...
bridge_patch = r'''
# GV-0066-2: use the verified native Colab/Android select bridge.
...
if source.count(bridge_open) != 1:
    raise RuntimeError("GV-0066-2 survey bridge selector was not found exactly once.")
...
if source.count('</script>') != 1:
    raise RuntimeError("GV-0066-2 closing script tag was not found exactly once.")
...
if wrapper.count(old_exec) != 1:
    raise RuntimeError("GV-0066-2 base execution line was not found exactly once.")
wrapper = wrapper.replace(old_exec, bridge_patch, 1)

exec(compile(wrapper, "GV-0066-2-wrapper.py", "exec"))
