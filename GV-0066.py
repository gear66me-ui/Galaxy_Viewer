...
# Repair the malformed extra parenthesis in the pinned GV-0066-1 wrapper.
...
if wrapper.count(bad_ned) != 1:
    raise RuntimeError("GV-0066-4 malformed NED block was not found exactly once.")
wrapper = wrapper.replace(bad_ned, good_ned, 1)

old_exec = 'exec(compile(source, "GV-0066.py", "exec"))'
bridge_patch = r'''
# GV-0066-4: verified native Colab/Android select bridge.
...
if source.count(bridge_open) != 1:
    raise RuntimeError("GV-0066-4 survey bridge selector was not found exactly once.")
...
if source.count('</script>') != 1:
    raise RuntimeError("GV-0066-4 closing script tag was not found exactly once.")
...
if wrapper.count(old_exec) != 1:
    raise RuntimeError("GV-0066-4 base execution line was not found exactly once.")
wrapper = wrapper.replace(old_exec, bridge_patch, 1)

# Parse the full generated Python wrapper before execution.
ast.parse(wrapper, filename="GV-0066-4-generated-wrapper.py")
exec(compile(wrapper, "GV-0066-4-generated-wrapper.py", "exec"))
