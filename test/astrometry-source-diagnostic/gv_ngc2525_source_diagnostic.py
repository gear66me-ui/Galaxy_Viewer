...
def exclusion_mask(h,w):
    yy,xx=np.mgrid[:h,:w]
    cx=(w-1)/2; cy=(h-1)/2
    # Mask the bright central galaxy plus a narrow edge band.
    rx=0.27*w; ry=0.27*h
...
def sep_sources(g,mask):
    ...
    for o in objs:
        ...
        rank=flux*compact
        rows.append({'x':x,'y':y,'flux':flux,'a':a,'b':b,'ellipticity':ell,'npix':npix,'peak':peak,'rank':rank})
    return uniformize(rows,w,h)


def dao_sources(g,mask):
    arr=g.copy(); arr[mask]=np.nan
    mean,med,std=sigma_clipped_stats(arr, sigma=3.0, maxiters=5)
    ...
    for row in tbl:
        x=float(row['xcentroid']); y=float(row['ycentroid']); flux=float(row['flux'])
        if mask[int(np.clip(round(y),0,h-1)),int(np.clip(round(x),0,w-1))]: continue
...
def image2xy_sources(provider_path,w,h):
    ...
    try:
        ...
        if p.returncode!=0: return [], (p.stderr or p.stdout)[-2000:]
        with fits.open(xyls) as hdul:
            ...
            rows=[]
            for i in range(min(len(tab),250)):
...
def write_xyls(rows,w,h,path):
    # Astrometry.net xylists use FITS pixel coordinates; center of first pixel is (1,1).
...
def solve_astrometry(xyls,w,h):
    ...
    req={
      'session':session,'publicly_visible':'n','allow_modifications':'d','allow_commercial_use':'d',
      ...
      'positional_error':2.0,'tweak_order':2,'crpix_center':True,'parity':2
    }
    with open(xyls,'rb') as fh:
        sub=api_post('https://nova.astrometry.net/api/upload',req,files={'file':('ngc2525_sep.xyls',fh,'application/octet-stream')})
    ...
    for name,url in {
      'solution.wcs':f'https://nova.astrometry.net/wcs_file/{job}',
      'annotated.jpg':f'https://nova.astrometry.net/annotated_display/{job}',
      'extraction.jpg':f'https://nova.astrometry.net/extraction_image_display/{job}',
      'corr.fits':f'https://nova.astrometry.net/corr_file/{job}'
    }.items():
        try:
            rr=requests.get(url,timeout=60); rr.raise_for_status(); (OUT/name).write_bytes(rr.content)
        except Exception as e: result.setdefault('download_errors',{})[name]=str(e)
    return result


def make_html(result,counts):
    cal=result.get('calibration') or {}
    ra=cal.get('ra',RA); dec=cal.get('dec',DEC)
    orient=cal.get('orientation',0); radius=cal.get('radius',CATALOG_FOV/2)
    fov=max(CATALOG_FOV,2*float(radius or CATALOG_FOV/2))
    html=f'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>NGC 2525 Source Diagnostic</title>
...
<style>body{{margin:0;background:#07101b;color:#eef;font-family:system-ui}}main{{padding:10px}}.grid{{display:grid;grid-template-columns:1fr 1fr;gap:10px}}img{{width:100%;background:#000}}#aladin{{height:70vh}}.card{{background:#101b29;border:1px solid #345;border-radius:10px;padding:10px;margin:8px 0}}@media(max-width:800px){{.grid{{grid-template-columns:1fr}}}}</style></head><body><main>
<h2>ASTROMETRY SOURCE DIAGNOSTIC — NGC 2525</h2><div class="card">SEP selected {counts['sep']} · DAOStarFinder {counts['dao']} · image2xy {counts['image2xy']}<br>Astrometry status: <b>{result.get('status')}</b><br>Calibration: RA {ra} · Dec {dec} · orientation {orient}° · display FOV {fov}°</div>
<div class="grid"><div><h3>SEP — controlled compact sources</h3><img src="02_SEP_SELECTED.jpg"></div><div><h3>DAOStarFinder</h3><img src="03_DAO_SELECTED.jpg"></div><div><h3>Astrometry image2xy</h3><img src="04_IMAGE2XY_SELECTED.jpg"></div><div><h3>Live Aladin DSS2 at Astrometry solution</h3><div id="aladin"></div></div></div>
...
def main():
    ...
    xyls=OUT/'sep_sources.xyls'; write_xyls(sep_rows,w,h,xyls)
    solve=solve_astrometry(xyls,w,h)
    payload={'schema':'gv-ngc2525-source-diagnostic-0001','elapsed_s':round(time.time()-t0,2),'image':{'width':w,'height':h},'catalog_hint':{'ra':RA,'dec':DEC,'fov_deg':CATALOG_FOV},'counts':{'sep':len(sep_rows),'dao':len(dao_rows),'image2xy':len(img_rows)},'image2xy_error':img_err,'astrometry':solve}
    (OUT/'result.json').write_text(json.dumps(payload,indent=2),encoding='utf-8')
    make_html(solve,payload['counts'])
