...
function formatMasText(valueArcsec) {
  const valueMas = Number(valueArcsec) * MAS_PER_ARCSEC;
  return Number.isFinite(valueMas) ? `${fmt(valueMas, 3)} MAS` : "Not available";
}
function sizeText(majorArcsec, minorArcsec, distanceLy) {
  ...
  const majMasText = formatMasText(maj);
  if (Number.isFinite(min)) {
    const minLy = distanceLy * min / 206264.806;
    const minMasText = formatMasText(min);
    return `${fmt(majLy,0)} × ${fmt(minLy,0)} ly (${majMasText} × ${minMasText})`;
  }
  return `${fmt(majLy,0)} ly (${majMasText})`;
}
...
function buildDebugDump(coords, simbadRows, ned, vizier, sdss, panstarrs, galex, surveyLog, finalSummary) {
  ...
  if (finalSummary) {
    ...
    text += `major axis mas: ${Number.isFinite(Number(finalSummary.galdim_majaxis)) ? fmt(Number(finalSummary.galdim_majaxis) * MAS_PER_ARCSEC, 3) : ""}\n`;
    text += `minor axis mas: ${Number.isFinite(Number(finalSummary.galdim_minaxis)) ? fmt(Number(finalSummary.galdim_minaxis) * MAS_PER_ARCSEC, 3) : ""}\n`;
