...
AR0="$W/viewer/GV-beta-0012AR-12-LAB.py"
RG0="$W/viewer/modules/random-galaxy/gv-random-galaxy-0093-LAB.js"
LAB0="$W/viewer/modules/diagnostics/gv-stutter-lab-0001.js"

AR="$W/viewer/GV-beta-0012AR-13-DOE.py"
RG="$W/viewer/modules/random-galaxy/gv-random-galaxy-0094-DOE.js"
LAB="$W/viewer/modules/diagnostics/gv-stutter-lab-0002.js"

D="/sdcard/Download"

echo "============================================================"
echo " GALAXY VIEWER — STUTTER DOE BUILD 0002"
echo "============================================================"

echo
echo "--- SOURCE GATE ---"

for f in "$AR0" "$RG0" "$LAB0"
do
  if [ ! -f "$f" ]; then
    echo "ABORT — SOURCE FILE MISSING: $f"
    exit 10
  fi
  echo "FOUND: $f"
done
...
do
  if [ -e "$f" ]; then
    echo "ABORT — DOE FILE ALREADY EXISTS: $f"
    exit 11
  fi
done

echo "COLLISION GATE: PASS"

cp "$AR0" "$AR"
cp "$RG0" "$RG"

echo "DOE BASELINE COPIES CREATED"

echo
echo "--- PATCH AR13 + RANDOM GALAXY 0094 ---"
...
replacements = [
    ("12AR-12-LAB", "12AR-13-DOE"),
    ("gv-random-galaxy-0093-LAB.js", "gv-random-galaxy-0094-DOE.js"),
    ("gv-stutter-lab-0001.js", "gv-stutter-lab-0002.js"),
    ("gvRandomGalaxy0093Lab", "gvRandomGalaxy0094Doe"),
    ("gvStutterLab0001", "gvStutterLab0002"),
]

for old, new in replacements:
    if old not in ar:
        raise SystemExit(f"AR PATCH ANCHOR MISSING: {old}")
    ar = ar.replace(old, new)

ar_path.write_text(ar)

# ============================================================
# RANDOM GALAXY 0094 DOE PATCH
# ============================================================

rg = rg_path.read_text()

old_version = "  const VERSION='0093';"

if old_version not in rg:
    raise SystemExit("RG VERSION ANCHOR MISSING")
...
# ------------------------------------------------------------
# Replace overlapping FOV choreography with true 3-phase DOE:
#
# PHASE 1 = zoom out
# PHASE 2 = hold wide + translate + rotate
# PHASE 3 = zoom in
# ------------------------------------------------------------

old_fov = """    #fovAt(t, startFov, destinationFov, immediate = false) {
      if (immediate) {
...
new_fov = """    #fovAt(t, startFov, destinationFov, immediate = false) {
      if (immediate) {
        const progress = clamp01(t);
        return Math.exp(
          Math.log(startFov) +
...
if old_fov not in rg:
    raise SystemExit("RG FOV CHOREOGRAPHY ANCHOR MISSING")

rg = rg.replace(old_fov, new_fov, 1)

# ------------------------------------------------------------
# DOE hook before duration / phase parameters are consumed.
# This allows the lab to set travelSeconds, translateStart,
# turnPoint and translationComplete on a run-by-run basis.
# ------------------------------------------------------------

old_prepare = """        const firstHomeTrip = this.#isHomeDeparture(source, startFov);
        GV_TRACE.enabled&&gvTrace(4639,'ROUTE_CALC_BEGIN',null);"""

new_prepare = """        const firstHomeTrip = this.#isHomeDeparture(source, startFov);
...
if old_prepare not in rg:
    raise SystemExit("RG PRE-TRAVEL DOE HOOK ANCHOR MISSING")

rg = rg.replace(old_prepare, new_prepare, 1)

# ------------------------------------------------------------
# Add explicit phase/timing telemetry to RAF setup trace.
# ------------------------------------------------------------

old_setup = """        GV_TRACE.enabled&&gvTrace(4658,'RAF_SETUP_COMPLETE',{baseDestinationRA,baseDestinationDec,baseDestinationFov,targetRotation,duration,turnPoint,alignmentStart});"""

new_setup = """        GV_TRACE.enabled&&gvTrace(4658,'RAF_SETUP_COMPLETE',{
          baseDestinationRA,
          baseDestinationDec,
          baseDestinationFov,
          targetRotation,
...
if old_setup not in rg:
    raise SystemExit("RG RAF SETUP TRACE ANCHOR MISSING")

rg = rg.replace(old_setup, new_setup, 1)

# ------------------------------------------------------------
# Completion hook.
# ------------------------------------------------------------

old_complete = """        if (this.options.prefetch) this.prefetch().catch(() => {});
        return destination;
      } catch (error) {"""

new_complete = """        if (this.options.prefetch) this.prefetch().catch(() => {});
...
if old_complete not in rg:
    raise SystemExit("RG POST-TRAVEL DOE HOOK ANCHOR MISSING")

rg = rg.replace(old_complete, new_complete, 1)

# ------------------------------------------------------------
# Abort hook.
# Failed DOE cells are NOT advanced; the next Random click
# repeats the same condition.
# ------------------------------------------------------------

old_abort = """        if (this.randomButton) this.randomButton.disabled = true;
        throw error;
      }
    }"""

new_abort = """        if (this.randomButton) this.randomButton.disabled = true;
...
if old_abort not in rg:
    raise SystemExit("RG DOE ERROR HOOK ANCHOR MISSING")

rg = rg.replace(old_abort, new_abort, 1)

rg_path.write_text(rg)

print("AR13 DOE PATCH: COMPLETE")
print("RANDOM GALAXY 0094 DOE PATCH: COMPLETE")
PY

echo
echo "--- PATCH ANCHOR CHECK ---"

grep -nE \
"12AR-13-DOE|gv-random-galaxy-0094-DOE|gv-stutter-lab-0002|gvRandomGalaxy0094Doe|gvStutterLab0002" \
"$AR" || {
  echo "ABORT — AR13 DOE ANCHOR CHECK FAILED"
  exit 20
}

grep -nE \
"VERSION='0094'|DOE 0094|prepareTravel|completeTravel|abortTravel|translateStart:Number|translationComplete:Number" \
"$RG" || {
  echo "ABORT — RANDOM GALAXY 0094 DOE ANCHOR CHECK FAILED"
  exit 21
}

echo
echo "AR13 + RANDOM GALAXY 0094 PATCH SECTION READY"

echo

echo
echo "--- WRITE STUTTER DOE LAB 0002 ---"
...
echo "STUTTER DOE LAB 0002 SOURCE WRITTEN"
