...
insert = r"""
source = source.replace("Galaxy Viewer — VIEWER-6", "Galaxy Viewer — VIEWER-8", 1)
...
if source.count(old_buttons) != 1:
    raise RuntimeError("VIEWER-8 controls token not found exactly once")
...
if source.count(old_show) != 1:
    raise RuntimeError("VIEWER-8 random galaxy display token not found exactly once")
...
for token in required:
    if token not in source:
        raise RuntimeError(f"VIEWER-8 required behavior missing: {token}")

exec(compile(source, "VIEWER-8.py", "exec"))
"""

if wrapper.count(old_exec) != 1:
    raise RuntimeError("VIEWER-8 VIEWER-6 execution token not found exactly once")
wrapper = wrapper.replace(old_exec, insert, 1)

compile(wrapper, "VIEWER-8-wrapper.py", "exec")
exec(compile(wrapper, "VIEWER-8-wrapper.py", "exec"))
