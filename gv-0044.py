...
<div id="gv0011-root">
  ...
  <div class="small-note">
    GV-0044 preserves the current viewer position, survey, and field of view in browser storage so switching tabs does not snap the viewer back to Andromeda.
  </div>
</div>
...
const VIEWER_STATE_KEY = "galaxy-viewer-gv0044-state";
...
function canonicalIdScore(id) {
  const s = cleanId(id).toUpperCase();
  let score = 0;
  if (/^(M\s*\d+|MESSIER\s*\d+|NGC\s*\d+|IC\s*\d+)$/.test(s)) score += 500;
  if (/\bM\s*31\b|MESSIER\s*31|NGC\s*224|ANDROMEDA/.test(s)) score += 200;
  if (/\bM\s*81\b|NGC\s*3031|BODE/.test(s)) score += 200;
  if (/\bM\s*51\b|NGC\s*5194|WHIRLPOOL/.test(s)) score += 200;
  if (s.includes("NAME P2B CLUSTER")) score -= 260;
  if (s.includes("[") || s.includes("LDC") || s.includes("2MASS") || s.includes("GALEX") || s.includes("SDSS") || s.includes("WISE")) score -= 160;
  return score;
}
...
function captureViewerState() {
  ...
  const coordText = document.getElementById("coordBox")?.value || `${fallback.ra.toFixed(6)} ${fallback.dec.toFixed(6)}`;
  let parsed = fallback;
  try { parsed = parseCoords(coordText); } catch(err) {}
  ...
  return { ra: parsed.ra, dec: parsed.dec, survey, fov };
}
...
(async () => {
  ...
  try {
    ...
    setDebugText("Viewer ready. Press Find Galaxy to print raw catalog responses in plain text.");
    saveViewerState();
  } catch(err) {
...
function scoreGalaxyCandidate(row) {
  ...
  score += canonicalIdScore(id);

  if (otype === "G") score += 160;
  else if (otype.startsWith("G")) score += 110;
  else if (["POG","LSB","IG","PAG"].includes(otype)) score += 35;

  const isCanonical = canonicalIdScore(id) >= 500;
  if (!isCanonical && /AGN|SY1|SY2|LINER|BLLAC|QSO|BLAZAR|SEYFERT|X/.test(otype)) score -= 130;
  if (!isCanonical && /STAR|CLUSTER|CLOUD|HII|SN|NOVA|XB/.test(otype)) score -= 80;

  if (Number.isFinite(maj)) score += Math.min(maj / 4, 120);
...
function summarizeGalaxyCandidates(rows, requested, ned=null, vizier=null) {
  ...
  const bestType = ranked.find(x => canonicalIdScore(x.row.main_id) >= 500 || !/STAR|CLUSTER|XB|X/i.test(String(x.row.otype ?? "")))?.row ?? bestIdentity;
...
async function findGalaxy() {
  try {
    ...
    setStatus("All catalog responses returned. Building final summary…");
    ...
    saveViewerState();
    setStatus("Search complete. GV-0044 kept your current view and did not reset to Andromeda.");
  } catch(err){
