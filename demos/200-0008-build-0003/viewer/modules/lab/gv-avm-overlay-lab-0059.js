...
const VERSION="0059";const trace=(code,detail={})=>{try{globalThis.GalaxyViewerDiagnostics?.recordAvm?.(code,detail)}catch(_){}};trace("AVM_LAB_BOOT",{version:VERSION});
const WATCH_MS=260;
...
async function fetchCanvasSafeBitmap(raw,key){
  ...
  const attempts=[
    {kind:'direct-cors',url:raw},
    {
      ...
      url:CANVAS_IMAGE_PROXY+encodeURIComponent(raw)+'&consumer=avm0052'
    }
  ];
