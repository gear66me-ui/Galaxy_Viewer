...
const VERSION="0060";const trace=(code,detail={})=>{try{globalThis.GalaxyViewerDiagnostics?.recordAvm?.(code,detail)}catch(_){}};trace("AVM_LAB_BOOT",{version:VERSION});
const WATCH_MS=260;
...
async function fetchCanvasSafeBitmap(raw,key){
  ...
  const st=stateOf();
  const prepared=
    st?.activeDestination?.preparedHdBlob ||
    st?.future?.[0]?.preparedHdBlob ||
    null;

  if(prepared&&Number(prepared.size)>0){
...
async function makeGlobalBlendSource(
  ...
  try{
    const acquired=await fetchCanvasSafeBitmap(raw,key);
    bmp=acquired.bitmap;
    source=bmp;
