...
function preloadUrl(url){
  ...
  try{
    ...
    if(preloadImages.length>40)preloadImages.splice(0,preloadImages.length-40);
  }catch(_){}
  try{
...
function preloadUpcoming(reason){
  ...
  if(count){
    try{console.info("GV AVM LAB 0011 PRELOADED",{reason,count})}catch(_){}
  }
}
...
function installTravelButtonGuard(){
  ...
  for(const b of buttons){
    if(!b||b.dataset.gvAvmGuard0011)continue;
    b.dataset.gvAvmGuard0011="1";
...
function panel(){
  ...
  let s=document.getElementById("gv-avm-overlay-lab-style-0011");
  if(!s){
    s=document.createElement("style");
    s.id="gv-avm-overlay-lab-style-0011";
    s.textContent=[
...
function applyExactImageFov(ra,dec,fov,destination,tag){
  ...
  if(fit!==null && fit>0){
    ...
    try{
      console.info("GV AVM LAB 0011 FOV FIT",{tag,fit,sourceFov,imageFovDeg:destination?.imageFovDeg,imageFovXDeg:destination?.imageFovXDeg,imageFovYDeg:destination?.imageFovYDeg,name:destination?.name,archiveId:destination?.archiveId});
    }catch(_){}
  }
}
  ...
  try{
    ...
    overlay=ARef.image(url,{
      ...
      successCallback:(ra,dec,fov,image)=>{
        ...
        console.info("GV AVM LAB 0011 AUTO LOADED",lastContext);
      },
      errorCallback:(error)=>{
        loadingKey="";
        console.warn("GV AVM LAB 0011 IMAGE LOAD FAILED",{reason,key,url,error});
      }
    });
    ...
    loadingKey="";
    console.error("GV AVM LAB 0011 FAILED",e);
    return false;
  }
}
