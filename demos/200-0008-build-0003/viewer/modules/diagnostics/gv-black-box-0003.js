...
(function(global){
  ...
  async function finishRun(runId,result={}) {
    ...
    const updated=await new Promise((resolve,reject)=>{
      ...
      req.onsuccess=()=>{
        ...
        record.status=result?.status==='ERROR'?'ERROR':result?.status==='ABORTED'?'ABORTED':'COMPLETE';
  ...
  function installEventBridge(){
    ...
    global.addEventListener('gv-black-box-nav-abort',e=>endNavigation({status:'ABORTED',...(e.detail||{})}));
