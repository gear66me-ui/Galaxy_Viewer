...
(() => {
  ...
  const FONT_URL = "../../../viewer/artwork/Fonts/Space%20Age%20Regular/Space%20Age%20Regular.otf";
  const ROW_HEIGHT = 36;
  const TILE_GAP = 2;
  const LEFT_INSET = 12;
  const TOP_INSET = 12;
...
.${DEFAULT_ROOT_CLASS} .gv-menu-proxy{appearance:none;-webkit-appearance:none;position:absolute;left:${LEFT_INSET}px;top:${TOP_INSET}px;display:flex;align-items:center;justify-content:center;width:${ROW_HEIGHT}px;height:${ROW_HEIGHT}px;margin:0;padding:0;overflow:hidden;background:linear-gradient(145deg,#081B3A 0%,#0B3177 42%,#1484DB 76%,#296DBD 100%);border:1px solid #7CCBFF;border-radius:6px;cursor:pointer;touch-action:manipulation;outline:none;box-shadow:inset 0 0 7px rgba(221,248,255,.16),0 0 10px rgba(88,191,255,.38);z-index:20}
