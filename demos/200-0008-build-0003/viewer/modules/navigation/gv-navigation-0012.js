...
(function(global){
  ...
  function flightDistanceProgress(t){
    ...
    function flightArchStateAt(t,startFov,destinationFov,maxFov){
      ...
      const zAt1=zm-A.Z1_BELOW_MAX;
      const zAt4=zm+(z1-zm)*(1-A.FINAL_ZOOM_SHARE);
      let x=0,z=z0;
      if(sec<=A.T1){
        x=0;
        z=flightHermiteSec(z0,zAt1,0,A.VZ_EXIT,sec,A.T0,A.T1);
      }else if(sec<=A.T2){
        x=flightHermiteSec(0,A.X2,0,A.VX_CRUISE,sec,A.T1,A.T2);
        z=flightHermiteSec(zAt1,zm,A.VZ_EXIT,0,sec,A.T1,A.T2);
      }else if(sec<=A.T3){
        x=flightHermiteSec(A.X2,A.X3,A.VX_CRUISE,A.VX_CRUISE,sec,A.T2,A.T3);
        z=zm;
      }else if(sec<=A.T4){
        x=flightHermiteSec(A.X3,1,A.VX_CRUISE,0,sec,A.T3,A.T4);
        z=flightHermiteSec(zm,zAt4,0,A.VZ_ENTRY,sec,A.T3,A.T4);
      }else{
        x=1;
        z=flightHermiteSec(zAt4,z1,A.VZ_ENTRY,0,sec,A.T4,A.T5);
      }
      return Object.freeze({translation:flightClamp01(x),fov:2**z,z});
    }

    function flightArchTranslationProgress(t,immediate=false){
      if(immediate)return flightClamp01(t);
      return flightArchStateAt(t,1,1,1).translation;
    }

    function flightArchRotationProgress(t){
      const sec=flightClamp01(t)*15;
      const A=FLIGHT_ARCH;
      if(sec<=A.T2)return 0;
      if(sec>=A.T3)return 1;
      return flightSmootherstep((sec-A.T2)/(A.T3-A.T2));
    }
    ...
    function flightArchFovAt(t,startFov,destinationFov,maxFov,immediate=false){
      ...
      return flightArchStateAt(t,startFov,destinationFov,maxFov).fov;
    }

    function flightBootstrapStateAt(t,startFov,destinationFov){
      const sec=flightClamp01(t)*9;
      const B=FLIGHT_BOOTSTRAP;
      const z0=Math.log2(Number(startFov));
      const z1=Math.log2(Number(destinationFov));
      const zBird=Math.log2(Math.min(Number(startFov),120));
      const zAt3=zBird+(z1-zBird)*(1-B.FINAL_ZOOM_SHARE);
      let x=0,z=z0;
      if(sec<=B.T1){
        x=flightHermiteSec(0,B.X1,0,B.VX1,sec,B.T0,B.T1);
        z=z0;
      }else if(sec<=B.T2){
        x=flightHermiteSec(B.X1,B.X2,B.VX1,B.VX2,sec,B.T1,B.T2);
        z=flightHermiteSec(z0,zBird,0,B.VZ2,sec,B.T1,B.T2);
      }else if(sec<=B.T3){
        x=flightHermiteSec(B.X2,1,B.VX2,0,sec,B.T2,B.T3);
        z=flightHermiteSec(zBird,zAt3,B.VZ2,B.VZ3,sec,B.T2,B.T3);
      }else{
        x=1;
        z=flightHermiteSec(zAt3,z1,B.VZ3,0,sec,B.T3,B.T4);
      }
      return Object.freeze({translation:flightClamp01(x),fov:2**z,z});
    }
    ...
    function flightBootstrapRotationProgress(t){
      const sec=flightClamp01(t)*9;
      const B=FLIGHT_BOOTSTRAP;
      if(sec<=B.T1)return 0;
      if(sec>=B.T2)return 1;
      return flightSmootherstep((sec-B.T1)/(B.T2-B.T1));
    }
...
async function flyViewport({
    ...
    if(!aladin)
      throw new Error('NAVIGATION 0012 ALADIN INSTANCE MISSING');
    ...
    if(activeFlightControl && !activeFlightControl.settled)
      throw new Error('NAVIGATION 0012 ACTIVE FLIGHT ALREADY EXISTS');
            ...
            if(blackBoxSample!==lastBlackBoxSample){
              ...
              try{
                ...
                const phase=firstHomeTrip
                  ? (
                      t<=1.5/9
                        ? 'BOOTSTRAP_TRANSLATE_ONLY'
                        : t<=4.5/9
                          ? 'BOOTSTRAP_TRANSLATE_ZOOM_ROTATE'
                          : t<=6/9
                            ? 'BOOTSTRAP_TRANSLATE_ZOOM'
