...
(function(global){
  ...
  function flightDistanceProgress(t){
    ...
    function flightBootstrapTranslationSeconds(sec){
      // FIRST TRIP: immediate half-arch. Translation begins at t=0,
      // reaches terminal speed once, and completes by T-2.5 s.
      // The final 2.5 s are destination-centered zoom-in only.
      const moveEnd=6.5;
      if(Number(sec)>=moveEnd)return 1;
      return flightTerminalProgressSeconds(sec,moveEnd,1.75,0.75);
    }
    ...
    function flightBootstrapStateAt(t,startFov,destinationFov){
      ...
      const z0=Math.log2(Number(startFov));
      const z1=Math.log2(Number(destinationFov));
      const x=flightBootstrapTranslationSeconds(sec);
      // Zoom-in starts immediately with translation and remains monotonic.
      // A smootherstep in log-FOV gives a rounded half-arch without a pause.
      const u=flightNavigationSmootherstep(sec/9);
      const z=z0+(z1-z0)*u;
      return Object.freeze({translation:x,fov:2**z,z});
    }

    function flightBootstrapTranslationProgress(t){
      return flightBootstrapTranslationSeconds(flightClamp01(t)*9);
    }
    ...
    function flightBootstrapRotationProgress(t){
      ...
      if(sec<=0.5)return 0;
      if(sec>=6.5)return 1;
      return flightNavigationSmootherstep((sec-0.5)/6);
    }
...
async function flyViewport({
    ...
    if(!aladin)
      throw new Error('NAVIGATION 0013 ALADIN INSTANCE MISSING');
    ...
    if(activeFlightControl && !activeFlightControl.settled)
      throw new Error('NAVIGATION 0013 ACTIVE FLIGHT ALREADY EXISTS');
