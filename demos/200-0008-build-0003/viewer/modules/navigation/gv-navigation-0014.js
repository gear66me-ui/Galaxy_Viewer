...
(function(global){
  ...
  function flightTravelDepartureBearingDeg(frame,ra,dec){
    const d2r=Math.PI/180,r2d=180/Math.PI;
    const r=Number(ra)*d2r,d=Number(dec)*d2r;
    const east=[-Math.sin(r),Math.cos(r),0];
    const north=[
    ...
    const dot=(a,b)=>a[0]*b[0]+a[1]*b[1]+a[2]*b[2];
    return Math.atan2(dot(frame.Ts,east),dot(frame.Ts,north))*r2d;
  }

  function flightNearestAxisAlignedRotation(bearing,currentRotation){
    const base=Number(bearing);
    const current=Number(currentRotation)||0;
    let best=base;
    let bestDelta=Infinity;
    for(let k=-3;k<=3;k++){
      const candidate=base+180*k;
      const delta=Math.abs(flightNormalizeRotationDelta(candidate-current));
      if(delta<bestDelta){
        best=candidate;
        bestDelta=delta;
      }
    }
    return current+flightNormalizeRotationDelta(best-current);
  }
  ...
  function flightDistanceProgress(t){
    ...
    function flightArchRotationProgress(t){
      const sec=flightClamp01(t)*15;
      if(sec<=3.5)return 0;
      if(sec>=11.5)return 1;
      return flightNavigationSmootherstep((sec-3.5)/8);
    }
    ...
    function flightBootstrapRotationProgress(t){
      return flightBootstrapStateAt(t,1,1).rotation;
    }

    function flightBootstrapCommandedRotationAt(
      t,
      rotationStart,
      preAlignRotation,
      targetRotation
    ){
      const sec=flightClamp01(t)*BOOTSTRAP_ENTRY.duration;
      const {preAlignEnd,arcStart,tangentTime}=BOOTSTRAP_ENTRY;
      const start=Number(rotationStart)||0;
      const align=Number(preAlignRotation)||0;
      const target=Number(targetRotation)||0;

      if(sec<=preAlignEnd){
        const u=preAlignEnd>0
          ? flightNavigationSmootherstep(sec/preAlignEnd)
          : 1;
        return start+flightNormalizeRotationDelta(align-start)*u;
      }

      if(sec<=arcStart)return align;

      if(sec<tangentTime){
        const u=flightNavigationSmootherstep(
          (sec-arcStart)/(tangentTime-arcStart)
        );
        return align+flightNormalizeRotationDelta(target-align)*u;
      }
...
async function flyViewport({
    ...
    if(!aladin)
      throw new Error('NAVIGATION 0014 ALADIN INSTANCE MISSING');
    ...
    const bootstrapDepartureBearing=firstHomeTrip
      ? flightTravelDepartureBearingDeg(travelFrame,Number(startRA),Number(startDec))
      : null;
    const bootstrapPreAlignRotation=firstHomeTrip
      ? flightNearestAxisAlignedRotation(
          bootstrapDepartureBearing,
    ...
    if(activeFlightControl && !activeFlightControl.settled)
      throw new Error('NAVIGATION 0014 ACTIVE FLIGHT ALREADY EXISTS');
      ...
      global.dispatchEvent(new CustomEvent('gv-black-box-nav-start',{detail:{
        ...
        choreography:{
          ...
          bootstrapProfile:{
            ...
            arcStartTranslation:0.10,
            departureBearingDeg:bootstrapDepartureBearing,
            preAlignRotationDeg:bootstrapPreAlignRotation,
            geometry:'PREALIGN_TRAVEL_PLANE_QUARTER_ELLIPSE_C1'
          },
          arch:{
            translateStart:3.5/15,
            apex:6.5/15,
          ...
          if(t<1&&aladinSample!==lastAladinSample){
            ...
            if(
              ...
              if(firstHomeTrip){
                commandSetRotation(
                  aladin,
                  flightBootstrapCommandedRotationAt(
                    t,
                    Number(rotationStart),
                    bootstrapPreAlignRotation,
                    targetRotation
                  )
                );
              }else{
                const rotationProgress=flightArchRotationProgress(t);
                commandSetRotation(
                  aladin,
                  Number(rotationStart)+
            ...
            if(blackBoxSample!==lastBlackBoxSample){
              ...
              try{
                ...
                if(firstHomeTrip){
                  commandedRotation=flightBootstrapCommandedRotationAt(
                    t,
                    Number(rotationStart),
                    bootstrapPreAlignRotation,
                    targetRotation
                  );
                }else if(t>=rotationEndPoint){
                  commandedRotation=targetRotation;
                }else if(t>=rotationStartPoint){
                  const rp=flightArchRotationProgress(t);
                  commandedRotation=
  ...
  function commandSetRotation(aladin,rotation){
    if(!aladin || typeof aladin.setRotation !== 'function')
      throw new Error('NAVIGATION 0014 setRotation UNAVAILABLE');
