...
(function(global){
  ...
  function fovOf(record){
    for(const v of [
      ...
      if(!fov.length) continue;

      fov = Math.max(...fov);

      if(
        text.includes('arcsec') ||
        text.includes('arcsecond')
      ){
        fov /= 3600;
      }else if(
        text.includes('arcmin') ||
        text.includes('arcminute')
      ){
        fov /= 60;
      }

      if(Number.isFinite(fov) && fov > 0) return fov;
    }
  ...
  function orientationOf(record){
    for(const v of [
      ...
      record?.orientation,
      record?.archiveOrientation
    ]){
      const n = finite(v);
      if(n !== null) return normalizeSigned(n);
      ...
      const m = text.match(
        /north\s+is\s+([0-9]+(?:\.[0-9]+)?)\s*°?\s*(right|left)\s+of\s+vertical/i
      );

      if(m){
        const amount = Number(m[1]);
        return normalizeSigned(
          (m[2].toLowerCase() === 'left' ? -1 : 1) * amount
        );
      }
    }
