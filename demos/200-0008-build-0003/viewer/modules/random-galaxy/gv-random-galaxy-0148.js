...
function setRandomButtonText(button,text,{busy=false,ready=false}={}){
  if(!(button instanceof Element))return;
  ...
  if(label)label.textContent=String(text||'RANDOM GALAXY');
  button.setAttribute('aria-label',String(text||'RANDOM GALAXY'));
        ...
        function startPrefetch(destination,priority=false){
            ...
            const promise=(async()=>{
                try{
                    ...
                    // AR118 hard gate: nothing enters the ready pool until the
                    // actual image AVM has supplied RA/Dec/FoV/rotation.
...
// AR120 - simple HD-only startup release gate.
// Diagnostics HD readiness is authoritative; AL is intentionally ignored.
const STARTUP_HD_READY_MIN=5;
const STARTUP_READY_FLASH_MS=900;
const PENDING_HD_WAIT_TIMEOUT_MS=45000;
let startupHdGateReleased=false;
let startupReadyTimer=0;

function startupHdReadyCount(){
  let ready=0;
  for(const record of orderedFutureRecords().slice(0,FUTURE_TARGET)){
    const hd=hdStateFor(record);
    if(Boolean(hd?.resource)&&hd.state==='READY')ready++;
  }
  return ready;
}
...
function updateStartupRandomButton(){
  ...
  const ready=startupHdReadyCount();
  const shown=Math.min(ready,STARTUP_HD_READY_MIN);

  setRandomButtonText(
    core.randomGalaxyButton,
    `DOWNLOADING ${shown} OF ${STARTUP_HD_READY_MIN}`,
    {busy:true}
  );

  if(ready<STARTUP_HD_READY_MIN)return false;

  startupHdGateReleased=true;
  setRandomButtonText(core.randomGalaxyButton,'READY',{ready:true});
...
async function waitForPendingHdReady(record,serial){
  ...
  for(;;){
    ...
    if(Boolean(hd?.resource)&&hd.state==='READY'){
      setRandomButtonText(core?.randomGalaxyButton,'RANDOM GALAXY');
      return true;
    }
    ...
    if(performance.now()-startedAt>=PENDING_HD_WAIT_TIMEOUT_MS)
      throw new Error('RANDOM PENDING HD WAIT TIMEOUT');
...
function updateRandomButtonReadyState(){
  ...
  if(
    ...
    !randomRequestPending &&
    !state.busy &&
    !core.randomGalaxyButton.classList.contains('gvrg-random-ready')
  )setRandomButtonText(core.randomGalaxyButton,'RANDOM GALAXY');
}
...
function install(){
  ...
  setRandomButtonText(
    core?.randomGalaxyButton,
    `DOWNLOADING 0 OF ${STARTUP_HD_READY_MIN}`,
    {busy:true}
  );
