...
`;
  ...
  function installHdArchiveIntegration(randomGalaxy, options = {}) {
    ...
    if(hdBackToSky){
        ...
        // BACK TO SKY presentation decoration.
        // Clone the real Random Galaxy star/comet nodes so this control
        // uses exactly the same visual language as the Random button.
        hdBackToSky.classList.add('gv-back-sky-fireballs');

        const sourceStar=randomGalaxy.randomButton?.querySelector('.gvrg-random-star')||null;
        const sourceComet=randomGalaxy.randomButton?.querySelector('.gvrg-random-comet')||null;

        if(sourceStar&&sourceComet&&!hdBackToSky.querySelector('.gv-back-sky-burst')){
            const burst=document.createElement('span');
            burst.className='gv-back-sky-burst';
            burst.setAttribute('aria-hidden','true');

            const specs=[
                ['-136px','-1px','1.20','0s'],
                ['-108px','-1px','0.80','-0.933333s'],
                ['-122px','5px','1.00','-1.866667s'],
                ['108px','-1px','0.80','-0.933333s'],
                ['136px','-1px','1.20','0s'],
                ['122px','5px','1.00','-1.866667s']
            ];

            for(const [x,y,scale,phaseDelay] of specs){
                const icon=document.createElement('span');
                icon.className='gv-back-sky-fireball';
                icon.style.setProperty('--gv-bts-x',x);
                icon.style.setProperty('--gv-bts-y',y);
                icon.style.setProperty('--gv-bts-scale',scale);
icon.style.setProperty('--gv-bts-phase-delay',phaseDelay);

                const starNode=sourceStar.cloneNode(true);
                const cometNode=sourceComet.cloneNode(true);
                cometNode.classList.add('gv-back-sky-comet-orbit');

                icon.append(starNode,cometNode);
                burst.appendChild(icon);
            }

            hdBackToSky.appendChild(burst);
        }
    }
    ...
    if(hdBackToSky)hdBackToSky.addEventListener('click',()=>{
        setTimeout(()=>{
            try{document.body.classList.remove('gv-hd-open')}catch(_){}
            onHideEarthReturn();
            requestAnimationFrame(()=>restoreAfterHdClose());
        },850);
    },true);
    if(hdDownloadButton){
    ...
    const handleViewerResize=()=>requestAnimationFrame(()=>{
        resetHdPresentationGeometry();
  ...
  function createRandomPreparationEngine(options = {}) {
    ...
    const avmMetadataRecords=
      Array.isArray(options.avmMetadataRecords)
        ? options.avmMetadataRecords
        : [];
    ...
    const HISTORY_PREPARED_TARGET=2;
    ...
    let prefetchDownloadWorkersActive=0;
    ...
    const priorityPrefetchKeys=new Set();
    const aladinPriorityTransactions=[];
    const aladinBackgroundTransactions=[];
    let aladinTransactionRunning=false;

    // 0189: runtime AVM authority is immutable for a given
    // destination/image URL during this application session.
    const runtimeAvmAuthorityCache=new Map();

    function pumpAladinTransactions(){
        if(aladinTransactionRunning)return;

        const entry=
            aladinPriorityTransactions.shift()||
            aladinBackgroundTransactions.shift();

        if(!entry)return;

        aladinTransactionRunning=true;

        Promise.resolve()
            .then(entry.task)
            .then(entry.resolve,entry.reject)
            .finally(()=>{
                aladinTransactionRunning=false;
                queueMicrotask(pumpAladinTransactions);
            });
    }

    function runAladinTransaction(task,priority=false){
        return new Promise((resolve,reject)=>{
            const entry={task,resolve,reject};

            if(priority)aladinPriorityTransactions.push(entry);
            else aladinBackgroundTransactions.push(entry);

            pumpAladinTransactions();
        });
    }
    ...
    const aladinPreparedReceipts=new Map();
        ...
        function gv0214CanonicalImageUrl(value){
            try{
        ...
        const gv0214AvmByImageUrl=new Map();
        const gv0214AvmByProviderArchive=new Map();
        const gv0214AvmArchiveCounts=new Map();
        const gv0214AvmUniqueArchive=new Map();

        for(const record of avmMetadataRecords){
            const archiveId=
                String(record?.archiveId||'')
                    .trim()
                    .toLowerCase();

            const providerKey=
                String(record?.providerKey||'')
                    .trim()
                    .toLowerCase();

            const imageUrl=
                gv0214CanonicalImageUrl(
                    record?.imageUrl
                );

            if(imageUrl)
                gv0214AvmByImageUrl.set(
                    imageUrl,
                    record
                );

            if(providerKey&&archiveId)
                gv0214AvmByProviderArchive.set(
                    providerKey+'|'+archiveId,
                    record
                );

            if(archiveId)
                gv0214AvmArchiveCounts.set(
                    archiveId,
                    Number(
                        gv0214AvmArchiveCounts.get(
                            archiveId
                        )||0
                    )+1
                );
        ...
        for(const record of avmMetadataRecords){
            ...
            if(
                archiveId &&
                gv0214AvmArchiveCounts.get(archiveId)===1
            )gv0214AvmUniqueArchive.set(
                archiveId,
                record
            );
        }

        function gv0214FindAvmMetadata(destination){
            const candidateUrls=[
                destination?.hdUrl,
            ...
            for(const value of candidateUrls){
                const key=
                    gv0214CanonicalImageUrl(value);

                if(
                    key &&
                    gv0214AvmByImageUrl.has(key)
                )return gv0214AvmByImageUrl.get(key);
            }
            ...
            if(providerKey&&archiveId){
                const exact=
                    gv0214AvmByProviderArchive.get(
                        providerKey+'|'+archiveId
                    );
        ...
        function releasePreparedItem(item){
            ...
            try{
                if(
                    item.objectUrl&&
                    String(item.objectUrl).startsWith('blob:')
                )URL.revokeObjectURL(item.objectUrl);
            }catch(_){}
            item.image=null;
            item.objectUrl='';
            item.blob=null;
            item.decodePromise=null;
        }
        ...
        async function gv0212JpegDimensionsFromBlob(blob){
            if(!(blob instanceof Blob)||blob.size<4)
                throw new Error('JPEG BLOB MISSING');

            const prefix=
                await blob
                    .slice(0,Math.min(blob.size,256*1024))
                    .arrayBuffer();

            const view=new DataView(prefix);
            if(view.byteLength<4||view.getUint16(0,false)!==0xFFD8)
                throw new Error('JPEG SOI MISSING');
            ...
            while(offset+4<=view.byteLength){
                ...
                if(sofMarkers.has(marker)){
                    if(length<7)throw new Error('JPEG SOF INVALID');
        ...
        async function gv0212EnsurePreparedItemDecoded(item){
            if(!item)return false;

            if(
                item.image instanceof HTMLImageElement &&
                item.image.complete &&
                Number(item.image.naturalWidth)>0 &&
                Number(item.image.naturalHeight)>0
            ){
                item.decoded=true;
                if(acceptedAvmAuthority(item.destination))
                    item.framingState='READY';
                setHdStatus(item.destination,'READY',item.sourceKind,item.sourceUrl);
                return true;
            }

            if(item.decodePromise)return item.decodePromise;
            ...
            const promise=(async()=>{
                ...
                GV_TRACE.enabled&&gvTrace(
                    22101,
                    'RANDOM_0221_FUTURE0_DECODE_STARTED',
                    {
                        key:String(item.key||''),
                ...
                setHdStatus(item.destination,'DECODING',item.sourceKind,item.sourceUrl);

                const prepared=
                    item.blob instanceof Blob
                        ? await decodePreparedBlob(item.blob)
                        : await loadPreparedImageDirect(item.sourceUrl);

                if(backgroundWorkSuspended){
                    try{
                        if(
                            prepared?.objectUrl &&
                            String(prepared.objectUrl).startsWith('blob:')
                        )URL.revokeObjectURL(prepared.objectUrl);
                    }catch(_){}
                    throw abortError();
                }

                item.image=prepared.image;
                item.objectUrl=prepared.objectUrl;
                item.decoded=true;

                const actualWidth=Number(item.image?.naturalWidth||0);
                const actualHeight=Number(item.image?.naturalHeight||0);

                if(
                    item.precomputedCatalogFallback===true &&
                    !acceptedAvmAuthority(item.destination)
                ){
                    item.destination=
                        gv0214PreparePrecomputedAvmAuthority(
                            item.destination,
                            {
                                width:actualWidth,
                                height:actualHeight
                    ...
                    item.pixelWidth=actualWidth;
                    item.pixelHeight=actualHeight;
                    item.framingState='READY';

                    GV_TRACE.enabled&&gvTrace(
                        21902,
                        'RANDOM_0219_CATALOG_FALLBACK_FUTURE0_READY',
                        {
                            key:item.key,
                            width:actualWidth,
                            height:actualHeight,
                ...
                const expectedWidth=Number(item.pixelWidth||0);
                const expectedHeight=Number(item.pixelHeight||0);
                ...
                if(dimensionsDiffer){
                    /*
                     * 0216:
                     * AVM ReferenceDimension is the coordinate-system raster
                     * size, not necessarily the provider's /screen/ JPEG size.
                     * The 0215 metadata-only fast path intentionally retained
                     * ReferenceDimension until FUTURE[0] performed its single
                     * visible-image decode.  Normalize WCS now to the real
                     * decoded raster instead of treating the legitimate
                     * provider resample as corruption.
                     */
                    if(
                        item.sourceKind==='AVM-CATALOG' &&
                        (
                            item.destination?.avmAuthority==='PRECOMPUTED_IMAGE_AVM' ||
                            item.destination?.avmAuthority==='PRECOMPUTED_CATALOG_FALLBACK'
                        ) &&
                        item.destination?.avmAuthorityWcs
                    ){
                        const normalizedWcs=
                            gv0211NormalizeWcsToDecodedImage(
                                item.destination.avmAuthorityWcs,
                                item.image
                            );

                        item.destination=Object.freeze({
                            ...item.destination,
                            avmAuthorityWcs:
                                Object.freeze({...normalizedWcs})
                        });
                        ...
                        GV_TRACE.enabled&&gvTrace(
                            21601,
                            'RANDOM_0216_AVM_SCREEN_RASTER_NORMALIZED',
                            {
                                key:item.key,
                                referenceWidth:expectedWidth,
                                referenceHeight:expectedHeight,
                                actualWidth,
                                actualHeight,
                                sourceUrl:String(item.sourceUrl||'')
                            }
                        ...
                        throw new Error(
                            'JPEG HEADER/DECODE DIMENSION MISMATCH'
                        );
                    }
                }
                ...
                GV_TRACE.enabled&&gvTrace(
                    21202,
                    'RANDOM_0212_FUTURE0_PIXEL_DECODE_READY',
                    {
                        key:item.key,
                        width:actualWidth,
                        height:actualHeight,
                        bytes:Number(item.blob?.size)||0,
                        elapsedMs:Math.round((performance.now()-started)*10)/10
                    }
                );
                return true;
            })().catch(error=>{
                const failedKey=String(item?.key||'').trim().toLowerCase();
                const failedSourceKind=String(item?.sourceKind||'');
                const failedSourceUrl=String(item?.sourceUrl||'');
                const failedIndex=prefetchReady.indexOf(item);
                const evicted=failedIndex>=0;

                if(evicted){
                    prefetchReady.splice(failedIndex,1);
                    releasePreparedItem(item);
                }
                ...
                GV_TRACE.enabled&&gvTrace(
                    22102,
                    'RANDOM_0221_FAILED_PREPARED_RESOURCE_EVICTED',
                    {
                        key:failedKey,
                        evicted,
                        sourceKind:failedSourceKind,
                ...
                if(item.decodePromise===promise)item.decodePromise=null;
            });

            item.decodePromise=promise;
            return promise;
        }
        ...
        function gv0215ValidatedHdImageUrl(value){
        ...
        function screenImageUrlFor(destination){
            // 0186: provider catalogs own their HD image URL.
            // ESO and NOIRLab must never be rewritten onto the
            // ESA/Hubble CDN.  normalizeCatalogGalaxy() has already
            // validated hdUrl as HTTPS before the destination reaches
            // Random Galaxy.
            const explicitHdUrl=
                gv0215ValidatedHdImageUrl(
                    destination?.hdUrl
                );

            if(explicitHdUrl)
                return explicitHdUrl;

            // Legacy fallback for older Hubble/Webb destinations that
            // genuinely lack an explicit catalog HD URL.
            const id=String(destination?.archiveId||'').trim();
            ...
            const webb=
                corpus.includes('esawebb.org') ||
                /^(?:weic|potm)/i.test(id);

            const hubble=
                corpus.includes('esahubble.org') ||
                corpus.includes('spacetelescope.org') ||
                /^(?:heic|opo|potw|ann)/i.test(id);
        ...
        function buildHdSourceCandidates(destination){
            const screen=screenImageUrlFor(destination);
        ...
        async function gv0217WarmCompressedImage(url,signal=null){
            ...
            try{
                ...
                GV_TRACE.enabled&&gvTrace(
                    21701,
                    'RANDOM_0217_COMPRESSED_CACHE_WARM_READY',
                    {
                        url:raw,
                ...
                GV_TRACE.enabled&&gvTraceError(
                    21702,
                    'RANDOM_0217_COMPRESSED_CACHE_WARM_FAILURE',
                    error
                );
                return false;
            }
        ...
        async function prepareHdDestinationFetch(destination,signal=null){
            /*
             * 0215: for true image-AVM records the sidecar already contains
             * the exact AVM reference raster dimensions. Use those dimensions
             * to build authoritative WCS immediately, without a background
             * cross-origin fetch of the JPEG. FUTURE[0] alone will load/decode
             * the exact catalog image when it must become visible.
             */
            const precomputedRecord=
                gv0214FindAvmMetadata(destination);

            const referenceDimension=
                Array.isArray(precomputedRecord?.referenceDimension)
                    ? precomputedRecord.referenceDimension.map(Number)
                    : [];

            const catalogImageUrl=
                gv0215ValidatedHdImageUrl(
                    precomputedRecord?.imageUrl
                );

            if(
                precomputedRecord?.authority==='PRECOMPUTED_IMAGE_AVM' &&
                referenceDimension.length>=2 &&
                referenceDimension.slice(0,2).every(
                    value=>Number.isFinite(value)&&value>0
                ) &&
                catalogImageUrl
            ){
                const dimensions=Object.freeze({
                    width:referenceDimension[0],
                    height:referenceDimension[1]
                });
                ...
                await gv0217WarmCompressedImage(
                    catalogImageUrl,
                    signal
                );
                ...
                GV_TRACE.enabled&&gvTrace(
                    21501,
                    'RANDOM_0215_PRECOMPUTED_METADATA_FAST_PATH',
                    {
                ...
                return {
                    ...
                    decoded:false,
                    decodePromise:null,
                    sourceUrl:catalogImageUrl,
                    sourceKind:'AVM-CATALOG'
                };
            }

            if(
                precomputedRecord?.authority==='PRECOMPUTED_CATALOG_FALLBACK' &&
                catalogImageUrl
            ){
                setHdStatus(
                    destination,
                    'DOWNLOADING',
                    'AVM-CATALOG-FALLBACK',
                    catalogImageUrl
                );
                ...
                GV_TRACE.enabled&&gvTrace(
                    21901,
                    'RANDOM_0219_CATALOG_FALLBACK_DEFERRED_TO_FUTURE0',
                    {
                        key:destinationKey(destination),
                        url:catalogImageUrl
                    }
                ...
                return {
                    ...
                    decodePromise:null,
                    sourceUrl:catalogImageUrl,
                    sourceKind:'AVM-CATALOG-FALLBACK',
                    precomputedCatalogFallback:true
                };
            }

            const sources=
                await orderHdSourcesBySize(
                    destination,
                    signal
                );
                ...
                try{
                    ...
                    if(signal?.aborted)
                        throw new DOMException(
                            'HD PRELOAD SUSPENDED',
                            'AbortError'
                        );

                    /*
                     * Read only the JPEG header for raster dimensions.
                     * No JPEG pixel surface is created here.
                     */
                    const dimensions=
                        await gv0212JpegDimensionsFromBlob(
                            blob
                        );

                    /*
                     * 0214: AVM/WCS authority comes only from the
                     * precomputed JSON catalog. Runtime JPEG XMP parsing
                     * is deliberately bypassed for this experiment.
                     */
                    const preparedDestination=
                        gv0214PreparePrecomputedAvmAuthority(
                            destination,
                    ...
                    GV_TRACE.enabled&&gvTraceError(
                        21403,
                        'RANDOM_0214_COMPRESSED_PREP_FAILURE',
                        error
                    );
                }
            }
            ...
            throw (
                lastError||
                new Error(
                    'HD PRELOAD HAS NO USABLE SOURCE'
                )
            );
        }
        ...
        async function prepareHdDestination(destination,signal=null){
            // 0203: one provider fetch/decode. AVM reuses this local blob URL.
            return prepareHdDestinationFetch(destination,signal);
        }
        ...
        function avmAuthorityUrlFor(destination){
            return screenImageUrlFor(destination);
        }
        ...
        function gv0211XmlValue(doc,name){
            const nodes=
                Array.from(
            ...
            for(const node of nodes){
                for(const attr of Array.from(node.attributes||[])){
                    ...
                    if(
                        local===name ||
                        qualified===name ||
                        qualified.endsWith(':'+name)
                    )return attr.value;
                }
            }

            // XMP element / rdf:Seq form.
            for(const node of nodes){
                const local=
                    String(
                        node.localName||
        ...
        function gv0211FindXmpPacket(text){
            const forms=[
                ['<x:xmpmeta','</x:xmpmeta>'],
                ['<xmpmeta','</xmpmeta>'],
                ['<rdf:RDF','</rdf:RDF>']
            ...
            for(const [startToken,endToken] of forms){
        ...
        async function gv0211RawAvmWcsFromBlob(blob){
            if(
                !(blob instanceof Blob) ||
                blob.size<1
            )throw new Error(
                'DIRECT AVM BLOB MISSING'
            );

            /*
             * AVM/XMP lives in image metadata, not in the pixel raster.
             * Read at most the first 1 MiB.
             *
             * The offline extraction proved that this is sufficient for the
             * working AVM corpus while avoiding a second full-image scan.
             */
            const bytes=
                await blob
                    .slice(
            ...
            const packet=
                gv0211FindXmpPacket(text);

            if(!packet)
                throw new Error(
                    'DIRECT AVM XMP PACKET NOT FOUND'
                );
            ...
            if(
                ...
                'DIRECT AVM REQUIRED WCS TAGS MISSING'
            );
            ...
            if(cd.length>=4){
                ...
                if(rotation.length)
                    wcs.CROTA2=rotation[0];
            }

            return wcs;
        }

        function gv0211NormalizeWcsToDecodedImage(
            sourceWcs,
            image
        ){
            const wcs={...sourceWcs};
            ...
            if(
                ...
                'DIRECT AVM IMAGE DIMENSIONS INVALID'
            );

            /*
             * This is the normalization 0210 omitted.
             *
             * AVM ReferenceDimension describes the coordinate system in
             * metadata space. The provider JPEG may have different pixel
             * dimensions. The WCS must therefore be rescaled before Aladin
             * sees the visible image.
             */
            if(actualWidth!==referenceWidth){
                ...
                if(Number.isFinite(Number(wcs.CD1_1)))
                    wcs.CD1_1=
                        Number(wcs.CD1_1)*
                        worldScale;

                if(Number.isFinite(Number(wcs.CD2_1)))
                    wcs.CD2_1=
                        Number(wcs.CD2_1)*
                        worldScale;
            ...
            if(actualHeight!==referenceHeight){
                const worldScale=
                    referenceHeight/actualHeight;

                const pixelScale=
                    actualHeight/referenceHeight;
                ...
                if(Number.isFinite(Number(wcs.CD1_2)))
                    wcs.CD1_2=
                        Number(wcs.CD1_2)*
                        worldScale;

                if(Number.isFinite(Number(wcs.CD2_2)))
                    wcs.CD2_2=
                        Number(wcs.CD2_2)*
                        worldScale;
            ...
            wcs.NAXIS1=actualWidth;
            wcs.NAXIS2=actualHeight;
            wcs.NAXIS=2;
        ...
        function gv0223GalacticToIcrs(lonDeg,latDeg){
            const l=Number(lonDeg)*Math.PI/180,b=Number(latDeg)*Math.PI/180;
            if(!Number.isFinite(l)||!Number.isFinite(b))return null;
            const cb=Math.cos(b),gx=cb*Math.cos(l),gy=cb*Math.sin(l),gz=Math.sin(b);
            const x=-0.0548755604162154*gx+0.4941094278755837*gy-0.8676661490190047*gz;
            const y=-0.8734370902348850*gx-0.4448296299600112*gy-0.1980763734312015*gz;
            const z=-0.4838350155487132*gx+0.7469822444972189*gy+0.4559837761750669*gz;
        ...
        function gv0223WcsPixelToSky(wcs,pixelX,pixelY){
            const crpix1=Number(wcs?.CRPIX1),crpix2=Number(wcs?.CRPIX2);
            const crval1=Number(wcs?.CRVAL1),crval2=Number(wcs?.CRVAL2);
            const x=Number(pixelX),y=Number(pixelY);
            if(![crpix1,crpix2,crval1,crval2,x,y].every(Number.isFinite))return null;
            const dx=x-crpix1,dy=y-crpix2;
            let xiDeg=null,etaDeg=null;
            const cd11=Number(wcs?.CD1_1),cd12=Number(wcs?.CD1_2),cd21=Number(wcs?.CD2_1),cd22=Number(wcs?.CD2_2);
            if([cd11,cd12,cd21,cd22].every(Number.isFinite)){
                xiDeg=cd11*dx+cd12*dy;etaDeg=cd21*dx+cd22*dy;
            }else{
                const cdelt1=Number(wcs?.CDELT1),cdelt2=Number(wcs?.CDELT2);
                if(!Number.isFinite(cdelt1)||!Number.isFinite(cdelt2))return null;
                const theta=Number.isFinite(Number(wcs?.CROTA2))?Number(wcs.CROTA2)*Math.PI/180:0;
                const c=Math.cos(theta),s=Math.sin(theta);
                xiDeg=cdelt1*dx*c-cdelt2*dy*s;
                etaDeg=cdelt1*dx*s+cdelt2*dy*c;
            }
            const ctype1=String(wcs?.CTYPE1||'RA---TAN').trim().toUpperCase();
            const ctype2=String(wcs?.CTYPE2||'DEC--TAN').trim().toUpperCase();
            const projection=ctype1.includes('SIN')?'SIN':ctype1.includes('TAN')?'TAN':'';
            if(!projection||!Number.isFinite(xiDeg)||!Number.isFinite(etaDeg))return null;
            const xi=xiDeg*Math.PI/180,eta=etaDeg*Math.PI/180,lon0=crval1*Math.PI/180,lat0=crval2*Math.PI/180;
            let lon,lat;
            if(projection==='TAN'){
                const d=Math.cos(lat0)-eta*Math.sin(lat0);
                lon=lon0+Math.atan2(xi,d);
                lat=Math.atan2(Math.sin(lat0)+eta*Math.cos(lat0),Math.hypot(d,xi));
            }else{
                const rho=Math.hypot(xi,eta);
                if(rho>1+1e-12)return null;
                if(rho<1e-15){lon=lon0;lat=lat0;}
                else{
                    const c=Math.asin(Math.min(1,rho)),sc=Math.sin(c),cc=Math.cos(c);
                    lat=Math.asin(cc*Math.sin(lat0)+eta*sc*Math.cos(lat0)/rho);
                    lon=lon0+Math.atan2(xi*sc,rho*Math.cos(lat0)*cc-eta*Math.sin(lat0)*sc);
                }
            }
            if(!Number.isFinite(lon)||!Number.isFinite(lat))return null;
            const lonDeg=((lon*180/Math.PI)%360+360)%360,latDeg=lat*180/Math.PI;
            const galactic=ctype1.startsWith('GLON')||ctype2.startsWith('GLAT');
            const sky=galactic?gv0223GalacticToIcrs(lonDeg,latDeg):{ra:lonDeg,dec:latDeg};
            if(!sky||!Number.isFinite(Number(sky.ra))||!Number.isFinite(Number(sky.dec)))return null;
            return Object.freeze({ra:Number(sky.ra),dec:Number(sky.dec),pixelX:x,pixelY:y,xiDeg,etaDeg,projection,coordinateFrame:galactic?'GALACTIC':'ICRS'});
        }

        function gv0223ImageCenterFromWcs(wcs,width,height){
            const w=Number(width||wcs?.NAXIS1),h=Number(height||wcs?.NAXIS2);
            if(!Number.isFinite(w)||w<=0||!Number.isFinite(h)||h<=0)return null;
            return gv0223WcsPixelToSky(wcs,w/2+0.5,h/2+0.5);
        }

        function gv0223TraceWcsCamera(stage,destination,wcs,center,hfov,vfov,ra,dec,fov,rotation){
            if(!GV_TRACE.enabled)return;
            const width=Number(wcs?.NAXIS1),height=Number(wcs?.NAXIS2);
            gvTrace(22301,'RANDOM_0223_WCS_CAMERA_AUTHORITY',{
                stage:String(stage||''),key:destinationKey(destination),
                crval:{ra:Number(wcs?.CRVAL1),dec:Number(wcs?.CRVAL2)},
                crpix:{x:Number(wcs?.CRPIX1),y:Number(wcs?.CRPIX2)},
                imageCenterPixel:{x:Number.isFinite(width)?width/2+0.5:null,y:Number.isFinite(height)?height/2+0.5:null},
                derivedCenter:center?{ra:Number(center.ra),dec:Number(center.dec)}:null,
                horizontalFov:Number(hfov),verticalFov:Number(vfov),
                finalCamera:{ra:Number(ra),dec:Number(dec),fov:Number(fov),rotation:Number(rotation)}
            });
        }


        function gv0214PreparePrecomputedAvmAuthority(
            destination,
            dimensions
        ){
            const record=
                gv0214FindAvmMetadata(
                    destination
                );

            if(!record)
                throw new Error(
                    'PRECOMPUTED AVM METADATA RECORD MISSING'
                );
            ...
            if(
                ...
                'PRECOMPUTED AVM JPEG DIMENSIONS INVALID'
            );
            ...
            const fovX=
                Number(record?.fovXDegrees);

            const fovY=
                Number(record?.fovYDegrees);
            ...
            if(
                ...
                'PRECOMPUTED AVM CORE GEOMETRY INVALID'
            );
            ...
            if(
                record?.authority===
                    'PRECOMPUTED_IMAGE_AVM'
            ){
                const referencePixel=
                    Array.isArray(
                        record?.referencePixel
                    )
                        ? record.referencePixel
                            .map(Number)
                        : [];
                ...
                const scale=
                    Array.isArray(record?.scale)
                        ? record.scale.map(Number)
                        : [];

                const cdMatrix=
                    Array.isArray(record?.cdMatrix)
                        ? record.cdMatrix.map(Number)
                        : [];

                if(
                    referencePixel.length<2 ||
                    referenceDimension.length<2 ||
                    !referencePixel
                        .slice(0,2)
                    ...
                    'PRECOMPUTED AVM WCS COMPONENTS INVALID'
                );
                ...
                if(cdMatrix.length>=4){
                    sourceWcs.CD1_1=cdMatrix[0];
                    sourceWcs.CD1_2=cdMatrix[1];
                    sourceWcs.CD2_1=cdMatrix[2];
                    sourceWcs.CD2_2=cdMatrix[3];
                }else{
                    sourceWcs.CDELT1=
                        scale[0];
                ...
                wcs={
                    ...
                    NAXIS1:actualWidth,
                    NAXIS2:actualHeight,
                    CRPIX1:(actualWidth+1)/2,
                    CRPIX2:(actualHeight+1)/2,
                    CRVAL1:ra,
                    CRVAL2:dec,
                    CTYPE1:'RA---TAN',
                    CTYPE2:'DEC--TAN',
                ...
                throw new Error(
                    'PRECOMPUTED AVM AUTHORITY TYPE INVALID'
                );
            }

            const centerAuthority=gv0223ImageCenterFromWcs(wcs,actualWidth,actualHeight);
            const centerRa=Number(centerAuthority&&centerAuthority.ra);
            const centerDec=Number(centerAuthority&&centerAuthority.dec);
            /* 0230: derivedCenter is AUDIT ONLY. Camera authority is catalog/AVM CRVAL RA-Dec. */
            const cameraRa=ra;
            const cameraDec=dec;
            ...
            const prepared=
                Object.freeze({
                    ...
                    /*
                     * 0223 catalog contract:
                     * FOV X is authoritative for Galaxy Viewer.
                     */
                    fovDegrees:fovX,
                    ...
                    avmHorizontalFovDegrees:
                        fovX,

                    avmVerticalFovDegrees:
                        fovY,
                    ...
                    avmMetadataSchema:
                        'gv-avm-runtime-catalog-0001',

                    avmMetadataProviderKey:
                        String(
                            record
                                ?.providerKey||
                            ''
            ...
            gv0223TraceWcsCamera(
                'PRECOMPUTED',destination,wcs,centerAuthority,
                fovX,fovY,cameraRa,cameraDec,fovX,cameraRotation
            );

            GV_TRACE.enabled&&gvTrace(
                21401,
                'RANDOM_0214_PRECOMPUTED_AVM_READY',
                {
                    key:
                        destinationKey(
                            destination
        ...
        async function gv0211PrepareDirectAvmAuthority(
            destination,
            blob,
            image
        ){
            ...
            const authorityUrl=
                avmAuthorityUrlFor(destination);

            if(!key)
                throw new Error(
                    'DIRECT AVM DESTINATION KEY MISSING'
                );

            if(!authorityUrl)
                throw new Error(
                    'DIRECT AVM AUTHORITY URL MISSING'
                );
            ...
            const rawWcs=
                await gv0211RawAvmWcsFromBlob(blob);
            ...
            const centerAuthority=gv0223ImageCenterFromWcs(
                wcs,
                Number(image?.naturalWidth||image?.width||wcs?.NAXIS1),
                Number(image?.naturalHeight||image?.height||wcs?.NAXIS2)
            );

            const avmRa=Number(wcs.CRVAL1);
            const avmDec=Number(wcs.CRVAL2);

            const rawRotation=
                avmRawRotationFromWcs(wcs);
            ...
            const horizontalFov=
                avmHorizontalFovFromWcs(
                    wcs,
                    null
                );

            const verticalFov=
                avmVerticalFovFromWcs(
                    wcs,
                    null
                );

            if(
                !Number.isFinite(avmRa) ||
                avmRa<0 ||
                avmRa>=360
            )throw new Error(
                'DIRECT AVM RA INVALID'
            );

            if(
                !Number.isFinite(avmDec) ||
                avmDec<-90 ||
                avmDec>90
            )throw new Error(
                'DIRECT AVM DEC INVALID'
            );
            ...
            if(
                ...
                'DIRECT AVM HORIZONTAL FoV INVALID'
            );
            ...
            if(
                ...
                'DIRECT AVM VERTICAL FoV INVALID'
            );
            ...
            if(
                ...
                'DIRECT AVM ROTATION INVALID'
            );

            /*
             * Preserve 0209's camera/FoV contract in this diagnostic
             * revision. We are isolating the extraction bottleneck only.
             */
            const authority=
                Object.freeze({
                    fovDegrees:horizontalFov,
            ...
            gv0223TraceWcsCamera(
                'DIRECT_AVM',destination,wcs,centerAuthority,
                horizontalFov,verticalFov,avmRa,avmDec,horizontalFov,cameraRotation
            );

            runtimeAvmAuthorityCache.set(
                authorityCacheKey,
                authority
            );
            ...
            GV_TRACE.enabled&&gvTrace(
                21101,
                'RANDOM_0211_DIRECT_AVM_READY',
                {
                    key,
                    bytes:
                        Number(blob?.size)||0,
                    referenceWidth:
                        Number(rawWcs.NAXIS1)||0,
                    referenceHeight:
                        Number(rawWcs.NAXIS2)||0,
                    decodedWidth:
                        Number(wcs.NAXIS1)||0,
                    decodedHeight:
                        Number(wcs.NAXIS2)||0,
        ...
        async function prepareRuntimeAvmAuthority(destination,preparedImageUrl=''){
            if(backgroundWorkSuspended)throw abortError();

            const key=destinationKey(destination);
            const authorityUrl=avmAuthorityUrlFor(destination);
            const preparedUrl=String(preparedImageUrl||'').trim();
            const url=preparedUrl||authorityUrl;

            if(!key)throw new Error('AVM AUTHORITY DESTINATION KEY MISSING');
            if(!authorityUrl)throw new Error('AVM AUTHORITY IMAGE URL MISSING');

            const authorityCacheKey=key+'|'+authorityUrl;
            const cachedAuthority=
                runtimeAvmAuthorityCache.get(authorityCacheKey);

            if(cachedAuthority){
                const cachedPrepared=Object.freeze({
                    ...destination,
                    ...cachedAuthority
                });
                ...
                GV_TRACE.enabled&&gvTrace(
                    11802,
                    'AR118_RUNTIME_AVM_AUTHORITY_CACHE_HIT',
                    {
                        key,
            ...
            if(!isolated)
                throw new Error('ISOLATED ALADIN PREWARM UNAVAILABLE');

            if(backgroundWorkSuspended)
                throw abortError();

            const isolatedA=aladinPrewarmHost?.contentWindow?.A;

            if(!isolatedA?.image)
                throw new Error('ISOLATED ALADIN A.image EXPORT MISSING');

            if(typeof isolated?.setOverlayImageLayer!=='function')
                throw new Error('ISOLATED ALADIN OVERLAY API MISSING');

            const layerName='GV AVM AUTHORITY PREFETCH '+key;
            ...
            return await new Promise((resolve,reject)=>{
                ...
                try{
                    const layer=isolatedA.image(url,{
                        name:layerName,
                        imgFormat:avmImageFormat(authorityUrl),
                        opacity:.01,
                        successCallback:(ra,dec,fov,image)=>{
                            try{
                                ...
                                gv0223TraceWcsCamera(
                                    'RUNTIME_ALADIN',destination,wcs,
                                    {ra:avmRa,dec:avmDec},
                                    horizontalFov,verticalFov,
                                    avmRa,avmDec,horizontalFov,cameraRotation
                                );

                                runtimeAvmAuthorityCache.set(
                                    authorityCacheKey,
                                    Object.freeze({
                                        fovDegrees:prepared.fovDegrees,
                                        aladinRotation:prepared.aladinRotation,
                                        avmRa:prepared.avmRa,
                                        avmDec:prepared.avmDec,
                                        avmHorizontalFovDegrees:
                                            prepared.avmHorizontalFovDegrees,
                                        avmVerticalFovDegrees:
                                            prepared.avmVerticalFovDegrees,
                                        avmCallbackFov:
                                            prepared.avmCallbackFov,
                                        avmRotation:
                                            prepared.avmRotation,
                                        avmCameraRotation:
                                            prepared.avmCameraRotation,
                                        avmAuthorityUrl:
                                            prepared.avmAuthorityUrl,
                                        avmAuthority:
                                            prepared.avmAuthority,
                                        avmAuthorityWcs:
                                            prepared.avmAuthorityWcs
                                    })
                                );
        ...
        function acceptedAvmAuthority(destination){
            return Boolean(
                destination?.avmAuthorityWcs &&
                (
                    destination?.avmAuthority==='RUNTIME_IMAGE_AVM' ||
                    destination?.avmAuthority==='CATALOG_IMAGE_WCS_FALLBACK' ||
                    destination?.avmAuthority==='PRECOMPUTED_IMAGE_AVM' ||
                    destination?.avmAuthority==='PRECOMPUTED_CATALOG_FALLBACK'
                )
            );
        }

        function catalogImageWcsFallback(destination,item,error=null){
            ...
            const horizontalFov=Number(destination?.fovDegrees);
            const cameraRotation=normalizeAvmCameraRotation(destination?.aladinRotation);
            const authorityUrl=avmAuthorityUrlFor(destination);
            ...
            if(
                ...
                !Number.isFinite(horizontalFov)||horizontalFov<=0||
                !Number.isFinite(cameraRotation)||
                !authorityUrl
            )return null;
            ...
            const verticalFov=
                2*Math.atan(Math.tan(horizontalRadians/2)*(height/width))*180/Math.PI;

            if(!Number.isFinite(verticalFov)||verticalFov<=0)return null;

            const wcs=Object.freeze({
                NAXIS1:width,NAXIS2:height,
                CRPIX1:(width+1)/2,CRPIX2:(height+1)/2,
                CRVAL1:ra,CRVAL2:dec,
                CTYPE1:'RA---TAN',CTYPE2:'DEC--TAN',
                CDELT1:-(horizontalFov/width),
                CDELT2:verticalFov/height,
                CROTA2:cameraRotation
            });
            ...
            const prepared=Object.freeze({
                ...
                avmHorizontalFovDegrees:horizontalFov,
                avmVerticalFovDegrees:verticalFov,
                avmCallbackFov:Math.max(horizontalFov,verticalFov),
                avmRotation:cameraRotation,
                avmCameraRotation:cameraRotation,
                avmAuthorityUrl:authorityUrl,
                avmAuthority:'CATALOG_IMAGE_WCS_FALLBACK',
                avmAuthorityWcs:wcs,
                avmAuthorityFallbackReason:String(
                    error?.message||error||'RUNTIME AVM UNAVAILABLE'
                )
            });

            const centerAuthority=gv0223ImageCenterFromWcs(wcs,width,height);
            gv0223TraceWcsCamera(
                'CATALOG_FALLBACK',destination,wcs,centerAuthority,
                horizontalFov,verticalFov,ra,dec,horizontalFov,cameraRotation
            );

            GV_TRACE.enabled&&gvTrace(
                20901,
                'RANDOM_0209_CATALOG_IMAGE_WCS_FALLBACK',
                {
                    key:destinationKey(destination),
                    name:String(destination?.name||''),
                    width,height,authorityUrl,
                    reason:prepared.avmAuthorityFallbackReason
                }
            );
        ...
        function aladinReceiptMatchesDestination(receipt,destination){
            if(!receipt||receipt.unavailable||!destination)return false;
            const key=destinationKey(destination);
            if(!key)return false;
            ...
            const expectedFov=Number(destination.fovDegrees);
            const expectedRotation=Number.isFinite(Number(destination.aladinRotation))
                ?Number(destination.aladinRotation)
                :0;
            return String(receipt.key||'').trim().toLowerCase()===key&&
                near(receipt.ra,destination.ra,1e-6)&&
                near(receipt.dec,destination.dec,1e-6)&&
                near(receipt.fov,expectedFov,Math.max(1e-6,Math.abs(expectedFov)*1e-6))&&
                near(receipt.rotation,expectedRotation,1e-4)&&
                String(receipt.projection||'MOL').trim().toUpperCase()==='MOL';
        }
        ...
        async function prepareAladinDestination(destination,force=false){
            ...
            try{
                ...
                if(typeof isolated.setProjection==='function')isolated.setProjection('MOL');
                ...
                const finalRotation=Number(destination.aladinRotation);
                if(Number.isFinite(finalRotation) && typeof isolated.setRotation==='function'){
                    window.GalaxyViewerNavigation.commandSetRotation(isolated,finalRotation);
                }
                ...
                if(
                    Number.isFinite(finalFov) &&
                    finalFov>0 &&
                    typeof isolated.setFov==='function'
                ){
                    const outerDwell=Math.round(ALADIN_PREWARM_DWELL_MS*.25);
                    const innerDwell=Math.round(ALADIN_PREWARM_DWELL_MS*.25);
                    const finalDwell=
                    ...
                    for(const [multiplier,milliseconds] of stages){
                        if(backgroundWorkSuspended){
                            completed=false;
                            break;
                        }
        ...
        function enqueuePrefetch(destination,priority=false){
            ...
            if(priority)priorityPrefetchKeys.add(key);
        ...
        function scheduleAladinEnhancement(item,destination,priority=false){
            ...
            const task=async()=>{
                ...
                if(acceptedAvmAuthority(item.destination)){
                    ...
                    GV_TRACE.enabled&&gvTrace(
                        11803,
                        'AR118_RUNTIME_AVM_AUTHORITY_ALREADY_READY',
                        {
                            key:destinationKey(item.destination),
                            name:String(item.destination?.name||'')
                        }
                    );
                ...
                try{
                    const preparedUrl=String(item?.objectUrl||'').trim();

                    if(/^blob:/i.test(preparedUrl)){
                        item.destination=
                            await prepareRuntimeAvmAuthority(
                                destination,
                                preparedUrl
                            );
                    }else{
                        item.destination=
                            catalogImageWcsFallback(
                                destination,
                                item,
                                new Error(
                                    'DIRECT IMAGE HAS NO CORS-READABLE LOCAL BLOB'
                                )
                            );

                        if(!item.destination)
                            throw new Error(
                                'CATALOG IMAGE WCS FALLBACK UNAVAILABLE'
                            );
                    }
                    ...
                    const fallback=
                        catalogImageWcsFallback(destination,item,error);
                    ...
                    console.warn(
                        'GALAXY VIEWER RUNTIME AVM AUTHORITY REJECTED',
                        error
                    );
                    throw error;
                }
            };
            return runAladinTransaction(task,priority);
        }
        ...
        function startPrefetch(destination,priority=false){
            ...
            if(prefetchDownloadWorkersActive>=PREFETCH_MAX_WORKERS){enqueuePrefetch(destination,priority);return}
            const controller=new AbortController();
            prefetchControllers.set(key,controller);
            const promise=(async()=>{
                let item;
                prefetchDownloadWorkersActive++;
                try{
                    try{
                        item=await prepareHdDestination(destination,controller.signal);
                    }finally{
                        prefetchDownloadWorkersActive=Math.max(0,prefetchDownloadWorkersActive-1);
                        queueMicrotask(fillPrefetchQueue);
                    }
                    const deferredCatalogFallback=Boolean(
                        item?.precomputedCatalogFallback===true &&
                        item?.sourceUrl
                    );

                    if(
                        acceptedAvmAuthority(
                            item?.destination
                        )
                    ){
                        /*
                         * 0214 precomputed AVM success:
                         * metadata authority is already complete and
                         * normalized to the JPEG header dimensions.
                         *
                         * Do NOT enter runAladinTransaction().
                         */
                        item.framingState='READY';

                        GV_TRACE.enabled&&gvTrace(
                            21402,
                            'RANDOM_0214_PRECOMPUTED_AVM_SERIAL_BYPASS',
                            {
                                key,
                                name:String(
                    ...
                    }else if(deferredCatalogFallback){
                        /*
                         * 0219:
                         * Catalog-fallback sidecar records do not have an
                         * AVM ReferenceDimension. Keep them metadata-only in
                         * FUTURE[1..9]. FUTURE[0] will direct-load exactly one
                         * image, obtain its real raster dimensions, and build
                         * the fallback WCS there.
                         */
                        item.destination=destination;
                        item.framingState='DEFERRED';
                    }else{
                        /*
                         * Compatibility path:
                         * preserve 0209 exactly when direct XMP extraction
                         * cannot provide complete authority.
                         */
                        item.destination=destination;
                        item.framingState='PREPARING';
                        ...
                        await scheduleAladinEnhancement(
                            item,
                            destination,
                            priority
                        );
                    ...
                    const pendingNavigation=
                        randomNavigationWindow?.getState?.().pending||null;
                    const pendingHistoryKey=
                        pendingNavigation&&
                        (pendingNavigation.kind==='back'||pendingNavigation.kind==='forward')
                            ?destinationKey(pendingNavigation.destination)
                            :'';
                    ...
                    if(key===activeTargetKey&&!activePreparedItem){
                        ...
                        if(
                            item.image instanceof HTMLImageElement &&
                            item.objectUrl
                        )window.GalaxyViewerRandomGalaxy?.setPreparedHdResource?.(
                            key,
                            item.objectUrl,
                            item.sourceKind,
                            item.image
                        );
                    }else if(key===pendingHistoryKey){
                        historyPreparedItems.push(item);
        ...
        function fillPrefetchQueue(){
            ...
            while(
                prefetchDownloadWorkersActive<PREFETCH_MAX_WORKERS &&
                ...
                const priority=
                    priorityPrefetchKeys.has(key)||
                    key===activeTargetKey;

                priorityPrefetchKeys.delete(key);
                startPrefetch(destination,priority);
        ...
        function prefetchHealthCheck(){
            ...
            if(!backgroundWorkSuspended&&(lastPrefetchHealth.total<PREFETCH_TARGET||(queued>0&&prefetchDownloadWorkersActive<PREFETCH_MAX_WORKERS)))fillPrefetchQueue();
    ...
    const api={
      ...
      ensureHdDecoded(key){
        const normalized=String(key||'').trim().toLowerCase();
        const item=
          activePreparedItem?.key===normalized
            ? activePreparedItem
            : prefetchReady.find(candidate=>candidate?.key===normalized)
              || historyPreparedItems.find(candidate=>candidate?.key===normalized)
              || null;

        if(!item)return Promise.resolve(false);
        return gv0212EnsurePreparedItemDecoded(item);
      },
      ...
      async ensureAladinPreparedForNavigation(destination){
        const key=destinationKey(destination);
        if(!key||aladinPrewarmUnavailable)return null;

        const existing=aladinPreparedReceipts.get(key)||null;

        if(existing&&!aladinReceiptMatchesDestination(existing,destination)){
          aladinPreparedReceipts.delete(key);
          aladinPrewarmedKeys.delete(key);
        }

        const retained=aladinPreparedReceipts.get(key)||null;

        if(
          retained&&
          aladinReceiptMatchesDestination(retained,destination)&&
          aladinPrewarm&&
          aladinPrewarmHost&&
          aladinPrewarmLastKey===key
        )return retained;
        ...
        const receipt=
          await runAladinTransaction(
            ()=>prepareAladinDestination(destination,true),
            true
          );

        return aladinReceiptMatchesDestination(receipt,destination)
          ? receipt
          : null;
      },
  ...
  class GalaxyRandomGalaxy {
    constructor(options = {}) {
      ...
      this._backClick = () => {
        if(this._backToSkyVisualPending)return;
        this._backToSkyVisualPending=true;

        try{
          this.backButton?.classList.add('gv-back-sky-green');
        }catch(_){}

        setTimeout(()=>{
          try{
            this.backButton?.classList.remove('gv-back-sky-green');
          }catch(_){}

          this._backToSkyVisualPending=false;
          this.backToSky();
        },850);
      };
...
#gv-travel-provider-icon{position:absolute;left:auto;right:12px;top:50%;transform:translateY(-50%);box-sizing:border-box;display:none;align-items:center;justify-content:center;overflow:hidden;width:48px;height:48px;padding:2px;border:2px solid transparent;border-radius:6px;background:linear-gradient(145deg,rgba(11,49,119,.98),rgba(20,132,219,.98)) padding-box,linear-gradient(145deg,#DDF8FF 0%,#7CCBFF 48%,#296DBD 100%) border-box;box-shadow:inset 0 0 7px rgba(225,248,255,.22),0 0 8px rgba(124,203,255,.42),0 0 14px rgba(41,109,189,.22);pointer-events:none;z-index:2}
...
#gv-travel-primary{position:relative;box-sizing:border-box;width:calc(100vw - 24px);padding:4px 70px 5px 7px;min-height:54px;border:1px solid rgba(124,203,255,.76);border-radius:6px;background:rgba(8,27,58,.72);box-shadow:0 0 8px rgba(88,191,255,.14);text-align:center}
...
/* BACK TO SKY — six Random Galaxy fireball/star decorations */
#gv-hd-control-row>.gvrg-back-button.gv-back-sky-fireballs{
    position:relative!important;
    isolation:isolate!important;
}
#gv-hd-control-row>.gvrg-back-button.gv-back-sky-fireballs>.gv-back-sky-burst{
...
#gv-hd-control-row>.gvrg-back-button.gv-back-sky-fireballs>*:not(.gv-back-sky-burst){
    position:relative!important;
    z-index:3!important;
}
.gv-back-sky-fireball{
    position:absolute;
    left:50%;
    top:50%;
    width:40px;
    ...
    transform:
        translate(
            calc(-50% + var(--gv-bts-x)),
            calc(-50% + var(--gv-bts-y))
        )
        scale(var(--gv-bts-scale));
    transform-origin:50% 50%;
    pointer-events:none;
    filter:
        drop-shadow(0 0 4px rgba(255,82,38,.58))
        drop-shadow(0 0 8px rgba(255,153,51,.34));
}
.gv-back-sky-fireball>.gvrg-random-star,
.gv-back-sky-fireball>.gv-back-sky-comet-orbit{
    position:absolute!important;
    left:50%!important;
    top:50%!important;
...
.gv-back-sky-fireball>.gv-back-sky-comet-orbit{
    ...
    filter:
        drop-shadow(0 0 5px rgba(255,80,50,.95))
        drop-shadow(0 0 11px rgba(255,140,40,.58));
}
.gv-back-sky-fireball>.gv-back-sky-comet-orbit i{
    display:block!important;
    opacity:1!important;
    background:linear-gradient(180deg,#FF3B30 0%,#FF6A1A 55%,#FFD05A 100%)!important;
    box-shadow:
        0 0 6px rgba(255,80,50,.95),
        0 0 12px rgba(255,140,40,.60)!important;
}
#gv-hd-control-row>.gvrg-back-button.gv-back-sky-green{
    ...
    box-shadow:
        inset 0 0 8px rgba(185,255,215,.28),
        0 0 10px rgba(120,255,171,.72),
        0 0 22px rgba(64,231,139,.42)!important;
    filter:brightness(1.10) saturate(1.14)!important;
}

/* ============================================================
   BACK TO SKY 0166 — AUTHORITATIVE SIX COMET ORBITS
   Each fireball group scales as one unit:
   1.20 large / 0.80 small / 1.00 nominal.
   ============================================================ */
...
.gv-back-sky-fireball>.gv-back-sky-comet-orbit{
    ...
    animation:gvBackSkyCometOrbit0166 2.8s linear infinite!important;
    animation-play-state:running!important;
animation-delay:var(--gv-bts-phase-delay)!important;
}
...
.gv-back-sky-fireball>.gv-back-sky-comet-orbit i{
    ...
    transform:rotate(var(--a)) translateY(-10px) scale(var(--s))!important;
    opacity:var(--o)!important;
    box-shadow:0 0 3px rgba(255,132,32,.85)!important;
}

.gv-back-sky-fireball>.gv-back-sky-comet-orbit i:nth-child(1){
    --a:0deg;
    --s:1;
    --o:1;
    background:#FF4414!important;
    box-shadow:
        0 0 2px 1px #FF4414,
        0 0 5px 1px #FF8420!important;
}

.gv-back-sky-fireball>.gv-back-sky-comet-orbit i:nth-child(2){
    --a:-15deg;--s:.88;--o:.84
}
.gv-back-sky-fireball>.gv-back-sky-comet-orbit i:nth-child(3){
    --a:-30deg;--s:.76;--o:.68
}
.gv-back-sky-fireball>.gv-back-sky-comet-orbit i:nth-child(4){
    --a:-45deg;--s:.64;--o:.52
}
.gv-back-sky-fireball>.gv-back-sky-comet-orbit i:nth-child(5){
    --a:-60deg;--s:.52;--o:.38
}
.gv-back-sky-fireball>.gv-back-sky-comet-orbit i:nth-child(6){
    --a:-75deg;--s:.42;--o:.26
}
.gv-back-sky-fireball>.gv-back-sky-comet-orbit i:nth-child(7){
    --a:-90deg;--s:.32;--o:.16
}
.gv-back-sky-fireball>.gv-back-sky-comet-orbit i:nth-child(8){
    --a:-105deg;--s:.24;--o:.08
}

@keyframes gvBackSkyCometOrbit0166{
...
`;
    ...
    #normalizeProviderCandidate(candidate) {
      ...
      // 0227 AUTHORITY CONTRACT:
      // Catalog RA/Dec are the sole navigation-center authority.
      // AVM/WCS RA/Dec remain image-registration metadata only.
      // AVM horizontal FoV and camera rotation remain framing authority.
    ...
    #showDistance(source,destination,route) {
      this.routeEl.textContent=`${source.name.toUpperCase()} TO ${destination.name.toUpperCase()}`;
      this.currentRouteDistanceAvailable=Boolean(!route?.unavailable&&finiteNumber(route?.value)>0);
      if(this.currentRouteDistanceAvailable)this.distanceRenderer.render(0);
      else this.distanceRenderer.renderUnavailable();
    ...
    previewDestination(destination) {
      ...
      try {
        const iconUrl=providerIconUrl(destination);        const img=this.providerIconButton?.querySelector("img");
    ...
    #showCard(destination) {
      ...
      if(d>0){
        ...
        this.distanceValueNumberEl.textContent=Math.round(d*dScale).toLocaleString('en-US');
      }else{
        if(dLabel)dLabel.textContent='DISTANCE';
        this.distanceValueNumberEl.textContent='—';
      }
    ...
    #r10ScreenFitFov(destination) {
      const verticalFov=finiteNumber(destination?.avmVerticalFovDegrees);
      return verticalFov!==null && verticalFov>0 ? verticalFov : null;
    }
    ...
    #formatTravelHudDistance(millionLy) {
      const available=this.travelHudDistanceAvailable===true;
      const total=finiteNumber(this.travelHudDistanceTotal);
      if(!available||!(total>0))
        return {integer:'—',fraction:'',unit:'',available:false};

      const value=Math.max(0,finiteNumber(millionLy)??0);
      let scale=1,unit='MILLION LIGHT-YEARS';
      if(total<1){scale=1_000;unit='THOUSAND LIGHT-YEARS'}
      else if(total>=1000){scale=1/1000;unit='BILLION LIGHT-YEARS'}

      const [integer,fraction='00']=(value*scale).toFixed(2).split('.');
      return {integer,fraction:fraction.padEnd(2,'0').slice(0,2),unit,available:true};
    }
    ...
    #beginTravelHud(destination, route, firstHomeTrip) {
      ...
      const distanceDecimalEl =
        document.getElementById('gv-travel-distance-decimal');
      ...
      const total=
        !route?.unavailable&&Number.isFinite(Number(route?.value))&&Number(route.value)>0
          ? Number(route.value)
          : null;
      this.travelHudDistanceTotal=total;
      this.travelHudDistanceAvailable=Boolean(total>0);
      ...
      const travelProviderUrl = providerIconUrl(destination);      if (travelProviderImage && travelProviderUrl) {
    ...
    #updateTravelHudDistance(millionLy) {
      const distanceIntegerEl=document.getElementById('gv-travel-distance-integer');
      const distanceDecimalEl=document.getElementById('gv-travel-distance-decimal');
      ...
      if(!distanceIntegerEl||!distanceDecimalEl||!distanceFractionEl||!distanceUnitEl)return;

      const shown=this.#formatTravelHudDistance(millionLy);
      distanceIntegerEl.textContent=shown.integer;
      distanceDecimalEl.textContent=shown.available?'.':'';
    ...
    async travelToRandom() {
      ...
      try {
        ...
        const route=routeDistanceMillionLy(source,destination,{homeDeparture:firstHomeTrip});
        ...
        // 0189: one destination-image reveal per physical trip.
        let avmDestinationShownForZoomIn=false;

        const showDestinationAvmForZoomIn=(reason)=>{
          if(avmDestinationShownForZoomIn)return true;

          const shown=Boolean(
            globalThis.GalaxyViewerAvmOverlayLab
              ?.showPendingForZoomIn?.(destination)
          );

          if(shown)
            avmDestinationShownForZoomIn=true;

          GV_TRACE.enabled&&gvTrace(
            4777,
            'AVM_0041_ZOOM_IN_TRIGGER',
            {
        ...
        await window.GalaxyViewerNavigation.flyViewport({
          ...
          onDistanceProgress:value=>{
            if(this.currentRouteDistanceAvailable)this.distanceRenderer.render(value);
            else this.distanceRenderer.renderUnavailable();
            this.#updateTravelHudDistance(value);
          },
          ...
          onPhaseChange:(phase,detail)=>{
            ...
            /*
             * User-designed two-image travel rule:
             *
             * Every phase before final zoom:
             *     current OFF
             *     next OFF
             *
             * Final zoom:
             ...
             *     next ON
             *
             * The destination was already loaded hidden by prepareForTravel().
             */
            /*
             * 0190 — TRUE TWO-IMAGE CONTINUITY.
             *
             * Do NOT hide CURRENT when Random is pressed.
             *
             * CURRENT remains opacity 1 while Aladin zooms out
             * and naturally leaves its sky position.
             *
             * NEXT was already staged at opacity 0.
             *
             * Only the final zoom-in turns NEXT on.
             * Arrival then commits NEXT and removes CURRENT.
             */
            if(
              phaseName.includes('FINAL') &&
              phaseName.includes('ZOOM')
            ){
              showDestinationAvmForZoomIn(
                'navigation-phase-'+phaseName
              );
            }

            GV_TRACE.enabled&&gvTrace(
              4776,
              'AVM_0041_NAVIGATION_PHASE',
              {
                phase:phaseName,
                previousPhase:
                  String(detail?.previousPhase||'')
              }
            );
          },
          trace:(code,label,detail)=>{
            if(
              String(label||'')===
              'ROTATION_SPEC_PRE_ZOOM_IN_OK'
            ){
              showDestinationAvmForZoomIn(
                'rotation-spec-pre-zoom-in'
              );
            }
        ...
        /*
         * 0188 / AVM 0041:
         * Destination is ALREADY visible from final zoom-in.
         * Arrival only commits pending -> active ownership.
         */
        try{
          const committed=
            globalThis.GalaxyViewerAvmOverlayLab
          ...
          GV_TRACE.enabled&&gvTrace(
            4790,
            'AVM_0041_ARRIVAL_COMMIT_RESULT',
            {
          ...
          console.warn(
            'AVM 0041 ARRIVAL COMMIT WARNING',
            error
          );
        }
...
let avmStageKey='';
let avmStagePromise=null;
let avmAutoStageFailedKey='';
...
function updateStartupRandomButton(){
  ...
  if(!headReady){
    ...
    const headState=
      head?.key
        ? String(hdStateFor(head).state||'QUEUED')
        : 'QUEUED';
    const downloading=
      ['QUEUED','DOWNLOADING','RETRY','SUSPENDED']
        .includes(headState);

    setRandomButtonText(
      button,
      downloading?'DOWNLOADING IMAGE':'PREPARING IMAGE',
...
function navigation0008Url(){
  ...
  if(owner?.src){
    return new URL(
      '../navigation/gv-navigation-0022.js',
      owner.src
    ).href;
  }

  return '/viewer/modules/navigation/gv-navigation-0022.js';
}
...
function ensureNavigationModule0013(){
  ...
  if(existing?.VERSION==='0022'){
    applyNavigation0013RuntimeConfig(existing);
    return Promise.resolve(existing);
  }

  if(existing && existing.VERSION!=='0022'){
...
function isUsableWebSourceUrl(value){
  try{
    const url=new URL(String(value||'').trim(),window.location.href);
...
function webStateFor(record){
  return record?.web||{key:String(record?.key||'').trim().toLowerCase(),state:'QUEUED',detail:'',nextRetryAt:0,sourceUrl:String(record?.destination?.sourceUrl||'').trim()};
}
...
function makeRecord(destination){
  ...
  return {
    ...
    web:{
      ...
      detail:'',
      nextRetryAt:0,
      sourceUrl:String(destination?.sourceUrl||'').trim()
    }
  };
}
...
function aladinReceiptMatchesRecord(receipt,record){
  const destination=record?.destination;
  if(!receipt||receipt.unavailable||!destination||!record?.key)return false;
  const near=(a,b,tolerance)=>{
    a=Number(a);b=Number(b);
    return Number.isFinite(a)&&Number.isFinite(b)&&Math.abs(a-b)<=tolerance;
  };
  const expectedFov=Number(destination.fovDegrees);
  const expectedRotation=Number.isFinite(Number(destination.aladinRotation))?Number(destination.aladinRotation):0;
  return String(receipt.key||'').trim().toLowerCase()===String(record.key||'').trim().toLowerCase()&&
    near(receipt.ra,destination.ra,1e-6)&&
    near(receipt.dec,destination.dec,1e-6)&&
    near(receipt.fov,expectedFov,Math.max(1e-6,Math.abs(expectedFov)*1e-6))&&
    near(receipt.rotation,expectedRotation,1e-4)&&
...
function queueAladinPreparation(record){
  ...
  const task=async()=>{
    ...
    if(suspended||preparationEngine?.getBackgroundWorkSuspended?.()){
    ...
    try{
      let receipt=await preparationEngine?.ensureAladinPreparedForNavigation?.(
        record.destination
      );

      if(!receipt&&preparationEngine?.isAladinPrepared?.(record.key)){
        receipt=preparationEngine?.getAladinPreparedReceipt?.(record.key)||null;
      }

      if(!aladinReceiptMatchesRecord(receipt,record)){
        throw new Error(
          'RANDOM GALAXY FUTURE ALADIN EXACT VIEWPORT RECEIPT MISMATCH'
        );
      }

      markAladinReady(record,receipt);
    }catch(error){
      const fallback=preparationEngine?.getAladinPreparedReceipt?.(record.key)||null;
      if(aladinReceiptMatchesRecord(fallback,record)){
        markAladinReady(record,fallback);
      }else{
        record.aladin={
          ...record.aladin,
          state:'RETRY',
  ...
  // The preparation engine owns the only hidden-Aladin transaction queue.
  // Future records request preparation but do not create a second queue.
  void task();
}
...
function startWeb(record){
  ...
  if(!isUsableWebSourceUrl(sourceUrl)){
    setWebState(record,'FAILED','INVALID SOURCE URL');
    return;
  }
  ...
  fetch(sourceUrl,{
    ...
    .finally(()=>{
      webControllers.delete(record.key);
      setTimeout(pumpWeb,0);
    });
}
function webPhaseAllowed(record){
  if(!record)return false;

  const hd=hdStateFor(record);
  const aladin=aladinStateFor(record);

  return Boolean(
    hd.state==='READY' &&
    hd.presentationReady &&
    aladin.state==='READY'
  );
}
function pumpWeb(){
  ...
  for(const record of candidates){
    if(webControllers.size>=WEB_MAX)break;
    if(!webPhaseAllowed(record))continue;
    startWeb(record);
  }
}

function pumpPreparationPhases(){
  if(suspended||preparationEngine?.getBackgroundWorkSuspended?.())return;
  ...
  const headAladin=aladinStateFor(head);
  const headReady=Boolean(
    headHd.state==='READY' &&
    headHd.presentationReady
  );

  /*
   * 0212 invariant:
   * FUTURE[0] is the only future record permitted to create a decoded
   * JPEG pixel surface. FUTURE[1..9] stop at compressed Blob + AVM/WCS.
   */
  if(headHd.state==='METADATA'){
    void preparationEngine
      ?.ensureHdDecoded?.(head.key)
      ?.then(()=>queueMicrotask(reconcileFutureQueue))
      .catch(error=>{
        GV_TRACE.enabled&&gvTraceError(
          21203,
          'RANDOM_0212_FUTURE0_PIXEL_DECODE_FAILURE',
          error
        );
        queueMicrotask(updateRandomButtonReadyState);
      });
  }else if(
    !['READY','DOWNLOADING','DECODING'].includes(headHd.state)
  ){
    if(headHd.state==='RETRY'){
      GV_TRACE.enabled&&gvTrace(
        22103,
        'RANDOM_0221_HEAD_RETRY_REQUESTED',
        {
          key:String(head.key||''),
          attempt:Number(head.hdRetryAttempts||0)+1
        }
      );
    }
  ...
  if(headAladin.state!=='READY')queueAladinPreparation(head);
  if(headReady)ensureHeadAvmStaged(head);

  for(const record of future.slice(1)){
    const hd=hdStateFor(record);

    if(
      ['METADATA','READY','DOWNLOADING','DECODING'].includes(hd.state)
    )continue;

    preparationEngine?.requestHdPrefetch?.(
      record.destination,
      false
    );
  }
...
function gv0213AcceptedAvmAuthority(destination){
  return Boolean(
    destination?.avmAuthorityWcs &&
    (
      destination?.avmAuthority==='RUNTIME_IMAGE_AVM' ||
...
function hdStateFor(record){
  ...
  const resource=
    (state==='READY'||state==='METADATA'||state==='DECODING')
      ? preparationEngine.getHdPreparedResource?.(record.key)||null
      : null;
  ...
  const identityMatch=
    resource
      ? Boolean(resourceKey===expectedKey)
  ...
  const framingReady=
    ...
    gv0213AcceptedAvmAuthority(resource?.destination);
...
function avmStageDestinationFor(record){
  if(!record?.destination||!record?.key)return null;
  const hd=hdStateFor(record);
  const resource=record.hd?.resource||null;
  if(!hd.presentationReady||!resource)return null;

  const preparedHdUrl=String(
    resource?.objectUrl||
    resource?.url||
    resource?.src||
    hd.resourceUrl||
  ...
  const preparedHdImage=
    resource?.image instanceof HTMLImageElement &&
    resource.image.complete &&
    Number(resource.image.naturalWidth)>0
      ? resource.image
      : null;

  if(!preparedHdUrl||!preparedHdImage)return null;

  return {
    ...record.destination,
    preparedHdUrl,
    preparedSource:String(resource?.sourceKind||record?.hd?.sourceKind||'').trim(),
    preparedHdImage
  };
}

function isHeadAvmStaged(record){
  const api=globalThis.GalaxyViewerAvmOverlayLab;
  if(!api||typeof api.isPreparedForTravel!=='function')return false;
  try{return api.isPreparedForTravel(record?.destination||record)===true}
  catch(_){return false}
}

function ensureHeadAvmStaged(record){
  if(!record?.key)return false;

  if(avmAutoStageFailedKey&&avmAutoStageFailedKey!==record.key)
    avmAutoStageFailedKey='';

  if(avmAutoStageFailedKey===record.key)
    return false;

  const api=globalThis.GalaxyViewerAvmOverlayLab;
  if(
    !api ||
    typeof api.stageForTravel!=='function' ||
    typeof api.isPreparedForTravel!=='function'
  )return false;

  const stageDestination=avmStageDestinationFor(record);
  if(!stageDestination)return false;

  /*
   * 0204 — UNIVERSAL AVM FILTER HANDOFF.
   * Persist the prepared local image and URL in the authoritative queued
   * destination. Hubble, JWST, ESO, NOIRLab and every future provider use
   * exactly the same filtered AVM path at click, zoom-in and arrival.
   */
  record.destination=stageDestination;

  if(api.isPreparedForTravel(stageDestination)===true)
    return true;

  if(avmStageKey===record.key&&avmStagePromise)
    return false;

  avmStageKey=record.key;
  const promise=Promise.resolve(
    api.stageForTravel(stageDestination,{hidePanel:false})
  ).then(result=>{
    if(result===true)avmAutoStageFailedKey='';

    GV_TRACE.enabled&&gvTrace(8432,'AVM_HEAD_STAGE_COMPLETE',{
      key:String(record.key||''),
      ready:Boolean(result)
    });
    return result;
  }).catch(error=>{
    const avmStageMessage=String(error?.message||error||');
    const avmStageSuperseded=avmStageMessage.indexOf('AVM PRETRAVEL BLEND SUPERSEDED')>=0;

    if(!avmStageSuperseded)avmAutoStageFailedKey=record.key;

    GV_TRACE.enabled&&gvTraceError(8438,'AVM_HEAD_STAGE_FAILURE',error);
    GV_TRACE.enabled&&gvTrace(22201,avmStageSuperseded?'RANDOM_0230_AVM_PRETRAVEL_SUPERSEDED_RETRY_ALLOWED':'RANDOM_0222_AVM_AUTO_RETRY_SUPPRESSED',{
      key:String(record.key||'),
      message:avmStageMessage
    });

    console.warn('RANDOM GALAXY FUTURE0 AVM STAGE WARNING',record.key,error);
    return false;
  }).finally(()=>{
    if(avmStagePromise===wrapped)avmStagePromise=null;
    setTimeout(updateRandomButtonReadyState,0);
  });
  const wrapped=promise;
  avmStagePromise=wrapped;
  return false;
}
...
function retirePoisonedHead(record){
  ...
  const promoted=future[1]||null;
  randomNavigationWindow.replaceFuture(future.slice(1));
  futureRecords.delete(key);

  GV_TRACE.enabled&&gvTrace(
    22104,
    'RANDOM_0221_HEAD_EVICTED',
    {
      key,
      retries:Number(record.hdRetryAttempts||0),
      quarantineMs:HEAD_HD_QUARANTINE_MS
    }
  );
  ...
  if(promoted){
    GV_TRACE.enabled&&gvTrace(
      22105,
      'RANDOM_0221_NEXT_HEAD_PROMOTED',
      {
        key:String(promoted.key||'')
      }
    );
  }
...
function healHeadHdFailure(){
  ...
  if(attempts>=HEAD_HD_MAX_RETRIES){
    const retired=retirePoisonedHead(record);

    if(retired){
      record.manualContinueRequired=false;
      return true;
    }

    record.manualContinueRequired=true;
    showFailedHeadButton(record);
    return false;
  }
...
function hasReadyNavigation(){
  ...
  return randomNavigationWindow.isNextReady(bundle=>{
    ...
    if(!hdReady)return false;

    const avmReady=isHeadAvmStaged(bundle);

    if(!avmReady)ensureHeadAvmStaged(bundle);
...
function updateRandomButtonReadyState(){
  ...
  const head=orderedFutureRecords()[0]||null;
  let headReady=false;
  let headHdState='QUEUED';

  if(head?.destination&&head?.key){
    const hd=hdStateFor(head);
    headHdState=String(hd.state||'QUEUED');
    const hdReady=Boolean(
      head.hd?.resource &&
      head.hd?.state==='READY' &&
      hd.presentationReady===true
    ...
    if(hdReady){
      const avmReady=isHeadAvmStaged(head);
      if(!avmReady)ensureHeadAvmStaged(head);
      headReady=avmReady;
    }
  }
  ...
  if(busy){
    setRandomButtonText(
      button,
      'TRAVELING',
      {busy:true,ready:false,imageWait:false}
    );
    return;
  }
...
async function requestRandomNavigation(){
  ...
  const preflightHd=hdStateFor(record);
  const preflightHdReady=Boolean(
    record.hd?.resource &&
    record.hd?.state==='READY' &&
    preflightHd.presentationReady===true
  );
  const preflightAvmReady=
    preflightHdReady &&
    isHeadAvmStaged(record);

  if(preflightHdReady&&!preflightAvmReady)
    ensureHeadAvmStaged(record);

  if(!preflightHdReady){
    updateRandomButtonReadyState();
    GV_TRACE.enabled&&gvTrace(5946,'REQUEST_BLOCKED_NOT_READY',{
      key:String(record?.key||''),
      hdState:String(record?.hd?.state||''),
      presentationReady:Boolean(preflightHd.presentationReady),
      avmReady:Boolean(preflightAvmReady)
    });
    return null;
  }
  ...
  /*
   * 0217 — AVM vignette staging is hidden preparation, not navigation authority.
   * Keep the current photograph during cruise. Start the destination vignette
   * opportunistically. commitPreparedArrival() can await the pending stage or
   * retry after arrival. A filter failure must never freeze Random navigation.
   */
  const avmApi=globalThis.GalaxyViewerAvmOverlayLab;
  const avmAlreadyStaged=Boolean(
    avmApi &&
    typeof avmApi.isPreparedForTravel==='function' &&
    avmApi.isPreparedForTravel(destination)===true
  );

  if(
    avmApi &&
    typeof avmApi.stageForTravel==='function' &&
    !avmAlreadyStaged
  ){
    void Promise.resolve(
      avmApi.stageForTravel(destination,{hidePanel:true})
    ).then(result=>{
      GV_TRACE.enabled&&gvTrace(
        11901,
        'AR119_VISIBLE_AVM_PRETRAVEL_READY',
        {
      ...
      GV_TRACE.enabled&&gvTraceError(
        11903,
        'AR119_VISIBLE_AVM_PRETRAVEL_FAILED_NONBLOCKING',
        error
      );
    });
  }else{
    GV_TRACE.enabled&&gvTrace(
      ...
      'AR119_VISIBLE_AVM_PRETRAVEL_READY',
      {
        ...
        name:String(destination?.name||''),
        url:String(destination?.avmAuthorityUrl||''),
        alreadyStaged:Boolean(avmAlreadyStaged),
        ready:Boolean(avmAlreadyStaged)
      }
    );
  }
...
async function requestHistoryNavigation(direction){
  ...
  try{
    const requestedKey=keyOf(requested);
    let destination=
      preparationEngine.activateQueuedDestination(
        requested,
        randomGalaxy?.currentGalaxy?.name||''
    ...
    if(destination&&keyOf(destination)!==requestedKey)
      throw new Error("HISTORY DESTINATION HANDOFF MISMATCH");

    /*
     * HISTORY CACHE MISS:
     * exact history identity is authoritative. If its decoded/prepared image
     * has been evicted, request that exact destination again and wait for the
     * retained resource. No queue substitution is permitted.
     */
    if(!destination){
      preparationEngine.requestHdPrefetch(requested,true);

      const historyWaitStarted=performance.now();

      for(;;){
        if(preparationEngine.getHdPreparedResource?.(requestedKey))break;

        if(performance.now()-historyWaitStarted>=PENDING_HD_WAIT_TIMEOUT_MS)
          throw new Error("HISTORY DESTINATION HD PREPARATION TIMEOUT");

        await new Promise(resolve=>setTimeout(resolve,POLL_MS));
      }

      const framingReady=
        await preparationEngine.ensureHdFramingReady?.(requested);

      if(!framingReady)
        throw new Error("HISTORY DESTINATION AVM PREPARATION FAILED");
      ...
      !['RUNTIME_IMAGE_AVM','CATALOG_IMAGE_WCS_FALLBACK','PRECOMPUTED_IMAGE_AVM','PRECOMPUTED_CATALOG_FALLBACK']
        .includes(destination.avmAuthority)||
      !Number.isFinite(Number(destination.ra))||
      destination.ra<0||destination.ra>=360||
      !Number.isFinite(Number(destination.dec))||
      destination.dec<-90||destination.dec>90||
      !Number.isFinite(Number(destination.avmHorizontalFovDegrees))||
      Number(destination.avmHorizontalFovDegrees)<=0||
      Number(destination.fovDegrees)!==Number(destination.avmHorizontalFovDegrees)||
    ...
    /*
     * Back/Forward uses the same two-image AVM handoff as Random Galaxy.
     * CURRENT stays visible. The exact history target is installed as NEXT
     * before travel; the existing travel choreography owns its reveal.
     */
    const avmApi=globalThis.GalaxyViewerAvmOverlayLab;
    ...
    )throw new Error("AR119 HISTORY AVM PRETRAVEL API MISSING");

    const avmInstalled=await avmApi.prepareForTravel(destination);

    if(!avmInstalled)
      throw new Error("AR119 HISTORY AVM PRETRAVEL INSTALL FAILED");
...
function aladinStateFor(record){
  ...
  let receipt=
    preparationEngine.getAladinPreparedReceipt?.(record.key)||
    record.aladin?.receipt||
    null;

  const receiptMatches=aladinReceiptMatchesRecord(receipt,record);

  if(receiptMatches){
    state='READY';
    progress=100;
  }else{
    if(preparationEngine.getBackgroundWorkSuspended?.()){
      state='SUSPENDED';
    }else{
      const live=preparationEngine.getAladinPrewarmState?.()||{};
  ...
  record.aladin={
    ...record.aladin,
    state,
    receipt:receiptMatches?receipt:null,
  ...
  const live=preparationEngine.getAladinPrewarmState?.()||{};
  const activeKey=String(live.activeKey||'').trim().toLowerCase();
  const identityMatch=Boolean(receipt&&aladinReceiptMatchesRecord(receipt,record));
  ...
  return {
    ...
    activeKey,
    identityMatch,
    ra:Number(record.aladin.ra),
    dec:Number(record.aladin.dec),
    fov:Number(record.aladin.fov),
    rotation:Number(record.aladin.rotation),
    receiptRa:Number.isFinite(Number(receipt?.ra))?Number(receipt.ra):null,
    receiptDec:Number.isFinite(Number(receipt?.dec))?Number(receipt.dec):null,
    receiptFov:Number.isFinite(Number(receipt?.fov))?Number(receipt.fov):null,
    receiptRotation:Number.isFinite(Number(receipt?.rotation))?Number(receipt.rotation):null
  };
}
...
function webTelemetry(record){
  const state=webStateFor(record);
  const expectedKey=String(record?.key||'').trim().toLowerCase();
  const stateKey=String(state?.key||'').trim().toLowerCase();
  const activeKey=webControllers.has(expectedKey)?expectedKey:'';
  const sourceUrl=String(state.sourceUrl||'').trim();
  const expectedSourceUrl=String(record?.destination?.sourceUrl||'').trim();

  const identityMatch=Boolean(
    stateKey===expectedKey &&
    (!activeKey||activeKey===expectedKey)
  );

  const sourceMatch=Boolean(
    isUsableWebSourceUrl(sourceUrl) &&
    sourceUrl===expectedSourceUrl
  );

  const truthfulReady=Boolean(
    state.state==='READY' &&
    identityMatch &&
    sourceMatch
  );
  ...
  const truthfulState=
    state.state==='READY' && !truthfulReady
      ? 'FAILED'
      : state.state;

  return {
    state:truthfulState,
    progress:truthfulReady?100:null,
    detail:state.detail||'',
    expectedKey,
    stateKey,
    activeKey,
...
function telemetry(){
  ...
  return Object.freeze({
    ...
    rows:Object.freeze(future.map((record,index)=>Object.freeze({
      ...
      aladin:Object.freeze(aladinStateFor(record)),
      web:Object.freeze(webTelemetry(record))
    })))
  });
}
...
function install(){
  ...
  window.addEventListener('beforeunload',()=>{
    ...
    for(const controller of webControllers.values()){
      try{controller.abort()}catch(_){}
    }
