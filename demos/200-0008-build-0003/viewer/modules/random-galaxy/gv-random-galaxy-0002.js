(function (global) {
  ...
  const FONT_NAMES = Object.freeze({
    spaceAge: 'GV Random Galaxy Space Age 0002',
    digits: 'GV Random Galaxy Digits 0002'
  });
  ...
  function extractDistanceMillionLy(value) {
    ...
    const matches = [...text.matchAll(/([0-9]+(?:\.[0-9]+)?)\s*(billion|million|thousand)?\s*(?:light[- ]?years?|ly)\b/gi)];
...
.gvrg-hd-loading{
  ...
  font:400 9px/1.4 \"${FONT_NAMES.spaceAge}\",sans-serif;letter-spacing:1px;color:#dff7ff
    ...
    async #candidateFromNasaItem(item, excludeName) {
      ...
      const distance = extractDistanceMillionLy(combined);
      const constellation = extractConstellation(combined);
    ...
    async #discoverHubbleCandidate(excludeName) {
      const firstTerm = NASA_SEARCH_TERMS[Math.floor(Math.random() * NASA_SEARCH_TERMS.length)];
      const firstUrl = new URL(this.options.nasaSearchUrl);
      firstUrl.searchParams.set('q', firstTerm);
      firstUrl.searchParams.set('media_type', 'image');
      firstUrl.searchParams.set('page', '1');

      const firstPayload = await this.#fetchNasaJson(firstUrl.href);
      ...
      const pageCap = Math.min(estimatedPages, 30);
      const attempts = Math.max(1, Number(this.options.discoveryAttempts) || 24);

      let payload = firstPayload;
      let items = Array.isArray(payload?.collection?.items) ? payload.collection.items.slice() : [];

      for (let attempt = 0; attempt < attempts; attempt += 1) {
        if (!items.length || attempt % 8 === 0) {
          const term = NASA_SEARCH_TERMS[Math.floor(Math.random() * NASA_SEARCH_TERMS.length)];
          const page = 1 + Math.floor(Math.pow(Math.random(), 1.7) * pageCap);
          const url = new URL(this.options.nasaSearchUrl);
          url.searchParams.set('q', term);
          url.searchParams.set('media_type', 'image');
          url.searchParams.set('page', String(page));
          payload = await this.#fetchNasaJson(url.href);
          items = Array.isArray(payload?.collection?.items) ? payload.collection.items.slice() : [];
        }
        ...
        try {
          ...
          // Reject the candidate and continue Hubble-only discovery.
        }
      }

      throw new Error('Could not find a usable Hubble galaxy on this attempt. Try RANDOM GALAXY again.');
    }
  ...
  GalaxyRandomGalaxy.PROVIDER_CONTRACT = Object.freeze({
    hubbleDiscovery: {
      ...
      selection: 'Hubble-image-backed galaxies only',
