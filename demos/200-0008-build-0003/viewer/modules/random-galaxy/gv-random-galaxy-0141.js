...
`;
  ...
  function installHdArchiveIntegration(randomGalaxy, options = {}) {
    ...
    if(!document.getElementById("gvrg-ar83-random-button-visible-style")){const st=document.createElement("style");st.id="gvrg-ar83-random-button-visible-style";st.textContent="body:not(.gv-hd-open) #gv-galaxy-nav,body:not(.gv-hd-open) #gv-random-galaxy{display:flex!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important}body:not(.gv-hd-open) #gv-galaxy-nav{z-index:7405!important}body.gv-hd-open #gv-galaxy-nav,body.gv-hd-open #gv-random-galaxy{display:none!important}";document.head.appendChild(st);}

    // AR83 / RANDOM 0141: removed unauthorized injected HD cleanup CSS.
    // Navigation/catalog fixes remain; viewport layout is not altered here.
...
`;
    ...
    // RANDOM 0141: safe presentation-only preview. No Aladin viewport writes.
...
async function requestRandomNavigation(){
  ...
  /*
   * RANDOM 0141 — PRE-TRAVEL DESTINATION PRESENTATION.
