  ...
  function createRandomPreparationEngine(options = {}) {
    ...
    const aladinPreparedReceipts=new Map();
        ...
        function catalogImageWcsFallback(destination,item,error=null){
            ...
            const verticalFov=Number(destination?.fovDegrees);
            ...
            if(
                ...
                !Number.isFinite(verticalFov)||verticalFov<=0||
                !Number.isFinite(cameraRotation)||
                !authorityUrl
            )return null;
            ...
            const horizontalFov=
                2*Math.atan(Math.tan(verticalRadians/2)*(width/height))*180/Math.PI;

            if(!Number.isFinite(horizontalFov)||horizontalFov<=0)return null;
