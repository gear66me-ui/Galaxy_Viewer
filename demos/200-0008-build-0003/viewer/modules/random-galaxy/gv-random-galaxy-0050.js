/*
GALAXY VIEWER ENGINEERING CHANGE ORDER — RANDOM GALAXY 0050
...
(function (global) {
  ...
  const FONT_NAMES = Object.freeze({
    spaceAge: 'GV Random Galaxy Space Age 0050',
    digits: 'GV Random Galaxy Digits 0050'
  });
...
`;
    ...
    async #getProviderCandidate(excludeName) {
      if (!this.provider) throw new Error('Random Galaxy 0050 requires a local galaxy provider.');
    ...
    async travelToRandom() {
      ...
      try {
        ...
        // Apply the authoritative final viewport before declaring arrival.
        const exactArrival=this.#exactActiveAladinState(destination);
        if(exactArrival){
          try{if(typeof this.aladin.setFrame==='function')this.aladin.setFrame('ICRSd')}catch(_){}
          try{if(exactArrival.projection&&typeof this.aladin.setProjection==='function')this.aladin.setProjection(exactArrival.projection)}catch(_){}
          try{if(Number.isFinite(Number(exactArrival.rotation))&&typeof this.aladin.setRotation==='function')this.aladin.setRotation(Number(exactArrival.rotation))}catch(_){}
          this.aladin.gotoRaDec(Number(exactArrival.ra),Number(exactArrival.dec));
          this.aladin.setFov(Number(exactArrival.fov));
        }
...
function updateRandomButtonReadyState(){
  ...
  core.randomGalaxyButton.disabled=Boolean(randomRequestPending||randomGalaxy?.getState?.().busy);
}
...
async function requestRandomNavigation(){
  ...
  reconcileFutureQueue();
  const locked=randomNavigationWindow.lockNext();
  if(!locked)return null;
  ...
  try{
    ...
    if(!destination||keyOf(destination)!==keyOf(locked)){
    ...
    randomGalaxy.prefetchPromise=null;
    try{
      if(typeof core?.aladin?.setFrame==='function')
        core.aladin.setFrame('ICRSd');
      if(typeof core?.aladin?.setRotation==='function')
        core.aladin.setRotation(0);
    }catch(error){
      console.warn('GALAXY VIEWER HOME FRAME RELEASE WARNING',error);
    }
    return await randomGalaxy.travelToRandom();
  }catch(error){
    if(randomNavigationWindow.getState().locked===locked)randomNavigationWindow.rollbackLocked();
    throw error;
  }finally{
    authoritativePendingDestination=null;
