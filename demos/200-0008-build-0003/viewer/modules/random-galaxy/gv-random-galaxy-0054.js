...
`;
  ...
  function installHdArchiveIntegration(randomGalaxy, options = {}) {
    ...
    function restoreNormalViewerPresentation(){
        ...
        bottom.random.disabled=Boolean(randomGalaxy.getState().busy);
        ...
        function scheduleAladinEnhancement(item,destination,priority=false){
            ...
            const task=async()=>{
                ...
                try{
                    ...
                    if(item&&framingCompleted)item.framingState='READY';
                }
            };
...
`;
    ...
    async #getProviderCandidate(excludeName) {
      if (!this.provider) throw new Error('Random Galaxy 0053 requires a local galaxy provider.');
    ...
    async travelToRandom() {
      ...
      try {
        ...
        let approachDestination=destination;
        let destinationFov=Number(
          approachDestination.fov
        );
        ...
        let targetRotation=
          finiteNumber(
            approachDestination.aladinRotation
          )??preservedUserRotation;
        const duration =
        ...
        await new Promise((resolve) => {
          const frame = (now) => {
            ...
            // Accept improved HD-guided framing only while
            // there is still time to blend it into the trip.
            // Once final approach begins, freeze the solution.
            if(t<alignmentStart){
              const candidate=this.activeDestination;

              if(candidate){
                const candidateArchive=
                  cleanText(candidate.archiveId);
                const destinationArchive=
                  cleanText(destination.archiveId);

                const sameDestination=
                  candidateArchive &&
                  destinationArchive
                    ? candidateArchive===destinationArchive
                    : cleanText(candidate.name)===
                      cleanText(destination.name);

                if(sameDestination){
                  const candidateFov=
                    finiteNumber(candidate.fov);

                  const candidateRotation=
                    finiteNumber(
                      candidate.aladinRotation
                    );
                  ...
                  if(
                    ...
                    approachDestination=candidate;
                    destinationFov=candidateFov;
                  }
            ...
            if(
              ...
              if(destinationCenterApplied&&alignmentProgress>0){
                ...
                const alignedDec=
                  baseDestinationDec+
                  (
                    Number(approachDestination.dec)-
                    baseDestinationDec
                  )*alignmentProgress;
        ...
        this.currentGalaxy={
          name:arrivedDestination.name,
          ra:arrivedDestination.ra,
          dec:arrivedDestination.dec,
          distance:arrivedDestination.distance
        };
        ...
        this.#hideDistance();
        this.#showCard(arrivedDestination);

        this.#setStatus(
          `ARRIVED ${arrivedDestination.name.toUpperCase()}`
        );
  ...
  const task=async()=>{
    ...
    try{
      ...
      if(!receipt&&core?.isAladinPrepared?.(record.key)){
        receipt=core?.getAladinPreparedReceipt?.(record.key)||null;
      }
      ...
      const fallback=core?.getAladinPreparedReceipt?.(record.key)||null;
...
async function consumeNext(excludeName=''){
  ...
  const locked=
    randomNavigationWindow.getState().locked ||
    randomNavigationWindow.lockNext();
  ...
  if(
    String(record.destination?.name||'').trim().toLowerCase()===excluded
  ){
    randomNavigationWindow.rollbackLocked();
    throw new Error(
      'RANDOM GALAXY LOCKED FUTURE0 MATCHES CURRENT GALAXY'
    );
  }
  ...
  if(!destination||keyOf(destination)!==lockedKey){
    activeRecord=null;
    randomNavigationWindow.rollbackLocked();
    throw new Error(
      'RANDOM GALAXY LOCKED FUTURE0 ACTIVATION MISMATCH'
    );
  }
  ...
  if(claimed!==record||keyOf(claimed)!==lockedKey){
    ...
    throw new Error(
      'RANDOM GALAXY CLAIMED BUNDLE IDENTITY MISMATCH'
    );
  }
...
function updateRandomButtonReadyState(){
  ...
  const next=randomNavigationWindow?.peekNext?.()||null;
...
async function requestRandomNavigation(){
  ...
  const expectedHead=
    randomNavigationWindow.peekNext?.()||null;
  ...
  const serial=++randomRequestSerial;
  ...
  try{
    const locked=
      randomNavigationWindow.lockNext?.()||null;

    if(
      !locked ||
      keyOf(locked)!==keyOf(expectedHead)
    ){
      throw new Error(
        'RANDOM FUTURE0 IMMEDIATE LOCK FAILED'
      );
    }

    // Priority preparation starts now.
    // It is deliberately NOT awaited before departure.
    core.requestHdPrefetch?.(locked.destination);
    queueAladinPreparation(locked);
    ...
    if(
      !destination ||
      keyOf(destination)!==keyOf(expectedHead)
    ){
      throw new Error(
        'RANDOM LOCKED FUTURE0 HANDOFF MISMATCH'
      );
    }

    authoritativePendingDestination=destination;

    // travelToRandom() consumes this exact object synchronously.
    randomGalaxy.prefetchedDestination=destination;
    randomGalaxy.prefetchPromise=null;
    randomGalaxy.activeDestination=destination;

    // Keep refining THIS SAME destination while the visible
    // 17-second journey is running. Never gate departure.
    const prepDuringTravel=(async()=>{
      const travelMs=Math.max(
        1000,
        Number(
          randomGalaxy?.options?.travelSeconds||17
        )*1000
      );

      // The travel engine freezes its arrival solution at
      // alignmentStart = 0.76. Do not alter it after that.
      const deadline=
        performance.now()+travelMs*.76;

      while(
        requestActive &&
        serial===randomRequestSerial &&
        performance.now()<deadline
      ){
        hdStateFor(locked);

        if(locked.aladin?.state!=='READY'){
          queueAladinPreparation(locked);
        }

        const prepared=locked.destination;

        if(
          prepared &&
          keyOf(prepared)===keyOf(destination)
        ){
          randomGalaxy.activeDestination=prepared;
        }

        if(
          Boolean(locked.hd?.resource) &&
          locked.hd?.state==='READY' &&
          locked.aladin?.state==='READY'
        ){
          return true;
        }
    ...
    prepDuringTravel.catch(error=>
      console.warn(
        'GALAXY VIEWER RANDOM DURING-TRAVEL PREPARATION WARNING',
        error
      )
    );

    // Visible motion begins immediately.
    return await randomGalaxy.travelToRandom();
...
function aladinStateFor(record){
  ...
  if(receiptMatches){
    ...
    if(core.getBackgroundWorkSuspended?.()){
      state='SUSPENDED';
    }else{
