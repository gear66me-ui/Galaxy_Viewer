  ...
  function createRandomPreparationEngine(options = {}) {
    ...
    const aladinPreparedReceipts=new Map();
        ...
        function screenImageUrlFor(destination){
            ...
            // Random Galaxy.
            const explicitHdUrl=
                String(destination?.hdUrl||'').trim();

            if(/^https:\/\//i.test(explicitHdUrl))
...
`;
    ...
    #normalizeProviderCandidate(candidate) {
      ...
      if (!['RUNTIME_IMAGE_AVM','CATALOG_IMAGE_WCS_FALLBACK'].includes(avmAuthority))
...
async function requestRandomNavigation(){
  ...
  if(
    !['RUNTIME_IMAGE_AVM','CATALOG_IMAGE_WCS_FALLBACK']
