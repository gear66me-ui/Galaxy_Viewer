...
`;
    ...
    #normalizeProviderCandidate(candidate) {
      ...
      // AR117: image FoV is acquired beside the already-authoritative
      // rotation metadata.  This is the FoV of the actual AVM/provider image,
      // not the older curated/context framing value.
      const imageFovDeg = finiteNumber(
        candidate.imageFovDeg ??
        candidate.imageFovDegrees ??
        candidate.image_fov_degrees
      );
      const imageFovXDeg = finiteNumber(
        candidate.imageFovXDeg ??
        candidate.imageFovXDegrees ??
        candidate.image_fov_x_degrees ??
        candidate.fieldOfViewDegrees?.widthDegrees
      );
      const imageFovYDeg = finiteNumber(
        candidate.imageFovYDeg ??
        candidate.imageFovYDegrees ??
        candidate.image_fov_y_degrees ??
        candidate.fieldOfViewDegrees?.heightDegrees
      );
      const imageFovCandidates = [imageFovDeg, imageFovXDeg, imageFovYDeg]
        .filter(value => value !== null && value > 0);
      const fovDegrees = imageFovCandidates.length
        ? Math.max(...imageFovCandidates)
        : null;
      ...
      if (fovDegrees == null || fovDegrees <= 0) throw new Error('Galaxy destination has no usable required image FoV metadata.');
      ...
      return Object.freeze({
        ...
        fovDegrees, imageFovDeg, imageFovXDeg, imageFovYDeg, aladinRotation,
        hdUrl: hdUrl.href,
...
async function requestRandomNavigation(){
  ...
  /*
   * RANDOM 0134 — PRE-TRAVEL DESTINATION PRESENTATION.
