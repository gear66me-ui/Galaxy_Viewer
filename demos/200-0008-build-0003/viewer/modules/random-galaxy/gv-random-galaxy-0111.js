...
function navigation0008Url(){
  ...
  if(owner?.src){
    return new URL(
      '../navigation/gv-navigation-0015.js',
      owner.src
    ).href;
  }

  return '/viewer/modules/navigation/gv-navigation-0015.js';
}
...
function ensureNavigationModule0013(){
  ...
  if(existing?.VERSION==='0015'){
    applyNavigation0013RuntimeConfig(existing);
    return Promise.resolve(existing);
  }

  if(existing && existing.VERSION!=='0015'){
  ...
  navigationModulePromise=new Promise((resolve,reject)=>{
    ...
    const finish=()=>{
      ...
      if(api?.VERSION!=='0015'){
        reject(
          new Error(
            'NAVIGATION 0015 EXPORT MISSING OR VERSION MISMATCH'
          )
        );
        return;
      }
    ...
    if(prior){
      ...
      prior.addEventListener(
        'error',
        ()=>reject(
          new Error('NAVIGATION 0015 SCRIPT LOAD FAILED')
        ),
        {once:true}
      );
      return;
    ...
    script.addEventListener(
      'error',
      ()=>reject(
        new Error(`NAVIGATION 0015 SCRIPT LOAD FAILED: ${url}`)
      ),
      {once:true}
    );
...
async function ensureNavigationPlanner(anchorDestination){
  ...
  navigationPlannerPromise=(async()=>{
    ...
    if(typeof api.create!=='function')
      throw new Error('NAVIGATION 0015 CREATE API MISSING');
    ...
    if(
      ...
      throw new Error('NAVIGATION 0015 PLANNER API INCOMPLETE');
    }
    ...
    if(navigationPlanner){
      queueMicrotask(()=>{
        try{
          ...
          console.error(
            'RANDOM 0105 NAVIGATION 0015 RECONCILE FAILURE',
            error
          );
        }
      });
...
async function advanceNavigationPlannerAfterArrival(arrived){
  ...
  if(!expected || keyOf(expected)!==arrivedKey){
    throw new Error(
      'RANDOM 0105 NAVIGATION 0015 COMMIT IDENTITY MISMATCH'
    );
  }
  ...
  if(!bootstrapDestinationKey){
    ...
    if(bootstrap&&addFuture(bootstrap)){
      ...
      void ensureNavigationPlanner(bootstrap).catch(error=>{
        console.error(
          'RANDOM 0105 NAVIGATION 0015 INITIALIZATION FAILURE',
          error
        );
      });
    }
