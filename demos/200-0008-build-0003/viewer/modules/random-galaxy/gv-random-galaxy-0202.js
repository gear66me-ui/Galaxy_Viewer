  ...
  function createRandomPreparationEngine(options = {}) {
    ...
    const aladinPreparedReceipts=new Map();
        ...
        async function prepareHdDestination(destination,signal=null){
            const key=destinationKey(destination);
            const url=screenImageUrlFor(destination);

            if(!key)
                throw new Error(
                    'SCREEN PRELOAD DESTINATION KEY MISSING'
                );

            if(!url)
                throw new Error(
                    'SCREEN PRELOAD URL MISSING'
                );
            ...
            await new Promise((resolve,reject)=>{
                ...
                image.onload=()=>{
                    if(
                        ...
                        finish(
                            reject,
                            new Error(
                                'SCREEN IMAGE HAS ZERO DIMENSIONS'
                            )
                        );
                        return;
                    }
                ...
                image.onerror=()=>finish(
                    reject,
                    new Error(
                        'SCREEN IMAGE DIRECT LOAD FAILED: '+url
                    )
                );
            ...
            if(
                ...
                'SCREEN IMAGE DECODE FAILED'
            );

            // The JPEG is complete.  It is NOT yet navigation-ready
            // until AVM/WCS authority completes.
            setHdStatus(
                destination,
                'PREPARING',
                'SCREEN',
...
`;
    ...
    #updateTravelHudDistance(millionLy) {
      ...
      const shown =
        this.#formatTravelHudDistance(millionLy);
...
function pumpPreparationPhases(){
  ...
  if(headAladin.state!=='READY'){
    queueAladinPreparation(head);
  }
  ...
  /*
   * STARTUP CONTRACT:
   * The settled JSON catalogs feed Navigation/Random first.
   * FUTURE[0] is then the ONLY galaxy allowed into HD/AVM
   * preparation until that authoritative first destination is
   * completely presentation-ready.
   */
  if(!headReady){
  ...
  /*
   * After FUTURE[0] releases the startup readiness gate, restore
   * the established ten-destination / three-worker background
   * preparation behavior for the remaining future window.
   */
  let allHdReady=headReady;
