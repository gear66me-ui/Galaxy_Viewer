...
/*
 * RANDOM GALAXY 0092 — NAVIGATION 0008 SEAM
 *
...
function navigation0008Url(){
  ...
  if(owner?.src){
    return new URL(
      '../navigation/gv-navigation-0008.js',
      owner.src
    ).href;
  }

  return '/viewer/modules/navigation/gv-navigation-0008.js';
}

function applyNavigation0007RuntimeConfig(api=null){
...
function ensureNavigationModule0007(){
  const existing=window.GalaxyViewerNavigation;

  if(existing?.VERSION==='0008'){
    applyNavigation0007RuntimeConfig(existing);
    return Promise.resolve(existing);
  }

  if(existing && existing.VERSION!=='0008'){
  ...
  navigationModulePromise=new Promise((resolve,reject)=>{
    ...
    const finish=()=>{
      ...
      if(api?.VERSION!=='0008'){
        reject(
          new Error(
            'NAVIGATION 0008 EXPORT MISSING OR VERSION MISMATCH'
          )
        );
        return;
      }

      applyNavigation0007RuntimeConfig(api);
      resolve(api);
    };
    ...
    if(prior){
      ...
      prior.addEventListener(
        'error',
        ()=>reject(
          new Error('NAVIGATION 0008 SCRIPT LOAD FAILED')
        ),
        {once:true}
      );
      return;
    ...
    script.addEventListener(
      'error',
      ()=>reject(
        new Error(`NAVIGATION 0008 SCRIPT LOAD FAILED: ${url}`)
      ),
      {once:true}
    );
...
async function ensureNavigationPlanner(anchorDestination){
  ...
  navigationPlannerPromise=(async()=>{
    const api=await ensureNavigationModule0007();

    if(typeof api.create!=='function')
      throw new Error('NAVIGATION 0008 CREATE API MISSING');
    ...
    if(
      ...
      throw new Error('NAVIGATION 0008 PLANNER API INCOMPLETE');
    }
    ...
    if(navigationPlanner){
      queueMicrotask(()=>{
        try{
          ...
          console.error(
            'RANDOM 0105 NAVIGATION 0008 RECONCILE FAILURE',
            error
          );
        }
      });
...
async function advanceNavigationPlannerAfterArrival(arrived){
  ...
  if(!expected || keyOf(expected)!==arrivedKey){
    throw new Error(
      'RANDOM 0105 NAVIGATION 0008 COMMIT IDENTITY MISMATCH'
    );
  }
...
function reconcileFutureQueue(){
  ...
  if(!bootstrapDestinationKey){
    ...
    if(bootstrap&&addFuture(bootstrap)){
      ...
      applyNavigation0007RuntimeConfig();

      void ensureNavigationPlanner(bootstrap).catch(error=>{
        console.error(
          'RANDOM 0105 NAVIGATION 0008 INITIALIZATION FAILURE',
          error
        );
      });
    }
...
async function requestRandomNavigation(){
  ...
  try{
    ...
    try{
      ...
      console.error(
        'RANDOM 0105 NAVIGATION 0008 POST-ARRIVAL FAILURE',
        error
      );
      ...
      void ensureNavigationPlanner(arrived).catch(recoveryError=>{
        console.error(
          'RANDOM 0105 NAVIGATION 0008 RECOVERY FAILURE',
          recoveryError
        );
      });
    }
