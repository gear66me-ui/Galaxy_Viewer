...
/*
 * RANDOM GALAXY 0092 — NAVIGATION 0003 SEAM
 *
 * Random Galaxy owns preparation, FIFO locking, travel, history and
 * commit/rollback. Navigation 0003 owns destination ordering after the
...
function navigation0003Url(){
  ...
  if(owner?.src){
    return new URL(
      '../navigation/gv-navigation-0003.js',
      owner.src
    ).href;
  }

  return '/viewer/modules/navigation/gv-navigation-0003.js';
}

function applyNavigation0003RuntimeConfig(api=null){
...
function ensureNavigationModule0003(){
  const existing=window.GalaxyViewerNavigation;

  if(existing?.VERSION==='0003'){
    applyNavigation0003RuntimeConfig(existing);
    return Promise.resolve(existing);
  }

  if(existing && existing.VERSION!=='0003'){
  ...
  navigationModulePromise=new Promise((resolve,reject)=>{
    const url=navigation0003Url();

    const prior=[...document.scripts].find(script=>
      script.dataset.gvNavigation0003==='1'
    );
    ...
    const finish=()=>{
      ...
      if(api?.VERSION!=='0003'){
        reject(
          new Error(
            'NAVIGATION 0003 EXPORT MISSING OR VERSION MISMATCH'
          )
        );
        return;
      }

      applyNavigation0003RuntimeConfig(api);
      resolve(api);
    };
    ...
    if(prior){
      ...
      prior.addEventListener(
        'error',
        ()=>reject(
          new Error('NAVIGATION 0003 SCRIPT LOAD FAILED')
        ),
        {once:true}
      );
      return;
    ...
    script.addEventListener(
      'error',
      ()=>reject(
        new Error(`NAVIGATION 0003 SCRIPT LOAD FAILED: ${url}`)
      ),
      {once:true}
    );
...
async function ensureNavigationPlanner(anchorDestination){
  ...
  navigationPlannerPromise=(async()=>{
    const api=await ensureNavigationModule0003();

    if(typeof api.create!=='function')
      throw new Error('NAVIGATION 0003 CREATE API MISSING');
    ...
    if(
      ...
      throw new Error('NAVIGATION 0003 PLANNER API INCOMPLETE');
    }
    ...
    if(navigationPlanner){
      queueMicrotask(()=>{
        try{
          ...
          console.error(
            'RANDOM 0092 NAVIGATION 0003 RECONCILE FAILURE',
            error
          );
        }
      });
...
async function advanceNavigationPlannerAfterArrival(arrived){
  ...
  if(!expected || keyOf(expected)!==arrivedKey){
    throw new Error(
      'RANDOM 0092 NAVIGATION 0003 COMMIT IDENTITY MISMATCH'
    );
  }
...
function reconcileFutureQueue(){
  ...
  if(!bootstrapDestinationKey){
    ...
    if(bootstrap&&addFuture(bootstrap)){
      ...
      applyNavigation0003RuntimeConfig();

      void ensureNavigationPlanner(bootstrap).catch(error=>{
        console.error(
          'RANDOM 0092 NAVIGATION 0003 INITIALIZATION FAILURE',
          error
        );
      });
    }
  ...
  /*
   * After the bootstrap slot, Navigation 0003 exclusively determines
...
async function requestRandomNavigation(){
  ...
  try{
    ...
    /*
     * Advance Navigation 0003 only after physical travel succeeded and
    ...
    try{
      ...
      console.error(
        'RANDOM 0092 NAVIGATION 0003 POST-ARRIVAL FAILURE',
        error
      );
      ...
      void ensureNavigationPlanner(arrived).catch(recoveryError=>{
        console.error(
          'RANDOM 0092 NAVIGATION 0003 RECOVERY FAILURE',
          recoveryError
        );
      });
    }
