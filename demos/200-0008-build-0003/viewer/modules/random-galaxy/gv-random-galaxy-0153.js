...
function updateStartupRandomButton(){
  ...
  const headHd=head?hdStateFor(head):null;
  const headPresentationReady=Boolean(
    headHd?.state==='READY' &&
    headHd?.presentationReady
  );

  if(ready<STARTUP_HD_READY_MIN){
    setRandomButtonText(
...
function hasReadyNavigation(){
  ...
  return randomNavigationWindow.isNextReady(bundle=>{
    ...
    return Boolean(bundle.hd?.resource)&&
      bundle.hd?.state==='READY'&&
      bundle.hd?.presentationReady===true&&
      bundle.aladin?.state==='READY';
  });
}
function updateRandomButtonReadyState(){
  ...
  if(
    ...
    setRandomButtonText(core.randomGalaxyButton,'PREPARING',{busy:true});
  }
}
