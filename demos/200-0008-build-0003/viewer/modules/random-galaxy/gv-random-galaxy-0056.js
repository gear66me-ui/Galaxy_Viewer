...
let randomInputTrace={documentDown:0,buttonDown:0,click:0,request:0,ready:null,lockAttempts:0,lockSuccesses:0,lastDocument:null,lastButton:null,lastClick:null,lastRequest:null,lastLock:null};
const inputElement=el=>el?`${String(el.tagName||'').toLowerCase()}${el.id?'#'+el.id:''}${el.className?'.'+String(el.className).trim().replace(/\s+/g,'.'):''}`:'';
...
async function requestRandomNavigation(){
  const busy=Boolean(randomGalaxy?.getState?.().busy);
  randomInputTrace.request++;
  randomInputTrace.lastRequest={at:Date.now(),pending:Boolean(randomRequestPending),busy,ready:null,exit:''};
  if(randomRequestPending||busy){randomInputTrace.lastRequest.exit='BUSY_OR_PENDING';return null;}
  authoritativePendingDestination=null;
  randomGalaxy.prefetchedDestination=null;
  randomGalaxy.prefetchPromise=null;
  reconcileFutureQueue();

  const ready=hasReadyNavigation();
  randomInputTrace.ready=ready;
  randomInputTrace.lastRequest={...randomInputTrace.lastRequest,ready,exit:ready?'READY':'NOT_READY'};
  if(!ready){
    updateRandomButtonReadyState();
    return null;
  }

  const expectedHead=randomNavigationWindow.peekNext?.()||null;
  if(!expectedHead){
    updateRandomButtonReadyState();
    return null;
  }
  ...
  try{
    randomInputTrace.lockAttempts++;
    const locked=randomNavigationWindow.lockNext?.()||null;
    randomInputTrace.lastLock={at:Date.now(),key:String(locked?.key||keyOf(locked)||'')};
    if(!locked||keyOf(locked)!==keyOf(expectedHead)){
      randomInputTrace.lastRequest.exit='LOCK_FAILED';
      throw new Error("RANDOM FUTURE0 IMMEDIATE LOCK FAILED");
    }
    randomInputTrace.lockSuccesses++;
    randomInputTrace.lastRequest.exit='LOCKED';

    core.requestHdPrefetch?.(locked.destination);
    queueAladinPreparation(locked);

    await waitForLockedReady(locked,serial);

    const destination=await consumeNext(randomGalaxy?.currentGalaxy?.name||'');
    if(!destination||keyOf(destination)!==keyOf(expectedHead)){
      throw new Error("RANDOM LOCKED FUTURE0 HANDOFF MISMATCH");
    }
    ...
    if(randomNavigationWindow.getState().locked)
      randomNavigationWindow.rollbackLocked();
    throw error;
  }finally{
...
function telemetry(){
  ...
  return Object.freeze({
    ...
    navigation:Object.freeze({
      ...
      inputBoundary:Object.freeze({...randomInputTrace})
    }),
...
function install(){
  ...
  const rb=core.randomGalaxyButton;
  document.addEventListener('pointerdown',event=>{
    const rect=rb?.getBoundingClientRect?.(); if(!rect)return;
    const x=event.clientX,y=event.clientY;
    if(x<rect.left||x>rect.right||y<rect.top||y>rect.bottom)return;
    randomInputTrace.documentDown++;
    randomInputTrace.lastDocument={at:Date.now(),target:inputElement(event.target),hit:inputElement(document.elementFromPoint(x,y)),disabled:Boolean(rb.disabled)};
  },true);
  rb?.addEventListener('pointerdown',event=>{
    randomInputTrace.buttonDown++;
    randomInputTrace.lastButton={at:Date.now(),target:inputElement(event.target),disabled:Boolean(rb.disabled)};
  },true);
  core.randomGalaxyButton?.addEventListener('click',event=>{
    randomInputTrace.click++;
    randomInputTrace.lastClick={at:Date.now(),target:inputElement(event.target),disabled:Boolean(rb.disabled)};
    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation();
