/*
RANDOM GALAXY 0199 — ZERO-WAIT PRETRAVEL STAGING
- FUTURE[0] invisible AVM/WCS layer is staged before RANDOM GALAXY becomes clickable.
- Click-time AVM preparation await is removed; click only verifies the layer is already staged.
- First accepted tap still gives immediate PREPARING TRIP + comet feedback.
- HD/AVM presentation readiness remains the Random gate; Aladin prewarm remains non-gating.
...
function hasReadyNavigation(){
  ...
  return randomNavigationWindow.isNextReady(bundle=>{
    ...
    if(isHeadAvmStaged(bundle))return true;

    ensureHeadAvmStaged(bundle);
    return false;
  });
}
...
function updateRandomButtonReadyState(){
  ...
  if(head?.destination&&head?.key){
    ...
    if(hdReady){
      headReady=isHeadAvmStaged(head);
...
async function requestRandomNavigation(){
  ...
  if(!preflightReady){
    ...
    GV_TRACE.enabled&&gvTrace(5946,'REQUEST_BLOCKED_NOT_READY',{
      ...
      avmStaged:Boolean(isHeadAvmStaged(record))
    });
    return null;
  }
  ...
  const avmInstalled=avmApi.isPreparedForTravel(destination)===true;
  if(!avmInstalled){
    randomNavigationWindow.rollbackPending?.();
    throw new Error('AR119 VISIBLE AVM PRETRAVEL WAS NOT STAGED BEFORE CLAIM');
  }
