...
/*
 * RANDOM GALAXY 0092 — NAVIGATION 0006 SEAM
 *
...
function navigation0006Url(){
  ...
  if(owner?.src){
    return new URL(
      '../navigation/gv-navigation-0006.js',
      owner.src
    ).href;
  }

  return '/viewer/modules/navigation/gv-navigation-0006.js';
}

function applyNavigation0006RuntimeConfig(api=null){
...
function ensureNavigationModule0006(){
  const existing=window.GalaxyViewerNavigation;

  if(existing?.VERSION==='0006'){
    applyNavigation0006RuntimeConfig(existing);
    return Promise.resolve(existing);
  }

  if(existing && existing.VERSION!=='0006'){
  ...
  navigationModulePromise=new Promise((resolve,reject)=>{
    const url=navigation0006Url();

    const prior=[...document.scripts].find(script=>
      script.dataset.gvNavigation0006==='1'
    );
    ...
    const finish=()=>{
      ...
      if(api?.VERSION!=='0006'){
        reject(
          new Error(
            'NAVIGATION 0006 EXPORT MISSING OR VERSION MISMATCH'
          )
        );
        return;
      }

      applyNavigation0006RuntimeConfig(api);
      resolve(api);
    };
    ...
    if(prior){
      ...
      prior.addEventListener(
        'error',
        ()=>reject(
          new Error('NAVIGATION 0006 SCRIPT LOAD FAILED')
        ),
        {once:true}
      );
      return;
    ...
    script.addEventListener(
      'error',
      ()=>reject(
        new Error(`NAVIGATION 0006 SCRIPT LOAD FAILED: ${url}`)
      ),
      {once:true}
    );
...
async function ensureNavigationPlanner(anchorDestination){
  ...
  navigationPlannerPromise=(async()=>{
    const api=await ensureNavigationModule0006();

    if(typeof api.create!=='function')
      throw new Error('NAVIGATION 0006 CREATE API MISSING');
    ...
    if(
      ...
      throw new Error('NAVIGATION 0006 PLANNER API INCOMPLETE');
    }
    ...
    if(navigationPlanner){
      queueMicrotask(()=>{
        try{
          ...
          console.error(
            'RANDOM 0104 NAVIGATION 0006 RECONCILE FAILURE',
            error
          );
        }
      });
...
async function advanceNavigationPlannerAfterArrival(arrived){
  ...
  if(!expected || keyOf(expected)!==arrivedKey){
    throw new Error(
      'RANDOM 0104 NAVIGATION 0006 COMMIT IDENTITY MISMATCH'
    );
  }
...
function reconcileFutureQueue(){
  ...
  if(!bootstrapDestinationKey){
    ...
    if(bootstrap&&addFuture(bootstrap)){
      ...
      applyNavigation0006RuntimeConfig();

      void ensureNavigationPlanner(bootstrap).catch(error=>{
        console.error(
          'RANDOM 0104 NAVIGATION 0006 INITIALIZATION FAILURE',
          error
        );
      });
    }
...
async function requestRandomNavigation(){
  ...
  /*
   * RANDOM 0102 — HD and Random navigation are mutually exclusive.
   ...
   */
  {
    ...
    if(
      ...
      global.GalaxyBlackBox?.recordCheckpoint?.('RANDOM_REQUEST_CANCELLED_HD_OWNER',{
  ...
  try{
    ...
    try{
      ...
      console.error(
        'RANDOM 0104 NAVIGATION 0006 POST-ARRIVAL FAILURE',
        error
      );
      ...
      void ensureNavigationPlanner(arrived).catch(recoveryError=>{
        console.error(
          'RANDOM 0104 NAVIGATION 0006 RECOVERY FAILURE',
          recoveryError
        );
      });
    }
