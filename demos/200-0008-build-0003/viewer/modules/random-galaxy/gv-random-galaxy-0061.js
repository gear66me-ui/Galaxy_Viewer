...
`;
  ...
  // REQ-017A / ECO-026F-A
  // Random Galaxy 0061 owns the active preparation engine.
...
`;
    ...
    async #getProviderCandidate(excludeName) {
      if (!this.provider) throw new Error('Random Galaxy 0061 requires a local galaxy provider.');
    ...
    installPreparationEngine(options = {}) {
      ...
      if(!randomNavigationWindow)
        throw new Error('RANDOM GALAXY 0061 NAVIGATION WINDOW MISSING');
...
async function consumeNext(excludeName=''){
  ...
  const exactReceipt=await preparationEngine.ensureAladinPreparedForNavigation?.(destination);
...
async function requestRandomNavigation(){
  ...
  try{
    ...
    preparationEngine.requestHdPrefetch?.(locked.destination);
    ...
    if(!arrived||pending?.kind!=='random')
      throw new Error("RANDOM NAVIGATION COMMIT INVARIANT FAILED");
    randomNavigationWindow.commitPending(arrived);
    updateHistoryControls();
    return arrived;
  }catch(error){
    const state=randomNavigationWindow.getState();
    if(state.locked)randomNavigationWindow.rollbackLocked();
    if(state.pending)randomNavigationWindow.rollbackPending();
    updateHistoryControls();
    throw error;
  }finally{
...
function install(){
  ...
  if(!preparationEngine)
    throw new Error('RANDOM GALAXY 0061 PREPARATION ENGINE MISSING');

  if(!archiveIntegration)
    throw new Error('RANDOM GALAXY 0061 HD/ARCHIVE INTEGRATION MISSING');
