/*
RANDOM GALAXY 0210 — DIRECT AVM BLOB METADATA FAST PATH
- Reads AVM/XMP directly from the already-downloaded JPEG Blob.
- Bypasses hidden Aladin A.image() for normal AVM authority extraction.
- Keeps the existing hidden-Aladin path as a compatibility fallback.
- Does not change AVM 0048 visible blending.
- Does not add an AVM sidecar database.
*/
/*
...
icon.style.setProperty('--gv-bts-phase-delay',phaseDelay);
  ...
  function createRandomPreparationEngine(options = {}) {
    ...
    const aladinPreparedReceipts=new Map();
        ...
        async function prepareHdDestinationFetch(destination,signal=null){
            ...
            for(const source of sources){
                try{
                    ...
                    return {key:destinationKey(destination),destination,image:prepared.image,blob,objectUrl:prepared.objectUrl,sourceUrl:source.url,sourceKind:source.kind};
                }catch(error){
                    ...
                    try{
                        ...
                        setHdStatus(destination,'READY',source.kind);
                        return {key:destinationKey(destination),destination,image:prepared.image,blob:null,objectUrl:prepared.objectUrl,sourceUrl:source.url,sourceKind:source.kind};
                    }catch(directError){
        ...
        async function prepareRuntimeAvmAuthorityViaAladin(destination,preparedImageUrl=''){
        ...
        function avmFastNumbers(value){
            if(value==null)return [];
            const raw=Array.isArray(value)?value:[value];
            const out=[];
            for(const item of raw){
                const matches=String(item??'').match(
                    /[-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?/g
                )||[];
        ...
        function avmFastXmlValue(doc,name){
            const all=Array.from(doc.getElementsByTagName('*'));

            for(const element of all){
                for(const attr of Array.from(element.attributes||[])){
                    const local=String(attr.localName||attr.name||'');
                    const node=String(attr.name||'');
                    if(local===name||node===name||node.endsWith(':'+name))
                        return attr.value;
                }
            }

            for(const element of all){
                const local=String(element.localName||element.nodeName||'');
                const node=String(element.nodeName||'');
                ...
                const values=Array.from(element.getElementsByTagName('*'))
                    .filter(child=>{
                        const c=String(child.localName||child.nodeName||'');
                        return c==='li'||String(child.nodeName||'').endsWith(':li');
                    })
        ...
        function avmFastPacketFromText(text){
            const starts=[
                ['<x:xmpmeta','</x:xmpmeta>'],
                ['<xmpmeta','</xmpmeta>']
            ];

            for(const [startToken,endToken] of starts){
        ...
        async function extractAvmWcsDirectFromBlob(blob){
            if(!(blob instanceof Blob)||blob.size<1)
                throw new Error('DIRECT AVM BLOB MISSING');

            /*
             * JPEG APP1/XMP normally lives near the beginning of the file.
             * Reading 256 KiB avoids converting a multi-megabyte image into
             * another complete ArrayBuffer just to recover a few KB of XML.
             */
            const prefixBytes=Math.min(blob.size,256*1024);
            const prefix=await blob.slice(0,prefixBytes).arrayBuffer();
            const text=new TextDecoder('utf-8',{fatal:false}).decode(prefix);
            const packet=avmFastPacketFromText(text);

            if(!packet)
                throw new Error('DIRECT AVM XMP PACKET NOT FOUND IN JPEG HEADER');

            const doc=new DOMParser().parseFromString(packet,'application/xml');

            if(doc.querySelector('parsererror'))
                throw new Error('DIRECT AVM XMP XML PARSE FAILED');

            const rv=avmFastNumbers(
                avmFastXmlValue(doc,'Spatial.ReferenceValue')
            );
            const rp=avmFastNumbers(
                avmFastXmlValue(doc,'Spatial.ReferencePixel')
            );
            const rd=avmFastNumbers(
                avmFastXmlValue(doc,'Spatial.ReferenceDimension')
            );
            const scale=avmFastNumbers(
                avmFastXmlValue(doc,'Spatial.Scale')
            );
            const cd=avmFastNumbers(
                avmFastXmlValue(doc,'Spatial.CDMatrix')
            );
            const rotation=avmFastNumbers(
                avmFastXmlValue(doc,'Spatial.Rotation')
            );
            const equinox=avmFastNumbers(
                avmFastXmlValue(doc,'Spatial.Equinox')
            );
            const frame=String(
                avmFastXmlValue(doc,'Spatial.CoordinateFrame')||'ICRS'
            ).trim();
            const projection=String(
                avmFastXmlValue(doc,'Spatial.CoordsystemProjection')||'TAN'
            ).trim();
            ...
            if(
                ...
                rp.length<2 ||
                rd.length<2 ||
                (!(scale.length>=2)&&!(cd.length>=4))
            )throw new Error('DIRECT AVM REQUIRED SPATIAL TAGS MISSING');
            ...
            const wcs={
                ...
                CRVAL2:rv[1],
                CTYPE1:
                    (frame.toUpperCase()==='GAL'?'GLON-':'RA---')+
                    projection,
                CTYPE2:
                    (frame.toUpperCase()==='GAL'?'GLAT-':'DEC--')+
                    projection
            };

            if(equinox.length)wcs.EQUINOX=equinox[0];
            ...
            if(cd.length>=4){
                ...
                wcs.CDELT1=scale[0];
                wcs.CDELT2=scale[1];
                if(rotation.length)wcs.CROTA2=rotation[0];
            }

            return wcs;
        }

        async function prepareRuntimeAvmAuthority(
            destination,
            preparedBlob=null,
            preparedImageUrl=''
        ){
            ...
            if(!key)
                throw new Error('AVM AUTHORITY DESTINATION KEY MISSING');

            if(!authorityUrl)
                throw new Error('AVM AUTHORITY IMAGE URL MISSING');
            ...
            if(preparedBlob instanceof Blob){
                try{
                    const wcs=await extractAvmWcsDirectFromBlob(preparedBlob);

                    const avmRa=Number(wcs.CRVAL1);
                    const avmDec=Number(wcs.CRVAL2);
                    const rawRotation=avmRawRotationFromWcs(wcs);
                    const cameraRotation=
                        normalizeAvmCameraRotation(rawRotation);
                    const horizontalFov=
                        avmHorizontalFovFromWcs(wcs,null);
                    const verticalFov=
                        avmVerticalFovFromWcs(wcs,null);

                    if(!Number.isFinite(avmRa)||avmRa<0||avmRa>=360)
                        throw new Error('DIRECT AVM RA INVALID');

                    if(!Number.isFinite(avmDec)||avmDec<-90||avmDec>90)
                        throw new Error('DIRECT AVM DEC INVALID');

                    if(
                        !Number.isFinite(horizontalFov)||
                        horizontalFov<=0
                    )throw new Error('DIRECT AVM HORIZONTAL FoV INVALID');

                    if(
                        !Number.isFinite(verticalFov)||
                        verticalFov<=0
                    )throw new Error('DIRECT AVM VERTICAL FoV INVALID');

                    if(
                        !Number.isFinite(rawRotation)||
                        !Number.isFinite(cameraRotation)
                    )throw new Error('DIRECT AVM ROTATION INVALID');

                    /*
                     * 0210 PERFORMANCE TEST:
                     * preserve 0209 FoV semantics exactly.
                     * The separate X-authoritative FoV correction is NOT mixed
                     * into this experiment, so performance is the only variable.
                     */
                    const authority=Object.freeze({
                        ...
                        avmVerticalFovDegrees:verticalFov,
                        avmCallbackFov:
                            Math.max(horizontalFov,verticalFov),
                        avmRotation:rawRotation,
                        avmCameraRotation:cameraRotation,
                        avmAuthorityUrl:authorityUrl,
                        avmAuthority:'RUNTIME_IMAGE_AVM',
                        avmAuthorityWcs:Object.freeze({...wcs})
                    });
                    ...
                    GV_TRACE.enabled&&gvTrace(
                        21001,
                        'RANDOM_0210_DIRECT_AVM_BLOB_READY',
                        {
                            key,
                            bytes:Number(preparedBlob.size)||0,
                            elapsedMs:
                                Math.round(
                    ...
                    GV_TRACE.enabled&&gvTrace(
                        21002,
                        'RANDOM_0210_DIRECT_AVM_BLOB_FALLBACK',
                        {
                            key,
                            bytes:Number(preparedBlob.size)||0,
                            elapsedMs:
            ...
            /*
             * Compatibility fallback for PNGs, unusual XMP placement,
             * CORS-direct Image paths, or malformed AVM packets.
             */
            return prepareRuntimeAvmAuthorityViaAladin(
                destination,
                preparedImageUrl
            );
        }
...
function pumpPreparationPhases(){
  ...
  const headReady=Boolean(headHd.state==='READY'&&headHd.presentationReady);

  if(headAladin.state!=='READY')queueAladinPreparation(head);

  if(headReady)ensureHeadAvmStaged(head);
  else preparationEngine?.requestHdPrefetch?.(head.destination,true);

  // FUTURE[0] stays priority; workers 2/3 prepare the next two trips immediately.
  if(!startupHdGateReleased){
    for(const record of future.slice(1,3)){
      const hd=hdStateFor(record);
      if(!(hd.state==='READY'&&hd.presentationReady))
  ...
  // Start WCS work for each destination as soon as that destination's HD is ready.
  for(const record of future.slice(1)){
    const hd=hdStateFor(record);
    const ready=Boolean(hd.state==='READY'&&hd.presentationReady);
    if(!ready){
      preparationEngine?.requestHdPrefetch?.(record.destination,false);
      continue;
    }
    const aladin=aladinStateFor(record);
    if(aladin.state!=='READY')queueAladinPreparation(record);
  }
