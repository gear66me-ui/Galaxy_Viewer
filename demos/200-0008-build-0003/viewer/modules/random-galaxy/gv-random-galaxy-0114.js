  ...
  navigationModulePromise=new Promise((resolve,reject)=>{
    ...
    const finish=()=>{
      ...
      if(api?.VERSION!=='0017'){
        reject(
          new Error(
            'NAVIGATION 0017 EXPORT MISSING OR VERSION MISMATCH'
          )
        );
        return;
      }
    ...
    if(prior){
      ...
      prior.addEventListener(
        'error',
        ()=>reject(
          new Error('NAVIGATION 0017 SCRIPT LOAD FAILED')
        ),
        {once:true}
      );
      return;
    ...
    script.addEventListener(
      'error',
      ()=>reject(
        new Error(`NAVIGATION 0017 SCRIPT LOAD FAILED: ${url}`)
      ),
      {once:true}
    );
...
async function ensureNavigationPlanner(anchorDestination){
  ...
  navigationPlannerPromise=(async()=>{
    ...
    if(typeof api.create!=='function')
      throw new Error('NAVIGATION 0017 CREATE API MISSING');
    ...
    if(
      ...
      throw new Error('NAVIGATION 0017 PLANNER API INCOMPLETE');
    }
    ...
    if(navigationPlanner){
      queueMicrotask(()=>{
        try{
          ...
          console.error(
            'RANDOM 0105 NAVIGATION 0017 RECONCILE FAILURE',
            error
          );
        }
      });
...
async function advanceNavigationPlannerAfterArrival(arrived){
  ...
  if(!expected || keyOf(expected)!==arrivedKey){
    throw new Error(
      'RANDOM 0105 NAVIGATION 0017 COMMIT IDENTITY MISMATCH'
    );
  }
  ...
  if(!bootstrapDestinationKey){
    ...
    if(bootstrap&&addFuture(bootstrap)){
      ...
      void ensureNavigationPlanner(bootstrap).catch(error=>{
        console.error(
          'RANDOM 0105 NAVIGATION 0017 INITIALIZATION FAILURE',
          error
        );
      });
    }
