...
(function (global) {
  ...
  const GV_TRACE=global.__GV_RANDOM_EXEC_TRACE__={
    version:'0067',
    source:'gv-random-galaxy-0066.js',
...
`;
  ...
  function createEarthReturnController(options = {}) {
    ...
    const updateEarthReturnIndicator=()=>{
        ...
        if(useHdEnvelope){
            ...
            safeLeft=apertureLeft;
            safeRight=apertureRight;
        ...
        const arrowCenterScale=Math.max(0,edgeScale-arrowTipInset);
        const x=apertureCenterX+dx*arrowCenterScale;
        const y=apertureCenterY+dy*arrowCenterScale;
        ...
        const minimumDistance=arrowBodyLength+calloutGap+iconRadialSupport;
        const edgeX=x+dx*arrowTipInset;
        const edgeY=y+dy*arrowTipInset;
        ...
        const startX=edgeX-iconCenterOffsetX;
        const startY=edgeY-iconCenterOffsetY;
        const axisInterval=(start,direction,min,max)=>{
            if(Math.abs(direction)<=0.001)
                return start>=min&&start<=max?[0,Infinity]:[Infinity,-Infinity];
            const a=(start-max)/direction;
            const b=(start-min)/direction;
            return [Math.min(a,b),Math.max(a,b)];
        };
        const [xMin,xMax]=axisInterval(startX,dx,minX,maxX);
        const [yMin,yMax]=axisInterval(startY,dy,minY,maxY);
        const radialMin=Math.max(0,minimumDistance,xMin,yMin);
        const radialMax=Math.min(xMax,yMax);
        let readoutX,readoutY;
        if(Number.isFinite(radialMin)&&radialMin<=radialMax){
            readoutX=startX-dx*radialMin;
            readoutY=startY-dy*radialMin;
        }else{
            const desiredX=startX-dx*minimumDistance;
            const desiredY=startY-dy*minimumDistance;
            readoutX=minX<=maxX?Math.min(maxX,Math.max(minX,desiredX)):apertureCenterX;
            readoutY=minY<=maxY?Math.min(maxY,Math.max(minY,desiredY)):apertureCenterY;
        }
        const rx=readoutX-x;
        const ry=readoutY-y;
  ...
  function createRandomPreparationEngine(options = {}) {
    ...
    const aladinPreparedReceipts=new Map();
        ...
        async function prepareAladinDestination(destination,force=false){
            ...
            const finalRa=Number(destination.ra);
            const finalDec=Number(destination.dec);
            const finalFov=Number(destination.fov);
            const finalRotation=Number.isFinite(Number(destination.aladinRotation))
                ? Number(destination.aladinRotation)
                : 0;

            if(
                !Number.isFinite(finalRa) ||
                !Number.isFinite(finalDec) ||
                !Number.isFinite(finalFov) ||
                finalFov<=0
            )throw new Error('ALADIN PREWARM FINAL VIEWPORT IS INVALID');

            if(
                typeof isolated.setFrame!=='function' ||
                typeof isolated.setProjection!=='function' ||
                typeof isolated.setRotation!=='function' ||
                typeof isolated.gotoRaDec!=='function' ||
                typeof isolated.setFov!=='function' ||
                typeof isolated.getRaDec!=='function' ||
                typeof isolated.getFov!=='function' ||
                typeof isolated.getRotation!=='function' ||
                typeof isolated.getProjection!=='function'
            )throw new Error('ALADIN PREWARM EXACT VIEWPORT API IS INCOMPLETE');

            let completed=true;
            try{
                isolated.setFrame('ICRSd');
                isolated.setProjection('SIN');
                isolated.setRotation(finalRotation);
                isolated.gotoRaDec(finalRa,finalDec);
                ...
                if(
                    destination.framingCorrected===true &&
                    Number.isFinite(finalFov) &&
                    finalFov>0 &&
                    typeof isolated.setFov==='function'
                ){
                    ...
                    if(typeof isolated.setFov==='function')
                        isolated.setFov(destination.fov);
            ...
            const actualCoordinates=isolated.getRaDec();
            const actualFovValue=isolated.getFov();
            const actualRa=Number(actualCoordinates?.[0]);
            const actualDec=Number(actualCoordinates?.[1]);
            const actualFov=Number(Array.isArray(actualFovValue)?actualFovValue[0]:actualFovValue);
            const actualRotation=Number(isolated.getRotation());
            const actualProjection=String(isolated.getProjection()||'').trim().toUpperCase();
            ...
            if(
                !near(actualRa,finalRa,1e-6) ||
                !near(actualDec,finalDec,1e-6) ||
                !near(actualFov,finalFov,Math.max(1e-6,Math.abs(finalFov)*1e-6)) ||
                !near(actualRotation,finalRotation,1e-4) ||
                actualProjection!=='SIN'
            )throw new Error('ALADIN PREWARM FINAL VIEWPORT READBACK MISMATCH');
        ...
        function deriveSourceFraming(destination,sourceImage){
            ...
            try{
                ...
                const baseFov=Number(destination.fov);
                ...
                const pointDelta=pointUsable
                    ? normalizeFullRotationDelta(pointFit.rotationDelta)
                    : null;

                let rotationDelta=0;
                let rotationSource='CATALOG';

                if(pointDelta!==null){
                    if(morphologyDelta!==null){
                        const alignedMorphology=
                            alignHalfTurnToReference(
                                morphologyDelta,
                                pointDelta
                            );
                        const agreement=Math.abs(
                            normalizeFullRotationDelta(
                                pointDelta-alignedMorphology
                            )
                        );
                        ...
                        if(
                            ...
                            rotationDelta=normalizeFullRotationDelta(
                                pointDelta*.70+
                                alignedMorphology*.30
                            );
                            rotationSource='POINT_MORPH_AGREE';
                        }else{
                            rotationDelta=pointDelta;
                            rotationSource=
                ...
                }else if(morphologyDelta!==null){
                    rotationDelta=morphologyDelta;
                    rotationSource='MORPHOLOGY';
                }

                // Hidden Aladin prewarm already uses the catalog orientation
                // when one exists, so the image-derived angle is a correction
                // relative to that prior. Null catalog orientation starts at 0.
                const rotation=
                    (baseRotation??0)+rotationDelta;
                ...
                if(!centerChanged&&!scaleChanged&&!rotationChanged)
                    gvTrace(2663,'TRAVEL_RETURN',{name:String(destination?.name||'')});
        return destination;
  ...
  class GalaxyRandomGalaxy {
    constructor(options = {}) {
      ...
      gvTrace(3328,'READY_CREATED',{state:this.__gvReadyState});
      this.ready = this.#initialize();
      this.ready.then(
        ()=>{
          this.__gvReadyState='RESOLVED';
          gvTrace(3328,'READY_RESOLVED',{state:this.__gvReadyState});
        },
        error=>{
          this.__gvReadyState='REJECTED';
          gvTraceError(3328,'READY_REJECTED',error);
        }
      );
    }
...
`;
    ...
    #normalizeProviderCandidate(candidate) {
      ...
      if (fov == null || fov <= 0) throw new Error('Galaxy destination has no usable FOV.');
    ...
    async travelToRandom() {
      gvTrace(4601,'TRAVEL_ENTER',{
        readyState:this.__gvReadyState||'UNKNOWN',
        busy:Boolean(this.busy),
        destroyed:Boolean(this.destroyed)
      });
      gvTrace(4602,'PORTRAIT_REQUEST');
      requestPortraitOrientation('random-travel').catch(() => {});
      gvTrace(4603,'READY_AWAIT_BEGIN',{state:this.__gvReadyState||'UNKNOWN'});
      await this.ready;
      gvTrace(4603,'READY_AWAIT_END',{state:this.__gvReadyState||'UNKNOWN'});
      gvTrace(4604,'TRAVEL_GUARD',{busy:Boolean(this.busy),destroyed:Boolean(this.destroyed)});
      ...
      try {
        gvTrace(4614,'STATUS_FINDING');
        this.#setStatus('FINDING GALAXY');
        gvTrace(4615,'CONSUME_BEGIN');
        const destination = await this.#consumeDestination();
        gvTrace(4615,'CONSUME_END',{
        ...
        gvTrace(4616,'ACTIVE_DESTINATION_SET',{name:String(destination?.name||'')});
        this.#setTravelStatus(destination.name);
        gvTrace(4618,'ALADIN_GET_RADEC_BEGIN',null);
        const coords = this.aladin.getRaDec();
        gvTrace(4618,'ALADIN_GET_RADEC_END',{coords:Array.isArray(coords)?coords.slice(0,2):coords});
        gvTrace(4619,'ALADIN_GET_FOV_BEGIN',null);
        const fov = this.aladin.getFov();
        gvTrace(4619,'ALADIN_GET_FOV_END',{fov:Array.isArray(fov)?fov.slice(0,2):fov});
        let preservedUserRotation = 0;
        try {
          if (typeof this.aladin.getRotation === 'function') {
            gvTrace(4623,'ALADIN_GET_ROTATION_BEGIN',null);
            const liveRotation = finiteNumber(this.aladin.getRotation());
            gvTrace(4623,'ALADIN_GET_ROTATION_END',{rotation:liveRotation});
            if (liveRotation !== null) preservedUserRotation = liveRotation;
          }
        } catch (_) {
          preservedUserRotation = 0;
        }
        gvTrace(4629,'NUMERIC_STATE_BEGIN',null);
        ...
        gvTrace(4632,'NUMERIC_STATE_COMPLETE',{startRA,startDec,startFov,destinationFov});
        ...
        gvTrace(4639,'ROUTE_CALC_BEGIN',null);
        const route = routeDistanceMillionLy(source, destination);
        gvTrace(4639,'ROUTE_CALC_END',{routeValue:Number(route?.value)});
        this.#beginTravelHud(destination, route, firstHomeTrip);
        this.#showDistance(source, destination, route);
        const baseDestinationRA=finiteNumber(destination.framingBaseRa)??Number(destination.ra);
        const baseDestinationDec=finiteNumber(destination.framingBaseDec)??Number(destination.dec);
        const baseDestinationFov=finiteNumber(destination.framingBaseFov)??destinationFov;
        const targetRotation=finiteNumber(destination.aladinRotation)??preservedUserRotation;
        const duration =
          Number(firstHomeTrip ? this.options.firstHomeTravelSeconds : this.options.travelSeconds) * 1000;
        const aladinTravelUpdateMs=32;
        const turnPoint=Number(this.options.turnPoint);
        const alignmentStart=Math.max(turnPoint,.76);
        const rotationStart=preservedUserRotation;
        ...
        let lastAladinViewportUpdate=-Infinity;
        gvTrace(4658,'RAF_SETUP_COMPLETE',{baseDestinationRA,baseDestinationDec,baseDestinationFov,targetRotation,duration,turnPoint,alignmentStart});
        const started = performance.now();
        await new Promise((resolve, reject) => {
          const frame = (now) => {
            try {
            const t = Math.min(1, (now - started) / duration);
            gvTrace(4662,'RAF_FRAME_EXECUTED',{t,now});

            if(
              t < 1 &&
              now-lastAladinViewportUpdate>=aladinTravelUpdateMs
            ){
              ...
              const alignmentProgress=t<=alignmentStart
                ? 0
                : smootherstep((t-alignmentStart)/(1-alignmentStart));
              const currentFov=alignmentProgress>0
                ? Math.exp(
                    Math.log(baseTravelFov)+
                    (Math.log(correctedTravelFov)-Math.log(baseTravelFov))*alignmentProgress
                  )
                : baseTravelFov;

              gvTrace(4690,'RAF_SET_FOV',{t,currentFov});
              this.aladin.setFov(currentFov);

              if(firstHomeTrip){
                const translationProgress=smootherstep(t);
                ...
                gvTrace(4696,'RAF_GOTO_HOME_ARC',{t,travelRa,travelDec,translationProgress});
                ...
                if(
                  ...
                  gvTrace(4696,'RAF_GOTO_BASE_PROGRESS',{
                    t,
                    travelRa,
                    travelDec,
                    translationProgress
              ...
              if(
                ...
                gvTrace(4712,'RAF_ROTATE_PROGRESS',{t,rotationProgress});
                ...
                gvTrace(4722,'RAF_ROTATE_TARGET_EARLY',{t,targetRotation});
                gvTrace(4722,'RAF_ROTATE_TARGET_FINAL',{t,targetRotation});
              this.aladin.setRotation(Number(targetRotation));
                finalRotationApplied=true;
              }

              if(destinationCenterApplied&&alignmentProgress>0){
                ...
                gvTrace(4732,'RAF_GOTO_ALIGNED',{t,alignedRa,alignedDec,alignmentProgress});
                this.aladin.gotoRaDec(alignedRa,alignedDec);
              }

              lastAladinViewportUpdate=now;
            }

            gvTrace(4738,'RAF_DISTANCE_RENDER',{t});
            ...
            if(t<1){
              gvTrace(4746,'RAF_SCHEDULE_NEXT',{t});
              requestAnimationFrame(frame);
              return;
            }
            ...
            gvTrace(4764,'RAF_FINAL_GOTO',{t,ra:Number(destination.ra),dec:Number(destination.dec)});
            this.aladin.gotoRaDec(
              Number(destination.ra),
              Number(destination.dec)
            );
            gvTrace(4768,'RAF_FINAL_FOV',{t,destinationFov});
            ...
            gvTrace(4774,'RAF_RESOLVE',{t});
            resolve();
            } catch (error) {
              gvTraceError(4775,'RAF_EXCEPTION',error);
              reject(error);
            }
          };
          requestAnimationFrame(frame);
        ...
        gvTrace(4783,'ARRIVAL_PAINT_WAIT_BEGIN',null);
        await new Promise(resolve=>
          requestAnimationFrame(()=>requestAnimationFrame(resolve))
        );
        gvTrace(4783,'ARRIVAL_PAINT_WAIT_END',null);

        gvTrace(4787,'ARRIVAL_SET_CURRENT',{name:String(destination?.name||'')});
        this.currentGalaxy = { name: destination.name, ra: destination.ra, dec: destination.dec, distance: destination.distance };
        gvTrace(4788,'ARRIVAL_TRUE',null);
        this.arrived = true;
        gvTrace(4789,'ARRIVAL_BUSY_FALSE',null);
        this.busy = false;
        setRandomWaitComet(this.randomButton, false);
        this.#hideDistance();
        gvTrace(4792,'ARRIVAL_SHOW_CARD',null);
        this.#showCard(destination);
        gvTrace(4793,'ARRIVAL_STATUS',null);
        ...
        gvTrace(4800,'ARRIVAL_CALLBACK',null);
        ...
        gvTraceError(4808,'TRAVEL_EXCEPTION',error);
        this.busy = false;
        setRandomWaitComet(this.randomButton, false);
        this.arrived = false;
        this.activeDestination = null;
...
function startWeb(record){
  ...
  const state=webStateFor(record);
  if(state.state==='READY'||state.state==='PRELOADED'||state.state==='DOWNLOADING')return;
  ...
  fetch(sourceUrl,{
    ...
    .then(()=>{
      setWebState(
        record,
        'PRELOADED',
        'OPAQUE PRELOAD COMPLETE — NOT VERIFIED READY'
      );
    })
    .catch(error=>{
      if(error?.name==='AbortError'){
...
function aladinReceiptMatchesRecord(receipt,record){
  ...
  const expectedFov=Number(destination.fov);
  ...
  return String(receipt.key||'').trim().toLowerCase()===String(record.key||'').trim().toLowerCase()&&
    ...
    String(receipt.projection||'SIN').trim().toUpperCase()==='SIN';
}
...
function hdStateFor(record){
  ...
  if(
    ...
    record.aladin={
      ...
      fov:Number(preparedDestination.fov),
  ...
  return {
    state:navigationState,
    progress:navigationState==='READY'?100:null,
...
async function requestRandomNavigation(){
  gvTrace(5937,'REQUEST_ENTER',{
    pending:Boolean(randomRequestPending),
    busy:Boolean(randomGalaxy?.getState?.().busy)
  });
  /*
   * RANDOM 0065 — NAVIGATION OWNS NAVIGATION.
   *
   * Download, HD, archive and Aladin-prewarm readiness may never veto
   * visible travel.  They are paused only to protect animation smoothness.
   */
  ...
  if(
    ...
    !Number.isFinite(Number(destination?.fov))||
    Number(destination.fov)<=0
  ){
  ...
  try{
    ...
    gvTrace(5983,'REQUEST_TRAVEL_AWAIT_BEGIN');
    const arrived=await randomGalaxy.travelToRandom();
    gvTrace(5983,'REQUEST_TRAVEL_AWAIT_END',{arrived:Boolean(arrived)});
    ...
    gvTraceError(6014,'REQUEST_EXCEPTION',error);
    randomNavigationWindow.rollbackPending?.();
    activeRecord=null;
    throw error;
...
function updateHistoryControls(){
  ...
  const busy=randomRequestPending||Boolean(randomGalaxy?.getState?.().busy);
...
function installHdFeedback(){
  ...
  const handle=async event=>{
    ...
    try{await waitForHd(key,2500);randomGalaxy.showHD()}catch(error){console.error('GALAXY VIEWER PREFETCH HD ENTRY FAILURE',error);try{randomGalaxy.showHD()}catch(_){} }finally{randomGalaxy.providerIconButton.classList.remove('gv-prefetch-hd-wait');hdFeedbackBusy=false}
  };
...
function install(){
  ...
  core.randomGalaxyButton?.addEventListener('click',event=>{
    gvTrace(6309,'PHYSICAL_RANDOM_CLICK',{
    ...
    requestRandomNavigation().catch(error=>{
      gvTraceError(6313,'PHYSICAL_RANDOM_REJECTION',error);
