    ...
    const aladinPreparedReceipts=new Map();
        ...
        function scheduleAladinEnhancement(item,destination,priority=false){
            ...
            const task=async()=>{
                ...
                try{
                    item.destination=await prepareRuntimeAvmAuthority(destination,item?.objectUrl||'');
                    item.framingState='READY';
                    return true;
                }catch(error){
                    if(error?.name==='AbortError'){
...
`;
    ...
    #normalizeProviderCandidate(candidate) {
      ...
      if (avmAuthority !== 'RUNTIME_IMAGE_AVM') throw new Error('Galaxy destination lost runtime AVM authority.');
