  ...
  function createRandomPreparationEngine(options = {}) {
    ...
    const aladinPreparedReceipts=new Map();
        ...
        async function gv0217WarmCompressedImage(url,signal=null){
            ...
            if(!raw)throw new Error('HD COMPRESSED IMAGE URL MISSING');
            try{
                /*
                 * 0234: retain the compressed JPEG bytes for every FUTURE slot.
                 * 0233 only issued an opaque no-cors cache warm and discarded
                 * the response, forcing FUTURE[0] to perform a second network
                 * load before it could decode.  Keep one compressed Blob per
                 * queued destination; decoding remains lazy to control RAM.
                 */
                ...
                if(!response.ok)
                    throw new Error('HD COMPRESSED IMAGE HTTP '+response.status);
                const blob=await response.blob();
                if(!(blob instanceof Blob)||blob.size<1)
                    throw new Error('HD COMPRESSED IMAGE EMPTY');
                GV_TRACE.enabled&&gvTrace(
                    23401,
                    'RANDOM_0234_COMPRESSED_BLOB_RETAINED',
                    {
                        url:raw,
                ...
                GV_TRACE.enabled&&gvTraceError(
                    23402,
                    'RANDOM_0234_COMPRESSED_BLOB_FAILURE',
                    error
                );
                throw error;
            }
        ...
        function gv0214PreparePrecomputedAvmAuthority(
            ...
            if(
                ...
                const rawFallbackRotation=
                    Number.isFinite(Number(record?.spatialRotationDeg))
                        ? Number(record.spatialRotationDeg)
                        : Number.isFinite(Number(record?.avmRotation))
                            ? Number(record.avmRotation)
                            : 0;
            ...
            let rawRotation=Number(record?.spatialRotationDeg);
            if(!Number.isFinite(rawRotation))
                rawRotation=Number(avmRawRotationFromWcs(wcs));
        ...
        async function gv0211PrepareDirectAvmAuthority(
            ...
            if(
                ...
                !Number.isFinite(registrationRotation)
            )throw new Error(
            ...
            const authority=
                Object.freeze({
                    ...
                    aladinRotation:registrationRotation,
                    ...
                    const layer=isolatedA.image(url,{
                        ...
                        successCallback:(ra,dec,fov,image)=>{
                            try{
                                ...
                                const registrationRotation=rawRotation;
                                ...
                                if(!Number.isFinite(rawRotation)||!Number.isFinite(registrationRotation))
                                ...
                                const prepared=Object.freeze({
                                    ...
                                    avmCameraRotation:registrationRotation,
                                ...
                                GV_TRACE.enabled&&gvTrace(11801,'AR118_RUNTIME_AVM_AUTHORITY',{
                                    ...
                                    runtime:{
                                        ...
                                        registrationRotation:prepared.aladinRotation
                                    }
                                });
        ...
        function catalogImageWcsFallback(destination,item,error=null){
            ...
            const registrationRotation=Number(destination?.aladinRotation);
            const authorityUrl=avmAuthorityUrlFor(destination);
            ...
            const prepared=Object.freeze({
                ...
                avmRotation:registrationRotation,
                avmCameraRotation:registrationRotation,
                avmAuthorityUrl:authorityUrl,
        ...
        /*
         * 0231 CAMERA-AUTHORITY PURGE
         *
         * Random Galaxy does not position, zoom, rotate, project, frame,
         * or otherwise command an Aladin camera.
         *
         * Catalog RA/Dec remain destination coordinates.
         * AVM/WCS remains image-registration metadata only.
         *
         * Camera execution belongs outside Random Galaxy.
         */
        ...
        /*
         * 0235: image-derived camera framing removed.
         * Catalog RA/Dec/FoV are the only navigation framing authority.
         * AVM/WCS is retained strictly for overlay image registration.
         */
...
`;
    ...
    #r10ScreenFitFov(destination) {
      /*
       * 0234 camera-authority containment:
       * navigation framing comes from the catalog destination only.
       * AVM vertical FoV belongs to image registration and may not command
       * the Aladin camera.
       */
      const catalogFov=finiteNumber(destination?.fovDegrees);
      return catalogFov!==null && catalogFov>0 ? catalogFov : null;
    }
