...
function navigation0008Url(){
  ...
  if(owner?.src){
    return new URL(
      '../navigation/gv-navigation-0012.js',
      owner.src
    ).href;
  }

  return '/viewer/modules/navigation/gv-navigation-0012.js';
}

function applyNavigation0012RuntimeConfig(api=null){
...
function ensureNavigationModule0012(){
  const existing=window.GalaxyViewerNavigation;

  if(existing?.VERSION==='0012'){
    applyNavigation0012RuntimeConfig(existing);
    return Promise.resolve(existing);
  }

  if(existing && existing.VERSION!=='0012'){
  ...
  navigationModulePromise=new Promise((resolve,reject)=>{
    ...
    const finish=()=>{
      ...
      if(api?.VERSION!=='0012'){
        reject(
          new Error(
            'NAVIGATION 0012 EXPORT MISSING OR VERSION MISMATCH'
          )
        );
        return;
      }

      applyNavigation0012RuntimeConfig(api);
      resolve(api);
    };
    ...
    if(prior){
      ...
      prior.addEventListener(
        'error',
        ()=>reject(
          new Error('NAVIGATION 0012 SCRIPT LOAD FAILED')
        ),
        {once:true}
      );
      return;
    ...
    script.addEventListener(
      'error',
      ()=>reject(
        new Error(`NAVIGATION 0012 SCRIPT LOAD FAILED: ${url}`)
      ),
      {once:true}
    );
...
async function ensureNavigationPlanner(anchorDestination){
  ...
  navigationPlannerPromise=(async()=>{
    const api=await ensureNavigationModule0012();

    if(typeof api.create!=='function')
      throw new Error('NAVIGATION 0012 CREATE API MISSING');
    ...
    if(
      ...
      throw new Error('NAVIGATION 0012 PLANNER API INCOMPLETE');
    }
    ...
    if(navigationPlanner){
      queueMicrotask(()=>{
        try{
          ...
          console.error(
            'RANDOM 0105 NAVIGATION 0012 RECONCILE FAILURE',
            error
          );
        }
      });
...
async function advanceNavigationPlannerAfterArrival(arrived){
  ...
  if(!expected || keyOf(expected)!==arrivedKey){
    throw new Error(
      'RANDOM 0105 NAVIGATION 0012 COMMIT IDENTITY MISMATCH'
    );
  }
...
function reconcileFutureQueue(){
  ...
  if(!bootstrapDestinationKey){
    ...
    if(bootstrap&&addFuture(bootstrap)){
      ...
      applyNavigation0012RuntimeConfig();

      void ensureNavigationPlanner(bootstrap).catch(error=>{
        console.error(
          'RANDOM 0105 NAVIGATION 0012 INITIALIZATION FAILURE',
          error
        );
      });
    }
...
async function requestRandomNavigation(){
  ...
  try{
    ...
    try{
      ...
      console.error(
        'RANDOM 0105 NAVIGATION 0012 POST-ARRIVAL FAILURE',
        error
      );
      ...
      void ensureNavigationPlanner(arrived).catch(recoveryError=>{
        console.error(
          'RANDOM 0105 NAVIGATION 0012 RECOVERY FAILURE',
          recoveryError
        );
      });
    }
