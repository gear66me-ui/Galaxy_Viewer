...
`;
  ...
  function createEarthReturnController(options = {}) {
    ...
    const updateEarthReturnIndicator=()=>{
        ...
        const safeLeft=apertureLeft+visibleHalfW+4;
        const safeRight=apertureRight-visibleHalfW-4;
        const apertureTop=Math.max(
            visibleHalfH+4,
            topControlsBottom+6+visibleHalfH
        );
        const apertureBottom=Math.max(
            apertureTop+1,
            Math.min(
                rootRect.height-visibleHalfH-4,
                lowerObstacleTop-8-visibleHalfH
            )
        );
  ...
  function installHdArchiveIntegration(randomGalaxy, options = {}) {
    ...
    function providerIconUrl(destination){
        ...
        const slug=String(destination?.provider||'')
            .trim()
            .toLowerCase()
            .replace(/[^a-z0-9]+/g,'-')
            .replace(/^-+|-+$/g,'');

        if(!slug)return DEFAULT_PROVIDER_ICON_URL;
    ...
    function syncHdProviderPresentation(destination=currentArchiveDestination()){
        ...
        const label=provider||telescope||'IMAGE';
        ...
        if(buttonIcon){
            ...
            buttonIcon.alt=`${label} ARCHIVE`;
            buttonIcon.onerror=()=>{
                buttonIcon.onerror=null;
                buttonIcon.src=DEFAULT_PROVIDER_ICON_URL;
            };
        }

        hdArchiveIcon.src=iconUrl;
        hdArchiveIcon.alt=`${label} ARCHIVE`;
        hdArchiveIcon.onerror=()=>{
            hdArchiveIcon.onerror=null;
            hdArchiveIcon.src=DEFAULT_PROVIDER_ICON_URL;
        };
...
`;
    ...
    async travelToRandom() {
      ...
      try {
        ...
        const duration =
          Number(this.options.travelSeconds) * 1000;
          ...
          const frame = (now) => {
            ...
            if(
              ...
              if(
                ...
                (firstHomeTrip || t>=turnPoint)
              ){
                gvTrace(4696,'RAF_GOTO_BASE',{t,ra:baseDestinationRA,dec:baseDestinationDec});
                this.aladin.gotoRaDec(baseDestinationRA,baseDestinationDec);
...
function hdStateFor(record){
  ...
  const preparedDestination=framingReady?resource?.destination||null:null;
  if(
    state==='READY' &&
    identityMatch &&
    preparedDestination &&
  ...
  return {
    ...
    resourceKey,
    identityMatch,
    resourceUrl:String(resource?.objectUrl||resource?.url||resource?.src||'')
  };
}
...
async function requestRandomNavigation(){
  ...
  const destination=record.destination||record;
  ...
  // travelToRandom consumes this directly.  It does not need an HD,
  // archive, download or prewarm receipt in order to move Aladin.
  ...
  try{
    ...
    // History/FIFO bookkeeping occurs AFTER successful visible travel.
    // It cannot prevent travel from starting.
    try{
      const locked=randomNavigationWindow.lockNext?.()||null;

      if(locked&&keyOf(locked)===keyOf(record)){
        const claimed=randomNavigationWindow.claimLocked?.()||null;

        if(claimed&&keyOf(claimed)===keyOf(record)){
          randomNavigationWindow.commitPending?.(arrived);
        }else{
          randomNavigationWindow.rollbackPending?.();
          console.error('RANDOM 0065 POST-TRAVEL CLAIM IDENTITY MISMATCH');
        }
      }else{
        randomNavigationWindow.rollbackLocked?.();
        console.error('RANDOM 0065 POST-TRAVEL FIFO LOCK MISMATCH');
      }
    }catch(error){
      randomNavigationWindow.rollbackLocked?.();
      randomNavigationWindow.rollbackPending?.();
      console.error('RANDOM 0065 POST-TRAVEL BOOKKEEPING FAILURE',error);
    }
...
function webTelemetry(record){
  ...
  return {
    state:state.state,
    progress:state.state==='READY'?100:null,
    detail:state.detail||'',
    expectedKey,
    stateKey,
    activeKey,
    identityMatch:Boolean(stateKey===expectedKey&&(!activeKey||activeKey===expectedKey)),
    sourceUrl:String(state.sourceUrl||''),
    expectedSourceUrl:String(record?.destination?.sourceUrl||'').trim()
  };
}
