...
(async()=>{
  ...
  position:'absolute',display:'none',color:NORTH_RED,
  ...
  return null;
    ...
    function setEarthMarkerInsideReticle(bearing){
        if(!Number.isFinite(bearing))return;
        const indicator=document.getElementById('gv-earth-return-indicator');
        if(!indicator||!indicator.classList.contains('gv-visible'))return;
        const arrow=indicator.querySelector('.gv-earth-return-arrow');
        const readout=indicator.querySelector('.gv-earth-return-readout');
        if(!arrow||!readout)return;

        // Keep the existing Earth-return pointer/readout.  Only relocate the
        // complete instrument to the inside of the 125 px compass ring.
        // The arrow points outward; its tip stops ~7 px short of the ring.
        const state=reticle.gvDirectional;
        const ringRadius=125;
        const arrowCenterRadius=108;
        const readoutInset=38;
        const radians=bearing*Math.PI/180;
        const centerRect=reticle.getBoundingClientRect();
        const rootRect=root.getBoundingClientRect();
        const cx=centerRect.left-rootRect.left+state.center;
        const cy=centerRect.top-rootRect.top+state.center;
        const x=cx+Math.sin(radians)*arrowCenterRadius;
        const y=cy-Math.cos(radians)*arrowCenterRadius;

        indicator.style.left=`${x}px`;
        indicator.style.top=`${y}px`;
        // Existing green arrow is drawn pointing right at 0deg.
        arrow.style.transform=`translate(-50%,-50%) rotate(${bearing-90}deg)`;

        // Existing Earth icon + distance stay behind the arrow, toward center.
        const rx=-Math.sin(radians)*readoutInset;
        const ry= Math.cos(radians)*readoutInset;
        readout.style.transform=`translate(calc(-50% + ${rx.toFixed(1)}px),calc(-50% + ${ry.toFixed(1)}px)) rotate(${bearing}deg)`;
    }
    ...
    const updateDirectionalReticle=()=>{
        ...
        // Earth bearing algorithm is unchanged.  Apply only presentation geometry
        // after Random Galaxy has done its normal update so our compass placement wins.
        const earthBearing=readEarthBearing(aladin,root);
        if(Number.isFinite(earthBearing)){
            queueMicrotask(()=>setEarthMarkerInsideReticle(earthBearing));
        }
    };
