...
#gv-earth-return-indicator .gv-earth-return-arrow{position:absolute;left:50%;top:50%;width:17px;height:12px;transform:translate(-50%,-50%) rotate(0deg);transform-origin:50% 50%;filter:drop-shadow(0 0 3px rgba(255,216,90,.48))}
#gv-earth-return-indicator .gv-earth-return-arrow::before{content:"";position:absolute;left:1px;top:1.5px;width:0;height:0;border-top:4.5px solid transparent;border-bottom:4.5px solid transparent;border-left:14px solid #FFD85A}
...
#gv-earth-return-indicator .gv-earth-return-icon{display:block;font:12px/12px system-ui,sans-serif;filter:grayscale(1) sepia(1) saturate(8) hue-rotate(355deg) brightness(1.18) drop-shadow(0 0 3px rgba(255,216,90,.55))}
#gv-earth-return-indicator .gv-earth-return-distance{display:flex;align-items:baseline;justify-content:center;gap:2px;margin-top:2px;color:#FFD85A}
#gv-earth-return-indicator .gv-earth-return-value,#gv-earth-return-indicator .gv-earth-return-unit{display:block;color:#FFD85A;font:600 10.5px/12px "Space Age",sans-serif;letter-spacing:.35px;font-variant-numeric:tabular-nums;text-shadow:0 0 4px rgba(255,242,168,.72),0 0 7px rgba(255,180,45,.28)}
...
(async()=>{
    ...
    const updateEarthReturnIndicator=()=>{
        ...
        const apertureTop=Math.max(4,topControlsBottom+6);
        const apertureBottom=Math.min(rootRect.height-4,navRect.top-rootRect.top-6);

        const apertureCenterX=(apertureLeft+apertureRight)/2;
        const apertureCenterY=(apertureTop+apertureBottom)/2;
        const halfW=Math.max(1,(apertureRight-apertureLeft)/2);
    ...
    function restoreNormalViewerPresentation(){
        ...
        if(!navigationPending&&!randomGalaxy.getState().busy)
            bottom.random.disabled=!window.GalaxyViewerPrefetch?.hasReadyNavigation?.();
        setHistoryControls();
    }
    ...
    window.GalaxyViewerCore=Object.freeze({version:VERSION,displayVersion:DISPLAY_VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,historyBackButton:bottom.back,historyForwardButton:bottom.forward,reticle,versionLabel:bottom.version,universeContext,homeOverlay,catalogCount:catalogRecordCount,eligibleCatalogCount:galaxyCatalog.length,catalogDatabaseCounts,startupMetrics,getPrefetchState,getDownloadStatus,getAladinPrewarmState,fillPrefetchQueue,getGalaxyCatalog:()=>Object.freeze([...galaxyCatalog]),beginTravelHud,activateQueuedDestination:(destination,excludeName='')=>consumeReady(destination,excludeName)||setUnpreparedActive(destination),requestHdPrefetch:destination=>{if(!destination)return '';enqueuePrefetch(destination,true);fillPrefetchQueue();return destinationKey(destination)},isHdPrepared:key=>{const normalized=String(key||'').trim().toLowerCase();return Boolean(activePreparedItem?.key===normalized||prefetchReady.some(item=>item?.key===normalized)||historyPreparedItems.some(item=>item?.key===normalized))},isAladinPrepared:key=>aladinPrewarmedKeys.has(String(key||'').trim().toLowerCase()),getBackgroundWorkSuspended:()=>backgroundWorkSuspended,getChandraTestOverrideState:()=>Object.freeze({chandraTestOverrideActive,chandraTestRemaining:chandraTestQueue.length,chandraTestTotal}),getGalaxyHistory:()=>({index:galaxyHistoryIndex,items:galaxyHistory.map(item=>({name:item.name,archiveId:item.archiveId,provider:providerFor(item)}))})});
...
function consumeNext(excludeName=''){
  ...
  const excluded=String(excludeName||'').trim().toLowerCase();
  const readyForNavigation=record=>
    core.isHdPrepared?.(record.key)&&
    aladinStateFor(record).state==='READY';
  const index=future.findIndex(record=>
    String(record.destination?.name||'').trim().toLowerCase()!==excluded&&
    record.key!==activeRecord?.key&&
    readyForNavigation(record)
  );
  if(index<0)return null;
...
function hasReadyNavigation(){
  return future.some(record=>core.isHdPrepared?.(record.key)&&aladinStateFor(record).state==='READY');
}
...
function install(){
  ...
  randomGalaxy.provider=async args=>{
    ...
    if(!destination)throw new Error('NO FUTURE-TEN GALAXY IS BOTH HD-READY AND ALADIN-READY');
    core.beginTravelHud?.(destination);
    return destination;
  };
  ...
  installHdFeedback();
  if(core.versionLabel){core.versionLabel.textContent='VERSION 11W';core.versionLabel.setAttribute('aria-label','GALAXY VIEWER VERSION 11W')}
  window.GalaxyViewerPrefetch=Object.freeze({version:VERSION,displayVersion:VERSION,core,randomGalaxy,getPrefetchTelemetry:telemetry,reconcileFutureQueue,hasReadyNavigation});
  installed=true;
