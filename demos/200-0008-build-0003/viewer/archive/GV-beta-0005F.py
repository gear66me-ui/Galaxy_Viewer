...
# GV-beta-0005F
# Standalone centered-reticle Galaxy Viewer release based only on GV-beta-0005E.
# Preserves the 5E UI, curated-first selection, random startup, Back/Next history, and version label.
# Adds only the centered GV-reticle-0001.svg overlay.
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){console.error("GV-beta-0005F STARTUP FAILURE: window.A was not created");return;}
        ...
        window.A.init.then(async() => {
            ...
            try{
                ...
                console.error("GV-beta-0005F CATALOG FAILURE:",error);
            }
            ...
            centerReticle.src=reticleUrl+"?v=5F-reticle-001";
            centerReticle.alt="";
            ...
            versionLabel.textContent="Galaxy Viewer 5F";
            ...
            async function openGalaxy(record){
                ...
                try{
                    ...
                    console.error("GV-beta-0005F TARGET FAILURE:",error);
                }finally{
                    navigating=false;
        ...
        }).catch(error=>console.error("GV-beta-0005F STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0005F STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
