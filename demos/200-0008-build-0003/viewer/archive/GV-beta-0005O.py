...
# GV-beta-0005O
# Standalone Galaxy Viewer release based only on the corrected GV-beta-0005N.
...
# Changes only the coordinate display to four decimal places and adds a comet-like spark that orbits the 32x32px Target button while inactive, then stops and becomes a steady glow while active.
...
#aladin-cosmic-command-test button.gv-target-proxy:active{
    ...
    flex:0 0 32px!important;margin:0 0 0 -1px!important;padding:0!important;box-sizing:border-box!important;overflow:visible!important;
...
#aladin-cosmic-command-test button.gv-target-proxy .gv-target-spark{
    position:absolute!important;left:0!important;top:0!important;width:4px!important;height:4px!important;border-radius:50%!important;
    background:#FFFFFF!important;box-shadow:0 0 4px 1px #FFFFFF,0 0 8px 3px #62D8FF,-5px 0 7px 1px rgba(98,216,255,.72)!important;
    offset-path:path("M 3 3 H 29 V 29 H 3 Z")!important;offset-rotate:0deg!important;animation:gv-target-comet-orbit 2.8s linear infinite!important;
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){console.error("GV-beta-0005O STARTUP FAILURE: window.A was not created");return;}
        window.A.init.then(async() => {
            ...
            catch(error){console.error("GV-beta-0005O CATALOG FAILURE:",error);}
            ...
            centerReticle.src=reticleUrl+"?v=5O-four-decimal-comet-001";
            centerReticle.alt="";
            ...
            function createTargetProxy(){
                ...
                proxy.innerHTML=`<img src="${targetIconUrl}?v=5O-four-decimal-comet-001" alt="" aria-hidden="true" draggable="false"><span class="gv-target-spark" aria-hidden="true"></span>`;
            ...
            versionLabel.textContent="Galaxy Viewer 5O";
            ...
            async function openGalaxy(record){
                ...
                }catch(error){nameLabel.textContent="Could not resolve "+String(record.primary_name||"galaxy");console.error("GV-beta-0005O TARGET FAILURE:",error);}
        ...
        }).catch(error=>console.error("GV-beta-0005O STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0005O STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
