...
# GV-beta-0006C
# Standalone Galaxy Viewer release based only on final GV-beta-0006B.
# Preserves the approved 36 px top row and Target system, mathematically centers the 2 px active Target star, and adds a five-row prototype pull-down menu with equal 126 px label tiles, empty 36 px icon placeholders, and temporary 700 ms selection glow.
# Preserves the four-decimal coordinate display, 1,000-record catalog, Spectacular Mode, centered reticle, safe FOV floor, navigation, SIMBAD behavior, Aladin 3.8.2 configuration, artwork, catalogs, and all unrelated viewer behavior.
...
#aladin-cosmic-command-test .gv-viewer-menu-label{
    ...
    padding:0 10px!important;font:700 12px/1.15 "Roboto Mono",Consolas,monospace!important;
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){console.error("GV-beta-0006C STARTUP FAILURE: window.A was not created");return;}
        window.A.init.then(async() => {
            ...
            catch(error){console.error("GV-beta-0006C CATALOG FAILURE:",error);}
            ...
            centerReticle.src=reticleUrl+"?v=6C-menu-frame-001";
            centerReticle.alt="";
            ...
            function createTargetProxy(){
                ...
                proxy.innerHTML=`<img src="${targetIconUrl}?v=6C-menu-frame-001" alt="" aria-hidden="true" draggable="false"><span class="gv-target-comet" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i></span>`;
            ...
            function bindTargetStatus(proxy,status){
                if(status.dataset.gv6cBound==="true")return;
                status.dataset.gv6cBound="true";
            ...
            versionLabel.textContent="Galaxy Viewer 6C";
            ...
            async function openGalaxy(record){
                ...
                }catch(error){nameLabel.textContent="Could not resolve "+String(record.primary_name||"galaxy");console.error("GV-beta-0006C TARGET FAILURE:",error);}
        ...
        }).catch(error=>console.error("GV-beta-0006C STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0006C STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
