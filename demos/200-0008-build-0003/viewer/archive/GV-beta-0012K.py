...
# Galaxy Viewer 12K repair loader.
# Frozen source baseline remains viewer/archive/GV-beta-0012F.py.
# 12I behavior preserved; only Download Analytics 0003 layering repair is added.

display(HTML("""
<style>html,body{background:#000}#gv-12k-loader{position:fixed;inset:0;background:#000;z-index:1}</style>
<div id="gv-12k-loader" aria-hidden="true"></div>
"""))
...
(async()=>{
  ...
  const SOURCE=RAW+'viewer/archive/GV-beta-0012F.py?v=12K-base';
  ...
  const extract=source=>{
    ...
    if(html.length!==1||js.length!==1)throw new Error('12K FROZEN 12F EXTRACTION FAILED');
    return [html[0][1],js[0][1]];
  };
  const once=(text,from,to,label)=>{
    const first=text.indexOf(from);
    if(first<0)throw new Error(`12K PATCH MISSING: ${label}`);
    if(text.indexOf(from,first+from.length)>=0)throw new Error(`12K PATCH NOT UNIQUE: ${label}`);
    return text.slice(0,first)+to+text.slice(first+from.length);
  };
  try{
    const source=await read(SOURCE);
    let [html,js]=extract(source);
    html=html.replace("version.textContent='VERSION 12F'","version.textContent='VERSION 12K'");
    js=once(js,"const DISPLAY_VERSION='12F';","const DISPLAY_VERSION='12K';",'DISPLAY VERSION');
    js=once(js,
...
`    const waitForAuthoritativeRandomReady=async(timeoutMs=10000)=>{
    ...
    const showRandomPreparationMessage=(text='PREPARING RANDOM GALAXY')=>{
        let note=document.getElementById('gv-random-preparing-12k');
        if(!note){note=document.createElement('div');note.id='gv-random-preparing-12k';note.style.cssText='position:fixed;left:50%;bottom:62px;z-index:2147483000;transform:translateX(-50%);padding:5px 8px;border:1px solid #7CCBFF;border-radius:5px;background:linear-gradient(145deg,rgba(8,27,58,.96),rgba(11,49,119,.94));color:#9BE5FF;font:7px monospace;letter-spacing:.35px;pointer-events:none;white-space:nowrap';document.body.appendChild(note)}
...
`,
    ...
    document.getElementById('gv-12k-loader')?.remove();
    document.body.insertAdjacentHTML('beforeend',html);
    const script=document.createElement('script');script.textContent=js;document.body.appendChild(script);
  }catch(error){
    document.getElementById('gv-12k-loader')?.remove();
    const failure=document.createElement('pre');failure.style.cssText='position:fixed;inset:0;z-index:2147483647;margin:0;padding:20px;overflow:auto;background:#000;color:#9BE5FF;font:11px monospace';failure.textContent='GALAXY VIEWER 12K FAILED TO LOAD\n\n'+String(error?.stack||error);document.body.appendChild(failure);
    throw error;
  }
})();
"""))
