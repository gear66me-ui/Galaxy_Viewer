...
(async()=>{
    ...
    const hamburger=window.GalaxyViewerHamburgerMenu.init({
        ...
        onProjectionSelected(name,detail){
            ...
            }catch(error){console.error('GV-9I PROJECTION FAILURE',name,detail?.code,error)}
        }
    });
    ...
    coordinate=window.GalaxyCoordinateOverlay.mount(coordinateHost,{onFrameChange(nextFrame){
        frame=nextFrame;
        try{if(typeof aladin.setFrame==='function')aladin.setFrame(frame==='GAL'?'galactic':'ICRSd')}catch(error){console.warn('GV-9I FRAME CHANGE WARNING',error)}
        renderCoordinates();
    }});
    ...
    function setHistoryControls(){
        const busy=navigationPending||Boolean(window.__gv9iRandomGalaxy?.getState?.().busy);
    ...
    const deferHdUntilPrepared=async event=>{
        ...
        try{
            ...
            console.error('GV-9I HUBBLE PREPARATION WAIT FAILURE',error);
...
})().catch(error=>{console.error('GALAXY VIEWER 9I STARTUP FAILURE:',error);document.dispatchEvent(new CustomEvent('gv-viewer-failed',{detail:{message:String(error?.stack||error)}}));});
"""))

# GV-beta-0010A staged