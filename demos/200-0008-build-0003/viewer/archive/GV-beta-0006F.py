...
# GV-beta-0006F
# Standalone Galaxy Viewer release based only on final GV-beta-0006E.
# Preserves the approved 36 px top row and Target system, mathematically centers the 2 px active Target star, and keeps the five-row pull-down menu with equal 126 px label tiles, empty 36 px icon placeholders, and temporary 700 ms selection glow.
# Changes only the pull-down menu label typography to the repository-hosted Space Age font while preserving all verified 6E behavior.
...
@font-face{
    ...
    src:url("https://raw.githubusercontent.com/gear66me-ui/Galaxy_Viewer/beta/viewer/artwork/Fonts/Space%20Age/space%20age.otf?v=6F-space-age-font-001") format("opentype");
...
#aladin-cosmic-command-test .gv-native-coordinate-frame{
    display:none!important;visibility:hidden!important;opacity:0!important;pointer-events:none!important;
}
...
#aladin-cosmic-command-test button.gv-menu-proxy .gv-menu-track{
    ...
    background:#D7F3FF!important;pointer-events:none!important;
    animation:gv-menu-track-pulse 3.2s ease-in-out infinite!important;
}
...
@keyframes gv-menu-track-pulse{
    0%,100%{box-shadow:0 0 2px rgba(215,243,255,.42),0 0 4px rgba(184,177,240,.22);filter:brightness(.88)}
    50%{box-shadow:0 0 4px rgba(215,243,255,.96),0 0 9px rgba(184,177,240,.78);filter:brightness(1.18)}
}
...
#aladin-cosmic-command-test .gv-viewer-menu.gv-open{
    visibility:visible!important;opacity:1!important;pointer-events:auto!important;
    transform:translateY(0)!important;transition:opacity .18s ease,transform .18s ease!important;
}
#aladin-cosmic-command-test .gv-viewer-menu-row{
    display:grid!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important;
    grid-template-columns:126px 36px!important;column-gap:2px!important;align-items:center!important;
    width:164px!important;height:36px!important;margin:0!important;padding:0!important;box-sizing:border-box!important;
}
...
#aladin-cosmic-command-test .gv-viewer-menu-label{
    width:126px!important;min-width:126px!important;max-width:126px!important;justify-content:flex-start!important;
    padding:0 10px!important;font:400 12px/1.15 "Space Age",sans-serif!important;
    letter-spacing:.55px!important;text-shadow:0 0 2px rgba(234,248,255,.58)!important;
    text-align:left!important;white-space:nowrap!important;overflow:hidden!important;
}
#aladin-cosmic-command-test .gv-viewer-menu-icon{
    width:36px!important;min-width:36px!important;max-width:36px!important;justify-content:center!important;padding:0!important;
}
#aladin-cosmic-command-test .gv-viewer-menu-row.gv-selected .gv-viewer-menu-label,
#aladin-cosmic-command-test .gv-viewer-menu-row.gv-selected .gv-viewer-menu-icon{
    background:rgba(35,31,70,.96)!important;
    box-shadow:0 0 5px rgba(240,237,255,.96),0 0 13px rgba(184,177,240,.90),inset 0 0 8px rgba(215,243,255,.24)!important;
...
#aladin-cosmic-command-test button.gv-target-proxy::before{
    content:"";position:absolute;inset:0;z-index:0;border-radius:5px;pointer-events:none;opacity:0;
}
#aladin-cosmic-command-test button.gv-target-proxy img{
    position:relative!important;z-index:2!important;display:block!important;width:34px!important;height:34px!important;object-fit:contain!important;
...
#aladin-cosmic-command-test button.gv-target-proxy[aria-pressed="true"]::before{
    ...
    box-shadow:inset 0 0 8px rgba(164,255,202,.62),inset 0 0 3px rgba(239,255,245,.70),0 0 8px rgba(85,255,136,.48);
}
...
#aladin-cosmic-command-test button.gv-target-proxy[aria-pressed="true"] .gv-target-comet i:nth-child(1){
    left:-1px!important;top:-1px!important;width:2px!important;height:2px!important;border-radius:50%!important;
    opacity:1!important;transform:none!important;background:#FFFFFF!important;
    box-shadow:0 0 2px 1px #FFFFFF,0 0 6px 2px #62D8FF!important;
}
...
@media (prefers-reduced-motion:reduce){
    #aladin-cosmic-command-test button.gv-menu-proxy .gv-menu-track{animation:none!important;box-shadow:0 0 4px rgba(215,243,255,.78)!important}
    ...
    #aladin-cosmic-command-test button.gv-target-proxy[aria-pressed="true"] .gv-target-comet{transform:translate(-50%,-50%)!important}
...
#aladin-cosmic-command-test .gv-target-status{
    grid-column:2 / 4!important;grid-row:2!important;justify-self:end!important;
    ...
    width:176px!important;min-width:176px!important;max-width:176px!important;height:36px!important;min-height:36px!important;max-height:36px!important;
    margin:8px 6px 0 0!important;padding:3px 8px!important;box-sizing:border-box!important;
    border:1px solid #B8B1F0!important;border-radius:6px!important;background:rgba(0,0,0,.88)!important;
    color:#D7F3FF!important;font:700 10px/1.12 "Roboto Mono",Consolas,monospace!important;text-align:center!important;
    white-space:nowrap!important;box-shadow:0 0 9px rgba(184,177,240,.34)!important;pointer-events:none!important;
}
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){console.error("GV-beta-0006F STARTUP FAILURE: window.A was not created");return;}
        window.A.init.then(async() => {
            ...
            catch(error){console.error("GV-beta-0006F CATALOG FAILURE:",error);}
            ...
            centerReticle.src=reticleUrl+"?v=6F-space-age-font-001";
            centerReticle.alt="";
            ...
            function getViewerMenu(){
                let panel=root.querySelector(".gv-viewer-menu");
                if(panel)return panel;
                panel=document.createElement("div");
                panel.className="gv-viewer-menu";
                panel.setAttribute("role","menu");
                panel.setAttribute("aria-label","Viewer options prototype");
                const labels=["Projection","Layers","Grid","Survey","Reticle On/Off"];
                labels.forEach(label=>{
                    ...
                    labelButton.className="gv-viewer-menu-label";
                    labelButton.setAttribute("role","menuitem");
                    labelButton.textContent=label;
                    const iconButton=document.createElement("button");
                    iconButton.type="button";
                    iconButton.className="gv-viewer-menu-icon";
                    iconButton.setAttribute("role","menuitem");
                    iconButton.setAttribute("aria-label",label+" icon placeholder");
                    const selectRow=event=>{
                        event.preventDefault();event.stopPropagation();
                        row.classList.remove("gv-selected");
                        void row.offsetWidth;
                        row.classList.add("gv-selected");
                        clearTimeout(row.gvSelectionTimer);
                        row.gvSelectionTimer=setTimeout(()=>row.classList.remove("gv-selected"),700);
                    };
                    labelButton.addEventListener("click",selectRow);
                    iconButton.addEventListener("click",selectRow);
                    row.append(labelButton,iconButton);
                    panel.appendChild(row);
                });
                root.appendChild(panel);
                return panel;
            }
            function createMenuProxy(){
                let menu=root.querySelector("button.gv-menu-proxy");
                if(menu)return menu;
                menu=document.createElement("button");
                menu.type="button";
                menu.className="gv-menu-proxy";
                menu.title="Viewer menu";
                menu.setAttribute("aria-label","Viewer menu");
                menu.setAttribute("aria-expanded","false");
                menu.innerHTML='<span class="gv-menu-track" aria-hidden="true"></span><span class="gv-menu-track" aria-hidden="true"></span><span class="gv-menu-track" aria-hidden="true"></span>';
                menu.addEventListener("click",event=>{
                    event.preventDefault();event.stopPropagation();
                    const panel=getViewerMenu();
                    const open=!panel.classList.contains("gv-open");
                    panel.classList.toggle("gv-open",open);
            ...
            function getTargetStatus(){
                ...
                status.innerHTML="<span>Select a galaxy or a star</span>";
                return status;
            }
            function createTargetProxy(){
                ...
                proxy.innerHTML=`<img src="${targetIconUrl}?v=6F-space-age-font-001" alt="" aria-hidden="true" draggable="false"><span class="gv-target-comet" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i></span>`;
            ...
            function bindTargetStatus(proxy,status){
                if(status.dataset.gv6fBound==="true")return;
                status.dataset.gv6fBound="true";
            ...
            function buildCoordinateTargetRow(){
                ...
                frame.classList.add("gv-native-coordinate-frame");
            ...
            versionLabel.textContent="Galaxy Viewer 6F";
            ...
            async function openGalaxy(record){
                ...
                }catch(error){nameLabel.textContent="Could not resolve "+String(record.primary_name||"galaxy");console.error("GV-beta-0006F TARGET FAILURE:",error);}
        ...
        }).catch(error=>console.error("GV-beta-0006F STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0006F STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
