...
(async()=>{
    'use strict';
    const VERSION='12AR-132AY';
    const DISPLAY_VERSION='12AR-132AY';
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0223.js';
    const NAVIGATION_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/navigation/gv-navigation-0021.js';
    const NAVIGATION_VERSION='0021';
    const RANDOM_GALAXY_VERSION='0223';
    const AVM_LAB_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/lab/gv-avm-overlay-lab-0052.js';
...
`;
    ...
    gvAvmTrace('VIEWER_AVM0052_LOAD_START',{url:AVM_LAB_URL});
    loadScriptGithubThenLocal(AVM_LAB_URL,'gvAvmOverlayLab0052').then(()=>{
        gvAvmTrace('VIEWER_AVM0052_LOAD_OK',{version:window.GalaxyViewerAvmOverlayLab?.VERSION||''});
        const lab=window.GalaxyViewerAvmOverlayLab;
        if(!lab){gvAvmTrace('VIEWER_AVM0052_EXPORT_MISSING',{});return}
        if(lab.VERSION!=='0052'){
            gvAvmTrace('VIEWER_AVM0052_VERSION_MISMATCH',{version:String(lab.VERSION||'')});
            return;
        }
        try{
            gvAvmTrace('VIEWER_AVM0052_INSTALL_START',{version:lab.VERSION});
            lab.install?.({A,aladin,viewerRoot:root,randomGalaxy});
            gvAvmTrace('VIEWER_AVM0052_INSTALL_OK',{version:lab.VERSION});
        }catch(error){
            gvAvmTrace('VIEWER_AVM0052_INSTALL_FAIL',{message:String(error?.message||error||'')});
            throw error;
        }
    }).catch(error=>{gvAvmTrace('VIEWER_AVM0052_LOAD_FAIL',{message:String(error?.message||error||'')});console.error('GV AVM LAB LOAD FAILURE',error)});
