...
(async()=>{
    'use strict';
    const VERSION='12AR-132BC';
    const DISPLAY_VERSION='12AR-132BC';
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0226.js';
    ...
    const RANDOM_GALAXY_VERSION='0226';
    const AVM_LAB_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/lab/gv-avm-overlay-lab-0053.js';
...
`;
    ...
    gvAvmTrace('VIEWER_AVM0053_LOAD_START',{url:AVM_LAB_URL});
    loadScriptGithubThenLocal(AVM_LAB_URL,'gvAvmOverlayLab0053').then(()=>{
        gvAvmTrace('VIEWER_AVM0053_LOAD_OK',{version:window.GalaxyViewerAvmOverlayLab?.VERSION||''});
        const lab=window.GalaxyViewerAvmOverlayLab;
        if(!lab){gvAvmTrace('VIEWER_AVM0053_EXPORT_MISSING',{});return}
        if(lab.VERSION!=='0053'){
            gvAvmTrace('VIEWER_AVM0053_VERSION_MISMATCH',{version:String(lab.VERSION||'')});
            return;
        }
        try{
            gvAvmTrace('VIEWER_AVM0053_INSTALL_START',{version:lab.VERSION});
            lab.install?.({A,aladin,viewerRoot:root,randomGalaxy});
            gvAvmTrace('VIEWER_AVM0053_INSTALL_OK',{version:lab.VERSION});
        }catch(error){
            gvAvmTrace('VIEWER_AVM0053_INSTALL_FAIL',{message:String(error?.message||error||'')});
            throw error;
        }
    }).catch(error=>{gvAvmTrace('VIEWER_AVM0053_LOAD_FAIL',{message:String(error?.message||error||'')});console.error('GV AVM LAB LOAD FAILURE',error)});
