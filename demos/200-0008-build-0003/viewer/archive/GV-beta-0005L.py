...
# GV-beta-0005L
# Standalone Galaxy Viewer release based only on the final GV-beta-0005K.
# Preserves the 1,000-record catalog, Spectacular Mode, centered reticle, safe FOV floor, navigation, and #A9A3D9 control color.
# Changes only the coordinate and Target control corner radii so each control has four rounded 6px corners.
...
#aladin-cosmic-command-test .gv-coordinate-target-row>.aladin-cooFrame{
    ...
    width:78px!important;min-width:78px!important;max-width:78px!important;height:32px!important;
    margin:0 7px 0 0!important;padding:0 22px 0 8px!important;box-sizing:border-box!important;
    border:1px solid #A9A3D9!important;border-radius:6px!important;
    ...
    box-shadow:0 0 10px rgba(169,163,217,.42)!important;pointer-events:auto!important;
}
...
#aladin-cosmic-command-test .gv-coordinate-target-row>.aladin-coordinates{
    ...
    box-sizing:border-box!important;overflow:hidden!important;border:1px solid #A9A3D9!important;border-radius:6px!important;
    background:rgba(0,0,0,.86)!important;color:#FFD166!important;
    font:700 12px/1.15 "Roboto Mono",Consolas,monospace!important;
    text-shadow:0 0 6px rgba(255,209,102,.42)!important;box-shadow:0 0 10px rgba(169,163,217,.42)!important;
...
#aladin-cosmic-command-test button.gv-target-proxy:active{
    ...
    background:rgba(0,0,0,.82)!important;border:1px solid #A9A3D9!important;border-radius:6px!important;
...
#aladin-cosmic-command-test button.gv-target-proxy img{
    display:block!important;width:28px!important;height:28px!important;object-fit:contain!important;
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){console.error("GV-beta-0005L STARTUP FAILURE: window.A was not created");return;}
        window.A.init.then(async() => {
            ...
            catch(error){console.error("GV-beta-0005L CATALOG FAILURE:",error);}
            ...
            centerReticle.src=reticleUrl+"?v=5L-rounded-controls-001";
            centerReticle.alt="";
            ...
            function createTargetProxy(){
                ...
                proxy.innerHTML=`<img src="${targetIconUrl}?v=5L-rounded-controls-001" alt="" aria-hidden="true" draggable="false">`;
            ...
            versionLabel.textContent="Galaxy Viewer 5L";
            ...
            async function openGalaxy(record){
                ...
                }catch(error){nameLabel.textContent="Could not resolve "+String(record.primary_name||"galaxy");console.error("GV-beta-0005L TARGET FAILURE:",error);}
        ...
        }).catch(error=>console.error("GV-beta-0005L STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0005L STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
