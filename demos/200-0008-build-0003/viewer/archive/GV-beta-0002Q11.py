...
# Q11 expands the active SIMBAD helper box with explicit exit guidance.
...
#aladin-cosmic-command-test .gv-helper-active-line{
...
A.init.then(() => {
    ...
    function setHelperIdle(){
        ...
        if(!stack)return;
        const row=stack.querySelector(".gv-helper-row");
        const arrow=stack.querySelector(".gv-arrow");
        const box=stack.querySelector(".gv-helper-box");
        row?.classList.remove("gv-active");
        if(arrow){
            arrow.style.color="var(--copy-blue)";
            arrow.style.animation="none";
        }
        if(box){
            box.innerHTML="Tap Target to Find Info";
            box.style.color="var(--copy-blue)";
            box.style.setProperty("border-color","#FFFFFF","important");
            box.style.animation="";
            box.style.textShadow="0 0 6px rgba(125,244,255,.60)";
        }
    }
    ...
    function setHelperActive(){
        ...
        if(box){
            box.innerHTML='<span class="gv-helper-active-line">Tap Galaxy / Star</span><span class="gv-helper-active-line">or Tap Target Again to Exit</span>';
            box.style.setProperty("color","#FFC857","important");
            box.style.setProperty("border-color","#FFFFFF","important");
            box.style.animation="none";
            box.style.setProperty("text-shadow","0 0 10px rgba(255,200,87,.75)","important");
        }
    }
    ...
    function ensureHelper(row,proxy){
        ...
        if(!stack){
            ...
            stack.innerHTML=`<div class="gv-helper-row"><div class="gv-arrow">◀</div><div class="gv-helper-box">Tap Target to Find Info</div></div><div class="gv-simbad-live-status" role="status" aria-live="polite"></div>`;
        }
