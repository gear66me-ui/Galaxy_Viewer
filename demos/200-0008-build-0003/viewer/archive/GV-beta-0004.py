...
# GV-beta-0004
# Standalone Galaxy Viewer release based on the known-good GV-beta-0003D state.
# Adds a fixed electric-cyan wireframe lock and "Pan Disabled" status inside
# the active Target helper box. The existing synchronized three-blue sequence
# remains limited to the active Target icon and exit instruction.
...
#aladin-cosmic-command-test button.gv-simbad-proxy.gv-active svg *{
    stroke:var(--gv-active-blue)!important;
}
#aladin-cosmic-command-test button.gv-simbad-proxy.gv-active svg{
...
<script src="https://gear66me-ui.github.io/Galaxy_Viewer/aladin-source-clone/dist/aladin.js" charset="utf-8"></script>
<script>
A.init.then(() => {
    ...
    function applyBlueSequenceColor(){
        root.style.setProperty("--gv-active-blue",blueSequence[blueSequenceIndex]);
    }
    function advanceBlueSequence(){
        blueSequenceIndex=(blueSequenceIndex+1)%blueSequence.length;
        applyBlueSequenceColor();
    }
    function startBlueSequence(){
        if(blueSequenceTimer)return;
        blueSequenceIndex=0;
        applyBlueSequenceColor();
        blueSequenceTimer=setInterval(advanceBlueSequence,500);
    }
    function stopBlueSequence(){
        if(blueSequenceTimer){clearInterval(blueSequenceTimer);blueSequenceTimer=null}
        blueSequenceIndex=0;
        applyBlueSequenceColor();
    }
    function syncBlueSequenceState(){
        ...
        if(active)startBlueSequence();else stopBlueSequence();
    }
    ...
    function setHelperActive(){
        ...
        if(arrow){arrow.style.color="var(--copy-blue)";arrow.style.animation="none"}
        if(box){box.innerHTML='<span class="gv-helper-active-line">✨ Tap Galaxy / Star</span><span class="gv-helper-active-line gv-helper-pan-line"><svg class="gv-helper-lock" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><rect x="4.5" y="10" width="15" height="10" rx="2"></rect><path d="M8 10V7.5a4 4 0 0 1 8 0V10"></path><circle cx="12" cy="14.5" r="1.2"></circle><path d="M12 15.7V18"></path></svg><span>Pan Disabled</span></span><span class="gv-helper-active-line">or Tap Target Again</span><span class="gv-helper-active-line">to Exit</span>';box.style.setProperty("color","#FFD166","important");box.style.setProperty("border-color","#FFFFFF","important");box.style.animation="none"}
        syncBlueSequenceState();
    }
    ...
    function applyPalette(){
        ...
        buildTargetRow();syncBlueSequenceState();
    }
