...
#aladin-cosmic-command-test button.gv-simbad-proxy img.gv-target-trademark{
    ...
    min-width:21px!important;min-height:21px!important;max-width:21px!important;max-height:21px!important;
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){
            console.error("GV-beta-0004H STARTUP FAILURE: window.A was not created");
            return;
        }
        ...
        }).catch(error=>console.error("GV-beta-0004H STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0004H STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
