...
# GV-beta-0003B
# Standalone Galaxy Viewer release based on the known-good GV-beta-0003A state.
# Replaces the flat whole-object color blinking with a smooth, fast,
# continuously scrolling diagonal rainbow field shared by the active Target
# icon and the "or Tap Target Again / to Exit" text.
...
#aladin-cosmic-command-test{
    ...
    --gv-rainbow-glow:rgba(111,231,255,.82);
}
...
#aladin-cosmic-command-test button.gv-simbad-proxy.gv-active svg *{
    stroke:url(#gv-0003b-rainbow)!important;
}
#aladin-cosmic-command-test button.gv-simbad-proxy.gv-active svg{
    filter:drop-shadow(0 0 2px var(--gv-rainbow-glow)) drop-shadow(0 0 5px var(--gv-rainbow-glow)) drop-shadow(0 0 9px var(--gv-rainbow-glow))!important;
    animation:gv-0003b-cloud-bloom 1.05s ease-in-out infinite!important;
}
...
#aladin-cosmic-command-test .gv-helper-row.gv-active .gv-helper-active-line:nth-child(3){
    ...
    filter:drop-shadow(0 0 2px var(--gv-rainbow-glow)) drop-shadow(0 0 5px var(--gv-rainbow-glow))!important;
}
...
@keyframes gv-0003b-cloud-bloom{0%,100%{opacity:.94;transform:scale(.99)}50%{opacity:1;transform:scale(1.025)}}
@media (prefers-reduced-motion:reduce){#aladin-cosmic-command-test button.gv-simbad-proxy.gv-active svg{animation:none!important}}
</style>
...
A.init.then(() => {
    ...
    function ensureGradient(svg){
        if(svg.querySelector("#gv-0003b-rainbow"))return;
        svg.insertAdjacentHTML("afterbegin",`<defs><linearGradient id="gv-0003b-rainbow" x1="0" y1="1" x2="1" y2="0"><stop offset="0%"></stop><stop offset="16%"></stop><stop offset="33%"></stop><stop offset="50%"></stop><stop offset="67%"></stop><stop offset="84%"></stop><stop offset="100%"></stop></linearGradient></defs>`);
    }
    function rainbowTick(now){
        if(!rainbowStart)rainbowStart=now;
        if(now>=nextSpeedChange){speedTarget=.19+Math.random()*.13;nextSpeedChange=now+700+Math.random()*900}
        speed+=(speedTarget-speed)*.012;
        const t=(now-rainbowStart)*speed;
        const wobble=Math.sin(now*.0017)*13+Math.sin(now*.00071)*9;
        const colors=hueOffsets.map((offset,i)=>hsl(t+offset+wobble+Math.sin(now*.0011+i)*10,94+Math.round(Math.sin(now*.0009+i)*4),64+Math.round(Math.sin(now*.0013+i)*5)));
        ...
        root.style.setProperty("--gv-rainbow-glow",colors[4]);
        const phase=((now-rainbowStart)*.11)%260;
        ...
        if(svg){
            ensureGradient(svg);
            const stops=svg.querySelectorAll("#gv-0003b-rainbow stop");
            stops.forEach((stop,i)=>stop.setAttribute("stop-color",colors[i]));
            const grad=svg.querySelector("#gv-0003b-rainbow");
            if(grad)grad.setAttribute("gradientTransform",`translate(${Math.sin(now*.0014)*.16} ${Math.cos(now*.0012)*.16}) rotate(${(now*.055)%360} .5 .5)`);
        }
