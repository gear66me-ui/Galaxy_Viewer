...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){
            console.error("GV-beta-0004L STARTUP FAILURE: window.A was not created");
            return;
        }
        ...
        window.A.init.then(() => {
            ...
            if(!versionLabel){
                ...
                versionLabel.textContent="Galaxy Viewer 4L";
        ...
        }).catch(error=>console.error("GV-beta-0004L STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0004L STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
