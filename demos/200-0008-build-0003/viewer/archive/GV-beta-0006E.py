...
# GV-beta-0006E
# Standalone Galaxy Viewer release based only on final GV-beta-0006C.
...
# Changes only the pull-down menu label typography to a visibly different rounded sans-serif stack while preserving all verified 6C behavior.
...
#aladin-cosmic-command-test .gv-viewer-menu-label{
    ...
    padding:0 10px!important;font:700 12px/1.15 "Arial Rounded MT Bold","Trebuchet MS","Segoe UI",sans-serif!important;
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){console.error("GV-beta-0006E STARTUP FAILURE: window.A was not created");return;}
        window.A.init.then(async() => {
            ...
            catch(error){console.error("GV-beta-0006E CATALOG FAILURE:",error);}
            ...
            centerReticle.src=reticleUrl+"?v=6E-menu-font-001";
            centerReticle.alt="";
            ...
            function createTargetProxy(){
                ...
                proxy.innerHTML=`<img src="${targetIconUrl}?v=6E-menu-font-001" alt="" aria-hidden="true" draggable="false"><span class="gv-target-comet" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i></span>`;
            ...
            function bindTargetStatus(proxy,status){
                if(status.dataset.gv6eBound==="true")return;
                status.dataset.gv6eBound="true";
            ...
            versionLabel.textContent="Galaxy Viewer 6E";
            ...
            async function openGalaxy(record){
                ...
                }catch(error){nameLabel.textContent="Could not resolve "+String(record.primary_name||"galaxy");console.error("GV-beta-0006E TARGET FAILURE:",error);}
        ...
        }).catch(error=>console.error("GV-beta-0006E STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0006E STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
