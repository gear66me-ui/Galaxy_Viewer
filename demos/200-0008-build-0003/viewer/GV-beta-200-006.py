...
# ECO: GV200-001
# ============================================================================
VIEWER_VERSION = "GV-beta-200-006"
...
function gvSmoothZoomTo(targetFov,destination){
    const target=Number(targetFov);
    if(!Number.isFinite(target)||target<=0)return Promise.resolve(false);
    return new Promise(resolve=>{
        function step(){
            if(directHdDestination!==destination){resolve(false);return}
            try{
                const raw=aladin.getFov?.();
                const current=Number(Array.isArray(raw)?raw[0]:raw);
                if(!Number.isFinite(current)||current<=0){resolve(false);return}
                if(current<=target*1.002){
                    aladin.setFov(target);
                    resolve(true);
                    return;
                }
                const next=Math.max(target,current*Math.exp(-1*0.018));
                aladin.setFov(next);
...
async function loadDirectHdOnArrival(destination){
    ...
    try{
        ...
        const wideFov=Math.max(finalFov,Number(destination?.fovDegrees)||finalFov);
        aladin.setProjection('TAN');
        aladin.setFoV(wideFov);
        ...
        const layer=A.image(imageObjectUrl,{
            ...
            successCallback:()=>{
                ...
                requestAnimationFrame(()=>requestAnimationFrame(()=>gvSmoothZoomTo(finalFov,destination)));
