...
insert = r'''
source = source.replace("Galaxy Viewer — VIEWER-6", "Galaxy Viewer — VIEWER-7-1", 1)
...
if source.count(old_buttons) != 1:
    raise RuntimeError("VIEWER-7-1 primary controls token not found exactly once")
...
if source.count(old_status) != 1:
    raise RuntimeError("VIEWER-7-1 status block token not found exactly once")
...
if source.count(old_show) != 1:
    raise RuntimeError("VIEWER-7-1 random galaxy display token not found exactly once")
...
for token in required:
    if token not in source:
        raise RuntimeError(f"VIEWER-7-1 required behavior missing: {token}")

exec(compile(source, "VIEWER-7-1.py", "exec"))
'''

if wrapper.count(old_exec) != 1:
    raise RuntimeError("VIEWER-7-1 VIEWER-6 execution token not found exactly once")
wrapper = wrapper.replace(old_exec, insert, 1)

exec(compile(wrapper, "VIEWER-7-1-wrapper.py", "exec"))
