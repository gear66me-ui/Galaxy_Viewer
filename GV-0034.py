...
function distanceFromNed(ned) {
  try {
    ...
    for (const row of rows) {
      const dMpc =
        Number(row.Distance_Mpc) ||
        Number(row.distance_mpc) ||
        Number(row.Redshift_Independent_Distance_Mpc) ||
        Number(row.redshift_independent_distance_mpc) ||
        Number(row.distance) ||
        Number(row.Distance);
...
function distanceFromLocalCatalog(obj, requested) {
  try {
    const name = String(obj?.main_id ?? "").replace(/^b['\"]|['\"]$/g,"").trim().toUpperCase();
    const ra = Number(obj?.ra ?? requested?.ra);
    const dec = Number(obj?.dec ?? requested?.dec);

    const localCatalog = [
      { names: ["M  31","M31","MESSIER 31","NGC 224","ANDROMEDA"], ra: 10.684708, dec: 41.268750, ly: 2537000 },
      { names: ["M  33","M33","MESSIER 33","NGC 598","TRIANGULUM"], ra: 23.462100, dec: 30.659941, ly: 2723000 },
      { names: ["M  32","M32","MESSIER 32","NGC 221"], ra: 10.674270, dec: 40.865170, ly: 2490000 },
      { names: ["M 110","M110","MESSIER 110","NGC 205"], ra: 10.091890, dec: 41.685410, ly: 2690000 },
      { names: ["LMC","LARGE MAGELLANIC CLOUD"], ra: 80.893900, dec: -69.756100, ly: 163000 },
      { names: ["SMC","SMALL MAGELLANIC CLOUD"], ra: 13.186700, dec: -72.828600, ly: 200000 },
      { names: ["NGC 6822","BARNARD'S GALAXY","BARNARDS GALAXY"], ra: 296.240000, dec: -14.803000, ly: 1630000 }
    ];

    for (const item of localCatalog) {
      if (item.names.some(n => name.includes(n))) return item.ly;
      if (Number.isFinite(ra) && Number.isFinite(dec)) {
        const dra = Math.abs(ra - item.ra);
        const ddec = Math.abs(dec - item.dec);
        if (dra < 0.2 && ddec < 0.2) return item.ly;
      }
    }
    return null;
  } catch(err) {
...
async function querySimbad(ra,dec) {
  ...
  const adql=`SELECT TOP 15 main_id,ra,dec,rvz_redshift,galdim_majaxis,galdim_minaxis,otype,sp_type,DISTANCE(POINT('ICRS',ra,dec),POINT('ICRS',${ra},${dec})) AS separation FROM basic WHERE 1=CONTAINS(POINT('ICRS',ra,dec),CIRCLE('ICRS',${ra},${dec},${radius})) ORDER BY separation ASC`;
...
function scoreGalaxyCandidate(row, requested) {
  let score = 0;
  ...
  const z = Number(row.rvz_redshift);

  if (/^(M\s*\d+|MESSIER\s*\d+|NGC\s*\d+|IC\s*\d+)$/.test(id)) score += 120;
  if (otype === "G") score += 140;
  else if (otype.startsWith("G")) score += 90;
  else if (["POG","LSB","IG","PAG"].includes(otype)) score += 70;

  if (/ANDROMEDA|TRIANGULUM|BODE|CIGAR|SOMBRERO|PINWHEEL|SOUTHERN PINWHEEL/i.test(id)) score += 40;

  if (/AGN|SY1|SY2|LINER|BLLAC|QSO|BLAZAR|SEYFERT/i.test(otype)) score -= 120;
  if (id.includes("[") || id.includes("LDC") || id.includes("2MASS") || id.includes("GALEX") || id.includes("SDSS") || id.includes("WISE")) score -= 70;

  if (Number.isFinite(maj)) score += Math.min(maj / 8, 100);
  if (Number.isFinite(min)) score += Math.min(min / 12, 40);
  if (Number.isFinite(sep)) score += Math.max(0, 40 - sep * 3600);

  if (Number.isFinite(z) && z > 0) score += 10;
  if (Number.isFinite(z) && z < -0.01) score -= 20;

  return score;
}
function summarizeGalaxyCandidates(rows, requested, ned=null) {
  if (!Array.isArray(rows) || rows.length === 0) return null;

  const ranked = rows
    .map(row => ({ row, score: scoreGalaxyCandidate(row, requested) }))
    .sort((a,b) => b.score - a.score);

  const bestIdentity = ranked[0]?.row ?? rows[0];
  const bestSize = ranked
    .filter(x => Number.isFinite(Number(x.row.galdim_majaxis)) && Number(x.row.galdim_majaxis) > 30)
    .sort((a,b) => Number(b.row.galdim_majaxis) - Number(a.row.galdim_majaxis))[0]?.row
    ?? ranked.find(x => Number.isFinite(Number(x.row.galdim_majaxis)))?.row
    ?? bestIdentity;

  const bestType = ranked
    .find(x => !/AGN|SY1|SY2|LINER|BLLAC|QSO|SEYFERT/i.test(String(x.row.otype ?? "")))?.row
    ?? bestIdentity;

  const bestRedshift = ranked
    .find(x => {
      const z = Number(x.row.rvz_redshift);
      return Number.isFinite(z) && z > 0 && !/AGN|SY1|SY2|LINER|BLLAC|QSO|SEYFERT/i.test(String(x.row.otype ?? ""));
    })?.row
    ?? ranked.find(x => Number.isFinite(Number(x.row.rvz_redshift)))?.row
    ?? bestIdentity;

  const merged = { ...bestIdentity };
  merged.galdim_majaxis = bestSize.galdim_majaxis;
  merged.galdim_minaxis = bestSize.galdim_minaxis;
  ...
  merged._pickedName = cleanId(bestIdentity.main_id);
  merged._pickedType = String(bestType.otype ?? "");
  ...
  if (!Number.isFinite(distanceLy) || distanceLy === null) distanceLy = distanceFromLocalCatalog(merged, requested);

  const majLy = angularSizeLy(merged.galdim_majaxis, distanceLy);
  if (Number.isFinite(majLy) && majLy < 5000) {
    const bigger = ranked
      .map(x => x.row)
      .filter(r => Number.isFinite(Number(r.galdim_majaxis)))
      .sort((a,b) => Number(b.galdim_majaxis) - Number(a.galdim_majaxis))[0];
    if (bigger && Number(bigger.galdim_majaxis) > Number(merged.galdim_majaxis) * 5) {
      merged.galdim_majaxis = bigger.galdim_majaxis;
      merged.galdim_minaxis = bigger.galdim_minaxis;
    }
  }
...
function renderResult(obj, requested, ned=null) {
  ...
  if(!Number.isFinite(distanceLy) || distanceLy===null) distanceLy=distanceFromNed(ned);
  if(!Number.isFinite(distanceLy) || distanceLy===null) distanceLy=distanceFromLocalCatalog(obj, requested);
  const coords=Number(obj.ra).toFixed(6)+" "+Number(obj.dec).toFixed(6);
  const name=cleanId(obj.main_id || "Not available");
  const info=[obj.otype?"Type: "+obj.otype:null,obj.sp_type?"Spectrum: "+obj.sp_type:null,obj._candidateCount?("Candidates: "+obj._candidateCount):null].filter(Boolean).join("; ")||"No additional SIMBAD classification";
...
async function findGalaxy() {
  try {
    ...
    const earlySummary=summarizeGalaxyCandidates(simbadRows,coords,null);
    renderResult(earlySummary,coords,null);
    ...
    renderResult(finalSummary,coords,ned||vizier);

    await Promise.allSettled(tasks);
    setStatus("Search complete. Galaxy table now summarizes multiple SIMBAD candidates and uses the most reliable whole-galaxy metadata it can assemble from the returned survey/catalog data.");
