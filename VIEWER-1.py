...
(async()=>{viewer1Setup();const s=viewer1Load();document.getElementById("viewer1CoordBox").value=`${s.ra.toFixed(6)} ${s.dec.toFixed(6)}`;document.getElementById("viewer1SurveySelect").value=s.survey;await A.init;window.viewer1Aladin=A.aladin("#viewer1-aladin",{target:`${s.ra} ${s.dec}`,survey:s.survey,fov:s.fov,cooFrame:"ICRS",showReticle:true,showZoomControl:true,showFullscreenControl:true,showLayersControl:true,showGotoControl:true,showCooGridControl:true,showSimbadPointerControl:true});viewer1Restore("Viewer ready. Restored last saved view.");viewer1Save()})().catch(e=>viewer1Status("Viewer initialization failed: "+e.message));
