...
<div id="gv0011-root">
  ...
  <div class="small-note">
    GV-0043 waits for the catalog pass to finish before filling the Galaxy figures table. The debug dump stays verbose so you can inspect exactly what each source returned and why a candidate won.
  </div>
</div>
...
async function findGalaxy() {
  try {
    ...
    setStatus("Catalog queries in flight… waiting for final aggregation before populating the Galaxy figures table.");
    ...
    setStatus("Search complete. GV-0043 waits for the full catalog pass before filling the Galaxy figures table.");
  } catch(err){
