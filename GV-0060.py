...
terminal = 'exec(compile(source, "GV-0058.py", "exec"))'
if terminal not in loader:
    raise RuntimeError("GV-0060 terminal patch target not found")

injected = r'''
_PREVIEW_SESSION = requests.Session()
_PREVIEW_SESSION.headers.update({
    "User-Agent": "Mozilla/5.0 GalaxyViewer GV-0060",
...
def _add_candidate(candidates, seen, url, page_url, source_url=None,
...
def _discover_candidates(page_url):
    response = _PREVIEW_SESSION.get(page_url, timeout=60, allow_redirects=True)
...
def _best_preview(page_urls):
    ...
    for page_url in page_urls:
        try:
            candidates.extend(_discover_candidates(page_url))
        except Exception:
            pass
    unique, seen = [], set()
    ...
    for candidate in unique[:50]:
        result = _inspect_candidate(candidate)
...
def _enrich_encoded(encoded, catalog):
...
_original_simbad_query = _simbad_query_official
_original_ned_query = _ned_query_official


def _simbad_query_with_preview(ra, dec):
    return _enrich_encoded(_original_simbad_query(ra, dec), "SIMBAD")


def _ned_query_with_preview(ra, dec):
    return _enrich_encoded(_original_ned_query(ra, dec), "NED")


if colab_output is not None:
    colab_output.register_callback("gv0058.simbad_query", _simbad_query_with_preview)
    colab_output.register_callback("gv0058.ned_query", _ned_query_with_preview)

source = source.replace("Galaxy Viewer — GV-0058", "Galaxy Viewer — GV-0060", 1)
source = source.replace(
    "GV-0058 uses the approved DEV-0003 official SIMBAD and NED catalog routines.",
    "GV-0060 preserves the approved DEV-0003 official SIMBAD and NED catalog routines and restores clickable catalog hotlinks and thumbnails.",
    1,
)
source = source.replace(
    "Search complete. GV-0058 used SIMBAD row 1 and NED row 1 from the 30-arcsecond search window.",
    "Search complete. GV-0060 used SIMBAD row 1 and NED row 1 from the 30-arcsecond search window.",
    1,
)
...
if "</style>" not in source:
    raise RuntimeError("GV-0060 style insertion target not found")
...
if old_result not in source:
    raise RuntimeError("GV-0060 result-row patch target not found")
source = source.replace(old_result, new_result, 1)

exec(compile(source, "GV-0060.py", "exec"))
'''

loader = loader.replace(terminal, injected, 1)
compile(loader, "GV-0060.py", "exec")
exec(compile(loader, "GV-0060.py", "exec"))
