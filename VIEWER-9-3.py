...
VIEWER92_BLOB_URL = "https://api.github.com/repos/gear66me-ui/Galaxy_Viewer/git/blobs/e75eaf6288a950d44da23f564bd63361861953c5"

with urllib.request.urlopen(VIEWER92_BLOB_URL, timeout=60) as response:
    payload = json.loads(response.read().decode("utf-8"))
source = base64.b64decode(payload["content"]).decode("utf-8")

source = source.replace("Galaxy Viewer — VIEWER-9-2", "Galaxy Viewer — VIEWER-9-3", 1)

old_random = '''const GV91_FALLBACK_GALAXIES=[
...
new_random = '''async function gv91RandomGalaxy(){gv91Busy("gv91-random",true,"Searching...","Random Galaxy");gv91Status("AI is selecting and verifying a new galaxy...");try{const r=await parent.google.colab.kernel.invokeFunction("viewer9.randomGalaxyAI",[]);const g=r.data["application/json"];if(!g)throw Error("No galaxy data returned.");gv91ShowGalaxy(g,"AI galaxy loaded");document.getElementById("gv91-panel").innerHTML='<div class="info-loading">Press Get Info to research '+gv91Esc(g.name)+'.</div>'}catch(e){const text=String(e.message||e);gv91Status("Random Galaxy failed: "+text);document.getElementById("gv91-panel").innerHTML='<div class="info-error">'+gv91Esc(text)+'</div>'}finally{gv91Busy("gv91-random",false,"","Random Galaxy")}}'''
if source.count(old_random) != 1:
    raise RuntimeError("VIEWER-9-3 random function token not found exactly once")
source = source.replace(old_random, new_random, 1)

old_info_call = 'parent.google.colab.kernel.invokeFunction("viewer9.getGalaxyInfo",[n,+c[0],+c[1]],{})'
new_info_call = 'parent.google.colab.kernel.invokeFunction("viewer9.getGalaxyInfo",[n,+c[0],+c[1]])'
if source.count(old_info_call) != 1:
    raise RuntimeError("VIEWER-9-3 Get Info callback token not found exactly once")
source = source.replace(old_info_call, new_info_call, 1)

# The source still contains the earlier random callback token replacement line in the
# Python patch wrapper. Prevent that replacement from changing the new function.
...
required = [
    ...
    'invokeFunction("viewer9.getGalaxyInfo",[n,+c[0],+c[1]])',
    'AI is selecting and verifying a new galaxy',
]
for token in required:
    if token not in source:
        raise RuntimeError(f"VIEWER-9-3 required behavior missing: {token}")
if "GV91_FALLBACK_GALAXIES" in source:
    raise RuntimeError("VIEWER-9-3 fixed fallback galaxy database was not removed")

compile(source, "VIEWER-9-3.py", "exec")
exec(compile(source, "VIEWER-9-3.py", "exec"))
