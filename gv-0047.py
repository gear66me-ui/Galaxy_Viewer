...
<div id="gv0011-root">
  ...
  <div class="small-note">
    GV-0047 preserves the current viewer position, survey, and field of view in browser storage so switching tabs does not snap the viewer back to Andromeda. SIMBAD is queried with a 30-arcsecond cone and the first returned SIMBAD row is used as the displayed primary object. Distance now displays in BLY and the Z column shows z / distance.
  </div>
</div>
...
const DEBUG_KEYS = [
  "main_id","name","id","otype","sp_type","rvz_redshift","rvz_radvel","separation",
  "ra","dec","RAJ2000","DEJ2000","galdim_majaxis","galdim_minaxis",
  "plx_value","plx_err","D25","d25","logD25","MajAxis","MinAxis","MajorAxis","MinorAxis",
  "Distance_Mpc","distance_mpc","Redshift_Independent_Distance_Mpc","distance","Distance"
];
const VIEWER_STATE_KEY = "galaxy-viewer-gv0047-state";
...
function captureViewerState() {
  ...
  let ra = fallback.ra, dec = fallback.dec;
  try {
...
function setDebugText(text) { document.getElementById("debugOutput").textContent = text; }
...
document.addEventListener("visibilitychange", () => {
  if (document.hidden) saveViewerState();
  else restoreViewerState("Viewer restored from saved tab state.");
});
...
function formatBlyFromLy(distanceLy, digits=6) {
  const n = Number(distanceLy);
  if (!Number.isFinite(n) || n <= 0) return "Not available";
  return `${fmt(n/1e9, digits)} BLY`;
}
...
function distanceFromZ(z){
  ...
  integral*=zz/N;
  return (C/H0)*integral*LY_PER_MPC;
}
function distanceFromParallax(plxMas) {
  const p = Number(plxMas);
  if (!Number.isFinite(p) || p <= 0) return null;
  return (1000 / p) * LY_PER_PARSEC;
}
...
function resolveDistanceLy(obj, ned=null) {
  return distanceFromZ(obj?.rvz_redshift) ?? distanceFromParallax(obj?.plx_value) ?? distanceFromNed(ned);
}
...
async function querySimbad(ra,dec) {
  ...
  const adql = `SELECT TOP 20 main_id,ra,dec,rvz_redshift,rvz_radvel,plx_value,plx_err,galdim_majaxis,galdim_minaxis,otype,sp_type,DISTANCE(POINT('ICRS',ra,dec),POINT('ICRS',${ra},${dec})) AS separation FROM basic WHERE 1=CONTAINS(POINT('ICRS',ra,dec),CIRCLE('ICRS',${ra},${dec},${radius})) ORDER BY separation ASC`;
...
function resolveWholeGalaxySize(primaryRow, ned, vizier, distanceLy) {
  ...
  for (const row of extractRows(vizier)) {
    ...
    if (Number.isFinite(majArcmin) && majArcmin > 1) candidates.push({source:"VizieR", majorArcsec:majArcmin*60, minorArcsec:null, quality:95});
    ...
    if (Number.isFinite(majArcsec) && majArcsec > 5) candidates.push({source:"VizieR", majorArcsec:majArcsec, minorArcsec:minArcsec, quality:majArcsec >= 30 ? 90 : 45});
  }
  for (const row of extractRows(ned)) {
    ...
    if (Number.isFinite(majArcsec) && majArcsec > 5) candidates.push({source:"NED", majorArcsec:majArcsec, minorArcsec:minArcsec, quality:majArcsec >= 30 ? 100 : 50});
  }
...
function renderResult(obj, requested, ned=null) {
  ...
  const distanceText = formatBlyFromLy(distanceLy, 6);
  ...
  body.innerHTML = `<tr><td>${safe(name)}</td><td style="font-family:monospace">${safe(coords)}</td><td>${safe(distanceText)}</td><td>${safe(sizeText(obj.galdim_majaxis,obj.galdim_minaxis,distanceLy))}</td><td>${safe(ageText)}</td><td>${safe(zDistanceText)}</td><td>${safe(info)}</td></tr>`;
}
...
function formatDebugSection(title, payload) {
  ...
  if (rows) {
    ...
    rows.slice(0,3).forEach((row, idx) => {
      out += `row ${idx+1}:\n`;
      const keys = Object.keys(row).filter(k => DEBUG_KEYS.includes(k) || /ra|dec|dist|axis|diam|redshift|type|name|id|plx/i.test(k));
...
function buildDebugDump(coords, simbadRows, ned, vizier, sdss, panstarrs, galex, surveyLog, finalSummary) {
  ...
  if (finalSummary) {
    const distanceLy = resolveDistanceLy(finalSummary, ned);
    text += "FINAL SUMMARY\n=============\n";
    ...
    text += `distance bly: ${String(formatBlyFromLy(distanceLy, 6))}\n`;
    text += `parallax mas: ${String(finalSummary.plx_value ?? "")}\n`;
...
async function findGalaxy() {
  try {
    ...
    saveViewerState();
    setStatus("Search complete. GV-0047 used the first SIMBAD row from the 30-arcsecond cone search, kept your current view, and now displays distance in BLY.");
  } catch(err) {
