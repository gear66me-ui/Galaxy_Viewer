...
# Dedicated bootstrap: exact proven generic-shell pattern, but fixed to frozen 10F + 10G INFO.
...
try{const started=performance.now();const viewerReady=startViewer();await delay(Math.max(0,3500-(performance.now()-started)));await runSplash();await viewerReady;p.remove();c?.remove();set10G()}catch(z){e.style.display='block';e.textContent='GALAXY VIEWER 10G FAILED TO LOAD\n\n'+String(z?.stack||z)}})();'''
...
print('10G proven-shell index bytes:', len(text_out.encode()))
