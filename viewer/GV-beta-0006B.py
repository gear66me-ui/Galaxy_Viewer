...
# GV-beta-0006B
# Standalone Galaxy Viewer release based only on final GV-beta-0006A.
# Uses 36 px Menu and Target tiles with 2 px gaps, restores the approved vector Target artwork, pulses only the centered cool-white menu bars, preserves the standby comet orbit, and applies a full-edge green active sea behind the protected Target island with a centered 2 px active star.
# Preserves the four-decimal coordinate display, 1,000-record catalog, Spectacular Mode, centered reticle, safe FOV floor, navigation, SIMBAD behavior, Aladin 3.8.2 configuration, and all unrelated viewer behavior.
...
#aladin-cosmic-command-test button.gv-target-proxy .gv-target-comet{
    position:absolute!important;left:18px!important;top:18px!important;width:0!important;height:0!important;
...
#aladin-cosmic-command-test button.gv-target-proxy[aria-pressed="true"] .gv-target-comet i:nth-child(1){
    left:-1px!important;top:-1px!important;width:2px!important;height:2px!important;
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){console.error("GV-beta-0006B STARTUP FAILURE: window.A was not created");return;}
        window.A.init.then(async() => {
            ...
            catch(error){console.error("GV-beta-0006B CATALOG FAILURE:",error);}
            ...
            centerReticle.src=reticleUrl+"?v=6B-target-menu-001";
            centerReticle.alt="";
            ...
            function createTargetProxy(){
                ...
                proxy.innerHTML=`<img src="${targetIconUrl}?v=6B-target-menu-001" alt="" aria-hidden="true" draggable="false"><span class="gv-target-comet" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i></span>`;
            ...
            function bindTargetStatus(proxy,status){
                if(status.dataset.gv6bBound==="true")return;
                status.dataset.gv6bBound="true";
            ...
            versionLabel.textContent="Galaxy Viewer 6B";
            ...
            async function openGalaxy(record){
                ...
                }catch(error){nameLabel.textContent="Could not resolve "+String(record.primary_name||"galaxy");console.error("GV-beta-0006B TARGET FAILURE:",error);}
        ...
        }).catch(error=>console.error("GV-beta-0006B STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0006B STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
