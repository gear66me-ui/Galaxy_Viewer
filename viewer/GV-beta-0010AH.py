...
# GV-beta-0010AH
# Standalone release derived directly from the exact GV-beta-0010AG baseline.
# 10AH authorized changes only: deterministically load the existing UHD viewport, repair VIEW ZOOMABLE
# activation, restore the provider control outside the HD gesture surface with comet feedback and
# Android-safe provider navigation, and expose non-visual runtime diagnostics.
...
(async()=>{
    ...
    const UHD_VIEWPORT_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/gv-uhd-viewport-0001.js?v=10AH-0001';
    ...
    const moduleLoads=Promise.all([
        ...
        loadScript(UHD_VIEWPORT_URL,'gvUhdViewport0001')
    ]);
    ...
    await moduleLoads;
    if(!window.GVUHDViewport||typeof window.GVUHDViewport.open!=='function')throw new Error('UHD VIEWPORT MODULE 0001 EXPORT / OPEN FUNCTION MISSING AFTER AWAITED LOAD');
    ...
    // 10AH HD presentation: exact scope is archive/source controls + Galaxy Info + BACK TO SKY + deterministic VIEW ZOOMABLE repair.
    ...
    function openProviderSource(value,control='PROVIDER CONTROL'){
        ...
        if(!sourceUrl){console.error(`GV-10AH ${control} INVALID HTTPS SOURCE`,{destination:currentArchiveDestination(),sourceUrl:value});return false}
        try{
            const opened=window.open(sourceUrl,'_blank','noopener,noreferrer');
            if(opened)return true;
        }catch(error){console.warn(`GV-10AH ${control} NEW-CONTEXT NAVIGATION FAILED; USING WEBVIEW FALLBACK`,error)}
        try{window.location.assign(sourceUrl);return true}catch(error){console.error(`GV-10AH ${control} NAVIGATION FAILED`,{destination:currentArchiveDestination(),sourceUrl,error});return false}
    }
    function wireUhdProviderNavigation(sourceUrl){
        const infoButton=document.querySelector('#gv-uhd-viewport-0001 .gvuhd-info');
        if(!infoButton){console.error('GV-10AH UHD PROVIDER CONTROL MISSING AFTER VIEWPORT OPEN',{destination:currentArchiveDestination(),sourceUrl});return false}
    ...
    hdArchiveButton.addEventListener('click',event=>{
        ...
        if(!destination||!sourceUrl){console.error('GV-10AH PROVIDER ICON ACTIVATION BLOCKED',{destination,sourceUrl:destination?.sourceUrl||''});setArchiveLoading(false);return}
    ...
    hdDownloadButton?.addEventListener('click',event=>{
        ...
        if(!destination||!sourceUrl||!previewUrl||!providerIcon||!viewport||typeof viewport.open!=='function'){
            console.error('GV-10AH VIEW ZOOMABLE ACTIVATION BLOCKED',{destination,sourceUrl,previewUrl,providerIcon,uhdLoaded:Boolean(viewport),openType:typeof viewport?.open});
            return;
        }
        ...
        if(!viewportRoot?.classList.contains('open'))console.error('GV-10AH VIEWPORT OPEN CALL COMPLETED WITHOUT .open ROOT STATE',{destination,payload});
    },true);
    ...
    window.GV10AHDiagnostics=Object.freeze({
        release:'10AH',
        get uhdModuleLoaded(){return Boolean(window.GVUHDViewport&&typeof window.GVUHDViewport.open==='function')},
...
})().catch(error=>{console.error('GALAXY VIEWER 10AH STARTUP FAILURE:',error);document.dispatchEvent(new CustomEvent('gv-viewer-failed',{detail:{message:String(error?.stack||error)}}));});
"""))

# GV-beta-0010AH staged