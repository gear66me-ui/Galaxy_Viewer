...
# Adds an explicit TAP HERE TO CLEAR control after a SIMBAD result appears.
# Uses Aladin's own hidePopup() and fire("default") methods.
...
#aladin-cosmic-command-test .gv-simbad-helper {
    ...
    text-shadow: 0 0 4px rgba(255,209,102,.45) !important;
...
A.init.then(() => {
    ...
    function findNativeSimbadPointer() {
        return root.querySelector(".aladin-simbadPointer-control") ||
    ...
    function clearSimbad() {
        try { aladin.hidePopup(); } catch (error) {
            root.querySelectorAll(".aladin-popup-container .aladin-closeBtn")
    ...
    function synchronizeTargetGeometry(coordinateBox, nativePointer) {
        ...
        [
            ["width", boxWidth + "px"], ["min-width", boxWidth + "px"], ["max-width", boxWidth + "px"],
            ["height", height + "px"], ["min-height", height + "px"], ["max-height", height + "px"],
            ["border-style", borderStyle], ["border-width", borderWidth],
            ["border-color", borderColor], ["border-radius", radius]
        ].forEach(([property, value]) => nativePointer.style.setProperty(property, value, "important"));
    }
            ...
            stack.innerHTML = `
                <div class="gv-simbad-helper" aria-live="polite">
                    <svg class="gv-simbad-helper-arrow" viewBox="0 0 24 14" aria-hidden="true">
                        <path d="M22 7H6"></path><path d="M11 2L5 7L11 12"></path>
                    </svg>
                    <span class="gv-simbad-helper-text">Click target, then click<br>a galaxy or star to find info</span>
                </div>
                <div class="gv-simbad-live-status" role="status" aria-live="polite"></div>`;
        }
    ...
    observer.observe(root, {childList:true, subtree:true, characterData:true, attributes:true, attributeFilter:["style","class"]});
