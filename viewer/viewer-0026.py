...
# Fixes the recursive MutationObserver loop.
# Adds a tappable SIMBAD result clear control using Aladin hidePopup() and fire("default").
...
A.init.then(() => {
    ...
    function clearSimbad(){
        ...
        pointer?.blur?.();
        pointer?.querySelectorAll("*").forEach(element=>element.blur?.());
    }
