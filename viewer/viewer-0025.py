...
# Fixes the SIMBAD target visual reset by using a Galaxy Viewer-controlled
# inactive state instead of guessing undocumented Aladin active classes.
...
/* Galaxy Viewer owns the inactive visual state. This overrides sticky
   :focus/:active/:hover behavior that can persist on touch devices. */
#aladin-cosmic-command-test .gv-native-simbad-moved.gv-simbad-inactive,
#aladin-cosmic-command-test .gv-native-simbad-moved.gv-simbad-inactive:hover,
#aladin-cosmic-command-test .gv-native-simbad-moved.gv-simbad-inactive:focus,
#aladin-cosmic-command-test .gv-native-simbad-moved.gv-simbad-inactive:focus-within,
#aladin-cosmic-command-test .gv-native-simbad-moved.gv-simbad-inactive:active {
    transform: none !important;
}
...
A.init.then(() => {
    ...
    function resetSimbadInterface(message) {
        ...
        if (text) {
            text.innerHTML = "Click target, then click<br>a galaxy or star to find info";
        }
        if (status && message) {
    ...
    function synchronizeTargetGeometry(coordinateBox, nativePointer) {
        ...
        const borderColor =
            ...
            borderStyle.borderRightColor !== "rgba(0, 0, 0, 0)"
                ? borderStyle.borderRightColor
                : "rgb(236, 236, 236)";
        const borderWidth = parseFloat(borderStyle.borderRightWidth) > 0
            ? borderStyle.borderRightWidth
            : "1px";
        const borderLineStyle = borderStyle.borderRightStyle !== "none"
        ...
        properties.forEach(([property, value]) => {
            nativePointer.style.setProperty(property, value, "important");
        });
    }

    function forceTargetInactiveVisual() {
        const nativePointer = findNativeSimbadPointer();
        const coordinateBox = findNativeCoordinateBox();
        if (!nativePointer) return;

        nativePointer.classList.add("gv-simbad-inactive");
        nativePointer.setAttribute("aria-pressed", "false");
        nativePointer.blur?.();
        nativePointer.querySelectorAll("*").forEach(element => {
            element.blur?.();
        });
        ...
        [...nativePointer.children].forEach(child => {
            ...
            child.style.setProperty("max-width", "81%", "important");
            child.style.setProperty("max-height", "81%", "important");
        });
    ...
    function scheduleTargetInactiveVisual() {
        [0, 40, 120, 300, 700].forEach(delay => {
            setTimeout(forceTargetInactiveVisual, delay);
        });
    }
    ...
    function ensureHelper(row, nativePointer) {
        ...
        if (!nativePointer.dataset.gvHelperBound) {
            ...
            nativePointer.addEventListener("click", () => {
                ...
                nativePointer.classList.remove("gv-simbad-inactive");
                nativePointer.removeAttribute("aria-pressed");
                ...
                [40, 180, 600].forEach(delay => {
                    setTimeout(mirrorNativeFeedback, delay);
                });
            });
        }
    ...
    function moveNativeSimbadPointer() {
        ...
        nativePointer.classList.add(
            "gv-native-simbad-moved",
            "gv-command",
            "gv-copy"
        );
        nativePointer.style.setProperty(
            "--command-filter",
            filters.copy,
            "important"
        );
    ...
    function mirrorNativeFeedback() {
        ...
        if (
            simbadModeActive &&
            !isInstructionalFeedback(text) &&
            !nativeMessage.dataset.gvSimbadAutoClosed
        ) {
            nativeMessage.dataset.gvSimbadAutoClosed = "true";
            setTimeout(() => {
                deactivateSimbadMode(
                    "SIMBAD complete — navigation restored."
                );
            }, 80);
        }
    }
