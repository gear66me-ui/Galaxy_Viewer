...
# viewer-0023
# Standalone Galaxy Viewer release copied from the approved Viewer 22 code.
# Changes only:
# 1. Enlarge the yellow SIMBAD status box by about 40 percent.
# 2. Make SIMBAD pointer mode single-use: cancel on a second target tap and
#    automatically restore normal navigation after a completed lookup.
# 3. Return the target helper and icon to their normal inactive state.
...
A.init.then(() => {
    ...
    function resetSimbadInterface(message) {
        ...
        helper?.classList.remove("gv-active");
        if (text) {
    ...
    function isInstructionalFeedback(text) {
        ...
        return (
            lower.includes("click again") ||
            lower.includes("click on an object") ||
            lower.includes("click an object") ||
            lower.includes("select an object") ||
            lower.includes("pointer mode") ||
    ...
    function mirrorNativeFeedback() {
        ...
        if (
            ...
            setTimeout(() => {
                deactivateSimbadMode("SIMBAD complete — navigation restored.");
            }, 80);
        }
    }
