...
(async()=>{
    ...
    const DISPLAY_VERSION='GV-beta-200-008';
  ...
  lastNorthBearing:null
    ...
    // 200-003 clean baseline: AVM metadata intentionally not loaded.
    ...
    const showAvmImage=destination=>{
        const url=String(destination?.githubImageUrl||destination?.hdUrl||'').trim();
        if(!url){console.error('AVM TEST IMAGE URL MISSING',destination);try{window.GalaxyViewerDiagnostics?.recordAvm?.('AVM_TEST_IMAGE_URL_MISSING',{name:destination?.name||'',provider:destination?.provider||''})}catch(_){}return;}
        try{
        ...
        try{
            const imageLayer=A.image(url,{
                ...
                successCallback:(ra,dec,fov,image)=>{
                    ...
                    applyAvmOpacity();
                    try{window.GalaxyViewerDiagnostics?.recordAvm?.('AVM_TEST_IMAGE_VISIBLE',{url,ra,dec,fov,wcs:Boolean(image?.options?.wcs)})}catch(_){}
                },
                errorCallback:error=>{
