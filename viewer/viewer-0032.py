...
# Fixed-size Galaxy Viewer SIMBAD proxy with resilient row placement, clearer helper flow,
# and smoother reset behavior for the native Aladin control.
...
#aladin-cosmic-command-test button.gv-simbad-proxy:focus-visible{
    outline:2px solid rgba(125,244,255,.95)!important;outline-offset:2px!important;
}
...
A.init.then(() => {
    ...
    function setHelperText(message,active=false){
        ...
        if(helper)helper.classList.toggle("gv-active",active);
        if(text)text.innerHTML=message;
    }

    function setStatus(message,clearReady=false){
        const stack=root.querySelector(".gv-simbad-helper-stack");
        if(!stack)return;
        ...
        if(clearReady){
            status.classList.add("gv-clear-ready");
    ...
    function ensureHelper(row,proxy){
        ...
        if(!stack){
            ...
            stack.innerHTML=`<div class="gv-simbad-helper" aria-live="polite"><svg class="gv-simbad-helper-arrow" viewBox="0 0 24 14" aria-hidden="true"><path d="M22 7H6"></path><path d="M11 2L5 7L11 12"></path></svg><span class="gv-simbad-helper-text">${helperDefaultText}</span></div><div class="gv-simbad-live-status" role="status" aria-live="polite"></div>`;
        }
    ...
    function bindProxy(proxy){
        ...
        proxy.addEventListener("click",activateProxy);
    ...
    function buildTargetRow(){
        ...
        }else if(coordinateBox.parentElement!==row){
            coordinateBox.parentElement?.removeChild(coordinateBox);
            row.appendChild(coordinateBox);
        }
    ...
    window.addEventListener("keydown",event=>{
        if(event.key==="Escape"&&(simbadModeActive||resultReady)){
            event.preventDefault();clearSimbad();
        }
    });
});
