...
(async()=>{
    'use strict';
    const VERSION='12AR-115';
    const DISPLAY_VERSION='12AR-115';
    ...
    const AVM_LAB_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/lab/gv-avm-overlay-lab-0031.js';
...
`;
    ...
    gvAvmTrace('VIEWER_AVM0031_LOAD_START',{url:AVM_LAB_URL});
    loadScriptGithubThenLocal(AVM_LAB_URL,'gvAvmOverlayLab0031').then(()=>{
        gvAvmTrace('VIEWER_AVM0031_LOAD_OK',{version:window.GalaxyViewerAvmOverlayLab?.VERSION||''});
        const lab=window.GalaxyViewerAvmOverlayLab;
        if(!lab){gvAvmTrace('VIEWER_AVM0026_EXPORT_MISSING',{});return}
        if(lab.VERSION!=='0031')gvAvmTrace('VIEWER_AVM0026_VERSION_MISMATCH',{version:String(lab.VERSION||'')});
        try{
            gvAvmTrace('VIEWER_AVM0031_INSTALL_START',{version:lab.VERSION});
            lab.install?.({A,aladin,viewerRoot:root,randomGalaxy});
            gvAvmTrace('VIEWER_AVM0031_INSTALL_OK',{version:lab.VERSION});
        }catch(error){
