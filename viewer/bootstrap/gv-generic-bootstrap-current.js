...
const waitForInteractiveViewer=()=>new Promise((ok,no)=>{const d=performance.now()+30000;const f=async()=>{try{const root=document.getElementById('aladin-cosmic-command-test');const rg=window.__gv10eRandomGalaxy;const button=document.getElementById('gv-random-galaxy');if(root?.querySelector('canvas')&&window.aladin_cosmic_command_test&&rg&&button){await rg.ready;if(rg.getState&&typeof rg.travelToRandom==='function'&&!button.disabled)return ok()}if(performance.now()>d)return no(new Error('10E interactive Viewer readiness timeout'));setTimeout(f,50)}catch(z){no(z)}};f()});
const startViewer=async payloadPromise=>{const[m,s]=await payloadPromise;if(m.version!=='10E')throw new Error('Manifest did not select 10E');warm(s);const[h,j]=extract(s);document.body.insertAdjacentHTML('beforeend',h);const z=document.createElement('script');z.textContent=j;document.body.appendChild(z);await waitForInteractiveViewer();return true};
