...
const waitForInteractiveViewer=()=>new Promise((ok,no)=>{const d=performance.now()+30000;let settled=false;const finishFail=err=>{if(settled)return;settled=true;document.removeEventListener('gv-viewer-failed',viewerFailed);no(err)};const finishOk=()=>{if(settled)return;settled=true;document.removeEventListener('gv-viewer-failed',viewerFailed);ok()};const viewerFailed=event=>finishFail(new Error(String(event?.detail?.message||activeVersion+' Viewer startup failed')));document.addEventListener('gv-viewer-failed',viewerFailed,{once:true});const f=async()=>{if(settled)return;try{const root=document.getElementById('aladin-cosmic-command-test');const rg=window.__gv10eRandomGalaxy||window.__gvRandomGalaxy;const button=document.getElementById('gv-random-galaxy');if(root?.querySelector('canvas')&&window.aladin_cosmic_command_test&&rg&&button){await rg.ready;if(rg.getState&&typeof rg.travelToRandom==='function'&&!button.disabled)return finishOk()}if(performance.now()>d)return finishFail(new Error(activeVersion+' interactive Viewer readiness timeout'));setTimeout(f,50)}catch(z){finishFail(z)}};f()});
