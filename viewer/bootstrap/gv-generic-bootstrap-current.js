...
const loadViewerPayload=async()=>{try{const m=JSON.parse(await text(R+'gv-current-viewer.json?t='+Date.now()));if(!validManifest(m))throw new Error('Invalid current Viewer manifest');return[m,await text(R+m.viewer+'?t='+Date.now())]}catch(_){const m=JSON.parse(await text('viewer/gv-current-viewer.json'));if(!validManifest(m))throw new Error('Invalid bundled Viewer manifest');return[m,await text('viewer/'+m.viewer)]}};
...
const waitForInteractiveViewer=()=>new Promise((ok,no)=>{const d=performance.now()+30000;const f=async()=>{try{const root=document.getElementById('aladin-cosmic-command-test');const rg=window.__gv10eRandomGalaxy||window.__gvRandomGalaxy;const button=document.getElementById('gv-random-galaxy');if(root?.querySelector('canvas')&&window.aladin_cosmic_command_test&&rg&&button){await rg.ready;if(rg.getState&&typeof rg.travelToRandom==='function'&&!button.disabled)return ok()}if(performance.now()>d)return no(new Error(activeVersion+' interactive Viewer readiness timeout'));setTimeout(f,50)}catch(z){no(z)}};f()});
const startViewer=async payloadPromise=>{const[m,s]=await payloadPromise;activeVersion=m.version;warm(s);const[h,j]=extract(s);document.body.insertAdjacentHTML('beforeend',h);const z=document.createElement('script');z.textContent=j;document.body.appendChild(z);await waitForInteractiveViewer();return true};
