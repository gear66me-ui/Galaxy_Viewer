...
# GV-beta-0009H
# Derived from exact GV-beta-0009G baseline blob ed915121d7b735cc0118c8d688ce7a28922d2d61.
# Authorized 9H change: calculate only the initial observable-universe context position so the arrow tip terminates at the top edge of the Mollweide oval.
# All unrelated 9G behavior remains frozen, including first-trip choreography, Hubble preload priority, ten-second HOME travel, eighteen-second later travel, and sequential ten-image prefetch.
...
(async()=>{
    ...
    function startPrefetch(destination,priority=false){
        ...
        const promise=prepareHdDestination(destination,controller.signal).then(item=>{
            if(key===activeTargetKey&&!activePreparedItem){
                ...
                window.__gv9hRandomGalaxy?.setPreparedHdResource?.(key,item.objectUrl,item.sourceKind,item.image);
                return;
            }
    ...
    function beginTravelHud(destination){
        ...
        const state=window.GV9H?.randomGalaxy?.getState?.()||window.__gv9hRandomGalaxy?.getState?.()||{};
    ...
    function createBottomControls(root){
        ...
        version.setAttribute('aria-label','GALAXY VIEWER VERSION 9H');
    ...
    const hamburger=window.GalaxyViewerHamburgerMenu.init({
        ...
        onProjectionSelected(name,detail){
            ...
            }catch(error){console.error('GV-9H PROJECTION FAILURE',name,detail?.code,error)}
        }
    });
    ...
    coordinate=window.GalaxyCoordinateOverlay.mount(coordinateHost,{onFrameChange(nextFrame){
        frame=nextFrame;
        try{if(typeof aladin.setFrame==='function')aladin.setFrame(frame==='GAL'?'galactic':'ICRSd')}catch(error){console.warn('GV-9H FRAME CHANGE WARNING',error)}
        renderCoordinates();
    }});
    ...
    function setHistoryControls(){
        const busy=navigationPending||Boolean(window.__gv9hRandomGalaxy?.getState?.().busy);
    ...
    function navigateHistory(index){
        ...
        randomGalaxy.travelToRandom().catch(error=>{
            forcedDestination=null;pendingHistoryIndex=null;navigationPending=false;endTravelHud();setHistoryControls();console.error('GV-9H HISTORY NAVIGATION FAILURE',error);
        });
    }
    ...
    const randomGalaxy=window.GalaxyRandomGalaxy.mount(randomGalaxyHost,{
        ...
        onError(error){
            navigationPending=false;pendingHistoryIndex=null;forcedDestination=null;endTravelHud();setHistoryControls();console.error('GV-9H RANDOM GALAXY FAILURE',error);
        }
    });
    window.__gv9hRandomGalaxy=randomGalaxy;
    ...
    const deferHdUntilPrepared=async event=>{
        ...
        try{
            ...
            console.error('GV-9H HUBBLE PREPARATION WAIT FAILURE',error);
    ...
    setHistoryControls();
    window.GV9H=Object.freeze({version:VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,historyBackButton:bottom.back,historyForwardButton:bottom.forward,reticle,versionLabel:bottom.version,universeContext,homeOverlay,catalogCount:catalogRecordCount,eligibleCatalogCount:galaxyCatalog.length,getHubblePrefetchState,startHubblePrefetch:fillPrefetchQueue,getGalaxyHistory:()=>({index:galaxyHistoryIndex,items:galaxyHistory.map(item=>({name:item.name,archiveId:item.archiveId}))})});
...
})().catch(error=>console.error('GALAXY VIEWER 9H STARTUP FAILURE:',error));
"""))

# GV-beta-0009H staged