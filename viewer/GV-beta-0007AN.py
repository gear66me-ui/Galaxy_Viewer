...
(async()=>{
  ...
  function enforceProjectionPulse(c=collect()){
    ...
    getGlobalPulseStart();
    const surfaceTargets=[c.leftLabels[0],c.leftIcons[0],...c.rightLabels,...c.rightIcons].filter(Boolean);
    ...
    const next=[];
    surfaceTargets.forEach(el=>cancelAnimations(el));
  ...
  function enforceMenuPulse(c=collect()){
    ...
    const options={duration:CYCLE,iterations:Infinity,easing:EASING,fill:"both"};
    const next=[];
    cancelAnimations(menuButton);
    const hamburgerGlow=ensureTileGlowLayer(menuButton);
    cancelAnimations(hamburgerGlow);
    hamburgerGlow.style.opacity="0";
    ...
    c.leftRows.slice(1).forEach(row=>{
      ...
      if(iconContent){
        iconContent.getAnimations().forEach(a=>{try{a.cancel()}catch(_){ }});
        iconContent.style?.setProperty?.("position","relative","important");
        iconContent.style?.setProperty?.("z-index","2","important");
        animateOwned(iconContent,iconFrames,options,next);
      }
    });
    menuAnimations=next;
    return true;
  ...
  function projectionPulseDiagnostics(c){
    const longLabels=[c.leftLabels[0],...c.rightLabels].filter(Boolean);
    const iconTiles=[c.leftIcons[0],...c.rightIcons].filter(Boolean);
    const iconGraphics=[c.leftIcons[0]?.querySelector("svg"),...c.rightIcons.map(icon=>icon?.querySelector("svg"))].filter(Boolean);
    const foreign=[];
    const owned=new Set(projectionAnimations);
    [...longLabels,...iconTiles].forEach(el=>el.getAnimations({subtree:true}).forEach(a=>{if(a.playState==="running"&&!owned.has(a)&&!foreign.includes(a))foreign.push(a)}));
    ...
    return {
      ...
      expectedCount:6,
      sharedStart:projectionAnimations.length===6&&projectionAnimations.every(a=>a.startTime===globalPulseStart),
      allRunning:projectionAnimations.length===6&&projectionAnimations.every(a=>a.playState==="running"),
      durationMatch:timings.length===6&&timings.every(t=>t.duration===CYCLE),
      easingMatch:timings.length===6&&timings.every(t=>normalizeEasing(t.easing)===normalizeEasing(EASING)),
      phaseSpreadMs:phaseSpread,
      phaseMatch:phaseSpread<=5,
      foreignRunningCount:foreign.length,
      noForeignRunning:foreign.length===0,
      longLabelsNoRunning:longLabels.every(label=>label.getAnimations({subtree:true}).filter(a=>a.playState==="running").length===0),
      longLabelGlowLayersInactive:longLabels.every(label=>!layerRunning(label)),
      iconTileSurfacesStatic:iconTiles.every(tile=>tile.getAnimations().filter(a=>a.playState==="running").length===0&&!layerRunning(tile)),
      iconGraphicsRunning:iconGraphics.length===6&&iconGraphics.every(graphic=>graphic.getAnimations().some(a=>owned.has(a)&&a.playState==="running")),
      startTime:globalPulseStart,
      cycleMs:CYCLE,
      easing:EASING
    };
  ...
  function menuPulseDiagnostics(c){
    ...
    const labels=inactive.map(row=>row.querySelector(".gv-viewer-menu-label")).filter(Boolean);
    const iconTiles=inactive.map(row=>row.querySelector(".gv-viewer-menu-icon")).filter(Boolean);
    const iconGraphics=iconTiles.map(icon=>contentTarget(icon,false)).filter(Boolean);
    const owned=new Set(menuAnimations);
    const layerRunning=tile=>tile?.querySelector(":scope > .gv-7ai-tile-glow")?.getAnimations().some(a=>a.playState==="running")||false;
    ...
    return {
      ...
      expectedCount:projectionOpen?0:iconGraphics.length,
      allRunning:projectionOpen?menuAnimations.length===0:menuAnimations.length===iconGraphics.length&&menuAnimations.every(a=>a.playState==="running"),
      sharedStart:projectionOpen?menuAnimations.length===0:menuAnimations.length===iconGraphics.length&&menuAnimations.every(a=>a.startTime===globalPulseStart),
      durationMatch:projectionOpen?menuAnimations.length===0:timings.length===iconGraphics.length&&timings.every(t=>t.duration===CYCLE),
      easingMatch:projectionOpen?menuAnimations.length===0:timings.length===iconGraphics.length&&timings.every(t=>normalizeEasing(t.easing)===normalizeEasing(EASING)),
      longLabelsNoRunning:labels.every(label=>label.getAnimations({subtree:true}).filter(a=>a.playState==="running").length===0),
      longLabelGlowLayersInactive:labels.every(label=>!layerRunning(label)),
      iconTileSurfacesStatic:iconTiles.every(tile=>tile.getAnimations().filter(a=>a.playState==="running").length===0&&!layerRunning(tile)),
      iconGraphicsRunning:projectionOpen?menuAnimations.length===0:iconGraphics.length>0&&iconGraphics.every(graphic=>graphic.getAnimations().some(a=>owned.has(a)&&a.playState==="running")),
      hamburgerTileHasOwnedGlow:!!hamburger?.querySelector(":scope > .gv-7ai-tile-glow"),
      hamburgerNoRunningAnimations:!!hamburger&&hamburger.getAnimations({subtree:true}).filter(a=>a.playState==="running").length===0,
  ...
  async function validate(baseline,centering){
    ...
    const projectionRunning=projection?.getAnimations({subtree:true}).filter(a=>a.playState==="running")||[];
    const mollweideRunning=mollweide?.getAnimations({subtree:true}).filter(a=>a.playState==="running")||[];
    ...
    const checks={
      ...
      projectionGlowRunning:projectionRunning.length>=1,
      longLabelTilesNoPulse:pulse.longLabelsNoRunning&&menuPulse.longLabelsNoRunning,
      longLabelGlowLayersInactive:pulse.longLabelGlowLayersInactive&&menuPulse.longLabelGlowLayersInactive,
      iconTileBackgroundsStatic:pulse.iconTileSurfacesStatic&&menuPulse.iconTileSurfacesStatic,
      iconGraphicsGlowRunning:pulse.iconGraphicsRunning&&menuPulse.iconGraphicsRunning,
      hamburgerGlowRemoved:menuPulse.hamburgerNoRunningAnimations,
      ...
      mollweideGlowRunning:mollweideRunning.length>=1,
  ...
  const projectionPulseReady=initializeProjectionPulse().catch(error=>{console.error("GV-BETA-0007AK PROJECTION PULSE INITIALIZATION FAILURE:",error);return false});
...
(async()=>{
  ...
  const nextPaint=()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
  const afterInheritedPulse=fn=>requestAnimationFrame(()=>requestAnimationFrame(()=>requestAnimationFrame(fn)));
  ...
  const ownedIconAnimations=new WeakMap();
  ...
  function cancelOwnedIcon(svg){
    const owned=ownedIconAnimations.get(svg);
    if(owned){try{owned.cancel()}catch(_){ }ownedIconAnimations.delete(svg)}
  }

  function cancelGraphicAnimations(svg){
    if(!svg)return;
    cancelOwnedIcon(svg);
  ...
  function sharedStart(){
    return window.GV7AK_COORDINATE_GLOW?.startTime??document.timeline.currentTime??performance.now();
  }

  function startBreathing(svg){
    if(!svg)return null;
    cancelGraphicAnimations(svg);
    ...
    ownedIconAnimations.set(svg,animation);
    return animation;
  }
  ...
  function makeSelectedSteady(svg){
    if(!svg)return false;
    cancelGraphicAnimations(svg);
    svg.style.setProperty("opacity",String(SELECTED_FRAME.opacity),"important");
    svg.style.setProperty("filter",SELECTED_FRAME.filter,"important");
    svg.style.setProperty("position","relative","important");
    svg.style.setProperty("z-index","2","important");
    return svg.getAnimations().filter(animation=>animation.playState==="running").length===0;
  }
  ...
  function reapplyVisualState(){
    ...
    const hamburgerGlow=c.menuButton?.querySelector(":scope > .gv-7ai-tile-glow");
    if(hamburgerGlow){
      hamburgerGlow.getAnimations().forEach(animation=>{try{animation.cancel()}catch(_){ }});
      hamburgerGlow.style.setProperty("opacity","0","important");
    }
    c.menuButton?.getAnimations({subtree:true}).forEach(animation=>{try{animation.cancel()}catch(_){ }});
    c.menuButton?.style.setProperty("animation","none","important");
    const stack=c.menuButton?.querySelector(".gv-menu-stack");
    if(stack){stack.getAnimations({subtree:true}).forEach(animation=>{try{animation.cancel()}catch(_){ }});stack.style.setProperty("animation","none","important")}

    c.rightIcons.forEach((tile,index)=>{
      const svg=tile?.querySelector("svg");
      if(!svg)return;
      if(selectedProjection===LABELS[index])makeSelectedSteady(svg);
    ...
    const projectionSvg=c.leftIcons[0]?.querySelector("svg");
    if(projectionSvg)startBreathing(projectionSvg);
  }
  ...
  async function fadeMenus(){
    ...
    const elements=[c.leftMenu,c.rightMenu,c.menuButton];
    ...
    c.menuButton.style.setProperty("visibility","hidden","important");
    c.menuButton.style.setProperty("opacity","0","important");
    c.menuButton.style.setProperty("pointer-events","none","important");
    c.leftMenu.style.setProperty("visibility","hidden","important");
    c.leftMenu.style.setProperty("pointer-events","none","important");
    c.rightMenu.style.setProperty("visibility","hidden","important");
    c.rightMenu.style.setProperty("pointer-events","none","important");
    menusDismissed=true;
    fadeInProgress=false;
    interaction.fadeCompleted=true;
    interaction.menusHidden=true;
    afterInheritedPulse(reapplyVisualState);
  }
  ...
  async function activateProjection(name,event){
    ...
    try{
      ...
      const c=collect();
      interaction.selectedSteady=makeSelectedSteady(c.rightIcons[LABELS.indexOf(name)]?.querySelector("svg"));
      await nextPaint();
      await fadeMenus();
    }catch(error){
  ...
  function reopenProjectionMenus(event){
    ...
    event.stopImmediatePropagation();
    const c=collect();
    [c.leftMenu,c.rightMenu,c.menuButton].forEach(restoreFadeStyle);
    c.menuButton.style.setProperty("display","flex","important");
    c.menuButton.style.setProperty("visibility","visible","important");
    c.menuButton.style.setProperty("opacity","1","important");
    c.menuButton.style.setProperty("pointer-events","auto","important");
  ...
  const projectionGraphics=[c.leftIcons[0]?.querySelector("svg"),...c.rightIcons.map(tile=>tile?.querySelector("svg"))].filter(Boolean);
  ...
  const checks={
    ...
    normalProjectionGraphicsBreathing:selectedProjection!==null||projectionGraphics.every(svg=>svg.getAnimations().some(animation=>animation.playState==="running"&&animation.effect.getTiming().duration===CYCLE)),
  ...
  stateObserver=new MutationObserver(()=>afterInheritedPulse(()=>{
    bindProjectionActions();
    bindTargetReopen();
    reapplyVisualState();
  }));
  stateObserver.observe(root,{subtree:true,childList:true,attributes:true,attributeFilter:["class"]});
...
(async()=>{
  ...
  const CENTER_TOL=.50;
  const LABELS=["MOLLWEIDE","SPHERICAL","ORTHO","TANGENTIAL","SINUSOIDAL"];
  const waitFor=(test,timeout=20000)=>new Promise((resolve,reject)=>{const end=performance.now()+timeout;const tick=()=>{let value=null;try{value=test()}catch(_){ }if(value){resolve(value);return}if(performance.now()>end){reject(new Error("GV-BETA-0007AN STARTUP TIMEOUT"));return}setTimeout(tick,50)};tick()});
  ...
  const inherited=await waitFor(()=>window.GV7AL_INTERACTION);
  const versionLabel=await waitFor(()=>root.querySelector("#gv-version-label"));
  versionLabel.textContent="V-7AN";
  ...
  function cloneForCenterTest(source,name,referenceRect){
    const clone=source.cloneNode(true);
    clone.removeAttribute("id");
    clone.dataset.gv7anProjection=name;
    clone.classList.add("gv-7an-centering-probe");
    clone.style.setProperty("position","fixed","important");
    clone.style.setProperty("left","-10000px","important");
    clone.style.setProperty("top","-10000px","important");
    ["width","min-width","max-width"].forEach(p=>clone.style.setProperty(p,`${referenceRect.width.toFixed(3)}px`,"important"));
    ["height","min-height","max-height"].forEach(p=>clone.style.setProperty(p,`${referenceRect.height.toFixed(3)}px`,"important"));
    clone.style.setProperty("display","grid","important");
    clone.style.setProperty("place-items","center","important");
    clone.style.setProperty("opacity","0","important");
    clone.style.setProperty("pointer-events","none","important");
    clone.style.setProperty("transition","none","important");
    clone.getAnimations({subtree:true}).forEach(a=>{try{a.cancel()}catch(_){ }});
    clone.querySelectorAll(".gv-7ai-tile-glow,.gv-7ab-inset,.gv-7al-static-inset").forEach(layer=>layer.getAnimations().forEach(a=>{try{a.cancel()}catch(_){ }}));
    return clone;
  }

  function measureAllFiveWithStatusGeometry(){
    const c=collect();
    const reference=c.statusTile;
    if(!reference||c.rightIcons.length!==5||c.rightIcons.some(icon=>!icon?.querySelector("svg")))return false;
    const rr=reference.getBoundingClientRect();
    if(!(rr.width>0&&rr.height>0))return false;
    LABELS.forEach((name,index)=>{
      const probe=cloneForCenterTest(c.rightIcons[index],name,rr);
      root.appendChild(probe);
      const measurement=centerPaintedStatusArtwork(probe);
      if(measurement)centeringByProjection[name]=measurement;
      probe.remove();
    });
    return LABELS.every(name=>centeringByProjection[name]?.pass===true);
  }
  ...
  function validate(){
    ...
    measureAllFiveWithStatusGeometry();
    const allFiveMeasured=LABELS.every(name=>!!centeringByProjection[name]);
    const allFiveCentered=LABELS.every(name=>centeringByProjection[name]?.pass===true);
    const statusSvg=c.statusTile?.querySelector("svg");
    const statusTiming=statusSvg?.getAnimations().find(a=>a.playState==="running")?.effect?.getTiming?.();
    const sinusoidal=updateSinusoidalRuntime();
    const structuralChecks={
      versionLabel:versionLabel.textContent==="V-7AN",
      ...
      allFiveMeasured,
      allFiveCentered,
      statusCyclePreserved:!c.statusTile||statusTiming?.duration===CYCLE,
    ...
    window.GV7AN_VALIDATION={
      ...
      baselineBlob:"01d1604596c8f2cd56c28017fa2abf4fc9ce0c78"
    };
  }
  ...
  function scheduleValidation(){
    ...
    afterLayout(()=>{
      ...
      try{validate()}catch(error){console.error("GV-BETA-0007AN VALIDATION FAILURE",error)}
    });
  }
  ...
  const observer=new MutationObserver(records=>{
    ...
    const relevant=records.some(record=>{
      ...
      return [...record.addedNodes,...record.removedNodes].some(node=>node.nodeType===1&&!node.matches?.(".gv-7an-centering-probe"));
    });
    if(statusAdded)scheduleCurrentCenter();
    if(relevant)scheduleValidation();
  });
  observer.observe(root,{subtree:true,childList:true,attributes:true,attributeFilter:["class","aria-checked"]});
  window.addEventListener("resize",scheduleCurrentCenter,{passive:true});

  scheduleCurrentCenter();
  scheduleValidation();
})().catch(error=>console.error("GV-BETA-0007AN STARTUP FAILURE:",error));
"""))

# GV-beta-0007AN staged
