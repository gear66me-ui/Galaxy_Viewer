...
(async()=>{
    'use strict';
    const VERSION='GV-beta-200-003';
    const DISPLAY_VERSION='GV-beta-200-003';
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0240.js';
    ...
    const RANDOM_GALAXY_VERSION='0240';
    const AVM_LAB_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/lab/gv-avm-overlay-lab-0063.js';
    ...
    const moduleLoads=Promise.all([
        ...
        loadScript(RANDOM_GALAXY_URL,`gvRandomGalaxy${RANDOM_GALAXY_VERSION}`)
    ]);
  ...
  lastNorthBearing:null
    ...
    // 200-003: Random Galaxy 0240 supplies only the clean random/HD presentation seam.
    if(window.GalaxyRandomGalaxy?.VERSION!==RANDOM_GALAXY_VERSION)
        throw new Error(`RANDOM GALAXY ${RANDOM_GALAXY_VERSION} EXPORT MISSING`);
    ...
    console.info('GV AVM METADATA CATALOG READY',{
        schema:avmMetadataCatalog.schema,
        fovPolicy:avmMetadataCatalog.fovPolicy,
        records:avmMetadataCatalog.recordCount
    });
    ...
    // ==================== RANDOM GALAXY 0240 — CLEAN BASELINE ====================
    // Preserve the complete Galaxy Viewer shell. Random Galaxy owns only:
    // shuffled destination selection, direct RA/Dec/FoV snap, banner and HD image.
    // Navigation, AVM, preparation, prefetch and travel choreography are intentionally absent.
    const shuffledGalaxyCatalog=[...galaxyCatalog];
    for(let i=shuffledGalaxyCatalog.length-1;i>0;i--){
        const j=Math.floor(Math.random()*(i+1));
        [shuffledGalaxyCatalog[i],shuffledGalaxyCatalog[j]]=
            [shuffledGalaxyCatalog[j],shuffledGalaxyCatalog[i]];
    }

    const randomGalaxy=window.GalaxyRandomGalaxy.mount(randomGalaxyHost,{
        aladin,
        catalog:shuffledGalaxyCatalog,
