...
(async()=>{
    ...
    const UHD_VIEWPORT_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/gv-uhd-viewport-0001.js?v=10AJ-0001';
    ...
    uhdViewportPromise=loadScript(UHD_VIEWPORT_URL,'gvUhdViewport0001').then(()=>{
        ...
        console.error('GV-10AJ NON-CRITICAL UHD VIEWPORT LOAD FAILURE',error);
        return null;
    });
    ...
    if(hdDownloadButton){
        ...
        hdDownloadButton.replaceChildren(zoomableLabel);
    ...
    function showZoomShell({previewUrl,providerIcon,sourceUrl,title}){
        document.getElementById('gv-aj-zoom-shell')?.remove();
        const shell=document.createElement('div');
        shell.id='gv-aj-zoom-shell';
        Object.assign(shell.style,{position:'fixed',inset:'0',zIndex:'2147483500',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:'10px',padding:'12px',boxSizing:'border-box',background:'#000',pointerEvents:'auto'});
        const image=document.createElement('img');
        image.src=previewUrl;
        image.alt=title||'ZOOMABLE GALAXY';
        Object.assign(image.style,{display:'block',width:'min(96vw,900px)',height:'min(76vh,900px)',objectFit:'contain',background:'#000',border:'2px solid #78FFAB',borderRadius:'8px',boxShadow:'0 0 10px rgba(87,255,147,.28),0 0 12px rgba(72,195,255,.18)'});
        const status=document.createElement('div');
        status.textContent='ZOOMABLE VIEW';
        Object.assign(status.style,{color:'#EAF8FF',font:'400 12px/1.2 "Space Age",sans-serif',letterSpacing:'.5px',textAlign:'center'});
        const controls=document.createElement('div');
        Object.assign(controls.style,{display:'flex',alignItems:'center',justifyContent:'center',gap:'10px'});
        const back=document.createElement('button');
        back.type='button';
        back.textContent='BACK TO GALAXY VIEWER';
        Object.assign(back.style,{height:'42px',padding:'0 12px',border:'2px solid #78FFAB',borderRadius:'6px',background:'#126941',color:'#E8FFF0',font:'400 11px/1 "Space Age",sans-serif'});
        back.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();shell.remove()});
        const provider=document.createElement('button');
        provider.type='button';
        provider.setAttribute('aria-label','OPEN ARCHIVE SOURCE');
        Object.assign(provider.style,{width:'42px',height:'42px',padding:'3px',border:'2px solid #78FFAB',borderRadius:'6px',background:'#126941'});
        const icon=document.createElement('img');
        icon.src=providerIcon;
        icon.alt='ARCHIVE SOURCE';
        Object.assign(icon.style,{width:'100%',height:'100%',objectFit:'contain'});
        provider.appendChild(icon);
        provider.addEventListener('click',event=>{
            event.preventDefault();event.stopPropagation();
            if(sourceUrl)try{window.open(sourceUrl,'_blank','noopener,noreferrer')}catch(_){}
        });
        controls.append(back,provider);
        shell.append(image,status,controls);
        document.body.appendChild(shell);
        return {shell,status};
    }
    function setZoomShellStatus(shellState,text){
        if(shellState?.status)shellState.status.textContent=text;
    }
    hdArchiveButton.addEventListener('click',event=>{
        ...
        if(!destination||!sourceUrl){
            console.error('GV-10AJ PROVIDER ICON ACTIVATION BLOCKED',{destination,sourceUrl:destination?.sourceUrl||''});
            setArchiveLoading(false);
            return;
        }
        ...
        try{
            ...
            archiveOpenRequested=false;
            setArchiveLoading(false);
            console.error('GV-10AJ PROVIDER ARCHIVE NAVIGATION FAILED',{destination,sourceUrl,error});
            return;
        }
        clearTimeout(providerLaunchTimer);
        providerLaunchTimer=setTimeout(()=>{
            if(!archiveOverlay.classList.contains('gv-open')){
                archiveOpenRequested=false;
                setArchiveLoading(false);
                console.error('GV-10AJ PROVIDER ARCHIVE NAVIGATION TIMEOUT',{destination,sourceUrl});
            }
        },8000);
    },true);
    hdDownloadButton?.addEventListener('click',async event=>{
        event.preventDefault();
        event.stopPropagation();
        const destination=currentArchiveDestination();
        const sourceUrl=validHttpsSource(destination?.sourceUrl);
        ...
        if(!destination||!sourceUrl||!previewUrl||!providerIcon){
            console.error('GV-10AJ VIEW ZOOMABLE ACTIVATION BLOCKED',{destination,sourceUrl,previewUrl,providerIcon});
            return;
        }
        const shellState=showZoomShell({
            previewUrl,
            providerIcon,
            sourceUrl,
            title:String(destination.commonName||destination.designation||destination.name||'GALAXY').trim()
        });
        setZoomShellStatus(shellState,'LOADING ZOOM ENGINE…');
        let viewport=window.GVUHDViewport;
        if(!viewport||typeof viewport.open!=='function'){
            try{viewport=await uhdViewportPromise}catch(error){uhdViewportLoadError=error;viewport=null}
        }
        if(!viewport||typeof viewport.open!=='function'){
            setZoomShellStatus(shellState,'ZOOM ENGINE COULD NOT LOAD');
            console.error('GV-10AJ VIEW ZOOMABLE UHD UNAVAILABLE',{uhdViewportReady,uhdViewportLoadError:String(uhdViewportLoadError?.message||uhdViewportLoadError||'')});
            return;
        }
        ...
        const opened=Boolean(viewportRoot?.classList.contains('open'));
        if(opened){
            shellState.shell.remove();
        }else{
            setZoomShellStatus(shellState,'ZOOMABLE VIEW COULD NOT OPEN');
            console.error('GV-10AJ VIEWPORT OPEN CALL COMPLETED WITHOUT .open ROOT STATE',{destination,payload});
        }
    },true);
...
})().catch(error=>{console.error('GALAXY VIEWER 10V STARTUP FAILURE:',error);document.dispatchEvent(new CustomEvent('gv-viewer-failed',{detail:{message:String(error?.stack||error)}}));});
"""))

# GV-beta-0010V staged