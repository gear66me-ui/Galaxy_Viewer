...
# Emergency surgical repair: HD overlay enters synchronously on tap; image loading continues asynchronously; visible HD action reads SEE NOW; friends/public website remains untouched.
...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/gv-random-galaxy-0028.js?v=625b8d889f67652fa19a39b8689e306ea1c1fb91-hd-now';
    ...
    randomGalaxy.viewHdButton.textContent='SEE NOW';
    const openHdImmediately=()=>{
        randomGalaxy.hdOverlay?.classList.add('gvrg-hd-open');
        randomGalaxy.hdOpen=true;
    };
    randomGalaxy.viewHdButton?.addEventListener('click',openHdImmediately,true);
    randomGalaxy.hubbleIconButton?.addEventListener('click',openHdImmediately,true);
    ...
    window.addEventListener('beforeunload',()=>{
        ...
        if(aladinPrewarmWaitResolve){const resolve=aladinPrewarmWaitResolve;aladinPrewarmWaitResolve=null;resolve(false)}
        randomGalaxy.viewHdButton?.removeEventListener('click',openHdImmediately,true);
        randomGalaxy.hubbleIconButton?.removeEventListener('click',openHdImmediately,true);
