...
# GV-beta-0010K
# Standalone release derived directly from GV-beta-0010H source.
# Authorized 10K changes only: keep DOWNLOAD IMAGE removed; preserve BACK TO SKY denomination
# and move that original bound control into the lower Galaxy Info panel; enlarge Galaxy Info
# with a lower-right text aperture; restore the approved green Hubble/JWST archive icon to the
# lower-right HD image corner; open each active catalog record's exact sourceUrl in the archive
# overlay; keep a persistent Galaxy Viewer-owned return control over that website.
...
(async()=>{
            ...
            if(url.startsWith('https://raw.githubusercontent.com/')){
                fetch(url,{cache:'no-store'}).then(response=>{
                    ...
                    if(datasetKey==='gvRandomGalaxy0031'){
                        ...
                        if(!source.includes("host === 'esawebb.org'")||!source.includes("provider: cleanText(candidate.provider"))throw new Error('RANDOM GALAXY 0031 JWST COMPATIBILITY PATCH FAILED');
                    }
    ...
    // DOWNLOAD IMAGE is completely removed; HD loading/prefetch remains untouched.
    if(randomGalaxy.downloadButton){
        try{if(randomGalaxy._downloadClick)randomGalaxy.downloadButton.removeEventListener('click',randomGalaxy._downloadClick)}catch(_){}
        try{randomGalaxy.downloadButton.remove()}catch(_){}
        randomGalaxy.downloadButton=null;
    }
    ...
    if(hdBackToSky){
        ...
        Object.assign(hdBackToSky.style,{
            ...
            minWidth:'138px',height:'42px',margin:'0',padding:'0 14px',
    ...
    archiveBack.append(archiveArrow,archiveTarget,archiveBackLabel);
    ...
    function renderHdInfoCandidate(words,count){
        ...
        if(hdBackToSky){
            ...
            const apertureWidth=Math.ceil(buttonRect.width)+18;
            const apertureHeight=Math.ceil(buttonRect.height)+8;
            Object.assign(aperture.style,{
                float:'right',display:'block',width:`${apertureWidth}px`,height:`${apertureHeight}px`,
                marginTop:`${Math.max(0,hdInfoBody.clientHeight-apertureHeight)}px`,marginLeft:'10px',
                pointerEvents:'none',visibility:'hidden'
            });
            hdInfoBody.appendChild(aperture);
        }
        hdInfoBody.appendChild(document.createTextNode(words.slice(0,count).join(' ')+(count<words.length?' …':'')));
    }
    ...
    function fitHdInfoText(destination=currentArchiveDestination()){
        ...
        while(lo<=hi){
            const mid=(lo+hi)>>1;
            renderHdInfoCandidate(words,mid);
            if(hdInfoBody.scrollHeight<=hdInfoBody.clientHeight+1){best=mid;lo=mid+1}else hi=mid-1;
        }
        renderHdInfoCandidate(words,best||words.length);
        hdInfoBody.dataset.fittedCharacters=String(hdInfoBody.textContent.length);
        hdInfoBody.dataset.fittedWords=String(best||words.length);
    }
    ...
    function positionHdInfoPanel(){
        ...
        const safeBottom=Math.min(overlayRect.bottom-HD_LAYOUT.edge,navRect.top-HD_LAYOUT.edge);
        ...
        const imageTarget=overlayRect.height*HD_LAYOUT.imageRatio;
        const scale=Math.min(1,available/Math.max(1,bannerTarget+imageTarget+150));
        const bannerHeight=Math.max(1,Math.floor(bannerTarget*scale));
        const imageHeight=Math.max(1,Math.floor(imageTarget*scale));
...
})().catch(error=>{console.error('GALAXY VIEWER 10K STARTUP FAILURE:',error);document.dispatchEvent(new CustomEvent('gv-viewer-failed',{detail:{message:String(error?.stack||error)}}));});
"""))

# GV-beta-0010K staged