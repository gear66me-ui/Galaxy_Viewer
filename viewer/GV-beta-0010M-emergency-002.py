...
(async()=>{
    ...
    async function orderHdSourcesBySize(destination,signal=null){
        ...
        for(const source of sources){
            const bytes=await probeHdSourceBytes(source,signal);
            source.bytes=bytes;
