...
(async()=>{
    ...
    function randomHubbleProvider({excludeName}={}){
        ...
        if(forcedDestination){
            ...
            const requested=archiveItem?.destination||galaxyCatalog[Math.floor(Math.random()*galaxyCatalog.length)];
            destination=consumeReady(requested,excludeName);
            if(!destination)destination=setUnpreparedActive(requested);
            if(archiveItem)bindActiveArchivePreload(archiveItem,destination);
        }
    ...
    const randomGalaxy=window.GalaxyRandomGalaxy.mount(randomGalaxyHost,{
        ...
        onArrival(destination){
            ...
            if(!activeArchivePreloadItem)preloadArchiveSource(destination);
    ...
    const archivePreloadHost=document.createElement('div');
    archivePreloadHost.id='gv-archive-preload-host';
    Object.assign(archivePreloadHost.style,{position:'fixed',left:'-4px',top:'-4px',width:'2px',height:'2px',overflow:'hidden',opacity:'0',pointerEvents:'none',zIndex:'-1'});
    document.body.appendChild(archivePreloadHost);
    ...
    const activeArchiveFrame=()=>activeArchivePreloadItem?.frame||archiveFrame;
    const mountActiveArchiveFrame=()=>{
        const frame=activeArchiveFrame();
        if(frame===archiveFrame){
            archiveFrame.id='gv-archive-frame';
            if(archiveFrame.parentNode!==archiveOverlay)archiveOverlay.insertBefore(archiveFrame,archiveBack);
            return frame;
        }
        archiveFrame.removeAttribute('id');
        if(archiveFrame.parentNode===archiveOverlay)archiveFrame.remove();
        frame.id='gv-archive-frame';
        frame.title='GALAXY ARCHIVE SOURCE';
        if(frame.parentNode!==archiveOverlay)archiveOverlay.insertBefore(frame,archiveBack);
        return frame;
    };
    const parkActiveArchiveFrame=()=>{
        if(!activeArchivePreloadItem?.frame)return;
        const frame=activeArchivePreloadItem.frame;
        frame.removeAttribute('id');
        frame.style.pointerEvents='none';
        if(frame.parentNode!==archivePreloadHost)archivePreloadHost.appendChild(frame);
        archiveFrame.id='gv-archive-frame';
        if(archiveFrame.parentNode!==archiveOverlay)archiveOverlay.insertBefore(archiveFrame,archiveBack);
    };
    const releaseActiveArchivePreload=()=>{
        if(!activeArchivePreloadItem)return;
        try{activeArchivePreloadItem.frame.remove()}catch(_){}
    ...
    const revealArchiveWhenReady=()=>{
        if(!archiveOpenRequested||archiveClosing||!archiveSourceUrl||archiveLoadedUrl!==archiveSourceUrl)return;
        ...
        const frame=mountActiveArchiveFrame();
    ...
    const chooseArchivePreloadDestination=()=>{
        if(!galaxyCatalog.length)return null;
        ...
        for(let attempt=0;attempt<Math.min(80,galaxyCatalog.length);attempt++){
            const candidate=galaxyCatalog[Math.floor(Math.random()*galaxyCatalog.length)];
            const key=archiveQueueKey(candidate);
            const sourceUrl=String(candidate?.sourceUrl||'').trim();
            if(key&&!excluded.has(key)&&/^https:\/\//i.test(sourceUrl))return candidate;
        }
        return galaxyCatalog.find(candidate=>{
            const key=archiveQueueKey(candidate);
            return key&&!excluded.has(key)&&/^https:\/\//i.test(String(candidate?.sourceUrl||'').trim());
        })||null;
    };
    const startNextArchivePreload=()=>{
        ...
        if(!item){
            ...
            const frame=document.createElement('iframe');
            frame.title='PRELOADED GALAXY ARCHIVE SOURCE';
            frame.setAttribute('aria-hidden','true');
            frame.tabIndex=-1;
            item={key:archiveQueueKey(destination),destination,sourceUrl,frame,state:'pending'};
            archivePreloadQueue.push(item);
            archivePreloadHost.appendChild(frame);
        }
        archivePreloadLoadingItem=item;
        item.state='loading';
        const finish=ready=>{
            if(archivePreloadLoadingItem===item)archivePreloadLoadingItem=null;
            if(!archivePreloadQueue.includes(item))return;
            if(ready){
                item.state='ready';
                ...
                try{item.frame.remove()}catch(_){}
            }
            if(!archivePreloadSuspended&&!backgroundWorkSuspended&&!navigationPending)queueMicrotask(ensureArchivePreloadQueue);
        };
        item.frame.onload=()=>{
            const href=String(item.frame.src||'');
            if(href==='about:blank')return;
            finish(true);
        };
        item.frame.onerror=()=>finish(false);
        item.frame.src=item.sourceUrl;
    };
    const ensureArchivePreloadQueue=()=>{
        ...
        while(archivePreloadQueue.length<ARCHIVE_PRELOAD_TARGET){
            ...
            frame.setAttribute('aria-hidden','true');
            frame.tabIndex=-1;
            archivePreloadQueue.push({key:archiveQueueKey(destination),destination,sourceUrl,frame,state:'pending'});
            archivePreloadHost.appendChild(frame);
        }
    ...
    const suspendArchivePreloads=()=>{
        ...
        if(item&&archivePreloadQueue.includes(item)&&item.state==='loading'){
            item.state='pending';
            item.frame.onload=null;
            item.frame.onerror=null;
            try{item.frame.src='about:blank'}catch(_){}
        }
    };
    ...
    const bindActiveArchivePreload=(item,destination)=>{
        ...
        archiveSourceUrl=item.sourceUrl;
        archiveLoadedUrl=item.state==='ready'?item.sourceUrl:'';
        archiveOpenRequested=false;
    ...
    archiveFrame.addEventListener('load',()=>{
        if(activeArchivePreloadItem||!archiveSourceUrl||archiveClosing)return;
    ...
    hdArchiveButton.addEventListener('click',event=>{
        ...
        if(activeArchivePreloadItem?.key===archiveQueueKey(destination)&&activeArchivePreloadItem.state==='ready'){
            archiveSourceUrl=activeArchivePreloadItem.sourceUrl;
            archiveLoadedUrl=activeArchivePreloadItem.sourceUrl;
        }else if(archiveSourceUrl!==sourceUrl){
    ...
    function syncHdProviderPresentation(destination=currentArchiveDestination()){
        ...
        if(isHdPresentationActive()&&activeArchivePreloadItem?.key!==archiveQueueKey(destination))preloadArchiveSource(destination);
