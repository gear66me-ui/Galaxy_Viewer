...
(async()=>{
    'use strict';
    const VERSION='12AR-132H';
    const DISPLAY_VERSION='12AR-132H';
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0188.js';
    ...
    const RANDOM_GALAXY_VERSION='0188';
    const AVM_LAB_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/lab/gv-avm-overlay-lab-0040.js';
    ...
    const DIAGNOSTICS_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/diagnostics/gv-diagnostics-0016.js';
    ...
    function normalizeCatalogGalaxy(candidate,index,catalogKey,catalogMeta={}){
        if(!candidate||typeof candidate!=='object')throw new Error(`CATALOG ${catalogKey} RECORD ${index} IS NOT AN OBJECT`);

        const name=String(candidate.name||candidate.title||candidate.objectName||candidate.archiveId||candidate.id||`CATALOG ${index+1}`).trim();
        ...
        // 132H: curated catalog records are never silently filtered.
        // Only hard navigation/image contract failures stop startup loudly.
        if(!Number.isFinite(ra)||ra<0||ra>=360)
            throw new Error(`CATALOG ${catalogKey} RECORD ${index} HAS INVALID RA`);
        if(!Number.isFinite(dec)||dec<-90||dec>90)
            throw new Error(`CATALOG ${catalogKey} RECORD ${index} HAS INVALID DEC`);
        if(!Number.isFinite(fovDegrees)||fovDegrees<=0)
            throw new Error(`CATALOG ${catalogKey} RECORD ${index} HAS INVALID FOV`);
        if(!Number.isFinite(aladinRotation))
            throw new Error(`CATALOG ${catalogKey} RECORD ${index} HAS INVALID ROTATION`);

        let hd=null;
        try{ hd=new URL(hdUrl); }catch(_){}
        if(!hd||hd.protocol!=='https:')
            throw new Error(`CATALOG ${catalogKey} RECORD ${index} HAS NO HTTPS IMAGE`);

        let source=null;
        try{
            if(sourceUrl){
                const candidateSource=new URL(sourceUrl);
                if(candidateSource.protocol==='https:')source=candidateSource;
            }
        }catch(_){}
    ...
    async function fetchJsonWithFallback(primaryUrl,label){
        ...
        const cacheMode='no-cache'; // 132H: never let stale provider JSON silently remove catalogs.
    ...
    async function loadMasterCatalog(){
        ...
        const sources=Object.entries(catalogs)
            .map(([rawKey,rawValue])=>{
                ...
                if(!key||!value)
                    throw new Error('MASTER CATALOG CONTAINS INVALID DATABASE POINTER');
                return Object.freeze({
                    key,
    ...
    async function loadGalaxyCatalog(catalogSource){
        ...
        if(Number.isFinite(declared)&&declared!==raw.length)
            console.warn(`GALAXY CATALOG ${key} DECLARED COUNT ${declared} != ACTUAL ${raw.length}; USING ACTUAL ENTRIES`);
...
`;
    ...
    gvAvmTrace('VIEWER_AVM0040_LOAD_START',{url:AVM_LAB_URL});
    loadScriptGithubThenLocal(AVM_LAB_URL,'gvAvmOverlayLab0040').then(()=>{
        gvAvmTrace('VIEWER_AVM0040_LOAD_OK',{version:window.GalaxyViewerAvmOverlayLab?.VERSION||''});
        const lab=window.GalaxyViewerAvmOverlayLab;
        if(!lab){gvAvmTrace('VIEWER_AVM0040_EXPORT_MISSING',{});return}
        if(lab.VERSION!=='0040')gvAvmTrace('VIEWER_AVM0040_VERSION_MISMATCH',{version:String(lab.VERSION||'')});
        try{
            gvAvmTrace('VIEWER_AVM0040_INSTALL_START',{version:lab.VERSION});
            lab.install?.({A,aladin,viewerRoot:root,randomGalaxy});
            gvAvmTrace('VIEWER_AVM0040_INSTALL_OK',{version:lab.VERSION});
        }catch(error){
            gvAvmTrace('VIEWER_AVM0040_INSTALL_FAIL',{message:String(error?.message||error||'')});
            throw error;
        }
    }).catch(error=>{gvAvmTrace('VIEWER_AVM0040_LOAD_FAIL',{message:String(error?.message||error||'')});console.error('GV AVM LAB LOAD FAILURE',error)});
