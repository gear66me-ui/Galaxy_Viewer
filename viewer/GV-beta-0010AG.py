...
# GV-beta-0010AG
# Standalone release derived directly from the exact GV-beta-0010AF baseline.
# 10AG authorized changes only: repair VIEW ZOOMABLE viewport activation, render VIEW / ZOOMABLE
# on two lines without reducing font size or colliding with the arrow, and restore direct provider links
# on the HD screen while preserving all unrelated 10AF behavior.

display(HTML("""
<link rel="stylesheet" href="https://aladin.cds.unistra.fr/AladinLite/api/v3/3.8.2/aladin.min.css" />
<script src="https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/gv-uhd-viewport-0001.js?v=10AG-0001"></script>
<style>
@font-face{
...
(async()=>{
    ...
    // 10AG HD presentation: exact scope is archive/source controls + Galaxy Info + BACK TO SKY + VIEW ZOOMABLE repair.
    ...
    hdArchiveButton.addEventListener('click',event=>{
        ...
        const sourceUrl=String(destination?.sourceUrl||'').trim();
        if(/^https:\/\//i.test(sourceUrl))window.open(sourceUrl,'_blank','noopener,noreferrer');
    },true);
    hdDownloadButton?.addEventListener('click',event=>{
        ...
        const destination=currentArchiveDestination();
        const sourceUrl=String(destination?.sourceUrl||'').trim();
        if(!destination||!/^https:\/\//i.test(sourceUrl)||!window.GVUHDViewport)return;
        const previewImage=randomGalaxy.hdViewport?.querySelector('img:not(#gv-hd-archive-button img)');
...
})().catch(error=>{console.error('GALAXY VIEWER 10AG STARTUP FAILURE:',error);document.dispatchEvent(new CustomEvent('gv-viewer-failed',{detail:{message:String(error?.stack||error)}}));});
"""))

# GV-beta-0010AG staged