...
(async()=>{
    'use strict';
    const VERSION='12AR-19-DOE';
    const DISPLAY_VERSION='12AR-19-DOE';
    ...
    const moduleLoads=Promise.all([
        ...
        loadScriptGithubThenLocal(STUTTER_LAB_URL,'gvStutterLab0007').then(()=>loadScriptGithubThenLocal(RANDOM_GALAXY_URL,'gvRandomGalaxy0096Doe')),
    ...
    if(
        window.GalaxyRandomGalaxy?.VERSION!=='0096' ||
        typeof window.GalaxyRandomGalaxy.bootstrapHomePresentation!=='function'
    )throw new Error('RANDOM GALAXY 0096 HOME BOOTSTRAP EXPORT MISSING');
    ...
    // ==================== RANDOM NAVIGATION ====================
    if(window.GalaxyRandomGalaxy?.VERSION!=='0096')throw new Error('RANDOM GALAXY 0096 EXPORT MISSING OR VERSION MISMATCH');
