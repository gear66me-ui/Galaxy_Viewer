...
(async()=>{
    ...
    archiveArrow.className='gvrg-back-chevron';
    ...
    function renderHdInfoCandidate(words,count){
        ...
        if(hdBackToSky){
            ...
            const overlapTop=Math.max(0,buttonRect.top-bodyRect.top-6);
            const textBottom=Math.max(overlapTop,Math.min(bodyRect.height,buttonRect.bottom-bodyRect.top-6));
            const apertureWidth=Math.max(0,Math.ceil(bodyRect.right-buttonRect.left)+6);
            const apertureHeight=Math.max(0,Math.ceil(textBottom-overlapTop));
            hdInfoBody.style.height=`${Math.max(0,Math.floor(textBottom))}px`;
            hdInfoBody.style.flex='0 0 auto';
            Object.assign(aperture.style,{
                float:'right',display:'block',width:`${apertureWidth}px`,height:`${apertureHeight}px`,
                marginTop:`${overlapTop}px`,marginLeft:'12px',
                pointerEvents:'none',visibility:'hidden'
            });
            hdInfoBody.appendChild(aperture);
        }else{
            hdInfoBody.style.height='';
            hdInfoBody.style.flex='1 1 auto';
        }
    ...
    function positionHdInfoPanel(){
        ...
        const versionRect=bottom.version.getBoundingClientRect();
        if(!overlayRect.height||!overlayRect.width)return;
        const safeBottom=Math.min(overlayRect.bottom-HD_LAYOUT.edge,navRect.top-HD_LAYOUT.edge,versionRect.top-HD_LAYOUT.edge);
