...
# viewer-0008
# Applies exactly two approved changes to viewer-0007:
# 1. Adds a white target box matching the coordinate control.
# 2. Changes the helper instruction text to yellow.
...
if old_target_css not in source:
    raise RuntimeError("viewer-0008: target CSS anchor was not found")
if old_helper_color not in source:
    raise RuntimeError("viewer-0008: helper color anchor was not found")
if old_helper_shadow not in source:
    raise RuntimeError("viewer-0008: helper shadow anchor was not found")
...
source = source.replace("# viewer-0007", "# viewer-0008", 1)

exec(compile(source, "viewer-0008.py", "exec"))
