...
<script>(()=>{const cover=document.getElementById('gv-apk-cover');if(!cover)return;const img=cover.querySelector('img');if(img)img.src='artwork/icon.svg';const version=document.createElement('div');version.className='gv-10e-version';version.textContent='VERSION 10G';cover.appendChild(version)})();</script>
"""))
...
function style(){if(document.getElementById('gv-info-module-0001-style'))return;const s=document.createElement('style');s.id='gv-info-module-0001-style';s.textContent=`
...
function sourceFrom(root){const inst=global.GV10G?.randomGalaxy||global.GV10E?.randomGalaxy||global.__gv10eRandomGalaxy||global.galaxyRandomGalaxy||global.randomGalaxy||null;const d=inst&&(inst.activeDestination||inst.currentGalaxy);return d||{};}
function sourceUrl(d){return clean(d.sourceUrl||d.source_url||d.archiveUrl||d.archive_url);}
function iconUrl(d){const src=clean(d.source||d.telescope||d.facility).toLowerCase();if(src.includes('hubble')||src.includes('esa'))return HUBBLE_ICON;return HUBBLE_ICON;}
function applySmartFrame(root,d){const img=root.querySelector('.gvrg-hd-viewport img,.gvrg-hd-image');if(!img)return;let x=50,y=50;const px=finite(d.targetPixelX??d.target_pixel_x),py=finite(d.targetPixelY??d.target_pixel_y),iw=finite(d.imagePixelWidth??d.image_pixel_width),ih=finite(d.imagePixelHeight??d.image_pixel_height);if(px!=null&&py!=null&&iw>0&&ih>0){x=Math.max(0,Math.min(100,px/iw*100));y=Math.max(0,Math.min(100,py/ih*100));img.dataset.gvCropMethod='wcs-pixel-target'}else{img.dataset.gvCropMethod='geometric-center';}img.style.setProperty('--gv-smart-x',x+'%');img.style.setProperty('--gv-smart-y',y+'%');const angle=finite(d.aladinRotationDeg??d.aladin_rotation_deg??d.positionAngleDeg??d.position_angle_deg);if(angle!=null){img.style.transform=`rotate(${angle}deg)`;img.dataset.gvOrientationMethod='metadata-angle';}else{img.style.transform='none';img.dataset.gvOrientationMethod='source-orientation';}img.dataset.gvPreferredMaxBytes=String(MAX_DISPLAY_BYTES);}
function build(root){style();const hd=root.querySelector('.gvrg-hd-science')?.parentElement||root.querySelector('.gvrg-hd-open,.gvrg-hd');if(!hd)return;let panel=hd.querySelector('.gv-info-panel-10g');if(!panel){panel=document.createElement('section');panel.className='gv-info-panel-10g';panel.setAttribute('aria-label','GALAXY INFORMATION');const title=document.createElement('div');title.className='gv-info-title-10g';title.textContent='INFO:';const text=document.createElement('p');text.className='gv-info-text-10g';text.textContent=PLACEHOLDER;text.dataset.characters=String(PLACEHOLDER_CHARACTERS);const link=document.createElement('a');link.className='gv-info-source-10g';link.target='_blank';link.rel='noopener noreferrer';link.setAttribute('aria-label','OPEN ORIGINAL IMAGE SOURCE');const icon=document.createElement('img');icon.alt='IMAGE SOURCE';link.appendChild(icon);panel.append(title,text,link);const footer=hd.querySelector('.gvrg-hd-footer');if(footer)hd.insertBefore(panel,footer);else hd.appendChild(panel)}const d=sourceFrom(root);const link=panel.querySelector('.gv-info-source-10g');const icon=link.querySelector('img');icon.src=iconUrl(d);const href=sourceUrl(d);if(href){link.href=href;link.style.pointerEvents='auto';link.style.opacity='1'}else{link.removeAttribute('href');link.style.pointerEvents='none';link.style.opacity='.72'}applySmartFrame(hd,d);const credit=hd.querySelector('.gvrg-credit');if(credit)credit.setAttribute('aria-hidden','true');}
function scan(){document.querySelectorAll('.gv-random-galaxy-module-root,.gvrg-root,#gv-random-galaxy-host').forEach(build)}
const observer=new MutationObserver(scan);function start(){style();scan();observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['class','src']});setInterval(scan,1200);}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
global.GalaxyViewerInfo10G=Object.freeze({version:VERSION,placeholder:PLACEHOLDER,placeholderCharacters:PLACEHOLDER_CHARACTERS,maxDisplayBytes:MAX_DISPLAY_BYTES,refresh:scan});
})(window);
"""))
