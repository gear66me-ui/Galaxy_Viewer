...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){
            console.error("GV-beta-0004K STARTUP FAILURE: window.A was not created");
            return;
        }
        ...
        window.A.init.then(() => {
            ...
            function syncProxyBorder(coordinateBox,proxy){proxy.style.setProperty("height","32px","important");proxy.style.setProperty("min-height","32px","important");proxy.style.setProperty("max-height","32px","important");proxy.style.setProperty("border","1px solid #FFFFFF","important");proxy.style.setProperty("border-radius","6px","important");}
        ...
        }).catch(error=>console.error("GV-beta-0004K STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0004K STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
