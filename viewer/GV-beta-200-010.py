...
#gv-avm-test-opacity{position:fixed;left:8px;top:50%;transform:translateY(-50%);z-index:7900;width:42px;height:190px;padding:8px 4px;border:1px solid rgba(111,234,255,.72);border-radius:12px;background:rgba(0,22,54,.82);box-shadow:0 0 12px rgba(76,205,255,.55);display:flex;flex-direction:column;align-items:center;gap:7px;pointer-events:auto}
#gv-avm-test-opacity label{font:700 7px/1.1 monospace;color:#9eefff;writing-mode:vertical-rl;transform:rotate(180deg);letter-spacing:.7px}
#gv-avm-test-opacity input{appearance:slider-vertical;-webkit-appearance:slider-vertical;width:24px;height:138px;writing-mode:vertical-lr;direction:rtl;accent-color:#6feaff}
#gv-avm-test-opacity output{font:700 8px/1 monospace;color:#fff}
#gv-galaxy-nav .gv-galaxy-history{display:flex!important;visibility:visible!important;opacity:1;align-items:center;justify-content:center;width:44px;min-width:44px;height:34px;border:1px solid rgba(111,234,255,.72);border-radius:9px;background:rgba(0,22,54,.78);color:#9eefff;font:700 22px/1 Arial,sans-serif;text-shadow:0 0 7px rgba(88,191,255,.85);pointer-events:auto}
#gv-galaxy-nav .gv-galaxy-history:disabled{opacity:.32!important}
#gv-prefetch-led-rail{position:fixed;right:6px;top:50%;transform:translateY(-50%);z-index:7890;width:min(178px,38vw);padding:7px 6px;border:1px solid rgba(111,234,255,.45);border-radius:10px;background:rgba(0,13,35,.78);font:700 8px/1.15 monospace;color:#c9f7ff;pointer-events:none}
#gv-prefetch-led-rail .gv-led-title{font-size:8px;letter-spacing:.8px;color:#6feaff;text-align:center;margin-bottom:5px}
#gv-prefetch-led-rail .gv-led-row{display:flex;align-items:center;gap:5px;height:17px;white-space:nowrap;overflow:hidden}
#gv-prefetch-led-rail .gv-led{width:8px;height:8px;min-width:8px;border-radius:50%;background:#5a2400;border:1px solid #ff9b35;box-shadow:0 0 4px rgba(255,120,20,.45)}
#gv-prefetch-led-rail .gv-led.ready{background:#42ff7b;border-color:#b7ffca;box-shadow:0 0 7px rgba(66,255,123,.95)}
#gv-prefetch-led-rail .gv-led.error{background:#ff3434;border-color:#ffb2b2;box-shadow:0 0 7px rgba(255,52,52,.85)}
#gv-prefetch-led-rail .gv-led-text{overflow:hidden;text-overflow:ellipsis}
</style>
...
(async()=>{
    'use strict';
    const VERSION='GV-beta-200-010';
    const DISPLAY_VERSION='GV-beta-200-010';
    ...
    const NAVIGATION_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/navigation/gv-navigation-0022.js';
    const NAVIGATION_VERSION='0022';
    const DIAGNOSTICS_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/diagnostics/gv-diagnostics-0019.js';
    ...
    const AVM_METADATA_URL='https://raw.githubusercontent.com/gear66me-ui/Galaxy_Viewer/beta/viewer/image-databases/master-database/avm-metadata/gv-avm-runtime-catalog-0001.json';
    ...
    const moduleLoads=Promise.all([
        ...
        loadScript(DIAGNOSTICS_URL,'gvDiagnostics0019')
    ]);
    ...
    async function loadAvmMetadataCatalog(){
        const payload=
            await fetchJsonWithFallback(
                AVM_METADATA_URL,
                'AVM METADATA CATALOG'
            );

        if(payload?.schema!=='gv-avm-runtime-catalog-0001')
            throw new Error('AVM METADATA CATALOG SCHEMA MISMATCH');

        if(payload?.fovPolicy!=='X')
            throw new Error('AVM METADATA CATALOG FOV POLICY MUST BE X');

        const records=payload?.records;
        const declared=Number(payload?.recordCount);
        const failures=Number(payload?.failureCount);
        ...
        if(
            ...
            'AVM METADATA CATALOG MUST CONTAIN 1871 RECORDS WITH ZERO FAILURES'
        );

        return Object.freeze({
            schema:payload.schema,
            fovPolicy:payload.fovPolicy,
  ...
  lastNorthBearing:null
    ...
    const avmMetadataCatalogPromise=loadAvmMetadataCatalog();
    ...
    const avmMetadataCatalog=await avmMetadataCatalogPromise;

    const canonicalImageUrl=value=>{
        try{const u=new URL(String(value||'').trim());u.hash='';return u.href}catch(_){return String(value||'').trim()}
    };
    const avmByImageUrl=new Map();
    const avmArchiveCounts=new Map();
    const avmUniqueArchive=new Map();
    for(const record of avmMetadataCatalog.records){
        const imageUrl=canonicalImageUrl(record?.imageUrl);
        const archiveId=String(record?.archiveId||'').trim().toLowerCase();
        if(imageUrl)avmByImageUrl.set(imageUrl,record);
        if(archiveId)avmArchiveCounts.set(archiveId,Number(avmArchiveCounts.get(archiveId)||0)+1);
    }
    for(const record of avmMetadataCatalog.records){
        const archiveId=String(record?.archiveId||'').trim().toLowerCase();
        if(archiveId&&avmArchiveCounts.get(archiveId)===1)avmUniqueArchive.set(archiveId,record);
    }
    const findAvmMetadata=destination=>{
        for(const value of [destination?.hdUrl,destination?.selectedImageUrl,destination?.largeUrl,destination?.screenUrl,destination?.avmSourceUrl,destination?.githubImageUrl]){
            const key=canonicalImageUrl(value);
            if(key&&avmByImageUrl.has(key))return avmByImageUrl.get(key);
        }
        const archiveId=String(destination?.archiveId||'').trim().toLowerCase();
        return archiveId?avmUniqueArchive.get(archiveId)||null:null;
    };
    const wcsFromAvmRecord=record=>{
        if(!record)return null;
        const width=Number(record?.referenceDimension?.[0]);
        const height=Number(record?.referenceDimension?.[1]);
        const crpix1=Number(record?.referencePixel?.[0]);
        const crpix2=Number(record?.referencePixel?.[1]);
        const ra=Number(record?.ra),dec=Number(record?.dec);
        const projection=String(record?.projection||'TAN').trim().toUpperCase()||'TAN';
        const frame=String(record?.coordinateFrame||'ICRS').trim().toUpperCase();
        if(![width,height,crpix1,crpix2,ra,dec].every(Number.isFinite)||width<=0||height<=0)return null;
        const galactic=frame==='GAL'||frame==='GALACTIC';
        const wcs={
            NAXIS:2,NAXIS1:width,NAXIS2:height,
            CRPIX1:crpix1,CRPIX2:crpix2,CRVAL1:ra,CRVAL2:dec,
            CTYPE1:(galactic?'GLON-':'RA---')+projection,
            CTYPE2:(galactic?'GLAT-':'DEC--')+projection
        };
        const cd=Array.isArray(record?.cdMatrix)?record.cdMatrix.map(Number):[];
        const scale=Array.isArray(record?.scale)?record.scale.map(Number):[];
        if(cd.length>=4&&cd.slice(0,4).every(Number.isFinite)){
            wcs.CD1_1=cd[0];wcs.CD1_2=cd[1];wcs.CD2_1=cd[2];wcs.CD2_2=cd[3];
        }else if(scale.length>=2&&scale.slice(0,2).every(Number.isFinite)){
            wcs.CDELT1=scale[0];wcs.CDELT2=scale[1];
            const rotation=Number(record?.spatialRotationDeg);
            if(Number.isFinite(rotation))wcs.CROTA2=rotation;
        }else return null;
        return wcs;
    };
    ...
    // ==================== TEMPORARY RANDOM HISTORY CONTROLLER ====================
    ...
    // ==================== NAVIGATION 0022 + TEN-IMAGE DECODE WINDOW ====================
    if(window.GalaxyViewerNavigation?.VERSION!==NAVIGATION_VERSION)
        throw new Error('NAVIGATION 0022 EXPORT MISSING OR VERSION MISMATCH');

    const navigationPlanner=window.GalaxyViewerNavigation.create({catalog:galaxyCatalog});
    const navigationState=await navigationPlanner.initialize(HOME);
    const navigationPlan=navigationPlanner.plan;
    if(!navigationPlan||navigationPlan.poolSize!==130||navigationPlan.routeLength!==100||navigationPlan.route.length!==100)
        throw new Error('NAVIGATION 130->100 CONTRACT FAILED');

    const navigationRoute=[...navigationPlan.route];
    console.info('GV NAVIGATION ROUTE READY',{
        poolSize:navigationPlan.poolSize,
        routeLength:navigationPlan.routeLength,
        sampleAttempt:navigationPlan.sampleAttempt,
        solverAttempt:navigationPlan.solverAttempt,
        solveTimeMs:navigationPlan.solveTimeMs
    });

    const ledRail=document.createElement('div');
    ledRail.id='gv-prefetch-led-rail';
    ledRail.innerHTML='<div class="gv-led-title">IMAGE QUEUE · 10</div>';
    const ledRows=[];
    for(let i=0;i<10;i++){
        const row=document.createElement('div');
        row.className='gv-led-row';
        row.innerHTML='<span class="gv-led"></span><span class="gv-led-text">'+String(i+1).padStart(2,'0')+' · QUEUED</span>';
        ledRail.appendChild(row);
        ledRows.push(row);
    }
    document.body.appendChild(ledRail);

    const predecodedImages=new Map();
    const labelForDestination=d=>{
        const provider=String(d?.provider||d?.telescope||d?.source||'CATALOG').trim().toUpperCase();
        const object=String(d?.designation||d?.commonName||d?.name||d?.archiveId||'GALAXY').trim();
        return provider+' · '+object;
    };
    const setLed=(index,state,d)=>{
        const row=ledRows[index];if(!row)return;
        const led=row.querySelector('.gv-led');
        const text=row.querySelector('.gv-led-text');
        led.classList.toggle('ready',state==='READY');
        led.classList.toggle('error',state==='ERROR');
        text.textContent=String(index+1).padStart(2,'0')+' · '+labelForDestination(d);
        row.title=state+' · '+labelForDestination(d);
    };
    const decodeRouteImage=async(d,index)=>{
        setLed(index,'LOADING',d);
        const record=findAvmMetadata(d);
        const url=String(record?.imageUrl||'').trim();
        const wcs=wcsFromAvmRecord(record);
        if(!url||!wcs){setLed(index,'ERROR',d);throw new Error('QUEUE AVM METADATA/WCS MISSING: '+labelForDestination(d))}
        const image=new Image();
        image.decoding='async';image.loading='eager';image.crossOrigin='anonymous';
        image.src=url;
        try{
            if(image.decode)await image.decode();
            else await new Promise((resolve,reject)=>{image.onload=resolve;image.onerror=reject});
            predecodedImages.set(destinationKey(d),Object.freeze({destination:d,record,url,wcs,image}));
            setLed(index,'READY',d);
            return true;
        }catch(error){
            setLed(index,'ERROR',d);
            console.error('GV QUEUE IMAGE DECODE FAILED',labelForDestination(d),url,error);
            return false;
        }
    };
    const decodeWindow=navigationRoute.slice(0,10);
    decodeWindow.forEach((d,index)=>{void decodeRouteImage(d,index)});

    // ==================== MAKESHIFT RANDOM GALAXY — DIRECT SNAP ONLY ====================
    // Temporary baseline controller. No Random Galaxy module, Navigation module,
    // AVM, preparation, prefetch, travel choreography or image registration.
    const shuffledGalaxyCatalog=[...navigationRoute];
    for(let i=shuffledGalaxyCatalog.length-1;i>0;i--){
        const j=Math.floor(Math.random()*(i+1));
        [shuffledGalaxyCatalog[i],shuffledGalaxyCatalog[j]]=[shuffledGalaxyCatalog[j],shuffledGalaxyCatalog[i]];
    }
    ...
    const destinationFov=destination=>{
        for(const value of [destination?.fov,destination?.fovDegrees,destination?.fieldOfView,destination?.fieldOfViewDegrees,destination?.imageFovDegrees]){
            const n=Number(value); if(Number.isFinite(n)&&n>0)return n;
        }
        return .15;
    };
    const AVM_TEST_LAYER='Galaxy Viewer AVM Test Image';
    let avmTestOverlay=null;
    let avmTestOpacity=1;
    const opacityPanel=document.createElement('div');
    opacityPanel.id='gv-avm-test-opacity';
    opacityPanel.innerHTML='<label for="gv-avm-opacity-range">AVM OPACITY</label><input id="gv-avm-opacity-range" type="range" min="0" max="100" value="100" step="1" orient="vertical"><output>100%</output>';
    document.body.appendChild(opacityPanel);
    const opacityRange=opacityPanel.querySelector('input');
    const opacityOutput=opacityPanel.querySelector('output');
    const applyAvmOpacity=()=>{
        avmTestOpacity=Number(opacityRange.value)/100;
        opacityOutput.textContent=Math.round(avmTestOpacity*100)+'%';
        const target=avmTestOverlay;
        try{target?.setOpacity?.(avmTestOpacity)}catch(_){}
        try{target?.setAlpha?.(avmTestOpacity)}catch(_){}
        try{if(target?.options)target.options.opacity=avmTestOpacity}catch(_){}
    };
    opacityRange.addEventListener('input',applyAvmOpacity);
    const showAvmImage=destination=>{
        const record=findAvmMetadata(destination);
        const url=String(record?.imageUrl||'').trim();
        const wcs=wcsFromAvmRecord(record);
        if(!record||!url||!wcs){
            console.error('AVM TEST METADATA/WCS MISSING',{destination,record});
            try{window.GalaxyViewerDiagnostics?.recordAvm?.('AVM_TEST_METADATA_WCS_MISSING',{name:destination?.name||'',archiveId:destination?.archiveId||'',matched:Boolean(record),url:Boolean(url),wcs:Boolean(wcs)})}catch(_){}
            return;
        }
        try{
            if(avmTestOverlay&&typeof aladin.removeOverlayImageLayer==='function')
                aladin.removeOverlayImageLayer(AVM_TEST_LAYER);
        }catch(_){}
        avmTestOverlay=null;
        try{
            const imageLayer=A.image(url,{
                name:AVM_TEST_LAYER,
                imgFormat:'jpeg',
                wcs,
                opacity:avmTestOpacity,
                successCallback:(ra,dec,fov,image)=>{
                    avmTestOverlay=imageLayer;
                    applyAvmOpacity();
                    console.info('AVM TEST IMAGE VISIBLE',{url,ra,dec,fov,archiveId:record?.archiveId,authority:record?.authority,wcs});
                    try{window.GalaxyViewerDiagnostics?.recordAvm?.('AVM_TEST_IMAGE_VISIBLE',{url,ra,dec,fov,archiveId:record?.archiveId||'',authority:record?.authority||'',wcs:true})}catch(_){}
                },
                errorCallback:error=>{
                    console.error('AVM TEST IMAGE LOAD FAILED',url,error);
                    try{window.GalaxyViewerDiagnostics?.recordAvm?.('AVM_TEST_IMAGE_FAILED',{url,message:String(error?.message||error||'')})}catch(_){}
                }
            });
            aladin.setOverlayImageLayer(imageLayer,AVM_TEST_LAYER);
        }catch(error){
            console.error('AVM TEST IMAGE INSTALL FAILED',url,error);
        }
    };
    const showDestination=destination=>{
        const ra=Number(destination?.ra),dec=Number(destination?.dec);
        if(!Number.isFinite(ra)||!Number.isFinite(dec))throw new Error('TEMP RANDOM DESTINATION INVALID RA/DEC');
        aladin.gotoRaDec(ra,dec);
        aladin.setFov(destinationFov(destination));
        showAvmImage(destination);
        return destination;
    };
    const syncHistoryButtons=()=>{
        bottom.back.disabled=historyIndex<=0;
        bottom.forward.disabled=historyIndex<0||historyIndex>=history.length-1;
    };
    const chooseRandom=()=>{
        if(!shuffledGalaxyCatalog.length)throw new Error('TEMP RANDOM CATALOG EMPTY');
        const destination=shuffledGalaxyCatalog[randomCursor%shuffledGalaxyCatalog.length];
        randomCursor++;
        if(historyIndex<history.length-1)history.splice(historyIndex+1);
        history.push(destination);historyIndex=history.length-1;
        showDestination(destination);syncHistoryButtons();
    };
    const goBack=()=>{
        if(historyIndex<=0)return;
        historyIndex--;showDestination(history[historyIndex]);syncHistoryButtons();
    };
    const goForward=()=>{
        if(historyIndex<0||historyIndex>=history.length-1)return;
        historyIndex++;showDestination(history[historyIndex]);syncHistoryButtons();
    };
    bottom.random.addEventListener('click',chooseRandom);
    bottom.back.addEventListener('click',goBack);
    bottom.forward.addEventListener('click',goForward);
    syncHistoryButtons();

    const randomGalaxy=Object.freeze({
        next:chooseRandom,
        back:goBack,
        forward:goForward,
    ...
    bottom.nav.style.visibility='visible';
    bottom.nav.style.opacity='1';
