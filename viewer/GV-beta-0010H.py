...
(async()=>{
    ...
    if(hdBackToViewer){
        const backLabel=hdBackToViewer.lastElementChild;
    ...
    hdArchiveButton.className='gvrg-button gvrg-hd-icon-button';
    ...
    const hdControls=randomGalaxy.hdFooter?.querySelector('.gvrg-hd-controls');
    if(hdControls)hdControls.insertBefore(hdArchiveButton,hdControls.firstChild);
    ...
    archiveBack.textContent='← BACK TO GALAXY VIEWER';
    ...
    const closeArchiveOverlay=()=>{archiveOverlay.classList.remove('gv-open');archiveFrame.src='about:blank'};
    ...
    function fitHdInfoText(destination=currentArchiveDestination()){
        ...
        let lo=0,hi=words.length,best='';
        while(lo<=hi){
            const mid=(lo+hi)>>1;
            const candidate=words.slice(0,mid).join(' ')+(mid<words.length?' …':'');
            hdInfoBody.textContent=candidate;
            if(hdInfoBody.scrollHeight<=hdInfoBody.clientHeight+1){best=candidate;lo=mid+1}else hi=mid-1;
        }
        hdInfoBody.textContent=best||full;
        hdInfoBody.dataset.fittedCharacters=String(hdInfoBody.textContent.length);
        hdInfoBody.dataset.fittedWords=String((hdInfoBody.textContent.match(/\S+/g)||[]).length);
    }
    ...
    function positionHdInfoPanel(){
        ...
        const scale=Math.min(1,available/Math.max(1,bannerTarget*2+imageTarget));
        ...
        if(hdBackToViewer)hdBackToViewer.style.pointerEvents='auto';
...
})().catch(error=>{console.error('GALAXY VIEWER 10H STARTUP FAILURE:',error);document.dispatchEvent(new CustomEvent('gv-viewer-failed',{detail:{message:String(error?.stack||error)}}));});
"""))

# GV-beta-0010H staged