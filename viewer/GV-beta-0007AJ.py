...
(async()=>{
  ...
  const waitFor=(test,timeout=20000)=>new Promise((resolve,reject)=>{const end=performance.now()+timeout;const tick=()=>{let value=null;try{value=test()}catch(_){ }if(value){resolve(value);return}if(performance.now()>end){reject(new Error("GV-BETA-0007AJ STARTUP TIMEOUT"));return}setTimeout(tick,50)};tick()});
  ...
  if(!htmlMatches.length||!jsMatches.length)throw new Error("GV-BETA-0007AJ COULD NOT EXTRACT 7AD BASELINE");
  ...
  versionLabel.textContent="V-7AJ";
  window.GV7AJ_VALIDATION={passed:false,pending:true,status:"AWAITING PROJECTION SUBMENU"};
  ...
  function menuPulseDiagnostics(c){
    ...
    return {
      ...
      allRunning:projectionOpen?menuAnimations.length===0:menuAnimations.length>0&&menuAnimations.every(a=>a.playState==="running"),
      sharedStart:projectionOpen?menuAnimations.length===0:menuAnimations.length>0&&menuAnimations.every(a=>a.startTime===globalPulseStart),
      durationMatch:projectionOpen?menuAnimations.length===0:timings.length>0&&timings.every(t=>t.duration===CYCLE),
      easingMatch:projectionOpen?menuAnimations.length===0:timings.length>0&&timings.every(t=>normalizeEasing(t.easing)===normalizeEasing(EASING)),
  ...
  function prepareAndPatch(){
    ...
    patched=true;
    observer?.disconnect();
    finalizeAndValidate(baseline).catch(error=>console.error("GV-BETA-0007AJ FINALIZATION FAILURE:",error));
    return true;
  }
  ...
  async function validate(baseline,centering){
    ...
    const checks={
      versionLabel:versionLabel.textContent==="V-7AJ",
    ...
    if(!passed)throw new Error("GV-BETA-0007AJ CONTRACT FAILED "+JSON.stringify(window.GV7AJ_VALIDATION));
    return true;
  }

  let coordinateGlowError=null;
  const coordinateGlowPromise=startCoordinateGlow().catch(error=>{coordinateGlowError=error;window.GV7AJ_COORDINATE_GLOW={passed:false,error:String(error)};return null});
  const projectionPulseReady=initializeProjectionPulse().catch(error=>{console.error("GV-BETA-0007AJ PROJECTION PULSE INITIALIZATION FAILURE:",error);return false});
...
})().catch(error=>console.error("GV-BETA-0007AJ STARTUP FAILURE:",error));
"""))

# GV-beta-0007AJ staged