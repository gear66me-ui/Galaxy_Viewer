...
AUTHORIZED BASELINE: gv-random-galaxy-0009.js blob aa001831a0e84633b10e528bca1b3d2c5ce551bf.
...
(function (global) {
  ...
  const FONT_NAMES = Object.freeze({
    spaceAge: 'GV Random Galaxy Space Age 0010',
    digits: 'GV Random Galaxy Digits 0010'
  });
...
.gvrg-card{position:absolute;left:50%;top:75%;transform:translate(-50%,-50%);width:min(286px,78vw);padding:7px 8px 8px;border:1px solid rgba(175,225,255,.66);border-radius:6px;background:rgba(0,8,17,.42);box-shadow:0 0 10px rgba(80,190,255,.16);opacity:0;transition:opacity .20s ease;pointer-events:none}
...
.gvrg-label{font:400 9px/1.15 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:.55px;color:#82b9d4;text-align:center;pointer-events:none}
.gvrg-value{min-width:0;min-height:14px;margin-top:1px;font:400 11px/1.18 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:.2px;color:#e9f9ff;text-align:center;white-space:normal;overflow-wrap:anywhere;pointer-events:none}
.gvrg-card-distance{display:block;min-width:0}
.gvrg-value-number{font:400 11px/1.18 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:.2px}
...
.gvrg-hd-loading{position:absolute;left:50%;top:50%;z-index:2;transform:translate(-50%,-50%);font:400 9px/1.4 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:1px;color:#dff7ff;pointer-events:none}`;
    ...
    async #getHubbleCandidate(excludeName) {
      if (!this.hubbleProvider) throw new Error('Random Galaxy 0010 requires the local Hubble catalog provider.');
    ...
    #normalizeHubbleCandidate(candidate) {
      ...
      return Object.freeze({
        ...
        hubble: true,
        name, ra, dec, distance, constellation, age, ageYears, physicalSizeLy, designation, commonName, preparedHdUrl, preparedSource,
    ...
    showHubbleHD() {
      ...
      const displayUrl = destination.preparedHdUrl || destination.hdUrl;
      this.#resetHdTransform();
      this.hdImage.removeAttribute('src');
      this.hdImage.alt = destination.name;
      this.hdLoading.textContent = destination.preparedHdUrl ? '' : 'LOADING HUBBLE HD';
      this.hdLoading.style.display = destination.preparedHdUrl ? 'none' : 'block';
      ...
      this.hdImage.onerror = () => { this.hdLoading.textContent = 'HUBBLE HD IMAGE COULD NOT LOAD'; this.hdLoading.style.display = 'block'; };
      this.hdImage.src = displayUrl;
      return displayUrl;
    }
    ...
    setPreparedHdResource(key, preparedHdUrl, preparedSource = '') {
      const destination = this.activeDestination;
      ...
      if (!destination || !requested || activeKey !== requested || !prepared) return false;
      this.activeDestination = Object.freeze({ ...destination, preparedHdUrl: prepared, preparedSource: cleanText(preparedSource) });
      return true;
    }
