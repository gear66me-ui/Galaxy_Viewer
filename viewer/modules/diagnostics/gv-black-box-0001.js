...
(function(global){
  ...
  function recordNavigation(sample={}){
    ...
    writeChain=writeChain.then(async()=>{const run=await activeRunPromise;if(run)await appendSample(run.id,enriched);}).catch(error=>console.error('BLACK BOX SAMPLE FAILURE',error));
    return true;
  }
  ...
  global.GalaxyBlackBox=Object.freeze({version:VERSION,openDb,startRun,appendSample,finishRun,getIncompleteRuns,getRunBundle,beginNavigation,recordNavigation,endNavigation});
})(window);
