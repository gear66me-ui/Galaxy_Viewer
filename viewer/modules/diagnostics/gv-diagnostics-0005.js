...
(() => {
  'use strict';
  const VERSION='0005',POLL_MS=1000,MAX_SNAPSHOTS=3600,MAX_ERRORS=500,MAX_EVENTS=12000;
  ...
  function ensureStyle(){
    if(document.getElementById('gv-diagnostics-0005-style'))return;
    const style=document.createElement('style');style.id='gv-diagnostics-0005-style';style.textContent=`
#gv-diagnostics-0005{position:fixed;right:5px;top:56px;z-index:2147483200;width:min(355px,calc(100vw - 10px));max-height:52vh;overflow:auto;padding:5px;border:1px solid #7CCBFF;border-radius:6px;background:linear-gradient(145deg,rgba(5,20,45,.97),rgba(10,48,112,.95));color:#DDF8FF;font:7px/1.16 monospace;box-shadow:0 0 10px rgba(88,191,255,.35)}
#gv-diagnostics-0005 *{box-sizing:border-box}
#gv-diagnostics-0005 header{display:flex;align-items:center;justify-content:space-between;gap:4px;color:#9BE5FF;font-weight:700;margin-bottom:3px}
#gv-diagnostics-0005 button{padding:2px 4px;border:1px solid #7CCBFF;border-radius:3px;background:#081B3A;color:#EAF8FF;font:6.5px monospace}
#gv-diagnostics-0005 table{width:100%;border-collapse:collapse;table-layout:fixed}
#gv-diagnostics-0005 td,#gv-diagnostics-0005 th{padding:1.5px 2px;border:1px solid rgba(124,203,255,.23);text-align:left;vertical-align:top;overflow-wrap:anywhere}
#gv-diagnostics-0005 th{color:#9BE5FF;background:#081B3A}
#gv-diagnostics-0005 .s{color:#9BE5FF;margin:3px 0 2px;font-weight:700}
#gv-diagnostics-0005 .status{color:#78FFAB;margin-bottom:3px}
#gv-diagnostics-0005 .nav{margin:2px 0;color:#DDF8FF;white-space:normal}
#gv-diagnostics-0005 .led{font-size:10px;line-height:1;display:inline-block;min-width:12px;text-align:center}
#gv-diagnostics-0005 .target{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:170px}`;
...
<td><span class="led" title="${esc(r.web.title||'')}">${ledGlyph(r.web.led)}</span></td>
  ...
  function download(){
    ...
    console.error('DIAGNOSTICS 0005 DOWNLOAD BRIDGE MISSING',filename);return false;
  }
