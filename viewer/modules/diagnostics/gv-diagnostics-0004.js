/* Galaxy Viewer automatic analytics diagnostics 0004. Collection starts immediately and never pauses. */
(() => {
  'use strict';
  const VERSION='0004',POLL_MS=1000,MAX_SNAPSHOTS=3600,MAX_ERRORS=500,MAX_EVENTS=12000;
  const startedAt=performance.now();
  const errors=[],snapshots=[],localEvents=[];
  let panel=null,statusEl=null,bodyEl=null,navEl=null,queueEl=null,eventEl=null,timer=0;
  ...
  function ensureStyle(){
    if(document.getElementById('gv-diagnostics-0004-style'))return;
    const style=document.createElement('style');style.id='gv-diagnostics-0004-style';style.textContent=`
#gv-diagnostics-0004{position:fixed;right:5px;top:56px;z-index:2147483200;width:min(355px,calc(100vw - 10px));max-height:52vh;overflow:auto;padding:5px;border:1px solid #7CCBFF;border-radius:6px;background:linear-gradient(145deg,rgba(5,20,45,.97),rgba(10,48,112,.95));color:#DDF8FF;font:7px/1.16 monospace;box-shadow:0 0 10px rgba(88,191,255,.35)}
#gv-diagnostics-0004 *{box-sizing:border-box}
#gv-diagnostics-0004 header{display:flex;align-items:center;justify-content:space-between;gap:4px;color:#9BE5FF;font-weight:700;margin-bottom:3px}
#gv-diagnostics-0004 button{padding:2px 4px;border:1px solid #7CCBFF;border-radius:3px;background:#081B3A;color:#EAF8FF;font:6.5px monospace}
#gv-diagnostics-0004 table{width:100%;border-collapse:collapse;table-layout:fixed}
#gv-diagnostics-0004 td,#gv-diagnostics-0004 th{padding:1.5px 2px;border:1px solid rgba(124,203,255,.23);text-align:left;vertical-align:top;overflow-wrap:anywhere}
#gv-diagnostics-0004 th{color:#9BE5FF;background:#081B3A}
#gv-diagnostics-0004 .s{color:#9BE5FF;margin:3px 0 2px;font-weight:700}
#gv-diagnostics-0004 .status{color:#78FFAB;margin-bottom:3px}
#gv-diagnostics-0004 .nav{margin:2px 0;color:#DDF8FF;white-space:normal}
#gv-diagnostics-0004 .led{font-size:10px;line-height:1;display:inline-block;min-width:12px;text-align:center}
#gv-diagnostics-0004 .target{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:170px}`;
    document.head.appendChild(style);
  }
  function render(s=snapshots.at(-1)||summary()){
    if(!panel)return;
    statusEl.textContent=`AUTO — COLLECTING FROM START · ${s.second}s · ${snapshots.length} SNAPSHOTS`;
    bodyEl.innerHTML=[
...
<td><span class="led" title="${esc(r.web.title||'')}">${ledGlyph(r.web.led)}</span></td>
  ...
  function download(){
    ...
    console.error('DIAGNOSTICS 0004 DOWNLOAD BRIDGE MISSING',filename);return false;
  }
