(function (global) {
  ...
  const FONT_NAMES = Object.freeze({
    spaceAge: 'GV Random Galaxy Space Age 0004',
    digits: 'GV Random Galaxy Digits 0004'
  });
  ...
  const DEFAULTS = Object.freeze({
    ...
    discoveryTermsPerScan: 4,
    discoveryPagesPerTerm: 2,
    discoveryQueueLimit: 500,
...
.gvrg-hd-loading{
  ...
  font:400 9px/1.4 \"${FONT_NAMES.spaceAge}\",sans-serif;letter-spacing:1px;color:#dff7ff
    ...
    async #discoverHubbleCandidate(excludeName) {
      ...
      const attempts = Math.max(20, Number(this.options.discoveryAttempts) || 120);
      const termCount = clamp(Number(this.options.discoveryTermsPerScan) || 4, 1, NASA_SEARCH_TERMS.length);
      const pagesPerTerm = Math.max(1, Number(this.options.discoveryPagesPerTerm) || 2);
      const queueLimit = Math.max(attempts, Number(this.options.discoveryQueueLimit) || 500);
