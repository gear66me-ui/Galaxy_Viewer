...
AUTHORIZED BASELINE: gv-random-galaxy-0005.js blob c0b3cc5e2bc7bb213c0aa00a900c93488d978101.
...
(function (global) {
  ...
  const FONT_NAMES = Object.freeze({
    spaceAge: 'GV Random Galaxy Space Age 0006',
    digits: 'GV Random Galaxy Digits 0006'
  });
...
.gvrg-card{position:absolute;left:50%;top:78%;transform:translate(-50%,-50%);width:min(390px,88%);padding:12px 14px;border:1px solid rgba(175,225,255,.78);border-radius:7px;background:rgba(0,8,17,.90);box-shadow:0 0 15px rgba(80,190,255,.24);opacity:0;transition:opacity .28s ease;pointer-events:none}
.gvrg-card-visible{opacity:1;pointer-events:auto}
.gvrg-name{font:400 18px/1.18 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:1.2px;color:#fff;text-shadow:0 0 7px rgba(145,220,255,.50);margin-bottom:9px;white-space:normal;overflow-wrap:anywhere}
.gvrg-row{display:grid;grid-template-columns:128px minmax(0,1fr);gap:10px;align-items:baseline;margin-top:6px}
.gvrg-label{font:400 8px/1.4 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:1px;color:#82b9d4}
.gvrg-value{min-width:0;font:400 11px/1.4 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:.55px;color:#e9f9ff;white-space:normal;overflow-wrap:anywhere}
.gvrg-card-distance{display:flex;align-items:baseline;gap:6px;min-width:0;flex-wrap:wrap}
.gvrg-value-number{font:400 13px/1.4 "${FONT_NAMES.digits}",sans-serif;letter-spacing:0}
.gvrg-value-unit{font:400 9px/1.4 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:.45px}
.gvrg-actions{display:flex;gap:7px;align-items:center;margin-top:11px;pointer-events:auto}
.gvrg-button{appearance:none;border:1px solid rgba(175,225,255,.84);border-radius:4px;background:rgba(7,27,42,.95);color:#effbff;padding:8px 11px;font:400 9px/1 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:.9px;cursor:pointer}
...
.gvrg-hd-loading{position:absolute;left:50%;top:50%;z-index:2;transform:translate(-50%,-50%);font:400 9px/1.4 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:1px;color:#dff7ff;pointer-events:none}`;
    ...
    #build() {
      ...
      const ageRow = makeRow('GALAXY AGE');
    ...
    async #getHubbleCandidate(excludeName) {
      if (!this.hubbleProvider) throw new Error('Random Galaxy 0006 requires the local Hubble catalog provider.');
