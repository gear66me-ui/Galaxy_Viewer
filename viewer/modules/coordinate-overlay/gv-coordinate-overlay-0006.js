(function(global){
  ...
  async function ensureFonts(){
    ...
    fontsPromise=(async()=>{
      ...
      await Promise.all([
        document.fonts.load(`400 12.5px "${FONT_NAMES.spaceAge}"`,'ICRSD'),
        document.fonts.load(`400 20px "${FONT_NAMES.digits}"`,'359.9999 -90.0000')
      ]);
  ...
  class GalaxyCoordinateOverlay{
    constructor(container,options={}){
      ...
      this.container=container;this.frame='GAL';this.longitude=0;this.latitude=0;this.destroyed=false;
...
.gvco-root{position:relative;width:${GEOMETRY.width}px;height:${GEOMETRY.height}px;overflow:hidden;background:linear-gradient(145deg,#081B3A 0%,#0B3177 44%,#123F86 70%,#296DBD 100%);border:1px solid #7CCBFF;border-radius:6px;box-shadow:inset 0 2px 2px rgba(225,251,255,.82),inset 0 -3px 5px rgba(0,0,0,.52),inset 0 0 13px rgba(41,153,255,.34),0 0 3px #DDF8FF,0 0 9px rgba(50,190,255,.72),0 0 18px rgba(20,116,219,.35);color:#DDF8FF;text-shadow:0 0 4px rgba(124,203,255,.42);user-select:none;-webkit-user-select:none}
...
`;
    ...
    async #init(){
      await ensureFonts();if(this.destroyed)return this;
      let widest=0;for(const c of '0123456789')widest=Math.max(widest,scan(this.ctx,this.canvas,c,FONT_NAMES.digits,20));
      this.metrics={digit:Math.ceil(widest),decimal:Math.ceil(scan(this.ctx,this.canvas,'.',FONT_NAMES.digits,20)),minus:Math.ceil(scan(this.ctx,this.canvas,'-',FONT_NAMES.digits,20))};
    ...
    #renderNumber(parent,value,anchor,integerSlots,signWidth){if(!this.metrics)return;const {digit,decimal}=this.metrics;parent.textContent='';const dl=anchor-decimal/2;this.#cell(parent,'.',dl,decimal);let cursor=dl,integer=value.integer.padStart(integerSlots,' ');for(let i=integer.length-1;i>=0;i--){cursor-=digit;if(integer[i]!==' ')this.#cell(parent,integer[i],cursor,digit)}if(signWidth){cursor-=signWidth;if(value.sign)this.#cell(parent,value.sign,cursor,signWidth)}cursor=dl+decimal;for(const c of value.fraction){this.#cell(parent,c,cursor,digit);cursor+=digit}}
