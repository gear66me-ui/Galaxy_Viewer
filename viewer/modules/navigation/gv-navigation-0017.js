...
(function(global){
  ...
  async function flyViewport({
    ...
    if(!aladin)throw new Error('NAVIGATION 0017 ALADIN INSTANCE MISSING');
    ...
    if(activeFlightControl && !activeFlightControl.settled)throw new Error('NAVIGATION 0017 ACTIVE FLIGHT ALREADY EXISTS');
...
if(flightControl.aborted){
  const error=new Error(`NAVIGATION 0017 ABORTED: ${flightControl.reason||'external-abort'}`);
...
resolve();
  ...
  function commandGotoRaDec(aladin,ra,dec){
    if(!aladin || typeof aladin.gotoRaDec !== 'function')
      throw new Error('NAVIGATION 0017 gotoRaDec UNAVAILABLE');
    aladin.gotoRaDec(ra,dec);
  }

  function commandSetFov(aladin,fov){
    if(!aladin || typeof aladin.setFov !== 'function')
      throw new Error('NAVIGATION 0017 setFov UNAVAILABLE');
    aladin.setFov(fov);
  }
