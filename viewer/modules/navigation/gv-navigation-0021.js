/*
 * GALAXY VIEWER — RANDOM NAVIGATION MODULE 0021
 *
 * 0021 — FLIGHT WATCHDOG + ACTIVE-FLIGHT CLEANUP
 *   - Bounds a flight to choreography duration + 10 seconds.
 *   - Clears activeFlightControl on success, abort, exception, or timeout.
 *   - Prevents a stalled requestAnimationFrame chain from leaving TRAVELING locked forever.
 *
 * PURPOSE
...
(function(global){
  ...
  const CONSTANTS = Object.freeze({
    ...
    TRANSLATION_MIN_DEG: 65,
    TRANSLATION_MAX_DEG: 125,
  ...
  const NAVIGATION_WORKER_VERSION='0009';
  ...
  class RoutePlanner{
    constructor({catalog,rng=Math.random}={}){
      ...
      this.selectionHistory=new Set();
      this.selectionHistoryLimit=1000;
      this.selectionHistoryResets=0;
    }

    async solvePlan(anchorRecord=null){
      const reservedKeys=new Set();

      if(this.plan?.route?.length){
        for(const record of this.plan.route.slice(this.cursor)){
          const key=identityOf(record);
          if(key)reservedKeys.add(key);
        }
      }

      if(this.refillPlan?.route?.length){
        for(const record of this.refillPlan.route){
          const key=identityOf(record);
          if(key)reservedKeys.add(key);
        }
      }

      const releaseCommittedHistory=
        this.selectionHistory.size>0 &&
        this.selectionHistory.size + this.remaining() >=
          this.selectionHistoryLimit;

      const eligibleCatalog=this.catalog.filter(record=>{
        const key=identityOf(record);
        if(!key)return true;
        if(reservedKeys.has(key))return false;
        return (
          releaseCommittedHistory ||
          !this.selectionHistory.has(key)
        );
      });
    ...
    commitNext(expectedRecord=null){
      ...
      if(current.key)this.selectionHistory.add(current.key);

      if(this.selectionHistory.size===this.selectionHistoryLimit){
        this.selectionHistory.clear();
        this.selectionHistoryResets++;
      }
    ...
    getState(){
      return Object.freeze({
        ...
        selectionHistorySize:this.selectionHistory.size,
        selectionHistoryLimit:this.selectionHistoryLimit,
        selectionHistoryResets:this.selectionHistoryResets,
  ...
  async function flyViewport({
    ...
    rotationStart=0,aladinTravelHz=15,onDistanceProgress=null,onTimelineProgress=null,onPhaseChange=null,trace=null,traceError=null
  }={}){
    ...
    const flightWatchdogMs=Math.max(15000,duration+10000);
    let flightWatchdog=0;
    ...
    try{
      await new Promise((resolve,reject)=>{
        ...
        flightWatchdog=setTimeout(()=>{
          flightControl.aborted=true;
          flightControl.reason='watchdog-timeout';
          const error=new Error(
            `NAVIGATION 0021 FLIGHT WATCHDOG TIMEOUT AFTER ${flightWatchdogMs}ms`
          );
          error.code='GV_NAV_TIMEOUT';
          emitTraceError(4774,'FLIGHT_WATCHDOG_TIMEOUT',error);
          finish(reject,error);
        },flightWatchdogMs);
...
if(flightControl.aborted){
  ...
  error.name='AbortError'; finish(reject,error); return;
}
...
if(t<1 && sample!==lastAladinSample){
  ...
  if(phase!==lastTravelPhase){
    const previousPhase=lastTravelPhase;
    lastTravelPhase=phase;
    ...
    if(typeof onPhaseChange==='function'){
      try{
        ...
        try{
          traceError?.(
            4776,
            'NAVIGATION_PHASE_CALLBACK_ERROR',
            error
          );
        }catch(_){}
      }
...
finish(resolve);
        }catch(error){emitTraceError(4775,'RAF_EXCEPTION',error);finish(reject,error);}
      };
