...
(function(global){
  ...
  const NAVIGATION_WORKER_VERSION='0003';
  ...
  function navigationWorkerUrl(){
    if(!NAVIGATION_MODULE_URL)
      throw new Error(
        'NAVIGATION 0008 WORKER BASE URL UNAVAILABLE'
      );

    return new URL(
      'gv-navigation-worker-0003.js',
  ...
  function solvePlanInWorker(catalog,anchorRecord=null){
    return new Promise((resolve,reject)=>{
      ...
      const timer=setTimeout(
        ()=>finish(
          reject,
          new Error('NAVIGATION 0008 WORKER TIMEOUT')
        ),
      ...
      worker.onmessage=event=>{
        ...
        if(
          ...
          finish(
            ...
            new Error(
              data.error ||
              'NAVIGATION 0008 WORKER RESULT INVALID'
            )
          );
          return;
        }
  ...
  function materializeWorkerPlan(
    ...
    if(
      ...
      throw new Error(
        'NAVIGATION 0008 WORKER ROUTE MATERIALIZATION FAILURE'
      );
    }
    ...
    if(!validation.ok){
      throw new Error(
        `NAVIGATION 0008 WORKER ROUTE VALIDATION FAILURE: ${
  ...
  function flightDistanceProgress(t){
    ...
    // ==========================================================
    // NAVIGATION 0008 — CONTINUOUS ARCH TRAJECTORY
    //
    ...
    const FLIGHT_ARCH=Object.freeze({
      T_TRANSLATE_START:3.5/15,
      T_APEX:6.5/15,
      T_DECEL:12/15,
      TRANSLATION_AT_DECEL:0.85,
      TRANSLATION_SLOPE_AT_DECEL:1.20,
      FOV_ASCENT_AT_TRANSLATE_START:0.55,
      FOV_ASCENT_SLOPE_AT_TRANSLATE_START:2.20,
      FOV_DESCENT_AT_DECEL:0.80,
      FOV_DESCENT_SLOPE_AT_DECEL:1.20
    });
    ...
    const FLIGHT_BOOTSTRAP=Object.freeze({
      T_GENTLE_END:1.5/9,
      T_DECEL:7.5/9,
      TRANSLATION_AT_GENTLE_END:0.10,
      TRANSLATION_SLOPE_AT_GENTLE_END:0.60,
      TRANSLATION_AT_DECEL:0.86,
      TRANSLATION_SLOPE_AT_DECEL:1.05,
      FOV_AT_GENTLE_END:0.06,
      FOV_SLOPE_AT_GENTLE_END:0.25,
      FOV_AT_DECEL:0.88,
      FOV_SLOPE_AT_DECEL:0.95
    });

    function flightBootstrapTranslationProgress(t){
      const uTime=flightClamp01(t);
      const {T_GENTLE_END,T_DECEL,
        TRANSLATION_AT_GENTLE_END,
        TRANSLATION_SLOPE_AT_GENTLE_END,
        TRANSLATION_AT_DECEL,
        TRANSLATION_SLOPE_AT_DECEL}=FLIGHT_BOOTSTRAP;
      if(uTime<=T_GENTLE_END){
        const span=T_GENTLE_END;
        const u=uTime/Math.max(span,.000001);
        return flightClamp01(flightHermiteScalar(
          0,TRANSLATION_AT_GENTLE_END,0,
          TRANSLATION_SLOPE_AT_GENTLE_END*span,u));
      }
      if(uTime<T_DECEL){
        const span=T_DECEL-T_GENTLE_END;
        const u=(uTime-T_GENTLE_END)/span;
        return flightClamp01(flightHermiteScalar(
          TRANSLATION_AT_GENTLE_END,TRANSLATION_AT_DECEL,
          TRANSLATION_SLOPE_AT_GENTLE_END*span,
          TRANSLATION_SLOPE_AT_DECEL*span,u));
      }
      if(uTime<1){
        const span=1-T_DECEL;
        const u=(uTime-T_DECEL)/span;
        return flightClamp01(flightHermiteScalar(
          TRANSLATION_AT_DECEL,1,
          TRANSLATION_SLOPE_AT_DECEL*span,0,u));
      }
      return 1;
    }

    function flightBootstrapRotationProgress(t){
      const uTime=flightClamp01(t);
      const {T_GENTLE_END,T_DECEL}=FLIGHT_BOOTSTRAP;
      if(uTime<=T_GENTLE_END)return 0;
      if(uTime<T_DECEL){
        const span=T_DECEL-T_GENTLE_END;
        const u=(uTime-T_GENTLE_END)/span;
        return flightClamp01(flightHermiteScalar(0,0.86,0,1.05*span,u));
      }
      if(uTime<1){
        const span=1-T_DECEL;
        const u=(uTime-T_DECEL)/span;
        return flightClamp01(flightHermiteScalar(0.86,1,1.05*span,0,u));
      }
      return 1;
    }
    ...
    function flightBootstrapFovAt(t,startFov,destinationFov){
      const uTime=flightClamp01(t);
      const {T_GENTLE_END,T_DECEL,
        FOV_AT_GENTLE_END,FOV_SLOPE_AT_GENTLE_END,
        FOV_AT_DECEL,FOV_SLOPE_AT_DECEL}=FLIGHT_BOOTSTRAP;
      let p;
      if(uTime<=T_GENTLE_END){
        p=0;
      }else if(uTime<T_DECEL){
        const span=T_DECEL-T_GENTLE_END;
        const u=(uTime-T_GENTLE_END)/span;
        p=flightHermiteScalar(0,FOV_AT_DECEL,
          0,
          FOV_SLOPE_AT_DECEL*span,u);
      }else if(uTime<1){
        const span=1-T_DECEL;
        const u=(uTime-T_DECEL)/span;
        p=flightHermiteScalar(FOV_AT_DECEL,1,
          FOV_SLOPE_AT_DECEL*span,0,u);
      }else p=1;
      const q=flightClamp01(p);
      return Math.exp(Math.log(startFov)+
        (Math.log(destinationFov)-Math.log(startFov))*q);
    }
    ...
    function flightArchTranslationProgress(t,immediate=false){
      ...
      const {
        ...
        T_DECEL,
        TRANSLATION_AT_DECEL,
        TRANSLATION_SLOPE_AT_DECEL
      }=FLIGHT_ARCH;

      if(uTime<=T_TRANSLATE_START)return 0;

      if(uTime<T_DECEL){
        const span=T_DECEL-T_TRANSLATE_START;
        const u=(uTime-T_TRANSLATE_START)/span;
        return flightClamp01(
          flightHermiteScalar(
            0,
            TRANSLATION_AT_DECEL,
            0,
            TRANSLATION_SLOPE_AT_DECEL*span,
            u
          )
        );
      }
    ...
    function flightArchFovAt(
      ...
      const {
        ...
        T_APEX,
        T_DECEL,
        FOV_ASCENT_AT_TRANSLATE_START,
        FOV_ASCENT_SLOPE_AT_TRANSLATE_START,
        FOV_DESCENT_AT_DECEL,
        FOV_DESCENT_SLOPE_AT_DECEL
      }=FLIGHT_ARCH;

      const zStart=Math.log2(Number(startFov));
      const zDestination=Math.log2(Number(destinationFov));
      const zMaximum=Math.log2(Number(maxFov));

      let z;

      if(uTime<=T_TRANSLATE_START){
        const span=T_TRANSLATE_START;
        const u=uTime/Math.max(span,.000001);
        const ascent=flightHermiteScalar(
          0,
          FOV_ASCENT_AT_TRANSLATE_START,
          0,
          FOV_ASCENT_SLOPE_AT_TRANSLATE_START*span,
          u
        );
        z=zStart+(zMaximum-zStart)*flightClamp01(ascent);

      }else if(uTime<T_APEX){
        const span=T_APEX-T_TRANSLATE_START;
        const u=(uTime-T_TRANSLATE_START)/span;
        const ascent=flightHermiteScalar(
          FOV_ASCENT_AT_TRANSLATE_START,
          1,
        ...
        const span=T_DECEL-T_APEX;
        const u=(uTime-T_APEX)/span;
        const descent=flightHermiteScalar(
          0,
          FOV_DESCENT_AT_DECEL,
          0,
          FOV_DESCENT_SLOPE_AT_DECEL*span,
          u
        );
        z=zMaximum+(zDestination-zMaximum)*flightClamp01(descent);
...
async function flyViewport({
    ...
    if(!aladin)
      throw new Error('NAVIGATION 0008 ALADIN INSTANCE MISSING');
    ...
    const rotationStartPoint=firstHomeTrip?0:Number(translateStart);

    const alignmentStart=Number(translationComplete);

    const rotationEndPoint=Number(translate90);
    ...
    if(activeFlightControl && !activeFlightControl.settled)
      throw new Error('NAVIGATION 0008 ACTIVE FLIGHT ALREADY EXISTS');
    ...
    try{
      global.dispatchEvent(new CustomEvent('gv-black-box-nav-start',{detail:{
        ...
        choreography:{
          ...
          bootstrapProfile:{
            durationSeconds:CONSTANTS.BOOTSTRAP_TRAVEL_SECONDS,
            gentleEnd:1.5/9,
            decelStart:7.5/9
          },
          arch:{
      ...
      const frame=(now)=>{
        try{
          if(flightControl.aborted){
            const error=new Error(`NAVIGATION 0008 ABORTED: ${flightControl.reason||'external-abort'}`);
          ...
          if(t<1&&aladinSample!==lastAladinSample){
            ...
            if(
              ...
              const rotationProgress=firstHomeTrip
                ? flightBootstrapRotationProgress(t)
                : translationProgress;
            ...
            if(blackBoxSample!==lastBlackBoxSample){
              ...
              try{
                ...
                const phase=firstHomeTrip
                  ? (
                      t<=1.5/9
                        ? 'BOOTSTRAP_GENTLE_TRANSLATE'
                        : t<=7.5/9
                          ? 'BOOTSTRAP_TRANSLATE_ZOOM_ROTATE'
                          : 'BOOTSTRAP_FINAL_DECEL_ALL_AXES'
                    )
                  : (
                      t<=3.5/15
                        ? 'ZOOM_OUT_ACCEL'
                        : t<=6.5/15
                          ? 'ARCH_TANGENTIAL_EXIT'
                          : t<=12/15
                            ? 'ARCH_TRANSLATE_ROTATE_ZOOM_IN'
                            : 'FINAL_DECEL_ALL_AXES'
                    );
