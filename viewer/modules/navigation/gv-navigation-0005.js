...
(function(global){
  ...
  function flightDistanceProgress(t){
    ...
    // ==========================================================
    // NAVIGATION 0005 — APPROVED DOME TRAJECTORY
    // 0.0–3.5 pure zoom-out
    // 3.5–6.5 broad tangential exit
    // 6.5–8.5 short flat-top translate + rotate
    // 8.5–11.5 broad tangential entry
    // 11.5–15.0 pure zoom-in
    // ==========================================================

    const FLIGHT_DOME=Object.freeze({
      T1:3.5/15,
      T2:6.5/15,
      T3:8.5/15,
      T4:11.5/15,
    ...
    function flightBezier1(p0,c1,c2,p3,u){
    ...
    function flightLerp(a,b,u){
      return Number(a)+(Number(b)-Number(a))*flightClamp01(u);
    }

    function flightDomeTranslationProgress(t,immediate=false){
      if(immediate)return flightClamp01(t);

      const uTime=flightClamp01(t);
      const {T1,T2,T3,T4,X_EXIT_FRAC,X_ENTRY_FRAC,KAPPA}=FLIGHT_DOME;

      if(uTime<=T1)return 0;

      if(uTime<T2){
        const u=(uTime-T1)/(T2-T1);
        const x0=0;
        const x3=X_EXIT_FRAC;
        const dx=x3-x0;
        return flightClamp01(
          flightBezier1(x0,x0,x3-KAPPA*dx,x3,u)
        );
      }

      if(uTime<=T3){
        const u=(uTime-T2)/(T3-T2);
        return flightClamp01(
          flightLerp(X_EXIT_FRAC,X_ENTRY_FRAC,u)
        );
      }

      if(uTime<T4){
        const u=(uTime-T3)/(T4-T3);
        const x0=X_ENTRY_FRAC;
        const x3=1;
        const dx=x3-x0;
        return flightClamp01(
          flightBezier1(x0,x0+KAPPA*dx,x3,x3,u)
        );
      }
    ...
    function flightDomeFovAt(
      ...
      const uTime=flightClamp01(t);
      const {T1,T2,T3,T4,Z_LEG_FRAC,KAPPA}=FLIGHT_DOME;
      ...
      const zExit=zStart+Z_LEG_FRAC*(zMaximum-zStart);
      const zEntry=zDestination+Z_LEG_FRAC*(zMaximum-zDestination);

      let z;

      if(uTime<=T1){
        const u=flightNavigationSmootherstep(
          flightClamp01(uTime/Math.max(T1,.000001))
        );
        z=flightLerp(zStart,zExit,u);

      }else if(uTime<T2){
        const u=(uTime-T1)/(T2-T1);
        const dz=zMaximum-zExit;
        z=flightBezier1(
          zExit,
          zExit+KAPPA*dz,
          zMaximum,
        ...
        const u=flightNavigationSmootherstep(
          flightClamp01(
            (uTime-T4)/Math.max(1-T4,.000001)
          )
        );
        z=flightLerp(zEntry,zDestination,u);
      }
        ...
        try{
          ...
          if(t<1&&aladinSample!==lastAladinSample){
            ...
            if(firstHomeTrip){
              ...
              const translationProgress=
                flightDomeTranslationProgress(t);
