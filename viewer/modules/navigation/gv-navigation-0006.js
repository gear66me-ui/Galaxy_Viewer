...
(function(global){
  ...
  function navigationWorkerUrl(){
    if(!NAVIGATION_MODULE_URL)
      throw new Error(
        'NAVIGATION 0006 WORKER BASE URL UNAVAILABLE'
      );
  ...
  function solvePlanInWorker(catalog,anchorRecord=null){
    return new Promise((resolve,reject)=>{
      ...
      const timer=setTimeout(
        ()=>finish(
          reject,
          new Error('NAVIGATION 0006 WORKER TIMEOUT')
        ),
      ...
      worker.onmessage=event=>{
        ...
        if(
          ...
          finish(
            ...
            new Error(
              data.error ||
              'NAVIGATION 0006 WORKER RESULT INVALID'
            )
          );
          return;
        }
  ...
  function materializeWorkerPlan(
    ...
    if(
      ...
      throw new Error(
        'NAVIGATION 0006 WORKER ROUTE MATERIALIZATION FAILURE'
      );
    }
    ...
    if(!validation.ok){
      throw new Error(
        `NAVIGATION 0006 WORKER ROUTE VALIDATION FAILURE: ${
  ...
  // Navigation 0006 owns trip timing, interpolation, RAF, and
  ...
  function flightDistanceProgress(t){
    ...
    // ==========================================================
    // NAVIGATION 0006 — CONTINUOUS ARCH TRAJECTORY
    //
...
async function flyViewport({
    ...
    if(!aladin)
      throw new Error('NAVIGATION 0006 ALADIN INSTANCE MISSING');
