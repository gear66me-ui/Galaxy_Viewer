/* Galaxy Viewer 11I diagnostics + scheduler/HD-return repair 0009.
   11I Viewer remains byte-identical to the verified 11H Viewer baseline.
   This removable runtime layer owns the authoritative ten-row Aladin preparation
   scheduler, reconciles suspended HD rows, restores the complete HD control row
   after provider Back, preserves the 11H active identity/highlight, and retains
   the diagnostic copy report. */
(()=>{
'use strict';
const VERSION='0009';
const ROOT_ID='gv-prefetch-diagnostics-0009';
const POLL_MS=120;
...
function restoreExistingNavigationShell(){
  ...
  if(!Boolean(core?.randomGalaxy?.getState?.().busy))controls.random.disabled=false;
  return true;
}
...
function installBackRestoration(){
  if(document.documentElement.dataset.gv11iBackBound==='true')return;
  document.documentElement.dataset.gv11iBackBound='true';
  document.addEventListener('click',event=>{
    const target=event.target instanceof Element?event.target:null;if(!target)return;
    if(target.closest('#gv-archive-back')){
      /* Document capture runs before the core button handler that stops propagation. */
...
function aladinStateForKey(key){
  const state=aladinState.get(String(key||'').toLowerCase())||'QUEUED';
  return {state,progress:state==='READY'?100:null};
}
...
async function ensureAladinPrewarm(){
  ...
  await window.A.init;
  if(!aladinHost){
    aladinHost=document.createElement('div');aladinHost.id='gv11i-aladin-prewarm';aladinHost.setAttribute('aria-hidden','true');
  ...
  aladinPrewarm=window.A.aladin('#gv11i-aladin-prewarm',{target:'266.41683 -29.00781',survey:'P/DSS2/color',fov:1,projection:'SIN',cooFrame:'ICRSd',showReticle:false,showZoomControl:false,showFullscreenControl:false,showLayersControl:false,showGotoControl:false,showCooGridControl:false,showSettingsControl:false,showSelectionModeControl:false,showColorPickerControl:false,showShareControl:false,showSimbadPointerControl:false,showProjectionControl:false,showStatusBar:false,showFrame:false,showFov:false,showCooLocation:false,showContextMenu:false,showCatalog:false,showCooGrid:false});
...
async function prepareAladinKey(key){
  ...
  const isolated=await ensureAladinPrewarm();if(!isolated)return false;
  ...
  if(!completed||token!==aladinRunToken||core?.getBackgroundWorkSuspended?.()){if(aladinState.get(key)!=='READY')aladinState.set(key,'SUSPENDED');aladinActiveKey='';return false}
  aladinState.set(key,'READY');aladinActiveKey='';return true;
}
async function pumpAladin(data,active){
  if(core?.getBackgroundWorkSuspended?.()||aladinActiveKey)return;
  ...
  setTimeout(()=>{try{const fresh=queueApi?.getPrefetchTelemetry?.()||{};pumpAladin(fresh,currentRuntimeDestination(fresh))}catch(_){}},0);
}
...
function reconcileHd(data,active){
  if(core?.getBackgroundWorkSuspended?.())return;
...
function repairedRows(data){
  return (Array.isArray(data?.rows)?data.rows.slice(0,10):[]).map(row=>Object.freeze({...row,aladin:Object.freeze(aladinStateForKey(row.key))}));
}
...
function reportText(){
  ...
  const lines=['GALAXY VIEWER 11I DIAGNOSTIC REPORT',`SUSPENDED: ${Boolean(core?.getBackgroundWorkSuspended?.())}`,`ALADIN ACTIVE: ${aladinQueue.activeKey||'NONE'}`,`ALADIN QUEUE: ${aladinQueue.queuedDestinations.join(', ')||'EMPTY'}`];
...
function render(){
  ...
  if(!core||!queueApi||typeof queueApi.getPrefetchTelemetry!=='function'){title.textContent='ACTIVE / TRAVELING TO — WAITING FOR 11I TELEMETRY';body.replaceChildren();return}
  try{
    ...
    const data=queueApi.getPrefetchTelemetry()||{};const sourceRows=Array.isArray(data.rows)?data.rows.slice(0,10):[];sourceRows.forEach(row=>{if(row?.key)lastRows.set(String(row.key).toLowerCase(),typeof structuredClone==='function'?structuredClone(row):JSON.parse(JSON.stringify(row)))});
    const active=currentRuntimeDestination(data);syncAladinStates(data,active);reconcileHd(data,active);
    const suspended=Boolean(core.getBackgroundWorkSuspended?.());if(suspended!==lastSuspended){lastSuspended=suspended;if(suspended)cancelAladinDwell();else{for(const [key,state] of aladinState)if(state==='SUSPENDED')aladinState.set(key,'QUEUED');setTimeout(()=>pumpAladin(data,active),0)}}else if(!suspended)setTimeout(()=>pumpAladin(data,active),0);
    const rows=repairedRows(data);const activeRow=activeResourceRow(active);const traveling=Boolean(core.randomGalaxy?.getState?.().busy||suspended);
    title.textContent=active?`ACTIVE / TRAVELING TO — ${active.name} — SEQ ${active.sequence}`:'ACTIVE / TRAVELING TO — NONE';
    let html=activeRow?rowHtml(activeRow,-1,{active:true,traveling}):'';html+=rows.map((row,index)=>rowHtml(row,index)).join('');body.innerHTML=html;
    ...
    installBackRestoration();captureControls();if(!archiveOpen()&&!hdOpen()&&!traveling)restoreExistingNavigationShell();
    if(core.versionLabel){core.versionLabel.textContent='VERSION 11I';core.versionLabel.setAttribute('aria-label','GALAXY VIEWER VERSION 11I')}
    const aladinQueue=getAladinQueueState(data,active);window.GV11I=Object.freeze({version:'11I',displayVersion:'11I',core,queueApi,getPrefetchTelemetry:()=>({active:activeRow,rows,suspended,aladin:aladinQueue}),getAladinPrewarmState:()=>aladinQueue,copyDiagnosticReport:copyReport,restoreHdViewportControls});
...
window.GalaxyViewerPrefetchDiagnostics0009=Object.freeze({version:VERSION,render,copyReport,restoreHdViewportControls});
})();
