...
`;
  ...
  function installHdArchiveIntegration(randomGalaxy, options = {}) {
    ...
    if(!document.getElementById("gvrg-ar81-random-button-visible-style")){const st=document.createElement("style");st.id="gvrg-ar81-random-button-visible-style";st.textContent="body:not(.gv-hd-open) #gv-galaxy-nav,body:not(.gv-hd-open) #gv-random-galaxy{display:flex!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important}body:not(.gv-hd-open) #gv-galaxy-nav{z-index:7405!important}body.gv-hd-open #gv-galaxy-nav,body.gv-hd-open #gv-random-galaxy{display:none!important}";document.head.appendChild(st);}

    // AR81 / RANDOM 0139: force HD viewport cleanup in the real document.
    // This suppresses stale HOME / WE ARE HERE leader artifacts while HD is open
    // and guarantees the archive/provider tile is positioned inside the HD image.
    if(!document.getElementById("gvrg-ar81-hd-cleanup-style")){
        const ar78HdStyle=document.createElement(style);
        ar78HdStyle.id="gvrg-ar81-hd-cleanup-style";
        ar78HdStyle.textContent=`
body.gv-hd-open #gv-we-are-here,
body.gv-hd-open #gv-universe-context,
body.gv-hd-open #gv-we-are-here *,
body.gv-hd-open #gv-universe-context *{
...
.gvrg-hd-viewport>#gv-hd-archive-button{
...
.gvrg-hd-viewport>#gv-hd-archive-button img{
...
.gvrg-hd-viewport>#gv-hd-archive-button .gv-hd-archive-comet{
  position:absolute!important;
  left:50%!important;
  top:50%!important;
...
.gvrg-hd-viewport>#gv-hd-archive-button.gv-archive-loading .gv-hd-archive-comet{
  opacity:1!important;
  animation:gvHdArchiveOrbit .9s linear infinite!important;
}`;
        document.head.appendChild(ar78HdStyle);
    }
    ...
    const aladinPreparedReceipts=new Map();
        ...
        function ensureAladinPrewarm(){
            ...
            aladinPrewarmReady=new Promise((resolve,reject)=>{
                ...
                frame.srcdoc=`<!doctype html><html><head><link rel="stylesheet" href="https://aladin.cds.unistra.fr/AladinLite/api/v3/3.8.2/aladin.min.css"><style>html,body,#gv-prewarm{margin:0;width:512px;height:512px;overflow:hidden;background:#000}
...
.gvrg-hd-science-value:empty,
.gvrg-hd-science-item:empty{
  display:none!important;
  opacity:0!important;
  visibility:hidden!important;
}
...
/* AR76-R3 FINAL: hotdog hidden; provider tile in HD viewport bottom-right */
.gvrg-hd-science-value:empty,
.gvrg-hd-science-item:empty{
  display:none!important;
  opacity:0!important;
...
/* AR76-R4 FINAL: 32px comet orbit around 45px HD provider tile */
.gvrg-hd-viewport>#gv-hd-archive-button{
  position:absolute!important;
  right:12px!important;
  bottom:12px!important;
...
</style></head><body><div id="gv-prewarm"></div><script src="${ALADIN_URL}"><\/script></body></html>`;
...
`;
    ...
    // RANDOM 0139: safe presentation-only preview. No Aladin viewport writes.
...
function refreshAuthoritativeCatalog(){
  const latestCatalog=preparationEngine?.getGalaxyCatalog?.();
...
async function requestRandomNavigation(){
  ...
  /*
   * RANDOM 0139 — PRE-TRAVEL DESTINATION PRESENTATION.
