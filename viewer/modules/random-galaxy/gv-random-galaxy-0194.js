...
icon.style.setProperty('--gv-bts-phase-delay',phaseDelay);
    ...
    function providerIdentityFor(destination){
        ...
        if(/(?:^|[^a-z0-9])(?:noirlab|noir lab|noirlabs|noir labs|noao|gemini)(?:[^a-z0-9]|$)/i.test(evidence))
            return Object.freeze({slug:'noirlab',label:'NOIRLAB'});

        if(/(?:^|[^a-z0-9])(?:eso|european southern observatory)(?:[^a-z0-9]|$)/i.test(evidence))
            return Object.freeze({slug:'eso',label:'ESO'});
    ...
    function providerIconUrl(destination){
        ...
        if(slug==='eso')
            return 'https://gear66me-ui.github.io/Galaxy_Viewer/viewer/artwork/ESO/eso_observatory_night_sky_icon.png';

        if(slug==='noirlab')
            return 'https://gear66me-ui.github.io/Galaxy_Viewer/viewer/artwork/NoirLabs/noirlab-icon.png';
...
`;
    ...
    previewDestination(destination) {
      ...
      try {
        ...
        if(/(?:^|[^a-z0-9])(?:noirlab|noir lab|noirlabs|noir labs|noao|gemini)(?:[^a-z0-9]|$)/i.test(evidence))
          slug="noirlab";
        else if(/(?:^|[^a-z0-9])(?:eso|european southern observatory)(?:[^a-z0-9]|$)/i.test(evidence))
          slug="eso";
        else if(/(?:^|[^a-z0-9])(?:hubble|hst|esahubble)(?:[^a-z0-9]|$)/i.test(evidence))
          slug="hubble";
        ...
        const iconUrl=/^https:\/\//i.test(explicit)
          ? explicit
          : slug
            ? (slug==="eso" ? "https://gear66me-ui.github.io/Galaxy_Viewer/viewer/artwork/ESO/eso_observatory_night_sky_icon.png" : slug==="noirlab" ? "https://gear66me-ui.github.io/Galaxy_Viewer/viewer/artwork/NoirLabs/noirlab-icon.png" : `https://gear66me-ui.github.io/Galaxy_Viewer/viewer/artwork/runtime/providers/${slug}/${slug}-icon.png`)
            : "";
    ...
    #beginTravelHud(destination, route, firstHomeTrip) {
      ...
      const travelProviderUrl = (()=>{
        ...
        return (slug==="eso" ? "https://gear66me-ui.github.io/Galaxy_Viewer/viewer/artwork/ESO/eso_observatory_night_sky_icon.png" : slug==="noirlab" ? "https://gear66me-ui.github.io/Galaxy_Viewer/viewer/artwork/NoirLabs/noirlab-icon.png" : `https://gear66me-ui.github.io/Galaxy_Viewer/viewer/artwork/runtime/providers/${slug}/${slug}-icon.png`);
      })();
...
function hasReadyNavigation(){
  ...
  return randomNavigationWindow.isNextReady(bundle=>{
    ...
    const hd=hdStateFor(bundle);
    const aladin=aladinStateFor(bundle);
...
function updateRandomButtonReadyState(){
  ...
  if(head?.destination&&head?.key){
    ...
    const aladin=aladinStateFor(head);
    headReady=Boolean(
      head.hd?.resource &&
      head.hd?.state==='READY' &&
      hd.presentationReady===true &&
