...
`;
    ...
    beginHdRequest() {
      if (this.destroyed || this.busy || this.interactionOwner === 'navigation') {
        global.GalaxyBlackBox?.recordCheckpoint?.('HD_REQUEST_BLOCKED_NAV_OWNER',{
      ...
      if (this.interactionOwner && this.interactionOwner !== 'hd') return false;
      ...
      global.GalaxyBlackBox?.recordCheckpoint?.('HD_REQUEST_LOCK_CLAIMED',{});
      return true;
    }
    ...
    async travelToRandom() {
      ...
      // First claimant wins. If HD was pressed first, Random cannot
      // cancel it or start motion underneath the HD overlay.
      if (this.hdOpen || this.interactionOwner === 'hd') {
        global.GalaxyBlackBox?.recordCheckpoint?.('RANDOM_BLOCKED_HD_OWNER',{
          hdOpen:Boolean(this.hdOpen),
          owner:String(this.interactionOwner||'')
        });
        return null;
