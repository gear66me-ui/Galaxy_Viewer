...
/*
 * RANDOM GALAXY 0093 — NAVIGATION 0003 SEAM
 *
...
function navigation0003Url(){
  ...
  const owner=
    scripts.find(script=>
      /\/random-galaxy\/gv-random-galaxy-0093\.js(?:[?#].*)?$/i.test(
        String(script.src||'')
      )
    ) ||
    ...
    if(navigationPlanner){
      queueMicrotask(()=>{
        try{
          ...
          console.error(
            'RANDOM 0093 NAVIGATION 0003 RECONCILE FAILURE',
            error
          );
        }
      });
...
async function advanceNavigationPlannerAfterArrival(arrived){
  ...
  if(!arrivedKey)
    throw new Error(
      'RANDOM 0093 ARRIVAL KEY MISSING'
    );
  ...
  if(!bootstrapCommitted){
    if(arrivedKey!==bootstrapDestinationKey){
      throw new Error(
        'RANDOM 0093 BOOTSTRAP ARRIVAL IDENTITY MISMATCH'
      );
    }
  ...
  if(!expected || keyOf(expected)!==arrivedKey){
    throw new Error(
      'RANDOM 0093 NAVIGATION 0003 COMMIT IDENTITY MISMATCH'
    );
  }
  ...
  if(!bootstrapDestinationKey){
    ...
    if(bootstrap&&addFuture(bootstrap)){
      ...
      void ensureNavigationPlanner(bootstrap).catch(error=>{
        console.error(
          'RANDOM 0093 NAVIGATION 0003 INITIALIZATION FAILURE',
          error
        );
      });
    }
...
async function requestRandomNavigation(){
  ...
  try{
    ...
    try{
      ...
      console.error(
        'RANDOM 0093 NAVIGATION 0003 POST-ARRIVAL FAILURE',
        error
      );
      ...
      void ensureNavigationPlanner(arrived).catch(recoveryError=>{
        console.error(
          'RANDOM 0093 NAVIGATION 0003 RECOVERY FAILURE',
          recoveryError
        );
      });
    }
