        ...
        async function prepareHdDestinationFetch(destination,signal=null){
            ...
            for(const source of sources){
                try{
                    ...
                    if(!response.ok)
                        throw new Error('HD PRELOAD RETURNED HTTP '+response.status);

                    const blob=await response.blob();
                    if(signal?.aborted)
                        throw new DOMException('HD PRELOAD SUSPENDED','AbortError');

                    const dimensions=
                        await gv0212JpegDimensionsFromBlob(blob);

                    let preparedDestination=destination;

                    try{
                        preparedDestination=
                            await gv0211PrepareDirectAvmAuthority(
                                destination,
                                blob,
                                dimensions
                            );
                        ...
                        GV_TRACE.enabled&&gvTrace(
                            21201,
                            'RANDOM_0212_METADATA_AVM_FALLBACK',
                            {
                    ...
                    setHdStatus(destination,'METADATA',source.kind,source.url);
                    ...
                    if(error?.name==='AbortError'){
                        setHdStatus(destination,'SUSPENDED',source.kind);
                        throw error;
                    }
                    ...
                    try{
                        ...
                        const prepared=
                            await loadPreparedImageDirect(source.url,signal);
                        ...
                        return {
                            ...
                            pixelWidth:Number(prepared.image?.naturalWidth||0),
                            pixelHeight:Number(prepared.image?.naturalHeight||0),
                            decoded:true,
                            decodePromise:null,
                            sourceUrl:source.url,
                            sourceKind:source.kind
            ...
            const promise=(async()=>{
                ...
                try{
                    ...
                    if(
                        ...
                        /*
                         * 0211 direct AVM success:
                         * metadata is already authoritative, normalized to
                         * the decoded JPEG dimensions, and cached.
                         *
