...
(function (global) {
  ...
  function routeDistanceMillionLy(source,destination,options={}) {
    ...
    if(options.homeDeparture===true){
      return dB>0
        ? {value:dB,exactRoute:true,unavailable:false,homeRoute:true}
        : {value:0,exactRoute:false,unavailable:true,homeRoute:true};
    }
    if(dA>0&&dB>0){
    ...
    return {value:0,exactRoute:false,unavailable:true};
  }
...
function setRandomButtonText(button,text,{busy=false,ready=false,imageWait=false}={}){
  ...
  class FixedDistanceRenderer {
    ...
    renderUnavailable() {
      ...
      this.unitEl.textContent='DISTANCE UNAVAILABLE';
      this.root.setAttribute('aria-label','DISTANCE UNAVAILABLE');
    }
    render(millionLy) {
...
`;
    ...
    #formatTravelHudDistance(millionLy) {
      ...
      if(!available||!(total>0))
        return {integer:'—',fraction:'',unit:'DISTANCE UNAVAILABLE',available:false};
...
async function requestRandomNavigation(){
  ...
  if(!avmAlreadyStaged){
    void Promise.resolve(
      ...
      GV_TRACE.enabled&&gvTrace(11902,'AR119_VISIBLE_AVM_STAGE_DURING_TRAVEL',{
        key:keyOf(destination),
        ready:Boolean(result)
      });
    }).catch(error=>{
      GV_TRACE.enabled&&gvTraceError(11903,'AR119_VISIBLE_AVM_STAGE_DURING_TRAVEL_FAILED',error);
      console.warn('RANDOM GALAXY AVM STAGE DURING TRAVEL WARNING',error);
    });
  }

  GV_TRACE.enabled&&gvTrace(11901,'AR119_VISIBLE_AVM_PRETRAVEL_DISPATCHED',{
    key:keyOf(destination),
    name:String(destination?.name||''),
