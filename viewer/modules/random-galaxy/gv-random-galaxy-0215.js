    ...
    const aladinPreparedReceipts=new Map();
        ...
        async function gv0212EnsurePreparedItemDecoded(item){
            ...
            const promise=(async()=>{
                ...
                if(
                    Number(item.pixelWidth)>0 &&
                    Number(item.pixelHeight)>0 &&
                    (
                        actualWidth!==Number(item.pixelWidth) ||
                        actualHeight!==Number(item.pixelHeight)
                    )
                )throw new Error('JPEG HEADER/DECODE DIMENSION MISMATCH');
