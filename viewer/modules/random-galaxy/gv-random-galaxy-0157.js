...
function installRandomWaitComet(button) {
  ...
  if (!document.getElementById('gvrg-random-wait-comet-0157-style')) {
    const style=document.createElement('style'); style.id='gvrg-random-wait-comet-0157-style';
    style.textContent=`#gv-random-galaxy .gvrg-random-layout{display:grid;grid-template-columns:20px auto 20px;align-items:center;justify-content:center;column-gap:13px}#gv-random-galaxy .gvrg-random-label{display:block;grid-column:2;text-align:center;white-space:nowrap;min-width:0}#gv-random-galaxy .gvrg-random-star-wrap{position:relative;display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;margin:0;flex:0 0 20px;line-height:20px;transform:none}#gv-random-galaxy .gvrg-random-star-wrap-left{grid-column:1}#gv-random-galaxy .gvrg-random-star-wrap-right{grid-column:3;transform:translateX(-2px)}#gv-random-galaxy .gvrg-random-star{display:block;font:16px/20px system-ui,sans-serif;transform:translateY(-.5px)}#gv-random-galaxy .gvrg-random-comet{position:absolute;left:50%;top:50%;width:0;height:0;opacity:0;pointer-events:none;z-index:2;animation:gvrg-random-comet-orbit-0031 2.8s linear infinite;animation-play-state:paused}#gv-random-galaxy .gvrg-random-comet i{position:absolute;left:-1.5px;top:-1.5px;width:3px;height:3px;border-radius:50%;background:#FF8A1C;transform:rotate(var(--a)) translateY(-8px) scale(var(--s));opacity:var(--o);box-shadow:0 0 4px rgba(255,105,24,.95)}#gv-random-galaxy .gvrg-random-comet i:nth-child(1){--a:0deg;--s:1;--o:1;background:#FF2415;box-shadow:0 0 2px 1px #FF5A24,0 0 6px 2px #FF1710,0 0 10px 2px rgba(255,92,24,.95)}#gv-random-galaxy .gvrg-random-comet i:nth-child(2){--a:-15deg;--s:.88;--o:.84;background:#FF3B16}#gv-random-galaxy .gvrg-random-comet i:nth-child(3){--a:-30deg;--s:.76;--o:.68;background:#FF5A18}#gv-random-galaxy .gvrg-random-comet i:nth-child(4){--a:-45deg;--s:.64;--o:.52;background:#FF741B}#gv-random-galaxy .gvrg-random-comet i:nth-child(5){--a:-60deg;--s:.52;--o:.38;background:#FF8F1F}#gv-random-galaxy .gvrg-random-comet i:nth-child(6){--a:-75deg;--s:.42;--o:.26;background:#FFA62A}#gv-random-galaxy .gvrg-random-comet i:nth-child(7){--a:-90deg;--s:.32;--o:.16;background:#FFB63B}#gv-random-galaxy .gvrg-random-comet i:nth-child(8){--a:-105deg;--s:.24;--o:.08;background:#FFC95A}#gv-random-galaxy.gvrg-random-busy .gvrg-random-comet{opacity:1;animation-play-state:running}#gv-random-galaxy .gvrg-random-comet-left{animation-delay:-1.4s}.gvrg-fov-sub{font-size:.72em;letter-spacing:.25px;white-space:nowrap}@keyframes gvrg-random-comet-orbit-0031{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`;
        ...
        function startPrefetch(destination,priority=false){
            ...
            const promise=(async()=>{
                try{
                    ...
                    // AR124: HD decode readiness is independent from AVM authority.
                    // Retain the decoded image immediately so HD workers can continue;
                    // AVM/WCS preparation remains a separate hard pre-travel gate.
                    ...
                    void scheduleAladinEnhancement(item,destination,priority).then(ready=>{
                        if(
                            !ready||
                            item.framingState!=='READY'||
                        ...
                        console.warn('GALAXY VIEWER RUNTIME AVM AUTHORITY PREPARATION WARNING',error);
                    });
                }catch(error){
                    if(error?.name==='AbortError'){
...
`;
    ...
    #normalizeProviderCandidate(candidate) {
      ...
      return Object.freeze({
        ...
        avmRa:ra, avmDec:dec, avmVerticalFovDegrees:fovDegrees, avmCameraRotation:aladinRotation,
        hdUrl: hdUrl.href,
...
// AR127 - FUTURE[0] owns startup readiness. The Random label never becomes
// a download counter; the existing comet spins until the exact head bundle is
// fully presentation-ready and its final AVM-derived Aladin view is prepared.
...
function updateStartupRandomButton(){
  ...
  setRandomButtonText(
    ...
    headReady?'RANDOM GALAXY':'DOWNLOADING IMAGE',
    {busy:!headReady,imageWait:!headReady}
  );

  if(!headReady)return false;

  startupHdGateReleased=true;
  core.randomGalaxyButton.disabled=false;
  return true;
}
...
async function waitForPendingHdReady(record,serial){
  ...
  for(;;){
    ...
    setRandomButtonText(core?.randomGalaxyButton,'DOWNLOADING IMAGE',{busy:true,imageWait:true});
...
function webPhaseAllowed(){
  const future=orderedFutureRecords();
  if(!future.length)return false;

  return future.every(record=>{
    const hd=hdStateFor(record);
...
function pumpWeb(){
  if(suspended||!webPhaseAllowed())return;
...
function pumpPreparationPhases(){
  ...
  // AR128-A: restore the proven AR126 FUTURE[0] lifecycle only.
  // Hidden Aladin may prepare while HD/AVM resolves. If runtime AVM later
  // replaces the destination, hdStateFor() invalidates that receipt and the
  // next pass prepares Aladin again against the final AVM-owned camera.
  if(headAladin.state!=='READY'){
    queueAladinPreparation(head);
  }

  // FUTURE[0] retains exclusive startup HD/AVM priority.
  if(!(headHd.state==='READY' && headHd.presentationReady)){
    preparationEngine?.requestHdPrefetch?.(head.destination,true);
    return;
  }

  // After AVM authority is final, require an exact matching Aladin receipt.
  const finalHeadAladin=aladinStateFor(head);
  if(finalHeadAladin.state!=='READY'){
    queueAladinPreparation(head);
    return;
  }

  // Phase 1: fill the remaining future batch with HD/AVM work first.
  let hdPhaseReady=true;
  ...
  if(!hdPhaseReady)return;

  // Phase 2: after every future HD/AVM is authoritative, prepare Aladin.
  let aladinPhaseReady=true;
  for(const record of future.slice(1)){
    const aladin=aladinStateFor(record);
    if(aladin.state!=='READY'){
      aladinPhaseReady=false;
      queueAladinPreparation(record);
    }
  }
  if(!aladinPhaseReady)return;

  // Phase 3: WEB/source preloads are deliberately last.
  pumpWeb();
}
function addFuture(destination,{prepare=true}={}){
  if(!destination)return false;
  const key=keyOf(destination);
  ...
  if(prepare)prepareRecord(record);
  return true;
}
...
function reconcileFutureQueue(){
   ...
   * Random future queue. Append the complete needed batch first; only then
   * begin HD/Aladin/web preparation for those records.
   */
  if(navigationPlanner){
    ...
    if(needed>0){
      ...
      for(const destination of upcoming){
        ...
        if(addFuture(destination,{prepare:false})){
          const record=futureRecords.get(key);
          if(record)addedRecords.push(record);
        }
      }
...
function updateRandomButtonReadyState(){
  ...
  const startupReady=startupHdGateReleased||updateStartupRandomButton();
  const navigationReady=startupReady&&hasReadyNavigation();

  core.randomGalaxyButton.disabled=Boolean(
    !startupReady||!navigationReady||randomRequestPending||state.busy
  );

  const imageWait=Boolean(startupReady&&!navigationReady&&!randomRequestPending&&!state.busy);
  if(imageWait){
    setRandomButtonText(core.randomGalaxyButton,'DOWNLOADING IMAGE',{busy:true,imageWait:true});
    return;
  }
  if(startupReady&&navigationReady&&!randomRequestPending&&!state.busy&&!core.randomGalaxyButton.classList.contains('gvrg-random-ready'))
    setRandomButtonText(core.randomGalaxyButton,'RANDOM GALAXY');
}
async function waitForLockedReady(locked,serial){
  ...
  for(;;){
    ...
    if(Boolean(locked.hd?.resource)&&locked.hd?.state==="READY"&&locked.aladin?.state==="READY"){
      setRandomButtonText(core?.randomGalaxyButton,'RANDOM GALAXY');
      return true;
    }
    setRandomButtonText(core?.randomGalaxyButton,'DOWNLOADING IMAGE',{busy:true,imageWait:true});
