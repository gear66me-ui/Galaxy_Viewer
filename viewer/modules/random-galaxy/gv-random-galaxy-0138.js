...
`;
  ...
  function installHdArchiveIntegration(randomGalaxy, options = {}) {
    ...
    if(!document.getElementById("gvrg-ar79-random-button-visible-style")){const st=document.createElement("style");st.id="gvrg-ar79-random-button-visible-style";st.textContent="body:not(.gv-hd-open) #gv-galaxy-nav,body:not(.gv-hd-open) #gv-random-galaxy{display:flex!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important}body:not(.gv-hd-open) #gv-galaxy-nav{z-index:7405!important}body.gv-hd-open #gv-galaxy-nav,body.gv-hd-open #gv-random-galaxy{display:none!important}";document.head.appendChild(st);}

    // AR79 / RANDOM 0138: force HD viewport cleanup in the real document.
    ...
    if(!document.getElementById("gvrg-ar79-hd-cleanup-style")){
        const ar78HdStyle=document.createElement(style);
        ar78HdStyle.id="gvrg-ar79-hd-cleanup-style";
...
`;
    ...
    // RANDOM 0138: safe presentation-only preview. No Aladin viewport writes.
...
async function requestRandomNavigation(){
  ...
  /*
   * RANDOM 0138 — PRE-TRAVEL DESTINATION PRESENTATION.
