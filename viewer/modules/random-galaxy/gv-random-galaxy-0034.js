...
#gv-we-are-here .gv-earth-icon{display:inline-flex;align-items:center;justify-content:center;font:22px/1 system-ui,sans-serif;filter:drop-shadow(0 0 5px rgba(88,191,255,.55))}
...
/* Travel distance text remains gold intentionally. */
...
function aladinStateFor(record){
  ...
  const receipt=core.getAladinPreparedReceipt?.(record.key)||record.aladin?.receipt||null;
  if(receipt){state='READY';progress=100}
  else if(core.getBackgroundWorkSuspended?.())state='SUSPENDED';
  else{
    const live=core.getAladinPrewarmState?.()||{};
    if(String(live.activeKey||'').toLowerCase()===record.key)state='PREPARING';
  }
  record.aladin={
    state,
    receipt,
