...
function updateRandomButtonReadyState(){
  ...
  if(head?.destination&&head?.key){
    ...
    if(headReady){
      ensureHeadAvmStaged(head);
      avmReady=isHeadAvmStaged(head);
    }
  }
...
async function requestRandomNavigation(){
  ...
  const preflightAvmReady=Boolean(
    preflightReady &&
    isHeadAvmStaged(record)
  );

  if(!preflightReady||!preflightAvmReady){
    if(preflightReady)ensureHeadAvmStaged(record);
    updateRandomButtonReadyState();
