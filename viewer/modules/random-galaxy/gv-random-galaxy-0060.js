...
`;
  ...
  // REQ-017A / ECO-026F-A
  // Random Galaxy 0060 owns the active preparation engine.
  function createRandomPreparationEngine(options = {}) {
    ...
    const api={
      ...
      async ensureAladinPreparedForNavigation(destination){
        ...
        const receipt=
          await prepareAladinDestination(destination,true);
...
`;
    ...
    async #getProviderCandidate(excludeName) {
      if (!this.provider) throw new Error('Random Galaxy 0060 requires a local galaxy provider.');
    ...
    installPreparationEngine(options = {}) {
      ...
      if(!randomNavigationWindow)
        throw new Error('RANDOM GALAXY 0060 NAVIGATION WINDOW MISSING');
...
function install(){
  ...
  if(!preparationEngine)
    throw new Error('RANDOM GALAXY 0060 PREPARATION ENGINE MISSING');

  if(!archiveIntegration)
    throw new Error('RANDOM GALAXY 0060 HD/ARCHIVE INTEGRATION MISSING');
