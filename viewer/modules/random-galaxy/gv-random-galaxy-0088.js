...
`;
    ...
    #fovAt(t, startFov, destinationFov, immediate = false) {
      ...
      if (t <= turn) {
        const progress = navigationSmootherstep(t / turn);
        return Math.exp(Math.log(startFov) + (Math.log(maximum) - Math.log(startFov)) * progress);
      }
      const progress = navigationSmootherstep((t - turn) / (1 - turn));
    ...
    async travelToRandom() {
      ...
      try {
        ...
        try {
          if (typeof this.aladin.getRotation === 'function') {
            GV_TRACE.enabled&&gvTrace(4623,'ALADIN_GET_ROTATION_BEGIN',null);
            const liveRotation = finiteNumber(this.aladin.getRotation());
            GV_TRACE.enabled&&gvTrace(4623,'ALADIN_GET_ROTATION_END',{rotation:liveRotation});
        ...
        GV_TRACE.enabled&&gvTrace(4658,'RAF_SETUP_COMPLETE',{baseDestinationRA,baseDestinationDec,baseDestinationFov,targetRotation,duration,turnPoint,alignmentStart});
          ...
          const frame = (now) => {
            ...
            if(
              ...
              if(firstHomeTrip){
                ...
                GV_TRACE.enabled&&gvTrace(4696,'RAF_GOTO_HOME_ARC',{t,travelRa,travelDec,translationProgress});
            ...
            GV_TRACE.enabled&&gvTrace(4764,'RAF_FINAL_GOTO',{t,ra:Number(destination.ra),dec:Number(destination.dec)});
...
/*
 * RANDOM GALAXY 0088 — NAVIGATION 0002 SEAM
 *
 * Random Galaxy owns preparation, FIFO locking, travel, history and
 * commit/rollback. Navigation 0002 owns destination ordering after the
...
function navigation0002Url(){
  const scripts=[...document.scripts];
  const owner=
    scripts.find(script=>
      /\/random-galaxy\/gv-random-galaxy-0088\.js(?:[?#].*)?$/i.test(
        String(script.src||'')
      )
    ) ||
  ...
  if(owner?.src){
    return new URL(
      '../navigation/gv-navigation-0002.js',
      owner.src
    ).href;
  }

  return '/viewer/modules/navigation/gv-navigation-0002.js';
}

function applyNavigation0002RuntimeConfig(api=null){
...
function ensureNavigationModule0002(){
  const existing=window.GalaxyViewerNavigation;

  if(existing?.VERSION==='0002'){
    applyNavigation0002RuntimeConfig(existing);
    return Promise.resolve(existing);
  }

  if(existing && existing.VERSION!=='0002'){
  ...
  navigationModulePromise=new Promise((resolve,reject)=>{
    const url=navigation0002Url();

    const prior=[...document.scripts].find(script=>
      script.dataset.gvNavigation0002==='1'
    );
    ...
    const finish=()=>{
      ...
      if(api?.VERSION!=='0002'){
        reject(
          new Error(
            'NAVIGATION 0002 EXPORT MISSING OR VERSION MISMATCH'
          )
        );
        return;
      }

      applyNavigation0002RuntimeConfig(api);
      resolve(api);
    };
    ...
    if(prior){
      ...
      prior.addEventListener(
        'error',
        ()=>reject(
          new Error('NAVIGATION 0002 SCRIPT LOAD FAILED')
        ),
        {once:true}
      );
      return;
    ...
    script.addEventListener(
      'error',
      ()=>reject(
        new Error(`NAVIGATION 0002 SCRIPT LOAD FAILED: ${url}`)
      ),
      {once:true}
    );
...
async function ensureNavigationPlanner(anchorDestination){
  ...
  navigationPlannerPromise=(async()=>{
    const api=await ensureNavigationModule0002();

    if(typeof api.create!=='function')
      throw new Error('NAVIGATION 0002 CREATE API MISSING');
    ...
    if(
      ...
      throw new Error('NAVIGATION 0002 PLANNER API INCOMPLETE');
    }
    ...
    if(navigationPlanner){
      queueMicrotask(()=>{
        try{
          ...
          console.error(
            'RANDOM 0088 NAVIGATION 0002 RECONCILE FAILURE',
            error
          );
        }
      });
...
async function advanceNavigationPlannerAfterArrival(arrived){
  ...
  if(!arrivedKey)
    throw new Error(
      'RANDOM 0088 ARRIVAL KEY MISSING'
    );
  ...
  if(!bootstrapCommitted){
    if(arrivedKey!==bootstrapDestinationKey){
      throw new Error(
        'RANDOM 0088 BOOTSTRAP ARRIVAL IDENTITY MISMATCH'
      );
    }
  ...
  if(!expected || keyOf(expected)!==arrivedKey){
    throw new Error(
      'RANDOM 0088 NAVIGATION 0002 COMMIT IDENTITY MISMATCH'
    );
  }
...
function reconcileFutureQueue(){
  ...
  if(!bootstrapDestinationKey){
    ...
    if(bootstrap&&addFuture(bootstrap)){
      ...
      applyNavigation0002RuntimeConfig();

      void ensureNavigationPlanner(bootstrap).catch(error=>{
        console.error(
          'RANDOM 0088 NAVIGATION 0002 INITIALIZATION FAILURE',
          error
        );
      });
    }
  ...
  /*
   * After the bootstrap slot, Navigation 0002 exclusively determines
...
async function requestRandomNavigation(){
  ...
  try{
    ...
    /*
     * Advance Navigation 0002 only after physical travel succeeded and
    ...
    try{
      ...
      console.error(
        'RANDOM 0088 NAVIGATION 0002 POST-ARRIVAL FAILURE',
        error
      );
      ...
      void ensureNavigationPlanner(arrived).catch(recoveryError=>{
        console.error(
          'RANDOM 0088 NAVIGATION 0002 RECOVERY FAILURE',
          recoveryError
        );
      });
    }
