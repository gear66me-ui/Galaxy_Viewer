...
`;
  ...
  function installHdArchiveIntegration(randomGalaxy, options = {}) {
    ...
    hdControlRow?.appendChild(hdArchiveButton);
    ...
    function positionHdInfoPanel(){
        ...
        if(!hdFixedViewportGeometry){
            ...
            const imageSide=Math.max(1,Math.floor(Math.min(viewportTargetWidth*layoutScale,available-bannerHeight-165)));
        ...
        const infoHeight=Math.max(1,Math.floor(safeBottom-(overlayRect.top+infoTop)));
        ...
        set(randomGalaxy.hdViewport,'top',`${imageTop}px`);set(randomGalaxy.hdViewport,'bottom','auto');set(randomGalaxy.hdViewport,'width',`${imageSide}px`);set(randomGalaxy.hdViewport,'max-width',`${imageSide}px`);set(randomGalaxy.hdViewport,'height',`${imageSide}px`);set(randomGalaxy.hdViewport,'max-height',`${imageSide}px`);
        ...
        function scheduleAladinEnhancement(item,destination,priority=false){
            ...
            const task=async()=>{
                ...
                try{
                    ...
                    if(error?.name==='AbortError')return;
                    console.warn('GALAXY VIEWER SERIAL ALADIN PREWARM WARNING',error);
                    item.destination=destination;
                    framingCompleted=true;
                }finally{
                    if(item){
                        if(framingCompleted)item.framingState='READY';
                        else if(item.framingState==='PREPARING')
                            item.framingState=backgroundWorkSuspended?'SUSPENDED':'RETRY';
                    }
                }
            };
  ...
  class GalaxyRandomGalaxy {
    constructor(options = {}) {
      ...
      // AR73 AVM overlay dormant until destination supplies AVM/WCS URL.
      this.avmOverlayLayer = null;
      this.avmOverlayUrl = '';
      this.avmOverlayOpacity = 0.72;
      this.avmOverlayActive = false;
      ...
      this._avmClick = () => this.toggleAvmOverlay().catch((error) => this.#handleError(error));
      this._avmOpacityInput = () => this.setAvmOverlayOpacity(Number(this.avmOpacitySlider?.value || 72) / 100);
      ...
      this.avmButton?.addEventListener('click', this._avmClick);
      this.avmOpacitySlider?.addEventListener('input', this._avmOpacityInput);
...
#gv-earth-return-indicator .gv-earth-return-icon{display:block;font:12px/12px system-ui,sans-serif;filter:drop-shadow(0 0 3px rgba(87,255,147,.92)) drop-shadow(0 0 7px rgba(77,255,143,.38))}
...
#gv-earth-return-indicator .gv-earth-return-value,#gv-earth-return-indicator .gv-earth-return-unit{display:block;color:#78FFAB;font:600 10.5px/12px "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:.35px;font-variant-numeric:tabular-nums;text-shadow:0 0 4px rgba(229,255,239,.88),0 0 8px rgba(87,255,147,.72),0 0 13px rgba(77,255,143,.34)}
...
.gv-galaxy-history:disabled{opacity:.62;cursor:default;box-shadow:inset 0 0 7px rgba(143,229,255,.18),0 0 6px rgba(41,109,189,.24)}#gv-travel-distance{box-sizing:border-box;display:flex;flex-direction:column;align-items:center;justify-content:center;width:172px;height:34px;padding:2px 6px 1px;border:1px solid rgba(124,203,255,.78);border-radius:5px;background:rgba(8,27,58,.76);color:#78FFAB;text-align:center;text-shadow:0 0 4px rgba(229,255,239,.82),0 0 9px rgba(87,255,147,.34);white-space:nowrap}
...
`;
    ...
    sizeRow.row.querySelector('.gvrg-label').append(document.createElement('br'), sizeFovSub);
      ...
      const avmControls = document.createElement('div');
      avmControls.className = 'gvrg-avm-controls';
      avmControls.hidden = true;
      avmControls.dataset.gvAr73AvmOverlayControls = 'above-hd-science-banner';
      const avmButton = document.createElement('button');
      avmButton.type = 'button';
      avmButton.className = 'gvrg-button gvrg-avm-button';
      avmButton.textContent = 'VIEW AVM OVERLAY';
      const avmSliderWrap = document.createElement('label');
      avmSliderWrap.className = 'gvrg-avm-slider-wrap';
      const avmSliderLabel = document.createElement('span');
      avmSliderLabel.textContent = 'AVM OPACITY';
      const avmOpacitySlider = document.createElement('input');
      avmOpacitySlider.type = 'range';
      avmOpacitySlider.min = '0';
      avmOpacitySlider.max = '100';
      avmOpacitySlider.step = '1';
      avmOpacitySlider.value = '72';
      avmOpacitySlider.className = 'gvrg-avm-opacity';
      avmSliderWrap.append(avmSliderLabel, avmOpacitySlider);
      avmControls.append(avmButton, avmSliderWrap);
      this.avmControls = avmControls;
      this.avmButton = avmButton;
      this.avmOpacitySlider = avmOpacitySlider;
    ...
    hdSize.item.querySelector('.gvrg-hd-science-label').append(document.createElement('br'), hdSizeFovSub);
      ...
      hd.append(viewport, loading, avmControls, hdScience, footer);
    ...
    #setTravelStatus(destinationName) {
      ...
      if (this.statusEl) {
        ...
        this.statusEl.classList.remove('gvrg-status-travel', 'gvrg-status-visible');
        this.statusEl.dataset.gvAr73TravelBannerRemoved = '1';
      }
      if (this.onStatus) this.onStatus(`TRAVEL ${destination}`, this);
    }
    ...
    #normalizeProviderCandidate(candidate) {
      ...
      const fovDegrees = finiteNumber(candidate.fovDegrees);
      const aladinRotation = finiteNumber(candidate.aladinRotation);
      ...
      if (fovDegrees == null || fovDegrees <= 0) throw new Error('Galaxy destination has no usable canonical fovDegrees.');
      if (aladinRotation == null) throw new Error('Galaxy destination has no usable canonical aladinRotation.');
    ...
    // RANDOM 0131: safe presentation-only preview. No Aladin viewport writes.
    ...
    #beginTravelHud(destination, route, firstHomeTrip) {
      ...
      if (!travelProviderIcon) {
        ...
        hud.dataset.gvAr73ProviderIconPlacement="distance-hud-right";
        hud.style.boxSizing="border-box";
        hud.style.paddingRight="56px";
      }
    ...
    async travelToRandom() {
      ...
      try {
        ...
        const destinationFov = Number(destination.fovDegrees);
    ...
    #avmOverlayUrlFor(destination = this.activeDestination) {
      const explicit = String(
        destination?.avmUrl ||
        destination?.avm_url ||
        destination?.avmImageUrl ||
        destination?.avm_image_url ||
        destination?.wcsImageUrl ||
        destination?.wcs_image_url ||
        destination?.overlayImageUrl ||
        destination?.overlay_image_url ||
        destination?.hdUrl ||
        destination?.hd_url ||
        ''
      ).trim();
    ...
    #syncAvmOverlayControls(destination = this.activeDestination) {
      const url = this.#avmOverlayUrlFor(destination);
      this.avmOverlayUrl = url;
      if (this.avmControls) {
        this.avmControls.hidden = !url;
        this.avmControls.classList.toggle('gv-avm-active', Boolean(this.avmOverlayActive));
      }
      if (this.avmButton) {
        this.avmButton.disabled = !url;
        this.avmButton.textContent = this.avmOverlayActive ? 'HIDE AVM OVERLAY' : 'VIEW AVM OVERLAY';
      }
      if (this.avmOpacitySlider)
        this.avmOpacitySlider.value = String(Math.round((this.avmOverlayOpacity || 0.72) * 100));
      return url;
    }

    #positionAvmOverlayControls() {
      if (!this.avmControls || !this.hdScience || this.avmControls.hidden) return;
      const scienceRect = this.hdScience.getBoundingClientRect?.();
      const overlayRect = this.hdOverlay?.getBoundingClientRect?.();
      const scienceTop = Number(scienceRect?.top || 0);
      const overlayTop = Number(overlayRect?.top || 0);
      const controlsHeight = Math.max(44, Number(this.avmControls.getBoundingClientRect?.().height || 44));
      const top = Math.max(8, scienceTop - overlayTop - controlsHeight - 14);
      this.avmControls.style.top = String(Math.round(top)) + 'px';
      this.avmControls.dataset.gvAr73AvmControlsTop = String(Math.round(top));
    }

    #applyAvmOverlayOpacity() {
      const layer = this.avmOverlayLayer;
      const opacity = Math.max(0, Math.min(1, Number(this.avmOverlayOpacity || 0)));
      try {
        if (typeof layer?.setOpacity === 'function') layer.setOpacity(opacity);
        else if (typeof layer?.setAlpha === 'function') layer.setAlpha(opacity);
        else if (layer) layer.opacity = opacity;
      } catch (_) {}
    }

    setAvmOverlayOpacity(opacity) {
      this.avmOverlayOpacity = Math.max(0, Math.min(1, Number(opacity || 0)));
      this.#applyAvmOverlayOpacity();
      return this.avmOverlayOpacity;
    }

    async toggleAvmOverlay(force = null) {
      const url = this.#syncAvmOverlayControls(this.activeDestination);
      if (!url) return false;
      const shouldShow = force === null ? !this.avmOverlayActive : Boolean(force);
      if (!shouldShow) {
        this.clearAvmOverlay();
        return false;
      }
      let layer = null;
      try {
        const Aapi = globalThis.A || null;
        if (typeof Aapi?.image === 'function')
          layer = Aapi.image(url, { name: 'AVM Overlay' });
        else
          layer = { url, name: 'AVM Overlay' };
      } catch (_) {
        layer = { url, name: 'AVM Overlay' };
      }
      try {
        if (typeof this.aladin?.setOverlayImageLayer === 'function')
          this.aladin.setOverlayImageLayer(layer);
        else if (typeof this.aladin?.addOverlay === 'function' && layer)
          this.aladin.addOverlay(layer);
      } catch (error) {
        this.clearAvmOverlay();
        throw error;
      }
      this.avmOverlayLayer = layer;
      this.avmOverlayActive = true;
      this.#applyAvmOverlayOpacity();
      this.#syncAvmOverlayControls(this.activeDestination);
      this.#positionAvmOverlayControls();
      return true;
    }
    ...
    clearAvmOverlay() {
      ...
      try {
        if (typeof this.aladin?.setOverlayImageLayer === 'function')
          this.aladin.setOverlayImageLayer(null);
        else if (layer && typeof this.aladin?.removeOverlay === 'function')
          this.aladin.removeOverlay(layer);
      } catch (_) {}
      this.avmOverlayLayer = null;
      this.avmOverlayActive = false;
      this.#syncAvmOverlayControls(this.activeDestination);
    ...
    showHD() {
      ...
      const destination = this.activeDestination;
      this.#syncAvmOverlayControls(destination);
      ...
      this.hdOpen = true;
      requestAnimationFrame(() => this.#positionAvmOverlayControls());
      if (preparedImage) {
    ...
    destroy() {
      ...
      this.avmButton?.removeEventListener('click', this._avmClick);
      this.avmOpacitySlider?.removeEventListener('input', this._avmOpacityInput);
      this.clearAvmOverlay();
...
async function ensureNavigationPlanner(anchorDestination){
...
async function advanceNavigationPlannerAfterArrival(arrived){
  ...
  /*
   * The first Random Galaxy is the one-time bootstrap destination.
   * It anchors the Monte Carlo route but is not itself a planner item.
   */
  if(!bootstrapCommitted){
    if(arrivedKey!==bootstrapDestinationKey){
      throw new Error(
        'RANDOM 0092 BOOTSTRAP ARRIVAL IDENTITY MISMATCH'
      );
    }
  ...
  await navigationPlanner.commitNext(arrived);
  rememberCommittedDestination(arrived);
...
function makeRecord(destination){
  ...
  return {
    ...
    web:{key:String(key||'').trim().toLowerCase(),state:'QUEUED',detail:'',nextRetryAt:0,sourceUrl:String(destination?.sourceUrl||'').trim()}
  };
}
...
function reconcileFutureQueue(){
  ...
  /*
   * Bootstrap exactly once per application launch.
   * This is the only standalone random destination.
   */
  if(!bootstrapDestinationKey){
    const bootstrap=chooseUniqueDestination();

    if(bootstrap&&addFuture(bootstrap)){
      bootstrapDestinationKey=keyOf(bootstrap);

      applyNavigation0013RuntimeConfig();
    }
  }

  /*
   * RANDOM 0121 — planner initialization may race progressive catalog loading.
   * Reconciliation is periodic, so retry from the authoritative bootstrap
   * destination until the live eligible catalog reaches the 130-item gate.
   * ensureNavigationPlanner() refreshes that catalog and serializes concurrent
   * attempts through navigationPlannerPromise.
   */
  if(bootstrapDestinationKey&&!navigationPlanner&&!navigationPlannerPromise){
    const bootstrap=
      catalogByKey.get(bootstrapDestinationKey) ||
      randomNavigationWindow.getFuture()
        .find(bundle=>bundle?.key===bootstrapDestinationKey)?.destination ||
      null;

    if(bootstrap){
      void ensureNavigationPlanner(bootstrap).catch(error=>{
        if(!String(error?.message||'').includes('NAVIGATION CATALOG NOT READY')){
          console.error(
            'RANDOM 0121 NAVIGATION 0018 INITIALIZATION FAILURE',
            error
          );
        }
      });
  ...
  /*
   * After the bootstrap slot, Navigation 0006 exclusively determines
   * destination order. Random Galaxy only prepares those destinations.
   */
  if(navigationPlanner){
...
async function consumeNext(excludeName=''){
  ...
  if(!aladinReceiptMatchesRecord(exactReceipt,record)){
    activeRecord=null;
    randomNavigationWindow.rollbackLocked();
    throw new Error('RANDOM GALAXY LOCKED BUNDLE ALADIN RECEIPT LOST');
  }
  ...
  activeRecord.aladin={
    ...
    dec:Number(destination.dec),
    fov:Number(destination.fovDegrees),
    rotation:Number.isFinite(Number(destination.aladinRotation))
...
function hdStateFor(record){
  ...
  const framingReady=
    state!=='READY' ||
    resource?.framingState!=='PREPARING';
...
function showFailedHeadButton(record){ if(!record)return; if(!failedHeadButton){ failedHeadButton=document.createElement('button'); failedHeadButton.textContent='CONTINUE TO NEXT GALAXY'; failedHeadButton.style.cssText='position:fixed;left:50%;bottom:86px;transform:translateX(-50%);z-index:2147483647;padding:12px 18px;font-weight:700'; failedHeadButton.onclick=()=>{const r=randomNavigationWindow.getFuture()[0]; if(r&&r.manualContinueRequired){retirePoisonedHead(r);failedHeadButton.remove();failedHeadButton=null;reconcileFutureQueue();updateRandomButtonReadyState();requestRandomNavigation().catch(error=>console.error('GALAXY VIEWER MANUAL CONTINUE FAILURE',error));}}; document.body.appendChild(failedHeadButton);} }
...
function hasReadyNavigation(){
  // A stale/broken HD URL must never hold the entire Random button hostage.
...
function updateRandomButtonReadyState(){
  ...
  core.randomGalaxyButton.disabled=Boolean(
...
async function waitForLockedReady(locked,serial){
  ...
  for(;;){
    ...
    healHeadHdFailure();
    hdStateFor(locked);
    aladinStateFor(locked);
    if(Boolean(locked.hd?.resource)&&locked.hd?.state==="READY"&&locked.aladin?.state==="READY")return true;
...
async function requestRandomNavigation(){
  ...
  if(
    ...
    !Number.isFinite(Number(destination?.fovDegrees))||
    Number(destination.fovDegrees)<=0
  ){
    throw new Error('RANDOM 0065 DESTINATION NAVIGATION VALUES ARE INVALID');
  }
  ...
  /*
   * RANDOM 0131 — PRE-TRAVEL DESTINATION PRESENTATION.
   * The FIFO destination is now exact and owned. Publish it immediately so
   * the destination banner/science card/provider identity can be inspected
   * before physical travel begins.
   */
  randomGalaxy.activeDestination=destination;
  try{
    randomGalaxy.previewDestination?.(destination);
  }catch(error){
    console.warn('GALAXY VIEWER PRE-TRAVEL PREVIEW WARNING',error);
  }
...
if(!install()){
  ...
  global.GalaxyViewerAvmOverlayDebug=()=>({
    active:Boolean(randomGalaxy?.avmOverlayActive),
    url:String(randomGalaxy?.avmOverlayUrl||''),
    opacity:Number(randomGalaxy?.avmOverlayOpacity||0),
    controlsHidden:Boolean(randomGalaxy?.avmControls?.hidden),
    controlsTop:String(randomGalaxy?.avmControls?.dataset?.gvAr73AvmControlsTop||''),
    hasLayer:Boolean(randomGalaxy?.avmOverlayLayer)
  });
