  ...
  function createEarthReturnController(options = {}) {
    ...
    const updateEarthReturnIndicator=()=>{
        ...
        if(Number.isFinite(earthX)&&Number.isFinite(earthY)){
            dx=earthX-apertureCenterX;
            dy=earthY-apertureCenterY;
        }
        ...
        const centerReticleStyle=centerReticle
            ...
            : null;
        const centerReticleVisible=Boolean(
            centerReticleRect &&
            centerReticleRect.width>0 &&
            centerReticleRect.height>0 &&
            centerReticleStyle?.display!=='none' &&
            centerReticleStyle?.visibility!=='hidden'
        );
...
`;
    ...
    #archProfile() {
      const requested=String(this.options.archProfileId||'B').trim().toUpperCase();
      const profiles={
        A:Object.freeze({
          id:'A',
          zoomOutOnly:0.30,
          zoomOutTranslate:0.10,
          translateRotate:0.20,
          translateZoomIn:0.10,
          zoomInOnly:0.30
        }),
        B:Object.freeze({
          id:'B',
          zoomOutOnly:0.25,
          zoomOutTranslate:0.15,
          translateRotate:0.20,
          translateZoomIn:0.15,
          zoomInOnly:0.25
        })
      };
      const profile=profiles[requested]||profiles.B;
      const total=
        profile.zoomOutOnly+
        profile.zoomOutTranslate+
        profile.translateRotate+
        profile.translateZoomIn+
        profile.zoomInOnly;
      if(Math.abs(total-1)>1e-9)
        throw new Error('ARCH CHOREOGRAPHY PHASES MUST SUM TO 1');
      ...
      return {
        ...
        apexFovDeg:
          Math.max(
            1,
            Number(this.options.archApexFovDeg)||150
          ),
        translationStart:profile.zoomOutOnly,
        zoomOutEnd:rotationStart,
        rotationStart,
        rotationEnd,
        zoomInStart:rotationEnd,
        translationEnd
      };
    }

    #archTranslationProgress(t) {
      const profile=this.#archProfile();
      if(t<=profile.translationStart)return 0;
      if(t>=profile.translationEnd)return 1;
      return navigationSmootherstep(
        (t-profile.translationStart)/
        Math.max(
          profile.translationEnd-profile.translationStart,
          .000001
        )
      );
    }

    #archFovAt(t,startFov,destinationFov) {
      const profile=this.#archProfile();
      const apex=profile.apexFovDeg;

      if(t<=profile.zoomOutEnd){
        const progress=navigationSmootherstep(
          clamp01(
            t/Math.max(profile.zoomOutEnd,.000001)
          )
        );
        return Math.exp(
          Math.log(startFov)+
          (Math.log(apex)-Math.log(startFov))*progress
        );
      }

      if(t<profile.zoomInStart)
        return apex;

      const progress=navigationSmootherstep(
        clamp01(
          (t-profile.zoomInStart)/
          Math.max(1-profile.zoomInStart,.000001)
        )
      );

      return Math.exp(
        Math.log(apex)+
        (Math.log(destinationFov)-Math.log(apex))*progress
      );
    }
    ...
    #fovAt(t, startFov, destinationFov, immediate = false) {
      ...
      // DOE 0096 — continuous production-style camera arc.
      //
    ...
    async travelToRandom() {
      ...
      try {
        ...
        // DOE condition is selected and applied before duration and
        // phase boundaries are read below. First-home remains unchanged
        // and is explicitly excluded from the experiment.
        global.GVStutterLab?.prepareTravel?.(
          this,
          {
            firstHomeTrip,
            source,
        ...
        const archProfile=firstHomeTrip?null:this.#archProfile();
        const turnPoint=firstHomeTrip
          ? Number(this.options.turnPoint)
          : archProfile.zoomOutEnd;
        const rotationStartPoint=firstHomeTrip
          ? 0
          : archProfile.rotationStart;
        const alignmentStart=firstHomeTrip
          ? firstHomeTranslationCompleteEnd
          : archProfile.translationEnd;
        const rotationEndPoint=firstHomeTrip
          ? alignmentStart
          : archProfile.rotationEnd;
        ...
        // DOE 0096 — measurement begins on the same authoritative
          ...
          const frame = (now) => {
            ...
            if(
              ...
              if(firstHomeTrip){
                ...
                const baseTravelFov=this.#archFovAt(
                  t,startFov,baseDestinationFov,false
                );
                const correctedTravelFov=this.#archFovAt(
              ...
              if(firstHomeTrip){
                ...
                const translationProgress=this.#archTranslationProgress(t);
            ...
            // LAB0004 samples the actual navigation callback after
            // this frame's camera work. No observer RAF runs in parallel.
            global.GVStutterLab?.sampleAnimationFrame?.(now);
        ...
        // DOE 0096 — stop measurement immediately after the
        ...
        global.GVStutterLab?.completeTravel?.({
          firstHomeTrip,
          destination,
          route
        });
        ...
        global.GVStutterLab?.abortTravel?.(error);
...
/*
 * RANDOM GALAXY 0092 — NAVIGATION 0004 SEAM
 *
 * Random Galaxy owns preparation, FIFO locking, travel, history and
 * commit/rollback. Navigation 0004 owns destination ordering after the
...
function navigation0004Url(){
  const scripts=[...document.scripts];
  const owner=
    scripts.find(script=>
      /\/random-galaxy\/gv-random-galaxy-0092\.js(?:[?#].*)?$/i.test(
        String(script.src||'')
      )
    ) ||
  ...
  if(owner?.src){
    return new URL(
      '../navigation/gv-navigation-0004.js',
      owner.src
    ).href;
  }

  return '/viewer/modules/navigation/gv-navigation-0004.js';
}
...
function applyNavigation0004RuntimeConfig(api=null){
  ...
  randomGalaxy.options.travelSeconds=normalSeconds;
  randomGalaxy.options.maxFov=birdseye;
  randomGalaxy.options.archProfileId=
    String(randomGalaxy.options.archProfileId||'B').toUpperCase();
  randomGalaxy.options.archApexFovDeg=
    Number(randomGalaxy.options.archApexFovDeg)||150;
}

function ensureNavigationModule0004(){
  const existing=window.GalaxyViewerNavigation;

  if(existing?.VERSION==='0004'){
    applyNavigation0004RuntimeConfig(existing);
    return Promise.resolve(existing);
  }

  if(existing && existing.VERSION!=='0004'){
  ...
  navigationModulePromise=new Promise((resolve,reject)=>{
    const url=navigation0004Url();

    const prior=[...document.scripts].find(script=>
      script.dataset.gvNavigation0004==='1'
    );
    ...
    const finish=()=>{
      ...
      if(api?.VERSION!=='0004'){
        reject(
          new Error(
            'NAVIGATION 0004 EXPORT MISSING OR VERSION MISMATCH'
          )
        );
        return;
      }

      applyNavigation0004RuntimeConfig(api);
      resolve(api);
    };
    ...
    if(prior){
      ...
      prior.addEventListener(
        'error',
        ()=>reject(
          new Error('NAVIGATION 0004 SCRIPT LOAD FAILED')
        ),
        {once:true}
      );
      return;
    ...
    script.addEventListener(
      'error',
      ()=>reject(
        new Error(`NAVIGATION 0004 SCRIPT LOAD FAILED: ${url}`)
      ),
      {once:true}
    );
...
async function ensureNavigationPlanner(anchorDestination){
  ...
  navigationPlannerPromise=(async()=>{
    const api=await ensureNavigationModule0004();

    if(typeof api.create!=='function')
      throw new Error('NAVIGATION 0004 CREATE API MISSING');
    ...
    if(
      ...
      throw new Error('NAVIGATION 0004 PLANNER API INCOMPLETE');
    }
    ...
    if(navigationPlanner){
      queueMicrotask(()=>{
        try{
          ...
          console.error(
            'RANDOM 0092 NAVIGATION 0004 RECONCILE FAILURE',
            error
          );
        }
      });
...
async function advanceNavigationPlannerAfterArrival(arrived){
  ...
  if(!expected || keyOf(expected)!==arrivedKey){
    throw new Error(
      'RANDOM 0092 NAVIGATION 0004 COMMIT IDENTITY MISMATCH'
    );
  }
...
function reconcileFutureQueue(){
  ...
  if(!bootstrapDestinationKey){
    ...
    if(bootstrap&&addFuture(bootstrap)){
      ...
      applyNavigation0004RuntimeConfig();

      void ensureNavigationPlanner(bootstrap).catch(error=>{
        console.error(
          'RANDOM 0092 NAVIGATION 0004 INITIALIZATION FAILURE',
          error
        );
      });
    }
  ...
  /*
   * After the bootstrap slot, Navigation 0004 exclusively determines
...
async function requestRandomNavigation(){
  ...
  try{
    ...
    /*
     * Advance Navigation 0004 only after physical travel succeeded and
     * Random Galaxy committed the exact arrived destination.
     */
    try{
      await advanceNavigationPlannerAfterArrival(arrived);
    }catch(error){
      console.error(
        'RANDOM 0092 NAVIGATION 0004 POST-ARRIVAL FAILURE',
        error
      );
      ...
      void ensureNavigationPlanner(arrived).catch(recoveryError=>{
        console.error(
          'RANDOM 0092 NAVIGATION 0004 RECOVERY FAILURE',
          recoveryError
        );
      });
    }
