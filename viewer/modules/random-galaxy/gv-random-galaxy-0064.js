...
`;
    ...
    async travelToRandom() {
      ...
      try {
        ...
        const fov = this.aladin.getFov();
        let preservedUserRotation = null;
        try {
          if (typeof this.aladin.getRotation === 'function')
            preservedUserRotation = finiteNumber(this.aladin.getRotation());
        } catch (_) {}
        if (preservedUserRotation === null)
          throw new Error('CURRENT ALADIN ROTATION IS INVALID');
...
async function requestRandomNavigation(){
    /*
     * 0064 SIMPLE RANDOM NAVIGATION
     *
     * Navigation contract:
     *   prepared future[0] destination
     *        -> existing travelToRandom()
     *        -> zoom out
     *        -> RA/Dec translation
     *        -> zoom in
     *        -> final FOV/orientation
     *
     * IMPORTANT:
     * Prefetch, HD, archive/source preparation, diagnostics and download
     * machinery remain untouched.  The FIFO transaction machinery is moved
     * AFTER visible travel so it cannot prevent the travel engine starting.
     */
    const state=randomNavigationWindow?.getState?.()||{};
    const record=state.future?.[0]||null;

    if(!record){
        throw new Error('RANDOM 0064 FUTURE0 DESTINATION IS NOT READY');
    }

    const destination=record.destination||record;

    if(!Number.isFinite(Number(destination?.ra)) ||
       !Number.isFinite(Number(destination?.dec))){
        throw new Error('RANDOM 0064 DESTINATION COORDINATES ARE INVALID');
    }

    // Preserve bundle ownership for HD/card/archive presentation.
    activeRecord=record;

    // Hand the already prepared destination directly to the existing
    // Random Galaxy travel engine.  No pre-travel lock/claim/receipt gate.
    ...
    if(!arrived){
        throw new Error('RANDOM 0064 DIRECT TRAVEL DID NOT ARRIVE');
    }

    /*
     * AFTER successful visible travel, let the existing navigation window
     * catch up for history/FIFO bookkeeping.  Bookkeeping failure must not
     * retroactively block the actual galactic travel.
     */
    try{
        ...
        if(locked && keyOf(locked)===keyOf(record)){
            ...
            if(claimed && keyOf(claimed)===keyOf(record)){
                ...
                randomNavigationWindow.rollbackPending?.();
                console.error('RANDOM 0064 POST-TRAVEL CLAIM IDENTITY MISMATCH');
            }
        }else{
            randomNavigationWindow.rollbackLocked?.();
            console.error('RANDOM 0064 POST-TRAVEL FIFO LOCK MISMATCH');
        }
    }catch(error){
        randomNavigationWindow.rollbackLocked?.();
        randomNavigationWindow.rollbackPending?.();
        console.error('RANDOM 0064 POST-TRAVEL BOOKKEEPING FAILURE',error);
    }
...
function install(){
  ...
  core.randomGalaxyButton?.addEventListener('click',event=>{
    ...
    event.stopPropagation();
    event.stopImmediatePropagation();
    requestRandomNavigation().catch(error=>console.error('GALAXY VIEWER RANDOM NAVIGATION REQUEST FAILURE',error));
  },true);
