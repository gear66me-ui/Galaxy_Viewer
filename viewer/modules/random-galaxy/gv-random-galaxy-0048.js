...
(function (global) {
  ...
  function isSupportedProviderHost(hostname) {
    ...
    return host === 'esahubble.org' || host.endsWith('.esahubble.org')
      ...
      || host === 'www.spitzer.caltech.edu' || host === 'spitzer.caltech.edu'
      || host === 'astropix.org' || host.endsWith('.astropix.org');
  }
...
`;
    ...
    #normalizeProviderCandidate(candidate) {
      ...
      if (telescope && !/(hubble|webb|chandra|spitzer)/i.test(telescope)) throw new Error('Rejected entry without supported telescope data.');
      return Object.freeze({
