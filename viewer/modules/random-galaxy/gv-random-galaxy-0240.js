...
`;
    ...
    async travelToRandom() {
      ...
      try{
        ...
        if(!destination)throw new Error('NO RANDOM GALAXY DESTINATION AVAILABLE');
        const ra=Number(destination.ra);
        const dec=Number(destination.dec);
        const destinationFov=this.#r10ScreenFitFov(destination);
        if(!Number.isFinite(ra)||!Number.isFinite(dec))
          throw new Error('DESTINATION RA/DEC IS INVALID');
        if(!Number.isFinite(destinationFov)||destinationFov<=0)
          throw new Error('DESTINATION FOV IS INVALID');
        ...
        if(typeof this.aladin.gotoRaDec!=='function')
          throw new Error('ALADIN gotoRaDec API MISSING');
        this.aladin.gotoRaDec(ra,dec);

        if(typeof this.aladin.setFov==='function')
        ...
        this.currentGalaxy={
          ...
          aladinRotation:finiteNumber(destination.aladinRotation)??0
        };
        this.arrived=true;
        this.#showCard(destination);
        this.#setStatus(`ARRIVED ${String(destination.name||'GALAXY').toUpperCase()}`);
...
async function consumeNext(excludeName=''){
  ...
  const readyForNavigation=bundle=>{
    ...
    return Boolean(destination&&key)&&
      String(destination.name||'').trim().toLowerCase()!==excluded&&
...
function install(){
  ...
  startupRoutePromise=(async()=>{
    ...
    reconcileFutureQueue();
    if(core?.randomGalaxyButton)core.randomGalaxyButton.disabled=false;
    setRandomButtonText(core?.randomGalaxyButton,'RANDOM GALAXY',{ready:true});
    return true;
  })();
  ...
  core.randomGalaxyButton?.addEventListener('click',event=>{
    ...
    randomGalaxy.travelToRandom().catch(error=>{
      ...
      console.error('GALAXY VIEWER RANDOM SNAP FAILURE',error);
    });
  },true);
  const monitor=setInterval(()=>{
    ...
    if(core?.randomGalaxyButton && !randomGalaxy?.busy) core.randomGalaxyButton.disabled=false;
    updateHistoryControls();
  },POLL_MS);
