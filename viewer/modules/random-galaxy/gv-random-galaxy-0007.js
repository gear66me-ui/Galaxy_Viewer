...
AUTHORIZED BASELINE: gv-random-galaxy-0006.js blob 8b32487130fb9146521786209ae7a0745721ceae.
...
(function (global) {
  ...
  const FONT_NAMES = Object.freeze({
    spaceAge: 'GV Random Galaxy Space Age 0007',
    digits: 'GV Random Galaxy Digits 0007'
  });
...
.gvrg-distance{position:absolute;left:50%;top:154px;transform:translateX(-50%);width:min(330px,88vw);padding:9px 11px 10px;border:1px solid rgba(183,255,208,.72);border-radius:6px;background:rgba(0,12,8,.66);box-shadow:0 0 13px rgba(77,255,143,.20);text-align:center;opacity:0;transition:opacity .20s linear}
...
.gvrg-card{position:absolute;left:50%;top:72%;transform:translate(-50%,-50%);width:min(410px,92%);padding:14px 15px 15px;border:1px solid rgba(175,225,255,.78);border-radius:7px;background:rgba(0,8,17,.92);box-shadow:0 0 15px rgba(80,190,255,.24);opacity:0;transition:opacity .28s ease;pointer-events:none}
.gvrg-card-visible{opacity:1;pointer-events:auto}
.gvrg-name{font:400 19px/1.18 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:1.2px;color:#fff;text-shadow:0 0 7px rgba(145,220,255,.50);margin-bottom:11px;white-space:normal;overflow-wrap:anywhere}
.gvrg-row{display:grid;grid-template-columns:122px minmax(0,1fr);gap:12px;align-items:baseline;margin-top:7px;padding-bottom:3px;border-bottom:1px solid rgba(130,185,212,.12)}
.gvrg-label{font:400 9px/1.4 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:1px;color:#82b9d4}
.gvrg-value{min-width:0;font:400 12px/1.4 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:.55px;color:#e9f9ff;white-space:normal;overflow-wrap:anywhere}
.gvrg-card-distance{display:flex;align-items:baseline;gap:6px;min-width:0;flex-wrap:wrap}
.gvrg-value-number{font:400 15px/1.4 "${FONT_NAMES.digits}",sans-serif;letter-spacing:0}
.gvrg-value-unit{font:400 10px/1.4 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:.45px}
.gvrg-actions{display:grid;grid-template-columns:minmax(0,1fr) 64px;gap:8px;align-items:stretch;margin-top:13px;pointer-events:auto}
.gvrg-button{appearance:none;border:1px solid rgba(175,225,255,.84);border-radius:5px;background:rgba(7,27,42,.95);color:#effbff;padding:8px 11px;font:400 9px/1 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:.9px;cursor:pointer}
.gvrg-button:disabled{opacity:.45;cursor:default}
.gvrg-hd-primary{min-width:0;height:64px;padding:8px 12px;font:400 18px/1.12 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:1px;text-align:center;box-shadow:inset 0 0 10px rgba(98,216,255,.10),0 0 9px rgba(98,216,255,.18)}
.gvrg-hd-icon-button{width:64px;height:64px;padding:4px;display:flex;align-items:center;justify-content:center;overflow:hidden}
...
.gvrg-hd-loading{position:absolute;left:50%;top:50%;z-index:2;transform:translate(-50%,-50%);font:400 9px/1.4 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:1px;color:#dff7ff;pointer-events:none}`;
    ...
    #build() {
      ...
      const distanceRow = makeRow('DISTANCE TO EARTH');
      ...
      card.append(name, distanceRow.row, constellationRow.row, ageRow.row, actions);
    ...
    async #getHubbleCandidate(excludeName) {
      if (!this.hubbleProvider) throw new Error('Random Galaxy 0007 requires the local Hubble catalog provider.');
    ...
    #normalizeHubbleCandidate(candidate) {
      ...
      const age = cleanText(candidate.age ?? candidate.ageEstimate ?? candidate.age_estimate ?? 'UNKNOWN');
      ...
      return Object.freeze({
        ...
        hubble: true,
        name, ra, dec, distance, constellation, age,
    ...
    #showCard(destination) {
      ...
      this.ageValueEl.textContent = cleanText(destination.age || 'UNKNOWN').toUpperCase();
