...
`;
    ...
    #normalizeProviderCandidate(candidate) {
      ...
      const name = cleanText(candidate.name || candidate.objectName || candidate.archiveId || candidate.id || 'CATALOG OBJECT');
      ...
      // 0188: no catalog-content filtering.  Distance, constellation,
      // category, image type, source page, and descriptive metadata are optional.
      // Runtime AVM/WCS and the image asset remain hard readiness requirements.
  ...
  GalaxyRandomGalaxy.PROVIDER_CONTRACT = Object.freeze({
    galaxyDiscovery: {
      ...
      selection: 'All curated catalog entries; no content filtering',
      liveArchiveScraping: false
    },
    provider: {
      request: { excludeName: 'optional previous destination name' },
      response: { destination: { name: 'catalog name or identifier', ra: 'runtime AVM ICRS degrees', dec: 'runtime AVM degrees', distanceMly: 'optional', constellation: 'optional', age: 'optional authoritative age text', physicalSizeLy: 'optional authoritative physical size in light-years', designation: 'optional catalog designation', commonName: 'optional common/title name', preparedHdUrl: 'optional retained runtime object URL', preparedHdImage: 'optional retained decoded HTMLImageElement for immediate HD display', fov: 'runtime AVM degrees', hdUrl: 'required image asset', sourceUrl: 'optional provider source URL', credit: 'optional/provider supplied', imageType: 'optional', category: 'optional', telescope: 'optional provider metadata' } }
    },
    geminiEndpoint: {
      method: 'POST',
      optional: true,
...
function navigation0008Url(){
  ...
  if(owner?.src){
    return new URL(
      '../navigation/gv-navigation-0019.js',
      owner.src
    ).href;
  }

  return '/viewer/modules/navigation/gv-navigation-0019.js';
}
...
function ensureNavigationModule0013(){
  ...
  if(existing?.VERSION==='0019'){
    applyNavigation0013RuntimeConfig(existing);
    return Promise.resolve(existing);
  }

  if(existing && existing.VERSION!=='0019'){
...
function pumpPreparationPhases(){
  ...
  /*
   * RANDOM 0184:
   * FUTURE[0] remains the sole Random-button readiness gate and
   * receives priority, but it no longer blocks preparation of the
   * remaining future window.
   *
   * The preparation engine may therefore keep three HD/AVM workers
   * active while FUTURE[0] is still becoming presentation-ready.
   */
  if(!headReady){
  ...
  /*
   * Immediately submit the rest of the already-selected future
   * window. PREFETCH_MAX_WORKERS owns actual concurrency; excess
   * destinations remain queued behind the active workers.
   */
