...
icon.style.setProperty('--gv-bts-phase-delay',phaseDelay);
  ...
  function createRandomPreparationEngine(options = {}) {
    ...
    const aladinPreparedReceipts=new Map();
        ...
        function gv0211NormalizeWcsToDecodedImage(sourceWcs,image){
            /*
             * 0237 FOUR-FIELD AUTHORITY:
             * Never transform celestial registration from decoded raster size.
             * Image width/height are not navigation inputs.
             */
            return Object.freeze({...sourceWcs});
        }
        ...
        function destinationWithPrepared(item){
            return {...item.destination,preparedHdUrl:item.objectUrl,preparedSource:item.sourceKind,preparedHdImage:item.image,preparedHdBlob:item.blob||null};
        }
  ...
  return {
    ...
    preparedHdBlob:
      resource?.blob instanceof Blob
        ? resource.blob
        : (record.destination?.preparedHdBlob instanceof Blob
            ? record.destination.preparedHdBlob
            : null)
  };
}
