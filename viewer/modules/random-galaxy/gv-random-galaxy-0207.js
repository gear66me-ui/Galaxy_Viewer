/*
RANDOM GALAXY 0207 — STARTUP AVM DEPENDENCY REPAIR
- Restores HD-only Random-button readiness; AVM staging is no longer a startup gate.
- Restores HD-only click admission.
- Click preflight waits for the AVM API when necessary and awaits filtered AVM staging before physical travel.
- Navigation 0021, vignette parameters, FIFO ownership, HD preparation, and travel choreography are unchanged.

RANDOM GALAXY 0206 — NAVIGATION LOADER PATH REPAIR
- Navigation resolver accepts every four-digit Random Galaxy module version, including 02xx.
- GitHub Pages fallback preserves the /Galaxy_Viewer repository prefix.
- No AVM, vignette, readiness, travel choreography, queue, or navigation behavior changes.

RANDOM GALAXY 0205 — PRETRAVEL AVM STAGING GATE
- RANDOM GALAXY stays disabled until FUTURE[0] HD presentation and filtered AVM staging are both ready.
- Click-time fallback awaits AVM staging before physical navigation; no full-image filter work is allowed to overlap flight.
- Existing vignette parameters and visual output are unchanged.
  ...
  function createRandomPreparationEngine(options = {}) {
    ...
    const aladinPreparedReceipts=new Map();
        ...
        async function prepareRuntimeAvmAuthority(destination){
            ...
            const authorityCacheKey=key+'|'+url;
...
function navigation0008Url(){
  ...
  const owner=
    scripts.find(script=>
      /\/random-galaxy\/gv-random-galaxy-\d{4}\.js(?:[?#].*)?$/i.test(
        String(script.src||'')
      )
    ) ||
  ...
  const repoPrefix=
    String(location.pathname||'').includes('/Galaxy_Viewer/')
      ? '/Galaxy_Viewer'
      : '';

  return `${location.origin}${repoPrefix}/viewer/modules/navigation/gv-navigation-0021.js`;
}
...
function hasReadyNavigation(){
  ...
  return randomNavigationWindow.isNextReady(bundle=>{
    ...
    if(hdReady)ensureHeadAvmStaged(bundle);
    return hdReady;
  });
}
...
async function waitForAvmPretravelApi(timeoutMs=15000){
  const started=performance.now();
  ...
  for(;;){
    ...
    if(performance.now()-started>=timeoutMs)
      throw new Error('AR119 VISIBLE AVM PRETRAVEL API WAIT TIMEOUT');
...
async function requestRandomNavigation(){
  ...
  try{
    avmApi=await waitForAvmPretravelApi();
  }catch(error){
    randomNavigationWindow.rollbackPending?.();
    throw error;
  }
  ...
  if(!avmReadyForFlight){
    ...
    GV_TRACE.enabled&&gvTrace(11902,'AR119_VISIBLE_AVM_STAGE_BEFORE_TRAVEL',{
