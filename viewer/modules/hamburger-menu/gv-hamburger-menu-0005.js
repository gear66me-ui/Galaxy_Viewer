...
(() => {
  ...
  const PROJECTION_LABELS = ["MOLLWEIDE", "SPHERICAL", "AITOFF-AIT", "TANGENTIAL", "SINUSOIDAL"];
...
.${DEFAULT_ROOT_CLASS} .gv-menu-proxy{appearance:none;-webkit-appearance:none;position:absolute;left:${LEFT_INSET}px;top:${TOP_INSET}px;display:flex;align-items:center;justify-content:center;width:${ROW_HEIGHT}px;height:${ROW_HEIGHT}px;margin:0;padding:0;overflow:hidden;background:linear-gradient(145deg,#081B3A 0%,#0B3177 42%,#1484DB 76%,#296DBD 100%);border:1px solid #7CCBFF;border-radius:6px;cursor:pointer;touch-action:manipulation;outline:none;box-shadow:inset 0 2px 2px rgba(225,251,255,.82),inset 0 -3px 5px rgba(0,0,0,.52),inset 0 0 13px rgba(41,153,255,.34),0 0 3px #DDF8FF,0 0 9px rgba(50,190,255,.72),0 0 18px rgba(20,116,219,.35);z-index:20}
...
.${DEFAULT_ROOT_CLASS} .gv-static-inset{position:absolute;inset:2px;border-radius:6px;pointer-events:none;z-index:0;opacity:1;background:linear-gradient(145deg,rgba(8,27,58,.28),rgba(20,132,219,.10));box-shadow:inset 0 0 0 1px rgba(124,203,255,.34),inset 0 0 7px rgba(88,191,255,.20),inset 0 0 11px rgba(41,109,189,.16)}
...
.${DEFAULT_ROOT_CLASS} .gv-viewer-menu-row.gv-selected .gv-viewer-menu-label,.${DEFAULT_ROOT_CLASS} .gv-viewer-menu-row.gv-selected .gv-viewer-menu-icon,.${DEFAULT_ROOT_CLASS} .gv-projection-option-row.gv-active .gv-projection-option-label{background:linear-gradient(145deg,#0B3177 0%,#1484DB 65%,#296DBD 100%)}
.${DEFAULT_ROOT_CLASS} .gv-projection-option-row.gv-active .gv-projection-option-icon{background:linear-gradient(145deg,#081B3A 0%,#0B3177 54%,#1484DB 100%)}
