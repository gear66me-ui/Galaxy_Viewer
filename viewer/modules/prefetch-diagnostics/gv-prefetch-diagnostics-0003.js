/* Galaxy Viewer Prefetch Diagnostics 0003 — rolling read-only telemetry, execution diary, and export. */
(()=>{
    'use strict';
    if(window.GVPrefetchDiagnostics0003)return;
    const VERSION='0003';
    ...
    const waitForApi=async()=>{
        for(let i=0;i<600;i++){
            ...
            if(api&&String(api.version)==='11C'&&typeof api.getHdSlots==='function'&&typeof api.getAladinSlots==='function'&&typeof api.getArchivePreloadState==='function')return api;
            await new Promise(resolve=>setTimeout(resolve,100));
        }
        throw new Error('11C TELEMETRY API TIMEOUT');
    };
    ...
    function installStyle(){
        const style=document.createElement('style');style.id='gv-prefetch-diag-style-0003';style.textContent=`
...
.gv-prefetch-diag-hidden{display:none!important}`;document.head.appendChild(style);
    ...
    function download(api){
        ...
        const report={schema:'GV-PREFETCH-DIAGNOSTIC-0003',viewerVersion:String(api.version||''),diagnosticsVersion:VERSION,sessionStartedAt,sessionDurationMs:Math.round((performance.now()-sessionStartedMs)*10)/10,downloadedAt:new Date().toISOString(),catalogDatabaseCounts:clone(api.catalogDatabaseCounts||{}),startupMetrics:clone(api.startupMetrics||{}),finalBackground:clone(api.getBackgroundWorkState?.()||null),finalHdSlots:clone(api.getHdSlots?.()||[]),finalAladinSlots:clone(api.getAladinSlots?.()||[]),finalWebPreload:clone(api.getArchivePreloadState?.()||null),finalActive:clone(api.getActiveDestination?.()||null),events,interactions:clone(interactions),perDestination:summarizeDestinations(events),resourceTiming:resourceTiming(),snapshots};
        const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a'),stamp=new Date().toISOString().replace(/[:.]/g,'-');a.href=url;a.download=`GV-11C-prefetch-diagnostic-${stamp}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),30000);
    }
    async function mount(){
        if(mounted)return;mounted=true;const api=await waitForApi();installStyle();
        const hdPanel=makePanel('gv-prefetch-diag-hd','HD','right',10),aladinPanel=makePanel('gv-prefetch-diag-aladin','ALDN','left',10),webPanel=makePanel('gv-prefetch-diag-web','WEB','left',10),activeLine=document.createElement('div');activeLine.className='gv-prefetch-diag-active';activeLine.textContent='ACTIVE —';hdPanel.appendChild(activeLine);
    ...
    window.GVPrefetchDiagnostics0003=Object.freeze({version:VERSION,mount,getSnapshots:()=>clone(snapshots),getInteractions:()=>clone(interactions)});
    mount().catch(error=>console.error('GV PREFETCH DIAGNOSTICS 0003 FAILURE',error));
})();
