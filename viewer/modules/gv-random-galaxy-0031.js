...
AUTHORIZED CHANGES: readable compact arrival presentation, five-field HD science banner, exact retained decoded-image handoff for immediate HD display, and 0012 identity. Touch-through interaction, 36px Hubble controls, top-centered HD viewing, no post-arrival reframing, and 24.075-second travel are preserved.
*/
(function (global) {
  'use strict';
...
.gvrg-card{position:absolute;left:50%;top:auto;bottom:64px;transform:translateX(-50%);width:calc(100vw - 24px);max-width:none;padding:4px 5px 4px;border:1px solid #78FFAB;border-radius:6px;background:rgba(0,12,8,.78);box-shadow:inset 0 0 10px rgba(120,255,171,.10),0 0 14px rgba(77,255,143,.34);opacity:0;transition:opacity .20s ease;pointer-events:none}
...
.gvrg-row{display:block;min-width:0;margin:0;padding:1px 4px 2px;border-right:1px solid rgba(120,255,171,.28);border-bottom:1px solid rgba(120,255,171,.28);text-align:center;pointer-events:none}
...
.gvrg-hd-primary{width:min(190px,calc(100% - 41px));flex:0 1 190px;min-width:0;height:36px;padding:4px 8px;background:linear-gradient(145deg,rgba(18,105,65,.96),rgba(31,176,96,.94));color:#E8FFF0;font:400 11px/1.05 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:.55px;text-align:center;text-shadow:0 0 4px rgba(229,255,239,.76);pointer-events:auto}
.gvrg-hd-icon-button{flex:0 0 36px;width:36px;height:36px;padding:2px;display:flex;align-items:center;justify-content:center;overflow:hidden;background:linear-gradient(145deg,rgba(18,105,65,.96),rgba(31,176,96,.94));pointer-events:auto}
.gvrg-hd-primary,.gvrg-hd-icon-button{border:2px solid #ff8214;box-shadow:inset 0 0 7px rgba(167,255,203,.28),0 0 8px rgba(255,130,20,.38),0 0 14px rgba(255,130,20,.18)}
...
.gvrg-back-chevron::before{border-width:5px;border-color:#78FFAB;filter:drop-shadow(0 0 4px rgba(87,255,147,.90));transform:translate(-38%,-50%) rotate(-135deg)}
...
.gvrg-hd-scale{position:absolute;left:50%;bottom:12px;z-index:6;transform:translateX(-50%);display:none;flex-direction:column;align-items:center;gap:4px;pointer-events:none;color:#FFD85A;font:400 9px/1.05 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:.7px;text-align:center;text-shadow:0 0 3px rgba(255,242,168,.72),0 0 7px rgba(255,180,45,.28);white-space:nowrap}
...
.gvrg-hd-loading{position:absolute;left:50%;top:50%;z-index:2;transform:translate(-50%,-50%);font:400 9px/1.4 "${FONT_NAMES.spaceAge}",sans-serif;letter-spacing:1px;color:#dff7ff;pointer-events:none}`;
    ...
    async #getHubbleCandidate(excludeName) {
      if (!this.hubbleProvider) throw new Error('Random Galaxy 0012 requires the local Hubble catalog provider.');
    ...
    async travelToRandom() {
      ...
      try {
        ...
        const duration = (firstHomeTrip ? 10 : Number(this.options.travelSeconds)) * 1000;
        const started = performance.now();
        let projectionState = firstHomeTrip ? this.options.wideProjection : this.options.arrivalProjection;
    ...
    async downloadHubbleHD() {
      ...
      const filename = `${stem}-Hubble-HD.jpg`;
