...
const VERSION="0048";const trace=(code,detail={})=>{try{globalThis.GalaxyViewerDiagnostics?.recordAvm?.(code,detail)}catch(_){}};trace("AVM_LAB_BOOT",{version:VERSION});
const WATCH_MS=260;
  ...
  try{
    ...
    if(preparedReady){
      ...
      try{
        ...
        trace('AVM_0048_BLEND_FETCH_FALLBACK',{
          key,
          sourceUrl:raw,
        ...
        return {
          url:raw,
          imgFormat:imageFormatFor(formatHint||raw),
          filtered:false,
          objectUrl:'',
          sourceKind:'direct-unfiltered',
    ...
    trace('AVM_0048_BLEND_UNFILTERED_FALLBACK',{
      key,
      sourceUrl:raw,
      sourceKind,
    ...
    return {
      ...
      objectUrl:'',
      sourceKind:'direct-unfiltered',
      sourceWidth:Number(source?.naturalWidth||source?.width||0)||0,
      sourceHeight:Number(source?.naturalHeight||source?.height||0)||0,
      outputWidth:Number(source?.naturalWidth||source?.width||0)||0,
      outputHeight:Number(source?.naturalHeight||source?.height||0)||0
    };
  }finally{
