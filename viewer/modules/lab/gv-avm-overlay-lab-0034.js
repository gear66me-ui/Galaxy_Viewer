(()=>{
'use strict';
const VERSION="0034";const trace=(code,detail={})=>{try{globalThis.GalaxyViewerDiagnostics?.recordAvm?.(code,detail)}catch(_){}};trace("AVM_LAB_BOOT",{version:VERSION});
const WATCH_MS=260;
const ARRIVAL_SETTLE_MS=650;
const LAYER_NAME="Galaxy Viewer AVM Overlay";
const PANEL_ID="gv-avm-overlay-lab";
const STYLE_ID="gv-avm-overlay-lab-style-0034";
...
function setPanelVisible(on){trace("AVM_PANEL_VISIBILITY_REQUEST",{on});const p=panel();if(!p)return;if(hdOpen()||randomInTransit())on=false;p.dataset.on=on?'1':'0';p.style.display=on?'block':'none'}
...
#${PANEL_ID} .gv-avm-title{position:absolute;left:1px;top:50%;transform:translateY(-50%);height:108px;display:flex;align-items:center;justify-content:center;writing-mode:vertical-rl;text-orientation:mixed;font:400 6px/1 "Space Age",sans-serif;letter-spacing:1.5px;color:#6feaff;text-shadow:0 0 4px rgba(190,250,255,.96),0 0 10px rgba(35,190,255,.9);user-select:none;pointer-events:none;white-space:nowrap}
...
function installTravelButtonGuard(){const buttons=document.querySelectorAll('#gv-random-galaxy,.gv-galaxy-history');for(const b of buttons){if(!b||b.dataset.gvAvmGuard0032)continue;b.dataset.gvAvmGuard0032='1';for(const ev of ['pointerdown','touchstart','click'])b.addEventListener(ev,hideOverlayForTravel,{capture:true,passive:true})}}
...
function prepareForTravel(d){
  trace('AVM_PRETRAVEL_REQUEST',{name:String(d?.name||''),archiveId:String(d?.archiveId||'')});
  ...
  if(!url||!key||!wcs)return Promise.reject(new Error('AVM PRETRAVEL DESTINATION/WCS INVALID'));

  clearArrivalTimer();
  try{
    if(typeof aladinRef.removeOverlayImageLayer==='function')
      aladinRef.removeOverlayImageLayer(LAYER_NAME);
  }catch(_){}
  overlay=null;
  lastKey='';
  loadingKey=key;
  ...
  return new Promise((resolve,reject)=>{
    ...
    const finish=(error,value)=>{
      ...
      if(error){
        loadingKey='';
        trace('AVM_PRETRAVEL_ERROR',{key,url,message:String(error?.message||error||'')});
        reject(error);
      }else resolve(value);
    };
    ...
    try{
      ...
      const imageLayer=ARef.image(url,{
        ...
        successCallback:(ra,dec,fov,image)=>{
          // AR119: observational only. Never touch camera here.
          if(key!==loadingKey){
          ...
          lastContext={
            version:VERSION,
            mode:'pretravel-visible-exact-wcs',
          ...
          trace('AVM_PRETRAVEL_READY',{
            key,url,ra,dec,fov,
            camera:cameraSnapshot('pretravel-visible-ready',d),
            catalog:catalogSummary(d)
          });
          finish(null,true);
        },
        errorCallback:error=>finish(
          error instanceof Error?error:new Error(String(error||'AVM PRETRAVEL IMAGE LOAD FAILED'))
        )
      });
      overlay=imageLayer;
      trace('AVM_PRETRAVEL_SET_LAYER',{key,url});
      aladinRef.setOverlayImageLayer(imageLayer,LAYER_NAME);
    }catch(error){
      finish(error);
    }
  });
}
function loadArrivedCandidate(candidate,reason){trace("AVM_LOAD_CANDIDATE_ENTER",{reason,key:candidate?.key});if(!candidate||!canCommandAladin(candidate.key)||candidate.key===loadingKey){trace("AVM_LOAD_CANDIDATE_REJECT",{reason,key:candidate?.key,loadingKey});return false}loadingKey=candidate.key;try{const url=candidate.url;const op=resetSliderOpaque();const beforeImageCall=cameraSnapshot("before-image-call",candidate.d);trace("AVM_AREF_IMAGE_CALL",{reason,key:candidate.key,url,imgFormat:imageFormatFor(url),opacity:op,camera:beforeImageCall,catalog:catalogSummary(candidate.d)});let callbackCamera=null;const imageLayer=ARef.image(url,{name:LAYER_NAME,imgFormat:imageFormatFor(url),opacity:op,successCallback:(ra,dec,fov,image)=>{const successCamera=cameraSnapshot("success-callback-enter",candidate.d);trace("AVM_AREF_IMAGE_SUCCESS",{key:candidate.key,ra,dec,fov,camera:successCamera,catalog:catalogSummary(candidate.d)});if(!canCommandAladin(candidate.key)){trace("AVM_SUCCESS_STALE",{key:candidate.key});loadingKey='';return}callbackCamera=applyAvmCallbackCamera(ra,dec,fov,candidate.d,candidate.key);lastKey=candidate.key;loadingKey='';lastContext={version:VERSION,mode:'native-avm-camera-roll-square-up',reason,destination:candidate.d,url,opacity:opacityNow(),ra,dec,fov,image,overlay:imageLayer};trace("AVM_LAST_CONTEXT_SET",{key:candidate.key,url});applyOpacity();setPanelVisible(true);trace("AVM_PANEL_SHOW",{key:candidate.key});try{console.info('GV AVM LAB 0032 ARRIVAL CURRENT LOADED',lastContext)}catch(_){}} ,errorCallback:(error)=>{trace('AVM_AREF_IMAGE_ERROR',{reason,key:candidate.key,url,message:String(error?.message||error||'')});loadingKey='';try{console.warn('GV AVM LAB 0032 IMAGE LOAD FAILED',{reason,key:candidate.key,url,error})}catch(_){}}});overlay=imageLayer;trace('AVM_SET_LAYER_ENTER',{key:candidate.key,phase:'immediate',camera:cameraSnapshot('before-layer-install',candidate.d),catalog:catalogSummary(candidate.d)});if(typeof aladinRef.setOverlayImageLayer==='function')aladinRef.setOverlayImageLayer(imageLayer,LAYER_NAME);const afterLayer=cameraSnapshot('after-layer-install',candidate.d);const cameraRoll=applyAvmCameraRoll(candidate.d,candidate.key);trace('AVM_SET_LAYER_DONE',{key:candidate.key,phase:'immediate',camera:afterLayer,cameraRoll,callbackCamera,catalog:catalogSummary(candidate.d)});return Boolean(imageLayer)}catch(e){trace('AVM_EXCEPTION',{reason,key:candidate?.key,message:String(e?.message||e||'')});loadingKey='';try{console.error('GV AVM LAB 0032 FAILED',e)}catch(_){}return false}}
function maybeLoadAfterArrival(reason){trace('AVM_LOAD_REQUEST',{reason});installTravelButtonGuard();ensurePanel();if(hdOpen()){trace('AVM_HD_OPEN_SKIP',{reason});clearOverlay();return false}if(!isArrivedStable()){trace('AVM_NOT_ARRIVED_STABLE',{reason});setPanelVisible(false);return false}const candidate=bestDestination();trace('AVM_CANDIDATE_RESULT',{reason,key:candidate?.key,url:candidate?.url});if(!candidate||!canCommandAladin(candidate.key)){trace('AVM_CANDIDATE_REJECT',{reason,key:candidate?.key});return false}if(candidate.key===lastKey){trace('AVM_ALREADY_LOADED',{reason,key:candidate.key});applyOpacity();setPanelVisible(true);return true}if(candidate.key===loadingKey||candidate.key===scheduledKey){trace('AVM_ALREADY_LOADING_OR_SCHEDULED',{reason,key:candidate.key,loadingKey,scheduledKey});return true}clearOverlay();scheduledKey=candidate.key;trace('AVM_SCHEDULE_SETTLE',{reason,key:candidate.key,delayMs:ARRIVAL_SETTLE_MS});arrivalTimer=setTimeout(()=>{arrivalTimer=0;const fresh=bestDestination();if(!fresh||fresh.key!==scheduledKey||!canCommandAladin(fresh.key)){trace('AVM_SETTLE_REJECT',{reason,key:fresh?.key,scheduledKey});scheduledKey='';return}scheduledKey='';loadArrivedCandidate(fresh,'arrival-settle-'+reason)},ARRIVAL_SETTLE_MS);return true}
function install({A,aladin,viewerRoot,randomGalaxy}={}){trace('AVM_INSTALL_ENTER',{hasA:!!A,hasAladin:!!aladin});if(!A)throw new Error('ALADIN A NAMESPACE MISSING');if(!aladin)throw new Error('ALADIN INSTANCE MISSING');ARef=A;aladinRef=aladin;randomRef=randomGalaxy;rootRef=viewerRoot||document.body;ensurePanel();installTravelButtonGuard();installInteractionDiagnostics();setPanelVisible(false);if(!watchTimer)watchTimer=setInterval(()=>{maybeLoadAfterArrival('watch')},WATCH_MS);try{console.info('GV AVM LAB 0032 INSTALLED arrival-gated native-avm-camera-roll slider-touch')}catch(_){}trace('AVM_INSTALL_EXIT',{version:VERSION});return {version:VERSION,panel:panel(),loadForBestDestination:maybeLoadAfterArrival}}
...
globalThis.GalaxyViewerAvmOverlayLab={VERSION,install,uninstall,prepareForTravel,loadForBestDestination:maybeLoadAfterArrival,get lastContext(){return lastContext}};
})();
