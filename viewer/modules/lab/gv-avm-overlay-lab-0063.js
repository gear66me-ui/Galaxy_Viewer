/*
AVM 0057
FORWARD-ONLY PRETRAVEL SUPERSEDE RECOVERY.
SUPERSEDED STALE BLEND RETURNS FALSE; IT DOES NOT BECOME A TERMINAL DOWNLOAD FAILURE.
*/
/*
AVM 0056 — ARRIVAL PRESENTATION GATE REPAIR
BASELINE: AVM 0053
AUTHORIZED CHANGE:
- Keep HD-open and Random-arrival authority.
- Allow prepared pending AVM layer to promote when navigationSeen is stale false.
- No raw-image fallback. Vignette filter remains mandatory.
*/
/*
...
*/
/*
AVM 0052 — CANVAS-SAFE VIGNETTE PIXEL TRANSPORT
BASELINE: AVM 0051
- Never draws the prepared provider HTMLImageElement into the vignette canvas.
- Acquires vignette pixels through a CORS-safe blob path: direct CORS fetch first, then the existing Galaxy Viewer Cloudflare image proxy.
- Decodes only CORS-safe blobs with createImageBitmap before canvas draw, eliminating the tainted-canvas toBlob failure.
- Keeps provider AVM/WCS and original HD URL authoritative; the proxy is pixel transport only.
- Preserves 0051 failure backoff, UI yields, and mandatory vignette filtering. No raw/unfiltered fallback.
*/
(()=>{
'use strict';
/* AVM 0043 — FUTURE[0] may stage its prepared blob/WCS layer before click; prepareForTravel reuses that ready layer. */
const VERSION="0063";const trace=(code,detail={})=>{try{globalThis.GalaxyViewerDiagnostics?.recordAvm?.(code,detail)}catch(_){}};trace("AVM_LAB_BOOT",{version:VERSION});
const WATCH_MS=260;
...
function markVignetteFailure(key,error,phase){
  ...
  trace('AVM_0052_VIGNETTE_BACKOFF_ARMED',{
    key:k,
    count:next.count,
...
function clearVignetteFailure(key,phase){
  ...
  trace('AVM_0052_VIGNETTE_BACKOFF_CLEARED',{
    key:k,
    phase:String(phase||'')
  });
  return true;
...
function vignetteBackoffActive(key,phase){
  ...
  if(Date.now()>=Number(state.until||0)){
    vignetteFailureState.delete(k);
    trace('AVM_0052_VIGNETTE_BACKOFF_EXPIRED',{
      key:k,
      phase:String(phase||'')
    });
    return false;
  ...
  if(!state.suppressionTraced){
    ...
    trace('AVM_0052_VIGNETTE_RETRY_SUPPRESSED',{
      key:k,
...
async function fetchCanvasSafeBitmap(raw,key,preparedBlob=null){
  if(typeof createImageBitmap!=='function')
    throw new Error('createImageBitmap unavailable');

  /*
   * 0060: consume Random Galaxy's already-downloaded compressed JPEG.
   * Do not download the same AVM image again for the vignette pipeline.
   */
  const prepared=
    preparedBlob instanceof Blob && Number(preparedBlob.size)>0
      ? preparedBlob
      : null;

  if(prepared){
    const bitmap=await createImageBitmap(prepared);
    trace('AVM_0060_PREPARED_BLOB_REUSED',{
      key,
      bytes:Number(prepared.size)||0,
      width:Number(bitmap?.width)||0,
      height:Number(bitmap?.height)||0
    });
    return {bitmap,transport:'prepared-hd-blob'};
  }

  const attempts=[
    {kind:'direct-cors',url:raw},
    {
      kind:'cloudflare-proxy',
      url:CANVAS_IMAGE_PROXY+encodeURIComponent(raw)+'&consumer=avm0060'
    }
  ];
  ...
  for(const attempt of attempts){
    try{
      trace('AVM_0052_CANVAS_SOURCE_ATTEMPT',{
        key,
        sourceUrl:raw,
      ...
      trace('AVM_0052_CANVAS_SOURCE_READY',{
        key,
        sourceUrl:raw,
        transport:attempt.kind,
        bytes:Number(blob.size)||0,
        mime:String(blob.type||response.headers.get('content-type')||''),
        width:Number(bitmap.width)||0,
        height:Number(bitmap.height)||0
      });

      return {bitmap,transport:attempt.kind};
    }catch(error){
      failures.push(
        attempt.kind+': '+
      ...
      trace('AVM_0052_CANVAS_SOURCE_FAILURE',{
        key,
        sourceUrl:raw,
        transport:attempt.kind,
        message:String(error?.message||error||'')
  ...
  throw new Error(
    'CANVAS-SAFE IMAGE ACQUISITION FAILED :: '+
    failures.join(' | ')
  );
}
...
async function makeGlobalBlendSource(
  ...
  let bmp=null,canvas=null,source=null,sourceKind='canvas-safe-blob';
  const started=performance.now();

  try{
    const acquired=await fetchCanvasSafeBitmap(raw,key,preparedBlob);
    bmp=acquired.bitmap;
    source=bmp;
    sourceKind=acquired.transport;
    ...
    /*
     * 0061: preserve source raster geometry exactly.
     * No resizing means no pixel-coordinate/register conversion is required.
     */
    const outW=w;
    const outH=h;
    ...
    trace('AVM_GLOBAL_BLEND_READY',{
      ...
      sourceKind,
      pipeline:'native-gradient-0052-canvas-safe',
...
function isArrivedStable(){const st=stateOf(),open=hdOpen();const randomArrived=st.arrived===true&&st.busy!==true;const pendingPrepared=pendingReady===true&&!!pendingOverlay&&!!pendingKey;const result=!open&&randomArrived&&(navigationSeen===true||pendingPrepared);trace("AVM_ARRIVAL_STATE",{arrived:st.arrived,busy:st.busy,hdOpen:open,navigationSeen,pendingPrepared,result});if(!result)trace("AVM_ARRIVAL_GATE_FALSE",{arrived:st.arrived,busy:st.busy,hdOpen:open,navigationSeen,pendingPrepared});return result}
...
function cloneWcsForVisible(d){
  ...
  /* 0058: preserve JPEG AVM/WCS handedness exactly; no synthetic CDELT rewrite. */

  return wcs;
}
function scaledWcsForBlend(baseWcs,blendSource){
  /*
   * 0059 FOUR-FIELD AUTHORITY:
   * Never rewrite celestial registration because the display raster changed.
   * Vignette dimensions are visual-only.
   */
  return baseWcs;
}
...
async function stageForTravel(d,{hidePanel=false}={}){
  ...
  const promise=(async()=>{
    const blendSource=await makeGlobalBlendSource(
      filterSourceUrl,
      key,
      d?.preparedHdImage||null,
      authorityUrl,
      d?.preparedHdBlob||null
    );
    ...
    if(loadingKey!==key||pendingKey!==key){
      ...
      trace('AVM_0057_PRETRAVEL_SUPERSEDED_RESOLVED',{key,authorityUrl,sourceUrl});
      return false;
    }
  ...
  wrapped=promise.catch(error=>{
    ...
    const superseded=
      message==='AVM PRETRAVEL BLEND SUPERSEDED' ||
      message==='AVM PRETRAVEL LOAD SUPERSEDED';

    if(superseded){
      clearVignetteFailure(key,'pretravel-superseded');
      trace('AVM_0053_PRETRAVEL_SUPERSEDED',{key,message});
    }else{
  ...
  void (async()=>{
    ...
    try{
      blendSource=await makeGlobalBlendSource(
        filterSourceUrl,
        key,
        candidate.d?.preparedHdImage||null,
        authorityUrl,
        candidate.d?.preparedHdBlob||null
      );
...
function maybeLoadAfterArrival(reason){
  ...
  if(
    ...
    trace('AVM_0063_WATCH_PENDING_OWNERSHIP_GUARD',{
