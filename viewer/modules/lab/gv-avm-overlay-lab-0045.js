...
const VERSION="0045";const trace=(code,detail={})=>{try{globalThis.GalaxyViewerDiagnostics?.recordAvm?.(code,detail)}catch(_){}};trace("AVM_LAB_BOOT",{version:VERSION});
const WATCH_MS=260;
...
async function makeGlobalBlendSource(sourceUrl,key){
  const raw=String(sourceUrl||'').trim();
  if(!raw)return {url:raw,imgFormat:imageFormatFor(raw),filtered:false,objectUrl:''};
  let bmp=null,canvas=null;
  try{
    const response=await fetch(raw,{cache:'default'});
    if(!response.ok)throw new Error('GLOBAL BLEND IMAGE FETCH '+response.status);
    const blob=await response.blob();
    if(typeof createImageBitmap!=='function')throw new Error('createImageBitmap unavailable');
    bmp=await createImageBitmap(blob);
    const w=Number(bmp.width)||0,h=Number(bmp.height)||0;
    if(w<1||h<1)throw new Error('GLOBAL BLEND IMAGE DIMENSIONS INVALID');
    canvas=document.createElement('canvas');
    canvas.width=w;canvas.height=h;
    const ctx=canvas.getContext('2d',{willReadFrequently:true});
    if(!ctx)throw new Error('GLOBAL BLEND 2D CONTEXT UNAVAILABLE');
    ctx.drawImage(bmp,0,0);
    const q=ctx.getImageData(0,0,w,h),d=q.data,cx=w/2,cy=h/2,p=GLOBAL_BLEND;
    const rx=w*.5*p.diameter,ry=h*.5*p.diameter;
    const core=Math.min(.98,p.core),blend=p.blend,decay=p.decay,curve=p.curve,dis=p.dissolve,dark=p.dark;
    const grain=Math.max(1,p.block|0);
    for(let yy=0;yy<h;yy++)for(let xx=0;xx<w;xx++){
      const i=(yy*w+xx)*4;
      const ex=(xx-cx)/rx,ey=(yy-cy)/ry,er=Math.hypot(ex,ey);
      const edgePx=Math.min(xx,yy,w-1-xx,h-1-yy);
      const edgeNorm=edgePx/Math.max(1,Math.min(w,h)*.5);
      let t;
      if(p.hardMode){
        ...
        t=Math.min(rectFade,ellipseFade);
      }else{
        if(er<=core)t=1;
        else if(er>=1)t=0;
        else t=1-(er-core)/Math.max(.001,1-core);
      }
      const smooth=t*t*(3-2*t);
      const expTail=(1-Math.exp(-Math.max(.1,decay)*t))/(1-Math.exp(-Math.max(.1,decay)));
      const mix=Math.max(0,Math.min(1,blend*1.8));
      let alpha=((1-mix)*Math.pow(smooth,curve)+mix*expTail)*p.hdOpacity*p.crossblend;
      const z=1-t,lum=(.2126*d[i]+.7152*d[i+1]+.0722*d[i+2])/255;
      const prob=dis*Math.pow(z,1.35)*((1-dark)+dark*(1-lum));
      if(globalBlendHash(Math.floor(xx/grain),Math.floor(yy/grain))<prob*.28)alpha*=.35;
    ...
    const png=await new Promise((resolve,reject)=>canvas.toBlob(
      value=>value?resolve(value):reject(new Error('GLOBAL BLEND PNG ENCODE FAILED')),
      'image/png'
    ));
    const objectUrl=URL.createObjectURL(png);
    trace('AVM_GLOBAL_BLEND_READY',{key,width:w,height:h,settings:GLOBAL_BLEND});
    return {url:objectUrl,imgFormat:'png',filtered:true,objectUrl};
  }catch(error){
    trace('AVM_GLOBAL_BLEND_FALLBACK',{key,sourceUrl:raw,message:String(error?.message||error||'')});
    return {url:raw,imgFormat:imageFormatFor(raw),filtered:false,objectUrl:''};
  }finally{
    try{bmp?.close?.()}catch(_){}
    if(canvas){try{canvas.width=1;canvas.height=1}catch(_){}}
  }
}
...
function commitPreparedArrival(d){
  ...
  if(pendingKey===key&&pendingPreparePromise){
    trace('AVM_0045_ARRIVAL_AWAIT_PENDING',{key,pendingReady,hasPendingOverlay:!!pendingOverlay});
    void Promise.resolve(pendingPreparePromise)
      ...
      .catch(error=>{
        trace('AVM_0045_ARRIVAL_AWAIT_PENDING_ERROR',{key,message:String(error?.message||error||'')});
        maybeLoadAfterArrival('arrival-await-pending-error');
      });
    return true;
  }

  trace('AVM_0045_ARRIVAL_COMMIT_REJECT',{
    key,
    lastKey,
    pendingKey,
    pendingReady,
...
async function stageForTravel(d,{hidePanel=false}={}){
  ...
  if(!authorityUrl||!sourceUrl||!key||!wcs)throw new Error('AVM PRETRAVEL DESTINATION/WCS INVALID');
  ...
  const promise=(async()=>{
    const blendSource=await makeGlobalBlendSource(sourceUrl,key);
...
function loadArrivedCandidate(candidate,reason){
  ...
  void (async()=>{
    const blendSource=await makeGlobalBlendSource(url,key);
    if(!canCommandAladin(key)||loadingKey!==key||pendingKey!==key){
      releaseBlendUrl(blendSource.objectUrl);
      if(loadingKey===key)loadingKey='';
      return;
    }
    ...
    try{
      ...
      const imageLayer=ARef.image(blendSource.url,{
        ...
        errorCallback:error=>{
          trace('AVM_AREF_IMAGE_ERROR',{reason,key,url,filtered:blendSource.filtered,message:String(error?.message||error||'')});
          if(pendingOverlay===imageLayer)clearPendingOverlay();
          if(loadingKey===key)loadingKey='';
        }
      });
      ...
      pendingOverlay=imageLayer;
      aladinRef.setOverlayImageLayer(imageLayer,stagedLayerName);
      trace('AVM_PENDING_LAYER_INSTALLED',{key,reason,filtered:blendSource.filtered,activeLayerName,pendingLayerName:stagedLayerName});
    }catch(error){
      if(pendingKey===key)clearPendingOverlay();
      else releaseBlendUrl(blendSource.objectUrl);
      if(loadingKey===key)loadingKey='';
