...
function preloadUrl(url){
  if(preloadCache.has(url))return false;
  ...
  if(preloadImages.length>80)preloadImages.splice(0,preloadImages.length-80);
  return true;
}
...
function preloadUpcoming(reason){
  ...
  for(const d of upcomingCandidates(randomRef)){
    ...
    if(preloadUrl(url))count++;
    if(count>=10)break;
  }
  if(count){
    try{console.info("GV AVM LAB 0012 PRELOADED",{reason,count})}catch(_){}
  }
}
...
function hideOverlayForTravel(){
  ...
  for(const ms of [350,700,1100,1600,2400,3400,4800,6600,8800,11200,14000,17200,20800,24400]){
...
function installTravelButtonGuard(){
  ...
  for(const b of buttons){
    if(!b||b.dataset.gvAvmGuard0012)continue;
    b.dataset.gvAvmGuard0012="1";
...
function panel(){
  ...
  let s=document.getElementById("gv-avm-overlay-lab-style-0012");
  if(!s){
    s=document.createElement("style");
    s.id="gv-avm-overlay-lab-style-0012";
    s.textContent=[
      ...
      "#gv-avm-overlay-lab{position:fixed;left:0;top:118px;z-index:2147482000;width:92px;height:195px;transform:none;background:transparent!important;border:0!important;box-shadow:none!important;padding:0!important;margin:0!important;pointer-events:none;display:none}",
      "#gv-avm-touch-shield{position:absolute;left:0;top:0;width:92px;height:195px;border-radius:20px;background:rgba(0,18,60,.10);pointer-events:auto;touch-action:none;user-select:none;-webkit-user-select:none;overscroll-behavior:contain}",
      "#gv-avm-fader-label{position:absolute;left:4px;top:50%;height:138px;transform:translateY(-50%) rotate(180deg);writing-mode:vertical-rl;text-orientation:mixed;font:400 7px/1 \"Space Age\",sans-serif;letter-spacing:.85px;color:#6DE2FF;text-shadow:0 0 7px rgba(109,226,255,.92),0 0 13px rgba(0,146,255,.58);pointer-events:none}",
      "#gv-avm-overlay-opacity{--gv-avm-pct:100%;-webkit-appearance:none;appearance:none;position:absolute;left:-92px;top:84px;width:158px;height:24px;margin:0;background:transparent!important;outline:none;display:block;transform:rotate(-90deg);transform-origin:center;direction:rtl;touch-action:none;pointer-events:auto}",
      "#gv-avm-overlay-opacity::-webkit-slider-runnable-track{height:8px;border-radius:999px;background:linear-gradient(90deg,#004DAD 0%,#007BE8 48%,#32C7FF 78%,#8EECFF 100%);box-shadow:0 0 8px rgba(109,226,255,.78),0 0 15px rgba(0,146,255,.52),inset 0 0 5px rgba(221,248,255,.34)}",
      "#gv-avm-overlay-opacity::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;width:19px;height:19px;border-radius:50%;background:#6DE2FF;border:1px solid rgba(221,248,255,.98);margin-top:-5.5px;box-shadow:0 0 11px rgba(109,226,255,.96),0 0 21px rgba(0,146,255,.68)}",
      "#gv-avm-overlay-opacity::-moz-range-track{height:8px;border-radius:999px;background:linear-gradient(90deg,#004DAD 0%,#007BE8 48%,#32C7FF 78%,#8EECFF 100%);box-shadow:0 0 8px rgba(109,226,255,.78),0 0 15px rgba(0,146,255,.52),inset 0 0 5px rgba(221,248,255,.34)}",
      "#gv-avm-overlay-opacity::-moz-range-thumb{width:19px;height:19px;border-radius:50%;background:#6DE2FF;border:1px solid rgba(221,248,255,.98);box-shadow:0 0 11px rgba(109,226,255,.96),0 0 21px rgba(0,146,255,.68)}",
      "#gv-avm-overlay-lab[data-on=\"1\"]{display:block}",
      "@media(max-height:720px){#gv-avm-overlay-lab{top:108px;width:88px;height:180px}#gv-avm-touch-shield{width:88px;height:180px}#gv-avm-fader-label{height:126px}#gv-avm-overlay-opacity{left:-84px;top:77px;width:146px}}",
      "@media(max-width:430px){#gv-avm-overlay-lab{left:0;top:112px;width:90px;height:188px}#gv-avm-touch-shield{width:90px;height:188px}#gv-avm-overlay-opacity{left:-88px;top:80px;width:152px}}",
      "#gv-avm-overlay-lab{position:fixed;left:0;top:112px;z-index:2147482000;width:104px;height:202px;transform:none;background:transparent!important;border:0!important;box-shadow:none!important;padding:0!important;margin:0!important;pointer-events:none;display:none}",
      "#gv-avm-touch-shield{position:absolute;left:0;top:0;width:104px;height:202px;border-radius:21px;background:rgba(0,18,60,.10);pointer-events:auto;touch-action:none;user-select:none;-webkit-user-select:none;overscroll-behavior:contain}",
      "#gv-avm-fader-label{position:absolute;left:4px;top:50%;height:140px;transform:translateY(-50%) rotate(180deg);writing-mode:vertical-rl;text-orientation:mixed;font:400 7px/1 \"Space Age\",sans-serif;letter-spacing:.85px;color:#6DE2FF;text-shadow:0 0 7px rgba(109,226,255,.92),0 0 13px rgba(0,146,255,.58);pointer-events:none}",
      "#gv-avm-overlay-opacity{--gv-avm-pct:100%;-webkit-appearance:none;appearance:none;position:absolute;left:-38px;top:86px;width:158px;height:24px;margin:0;background:transparent!important;outline:none;display:block;transform:rotate(-90deg);transform-origin:center;direction:rtl;touch-action:none;pointer-events:auto}",
      "#gv-avm-overlay-opacity::-webkit-slider-runnable-track{height:8px;border-radius:999px;background:linear-gradient(90deg,#004DAD 0%,#007BE8 48%,#32C7FF 78%,#8EECFF 100%);box-shadow:0 0 8px rgba(109,226,255,.78),0 0 15px rgba(0,146,255,.52),inset 0 0 5px rgba(221,248,255,.34)}",
      "#gv-avm-overlay-opacity::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;width:19px;height:19px;border-radius:50%;background:#6DE2FF;border:1px solid rgba(221,248,255,.98);margin-top:-5.5px;box-shadow:0 0 11px rgba(109,226,255,.96),0 0 21px rgba(0,146,255,.68)}",
      "#gv-avm-overlay-opacity::-moz-range-track{height:8px;border-radius:999px;background:linear-gradient(90deg,#004DAD 0%,#007BE8 48%,#32C7FF 78%,#8EECFF 100%);box-shadow:0 0 8px rgba(109,226,255,.78),0 0 15px rgba(0,146,255,.52),inset 0 0 5px rgba(221,248,255,.34)}",
      "#gv-avm-overlay-opacity::-moz-range-thumb{width:19px;height:19px;border-radius:50%;background:#6DE2FF;border:1px solid rgba(221,248,255,.98);box-shadow:0 0 11px rgba(109,226,255,.96),0 0 21px rgba(0,146,255,.68)}",
      "#gv-avm-overlay-lab[data-on=\"1\"]{display:block}",
      "@media(max-height:720px){#gv-avm-overlay-lab{top:104px;width:100px;height:188px}#gv-avm-touch-shield{width:100px;height:188px}#gv-avm-fader-label{height:130px}#gv-avm-overlay-opacity{left:-34px;top:80px;width:148px}}",
      "@media(max-width:430px){#gv-avm-overlay-lab{left:0;top:108px;width:102px;height:196px}#gv-avm-touch-shield{width:102px;height:196px}#gv-avm-overlay-opacity{left:-36px;top:83px;width:154px}}",
    ].join("");
...
function applyExactImageFov(ra,dec,fov,destination,tag){
  ...
  if(fit!==null && fit>0){
    ...
    try{
      console.info("GV AVM LAB 0012 FOV FIT",{tag,fit,sourceFov,imageFovDeg:destination?.imageFovDeg,imageFovXDeg:destination?.imageFovXDeg,imageFovYDeg:destination?.imageFovYDeg,name:destination?.name,archiveId:destination?.archiveId});
    }catch(_){}
  }
}
...
function loadForBestDestination(reason){
  ...
  if(suppressed()){
    ...
    if(travelOldKey && candidate.key===travelOldKey)return false;
    suppressUntil=0;
    travelOldKey="";
    travelMinUntil=0;
  }
  ...
  try{
    ...
    overlay=ARef.image(url,{
      ...
      successCallback:(ra,dec,fov,image)=>{
        ...
        console.info("GV AVM LAB 0012 AUTO LOADED",lastContext);
      },
      errorCallback:(error)=>{
        loadingKey="";
        console.warn("GV AVM LAB 0012 IMAGE LOAD FAILED",{reason,key,url,error});
      }
    });
    ...
    loadingKey="";
    console.error("GV AVM LAB 0012 FAILED",e);
    return false;
  }
}
...
function install({A,aladin,viewerRoot,randomGalaxy}={}){
  ...
  if(!global.__gvAvmPreloadInterval0012){global.__gvAvmPreloadInterval0012=setInterval(()=>preloadUpcoming("background"),1200);}
