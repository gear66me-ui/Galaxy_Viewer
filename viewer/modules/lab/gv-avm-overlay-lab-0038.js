(()=>{
'use strict';
const VERSION="0038";const trace=(code,detail={})=>{try{globalThis.GalaxyViewerDiagnostics?.recordAvm?.(code,detail)}catch(_){}};trace("AVM_LAB_BOOT",{version:VERSION});
const WATCH_MS=260;
...
let pendingOverlay=null,pendingKey="",pendingReady=false,pendingContext=null;
...
function promotePending(d,key,reason){
  ...
  applyPreparedAuthorityCamera(d,key);
...
function prepareForTravel(d){
  ...
  return new Promise((resolve,reject)=>{
    ...
    try{
      ...
      trace('AVM_PRETRAVEL_SET_PENDING_LAYER',{key,url,activeLayerName,pendingLayerName:stagedLayerName});
...
function maybeLoadAfterArrival(reason){
  ...
  if(candidate.key===lastKey){
    applyPreparedAuthorityCamera(candidate.d,candidate.key);
    applyOpacity();
    setPanelVisible(true);
    return true;
  }
