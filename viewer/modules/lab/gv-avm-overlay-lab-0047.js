...
const VERSION="0047";const trace=(code,detail={})=>{try{globalThis.GalaxyViewerDiagnostics?.recordAvm?.(code,detail)}catch(_){}};trace("AVM_LAB_BOOT",{version:VERSION});
const WATCH_MS=260;
...
async function makeGlobalBlendSource(sourceUrl,key,preparedImage=null){
  ...
  try{
    ...
    const cx=w/2,cy=h/2,p=GLOBAL_BLEND;
    const rx=w*.5*p.diameter,ry=h*.5*p.diameter;
    const core=Math.min(.98,p.core);
    const blend=p.blend;
    const decay=p.decay;
    const curve=p.curve;
    const dis=p.dissolve;
    const dark=p.dark;
    const grain=Math.max(1,p.block|0);

    // Keep each JavaScript burst near ~64k pixels. This preserves 0046's
    // exact per-pixel output while avoiding one full-frame ImageData copy.
    const rowsPerSlice=Math.max(
      4,
      Math.min(64,Math.floor(65536/Math.max(1,w)))
    );

    for(let y0=0;y0<h;y0+=rowsPerSlice){
      const sliceHeight=Math.min(rowsPerSlice,h-y0);
      const q=ctx.getImageData(0,y0,w,sliceHeight);
      const d=q.data;
      ...
      for(let localY=0;localY<sliceHeight;localY++){
        ...
        for(let xx=0;xx<w;xx++){
          ...
          let t;
          if(p.hardMode){
            const rectFade=Math.max(
              0,
              Math.min(1,edgeNorm/Math.max(.01,blend))
            );
            const ellipseFade=
              er<=core
                ? 1
          ...
          const expTail=
            (1-Math.exp(-Math.max(.1,decay)*t)) /
            (1-Math.exp(-Math.max(.1,decay)));
          const mix=Math.max(0,Math.min(1,blend*1.8));
          let alpha=
            ((1-mix)*Math.pow(smooth,curve)+mix*expTail) *
            p.hdOpacity *
            p.crossblend;

          const z=1-t;
          const lum=
            (.2126*d[i]+.7152*d[i+1]+.0722*d[i+2])/255;
          const prob=
            dis*Math.pow(z,1.35)*((1-dark)+dark*(1-lum));
          ...
          d[i+3]=Math.round(
            d[i+3]*Math.max(0,Math.min(1,alpha))
          );
        }
      }

      ctx.putImageData(q,0,y0);
    ...
    trace('AVM_GLOBAL_BLEND_REQUIRED_FAILED',{
      key,
      sourceUrl:raw,
      sourceKind,
...
function loadArrivedCandidate(candidate,reason){
  ...
  const sourceUrl=preparedUrl||authorityUrl;
  const preparedImage=candidate.d?.preparedHdImage||null;
  const stagedLayerName=pendingLayerName;
  ...
  void (async()=>{
    ...
    try{
      ...
      const imageLayer=ARef.image(
        blendSource.url,
        {
          ...
          errorCallback:error=>{
            ...
            setTimeout(
              ()=>maybeLoadAfterArrival('image-layer-error-retry'),
              250
            );
          }
        }
      ...
      setTimeout(
        ()=>maybeLoadAfterArrival('filtered-load-retry'),
        250
      );
    }
  })();
