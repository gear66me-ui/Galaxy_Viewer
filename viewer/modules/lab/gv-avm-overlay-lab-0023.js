...
let ARef=null,aladinRef=null,randomRef=null,rootRef=null,overlay=null,watchTimer=null,arrivalTimer=0,lastKey="",loadingKey="",scheduledKey="",lastContext=null;
...
function isArrivedStable(){const st=stateOf();return !hdOpen()&&st.arrived===true&&st.busy!==true}
...
function hideOverlayForTravel(){const sl=slider();if(sl)sl.value=DEFAULT_SLIDER;clearOverlay();setPanelVisible(false)}
function installTravelButtonGuard(){const buttons=document.querySelectorAll('#gv-random-galaxy,.gv-galaxy-history');for(const b of buttons){if(!b||b.dataset.gvAvmGuard0023)continue;b.dataset.gvAvmGuard0023='1';for(const ev of ['pointerdown','touchstart','click'])b.addEventListener(ev,hideOverlayForTravel,{capture:true,passive:true})}}
