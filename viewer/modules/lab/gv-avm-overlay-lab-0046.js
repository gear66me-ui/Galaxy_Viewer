...
const VERSION="0046";const trace=(code,detail={})=>{try{globalThis.GalaxyViewerDiagnostics?.recordAvm?.(code,detail)}catch(_){}};trace("AVM_LAB_BOOT",{version:VERSION});
const WATCH_MS=260;
...
async function makeGlobalBlendSource(sourceUrl,key,preparedImage=null){
  ...
  try{
    ...
    const q=ctx.getImageData(0,0,w,h);
    const d=q.data,cx=w/2,cy=h/2,p=GLOBAL_BLEND;
      ...
      for(let xx=0;xx<w;xx++){
        ...
        if(p.hardMode){
          ...
          const ellipseFade=
            er<=core
              ? 1
              : Math.max(0,1-(er-core)/Math.max(.001,1-core));
