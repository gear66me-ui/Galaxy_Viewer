/* Galaxy Viewer Prefetch Diagnostics 0002 — rolling read-only telemetry, execution diary, and export. */
(()=>{
    'use strict';
    if(window.GVPrefetchDiagnostics0002)return;
    const VERSION='0002';
    ...
    const waitForApi=async()=>{
        for(let i=0;i<600;i++){
            ...
            if(api&&String(api.version)==='11B'&&typeof api.getHdSlots==='function'&&typeof api.getAladinSlots==='function'&&typeof api.getArchivePreloadState==='function')return api;
            await new Promise(resolve=>setTimeout(resolve,100));
        }
        throw new Error('11B TELEMETRY API TIMEOUT');
    };
    const clone=value=>JSON.parse(JSON.stringify(value??null));
    const shortProvider=value=>value==='CHANDRA'?'CHA':value==='JWST'?'JWS':'HUB';
    const shortState=value=>({QUEUED:'Q',DOWNLOADING:'DL',DECODING:'DEC',RETRY:'RET',READY:'✓',EMPTY:'—',WAIT:'WAIT',LOADING:'LOAD',PENDING:'PEND',FAILED:'FAIL'}[String(value||'').toUpperCase()]||String(value||'').slice(0,5));
    ...
    function installStyle(){
        const style=document.createElement('style');style.id='gv-prefetch-diag-style-0002';style.textContent=`
...
.gv-prefetch-diag-hidden{display:none!important}`;document.head.appendChild(style);
    ...
    function snapshot(api,reason='interval'){
        ...
        const record={at:new Date().toISOString(),ms:performance.now(),reason,background:clone(api.getBackgroundWorkState?.()||null),hdSlots:clone(api.getHdSlots?.()||[]),aladinSlots:clone(api.getAladinSlots?.()||[]),web:clone(api.getArchivePreloadState?.()||null),active:clone(api.getActiveDestination?.()||null),newEvents:clone(events.slice(lastEventCount))};
    ...
    function renderRows(panel,items,rowCount){
        ...
        rows.forEach((row,index)=>{
            ...
            row.querySelector('.gv-prefetch-diag-name').textContent=item.key?`${shortProvider(item.provider)} ${item.name}`:'EMPTY';
            row.querySelector('.gv-prefetch-diag-state').textContent=shortState(item.state);
        });
    }
    function render(api,hdPanel,aladinPanel,webPanel,activeLine){
        ...
        renderRows(hdPanel,hd,10);renderRows(aladinPanel,aladin,10);renderRows(webPanel,webItems,3);
        const active=api.getActiveDestination?.();activeLine.textContent=active?`ACTIVE #${active.sequence||'—'} ${active.name}`:'ACTIVE —';
    ...
    function summarizeDestinations(events){
        const bySeq=new Map();
        const bucket=sequence=>{
            const key=Number(sequence)||0;if(!key)return null;
            if(!bySeq.has(key))bySeq.set(key,{sequence:key,name:'',provider:'',events:[],metrics:{},errors:[]});
            return bySeq.get(key);
        };
        for(const event of events){
            const item=bucket(event.sequence);if(!item)continue;
            item.events.push(clone(event));if(event.name)item.name=event.name;if(event.provider)item.provider=event.provider;
            if(/fail|error|retry/i.test(String(event.type||'')))item.errors.push({type:event.type,error:event.error||''});
        }
        for(const item of bySeq.values()){
            const first=type=>item.events.find(e=>e.type===type)?.ms;
            item.metrics={
                randomClickToTravelSuspendMs:duration(first('random-click'),first('travel-suspend')),
                travelMs:duration(first('travel-suspend'),first('travel-arrival')),
                hdClickToVisibleMs:duration(first('hd-entry-click'),first('hd-visible')),
                providerClickToFrameLoadMs:duration(first('archive-click'),first('archive-frame-load')),
                providerClickToVisibleMs:duration(first('archive-click'),first('archive-visible')),
                websitePreloadMs:duration(first('archive-preload-start'),first('archive-preload-ready')),
                websiteStateAtClick:item.events.find(e=>e.type==='archive-click')?.sourceState??null
            };
        }
    ...
    function download(api){
        ...
        const report={schema:'GV-PREFETCH-DIAGNOSTIC-0002',viewerVersion:String(api.version||''),diagnosticsVersion:VERSION,sessionStartedAt,sessionDurationMs:Math.round((performance.now()-sessionStartedMs)*10)/10,downloadedAt:new Date().toISOString(),catalogDatabaseCounts:clone(api.catalogDatabaseCounts||{}),startupMetrics:clone(api.startupMetrics||{}),finalBackground:clone(api.getBackgroundWorkState?.()||null),finalHdSlots:clone(api.getHdSlots?.()||[]),finalAladinSlots:clone(api.getAladinSlots?.()||[]),finalWebPreload:clone(api.getArchivePreloadState?.()||null),finalActive:clone(api.getActiveDestination?.()||null),events,interactions:clone(interactions),perDestination:summarizeDestinations(events),resourceTiming:resourceTiming(),snapshots};
        const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a'),stamp=new Date().toISOString().replace(/[:.]/g,'-');a.href=url;a.download=`GV-11B-prefetch-diagnostic-${stamp}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),30000);
    }
    async function mount(){
        if(mounted)return;mounted=true;const api=await waitForApi();installStyle();
        const hdPanel=makePanel('gv-prefetch-diag-hd','HD','right',10),aladinPanel=makePanel('gv-prefetch-diag-aladin','ALDN','left',10),webPanel=makePanel('gv-prefetch-diag-web','WEB','left',3),activeLine=document.createElement('div');activeLine.className='gv-prefetch-diag-active';activeLine.textContent='ACTIVE —';hdPanel.appendChild(activeLine);
        const button=document.createElement('button');button.id='gv-prefetch-download-report';button.type='button';button.textContent='DOWNLOAD REPORT';button.addEventListener('click',()=>download(api));document.addEventListener('click',captureInteraction,true);document.body.append(aladinPanel,hdPanel,webPanel,button);
        snapshot(api,'mount');render(api,hdPanel,aladinPanel,webPanel,activeLine);
        timer=setInterval(()=>{snapshot(api,'5-second');render(api,hdPanel,aladinPanel,webPanel,activeLine)},5000);
    ...
    window.GVPrefetchDiagnostics0002=Object.freeze({version:VERSION,mount,getSnapshots:()=>clone(snapshots),getInteractions:()=>clone(interactions)});
    mount().catch(error=>console.error('GV PREFETCH DIAGNOSTICS 0002 FAILURE',error));
})();
