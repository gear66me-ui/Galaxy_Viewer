...
(async()=>{
    ...
    const UHD_VIEWPORT_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/gv-uhd-viewport-0001.js?v=10AK-0001';
    ...
    uhdViewportPromise=loadScript(UHD_VIEWPORT_URL,'gvUhdViewport0001').then(()=>{
        ...
        console.error('GV-10AK NON-CRITICAL UHD VIEWPORT LOAD FAILURE',error);
        return null;
    });
    ...
    hdArchiveButton.addEventListener('click',event=>{
        ...
        if(!destination||!sourceUrl){
            console.error('GV-10AK PROVIDER ICON ACTIVATION BLOCKED',{destination,sourceUrl:destination?.sourceUrl||''});
            setArchiveLoading(false);
            return;
        }
        ...
        try{
            ...
            archiveOpenRequested=false;
            setArchiveLoading(false);
            console.error('GV-10AK PROVIDER ARCHIVE NAVIGATION FAILED',{destination,sourceUrl,error});
            return;
        }
        ...
        providerLaunchTimer=setTimeout(()=>{
            if(!archiveOverlay.classList.contains('gv-open')){
                archiveOpenRequested=false;
                setArchiveLoading(false);
                console.error('GV-10AK PROVIDER ARCHIVE NAVIGATION TIMEOUT',{destination,sourceUrl});
            }
        },8000);
    },true);
    hdDownloadButton?.addEventListener('click',async event=>{
        ...
        if(!destination||!sourceUrl||!previewUrl||!providerIcon){
            console.error('GV-10AK VIEW ZOOMABLE ACTIVATION BLOCKED',{destination,sourceUrl,previewUrl,providerIcon});
            return;
        }
        ...
        if(!viewport||typeof viewport.open!=='function'){
            ...
            console.error('GV-10AK VIEW ZOOMABLE UHD UNAVAILABLE',{uhdViewportReady,uhdViewportLoadError:String(uhdViewportLoadError?.message||uhdViewportLoadError||'')});
            return;
        }
        ...
        if(opened){
            ...
            console.error('GV-10AK VIEWPORT OPEN CALL COMPLETED WITHOUT .open ROOT STATE',{destination,payload});
        }
    },true);
