...
# viewer-0028
# Standalone Galaxy Viewer release copied from frozen Viewer 27.
# Only change: persistently restore the real Aladin SIMBAD button and nested icon after clear.
...
#aladin-cosmic-command-test .gv-native-simbad-moved.gv-target-normal:hover,
#aladin-cosmic-command-test .gv-native-simbad-moved.gv-target-normal:focus,
...
A.init.then(() => {
    ...
    function findNativeSimbadPointer(){return root.querySelector(".aladin-simbadPointer-control")||root.querySelector(".aladin-simbadPointerControl")||root.querySelector("[class*='simbadPointer']")}
    function getNativeSimbadButton(){
        const candidate=findNativeSimbadPointer();
        if(!candidate)return null;
        if(candidate.matches?.("button.aladin-btn"))return candidate;
        return candidate.querySelector?.("button.aladin-btn")||candidate.closest?.("button.aladin-btn")||null;
    }
    ...
    function restoreTargetVisual(){
        const button=getNativeSimbadButton();
    ...
    function synchronizeTargetGeometry(coordinateBox,nativePointer){
        ...
        [["width",boxWidth+"px"],["min-width",boxWidth+"px"],["max-width",boxWidth+"px"],["height",height+"px"],["min-height",height+"px"],["max-height",height+"px"],["border-style",borderStyle],["border-width",borderWidth],["border-color",borderColor],["border-radius",radius]].forEach(([property,value])=>{
            if(nativePointer.style.getPropertyValue(property)!==value){
    ...
    function ensureHelper(row,nativePointer){
        ...
        if(!status.dataset.gvClearBound){
            ...
            status.addEventListener("keydown",event=>{
                if((event.key==="Enter"||event.key===" ")&&status.classList.contains("gv-clear-ready")){
                    event.preventDefault(); clearSimbad();
                }
            });
        }
        if(!nativePointer.dataset.gvHelperBound){
            ...
            nativePointer.addEventListener("click",()=>{
                ...
                const text=stack.querySelector(".gv-simbad-helper-text");
                if(simbadModeActive){
                    getNativeSimbadButton()?.classList.remove("gv-target-normal");
