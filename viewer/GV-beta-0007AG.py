...
(async()=>{
  ...
  const waitFor=(test,timeout=20000)=>new Promise((resolve,reject)=>{const end=performance.now()+timeout;const tick=()=>{let value=null;try{value=test()}catch(_){ }if(value){resolve(value);return}if(performance.now()>end){reject(new Error("GV-BETA-0007AG STARTUP TIMEOUT"));return}setTimeout(tick,50)};tick()});
  ...
  if(!htmlMatches.length||!jsMatches.length)throw new Error("GV-BETA-0007AG COULD NOT EXTRACT 7AD BASELINE");
  ...
  versionLabel.textContent="V-7AG";
  window.GV7AG_VALIDATION={passed:false,pending:true,status:"AWAITING PROJECTION SUBMENU"};
  ...
  function enforceProjectionPulse(c=collect()){
    ...
    if(projectionPulseStart===null)projectionPulseStart=document.timeline.currentTime??performance.now();
    ...
    const next=[];
    tileTargets.forEach(el=>{const a=el.animate(insetFrames,options);a.startTime=projectionPulseStart;next.push(a)});
    svgTargets.forEach(svg=>{const a=svg.animate(iconFrames,options);a.startTime=projectionPulseStart;next.push(a)});
  ...
  async function initializeProjectionPulse(){
    ...
    await waitFor(()=>collect().leftLabels[0]&&collect().leftIcons[0]);
    if(projectionPulseStart===null)projectionPulseStart=document.timeline.currentTime??performance.now();
    enforceProjectionPulse();
    ensurePulseObserver();
    return true;
  ...
  async function startCoordinateGlow(){
    ...
    coordinateGlowStart=document.timeline.currentTime??performance.now();
    coordinateAnimation=frame.animate(coordinateFrames,{duration:CYCLE,iterations:Infinity,easing:EASING,fill:"both"});
    coordinateAnimation.startTime=coordinateGlowStart;
    window.GV7AG_COORDINATE_GLOW={
      passed:coordinateInitialFrame==="ICRSD",
      initialFrame:coordinateInitialFrame,
      cycleMs:CYCLE,
      easing:EASING,
      startTime:coordinateGlowStart,
      playState:coordinateAnimation.playState
    };
    return frame;
  }
  ...
  async function validateCoordinateGlow(){
    ...
    if(!frame||!coordinateAnimation)return {passed:false,error:String(coordinateGlowError||"coordinate frame unavailable")};
    ...
    const sameAnimationAfterFirst=coordinateAnimation===animationRef&&frame.getAnimations().includes(animationRef)&&animationRef.playState==="running";
    frame.click();
    await nextPaint();
    ...
    const directRunning=frame.getAnimations().filter(a=>a.playState==="running");
    ...
    const result={
      ...
      expectedFirst,
      sameNodeAfterFirst,
      sameAnimationAfterFirst,
      sameNodeAfterRestore:shadow?.querySelector(".gvco-frame")===frame,
      sameAnimationAfterRestore:coordinateAnimation===animationRef&&frame.getAnimations().includes(animationRef)&&animationRef.playState==="running",
      exactlyOneRunningGlow:directRunning.length===1&&directRunning[0]===animationRef,
      cycleMs:timing.duration,
      easing:timing.easing,
      startTime:animationRef.startTime,
    ...
    result.passed=
      ...
      result.sameNodeAfterFirst&&result.sameAnimationAfterFirst&&
      result.sameNodeAfterRestore&&result.sameAnimationAfterRestore&&
      result.exactlyOneRunningGlow&&
      result.cycleMs===CYCLE&&normalizeEasing(result.easing)===normalizeEasing(EASING)&&
  ...
  function projectionPulseDiagnostics(c){
    ...
    return {
      ...
      expectedCount:18,
      sharedStart:projectionAnimations.length===18&&projectionAnimations.every(a=>a.startTime===projectionPulseStart),
      ...
      startTime:projectionPulseStart,
      cycleMs:CYCLE,
      easing:EASING
    };
  }
  ...
  function prepareAndPatch(){
    ...
    patched=true;
    observer?.disconnect();
    finalizeAndValidate(baseline).catch(error=>console.error("GV-BETA-0007AG FINALIZATION FAILURE:",error));
    return true;
  }
  ...
  async function validate(baseline,centering){
    ...
    const checks={
      versionLabel:versionLabel.textContent==="V-7AG",
    ...
    if(!passed)throw new Error("GV-BETA-0007AG CONTRACT FAILED "+JSON.stringify(window.GV7AG_VALIDATION));
    return true;
  }

  let coordinateGlowError=null;
  const coordinateGlowPromise=startCoordinateGlow().catch(error=>{coordinateGlowError=error;window.GV7AG_COORDINATE_GLOW={passed:false,error:String(error)};return null});
  const projectionPulseReady=initializeProjectionPulse().catch(error=>{console.error("GV-BETA-0007AG PROJECTION PULSE INITIALIZATION FAILURE:",error);return false});
...
})().catch(error=>console.error("GV-BETA-0007AG STARTUP FAILURE:",error));
"""))

# GV-beta-0007AG staged
