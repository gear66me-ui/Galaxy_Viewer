...
#gv-center-reticle img{display:none!important;width:32px;height:32px}
body.gv-hd-open #gv-center-reticle{display:none!important}
#gv-rotation-hotdog{position:absolute;left:50%;top:max(43px,calc(env(safe-area-inset-top) + 39px));z-index:7600;transform:translateX(-50%);max-width:calc(100vw - 18px);box-sizing:border-box;padding:4px 12px;border:1px solid rgba(255,65,65,.72);border-radius:999px;background:rgba(28,0,0,.62);color:#FFD6D6;font:700 9px/1.2 "Roboto Mono",Consolas,monospace;letter-spacing:.35px;text-align:center;text-shadow:0 0 5px rgba(255,0,0,.55);white-space:nowrap;pointer-events:none}
...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0126.js';
    ...
    const RANDOM_GALAXY_VERSION='0126';
  ...
  boxSizing:'border-box'
        ...
        for(const angle of [0,90]){
            ...
            Object.assign(line.style,{
                ...
                boxShadow:'0 0 5px rgba(255,0,0,.16)',
  ...
  return null;
    ...
    // ==================== 12AR-66 EARTH COMPASS PRESENTATION ====================
...
#gv-earth-return-indicator.gv-visible{opacity:0.001 !important;visibility:visible !important}
`;
...
`;
    ...
    // GV AR66 ROTATION HOTDOG — BEGIN
    (()=>{
        const host=document.createElement('div');
        host.id='gv-rotation-hotdog';
        host.textContent='ROTATION —';
        root.appendChild(host);

        const fmt=value=>Number.isFinite(Number(value))?Number(value).toFixed(2):'—';
        const labelFor=target=>{
            const raw=String(target?.designation||target?.catalogDesignation||target?.id||target?.name||target?.title||'CURRENT GALAXY').replace(/\s+/g,' ').trim();
            return raw.slice(0,38)||'CURRENT GALAXY';
        };
        const getTarget=()=>{
            try{return window.GalaxyRandomGalaxy?.currentDestination||randomGalaxy?.currentDestination||randomGalaxy?.destination||null}catch(_){return null}
        };
        const update=()=>{
            try{
                const target=getTarget();
                const hdOpen=Boolean(randomGalaxy?.getState?.().hdOpen);
                document.body.classList.toggle('gv-hd-open',hdOpen);
                if(!target){host.textContent='ROTATION —';return}
                const catalog=Number(target.aladinRotation??target.rotation??target.orientation??NaN);
                host.textContent=`${labelFor(target)}  •  CATALOG ROT ${fmt(catalog)}°`;
            }catch(_){
                host.textContent='ROTATION —';
            }
        };
