...
# GV-beta-0010AI
# Standalone release derived directly from the exact GV-beta-0010AH baseline.
# 10AI authorized changes only: restore core startup isolation so UHD loading cannot gate
# Random Galaxy or the main viewer, while preserving the authorized 10AH HD controls unchanged.
...
(async()=>{
    ...
    const UHD_VIEWPORT_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/gv-uhd-viewport-0001.js?v=10AI-0001';
    ...
    let coreViewerReady=false;
    let randomGalaxyReady=false;
    let uhdViewportReady=false;
    let uhdViewportLoadError=null;
    let uhdViewportPromise=null;
    ...
    async function loadCombinedGalaxyCatalog(){
        const [hubbleData,jwstData]=await Promise.all([loadGalaxyCatalog(),loadJwstCatalog()]);
        const hubble=hubbleData.eligible,jwst=jwstData.eligible;
        const combined=Object.freeze([...hubble,...jwst]);
        catalogRecordCount=combined.length;
        catalogDatabaseCounts=Object.freeze({hubble:hubbleData.rawCount,jwst:jwstData.rawCount,total:hubbleData.rawCount+jwstData.rawCount,eligibleHubble:hubble.length,eligibleJwst:jwst.length,eligibleTotal:combined.length});
    ...
    function navigateHistory(index){
        ...
        randomGalaxy.travelToRandom().catch(error=>{
            forcedDestination=null;pendingHistoryIndex=null;navigationPending=false;resumeBackgroundWork();resumeArchivePreloads();endTravelHud();setHistoryControls();console.error('GV-10G HISTORY NAVIGATION FAILURE',error);
        });
    }
    ...
    const randomGalaxy=window.GalaxyRandomGalaxy.mount(randomGalaxyHost,{
        ...
        onError(error){
            navigationPending=false;pendingHistoryIndex=null;forcedDestination=null;endTravelHud();setHistoryControls();resumeBackgroundWork();resumeArchivePreloads();console.error('GV-10G RANDOM GALAXY FAILURE',error);
        }
    });
    ...
    if(hdDownloadButton){
        ...
        hdDownloadButton.setAttribute('aria-label','VIEW ZOOMABLE');
        hdDownloadButton.setAttribute('title','VIEW ZOOMABLE');
        const zoomableTelescopeTile=document.createElement('span');
        zoomableTelescopeTile.className='gv-zoomable-telescope-tile';
        zoomableTelescopeTile.setAttribute('aria-hidden','true');
        const zoomableTelescopeIcon=document.createElement('img');
        zoomableTelescopeIcon.className='gv-zoomable-telescope-icon';
        zoomableTelescopeIcon.src='https://raw.githubusercontent.com/gear66me-ui/Galaxy_Viewer/beta/viewer/artwork/gv-telescope-exact-pixel.svg';
        zoomableTelescopeIcon.alt='';
        const zoomableLabel=document.createElement('span');
        zoomableLabel.className='gv-zoomable-label';
        const zoomableLine1=document.createElement('span');
        zoomableLine1.textContent='VIEW';
        const zoomableLine2=document.createElement('span');
        zoomableLine2.textContent='ZOOMABLE';
        zoomableLabel.append(zoomableLine1,zoomableLine2);
        const zoomableArrow=document.createElement('span');
        zoomableArrow.className='gv-zoomable-arrow';
        zoomableArrow.setAttribute('aria-hidden','true');
        zoomableTelescopeTile.appendChild(zoomableTelescopeIcon);
        hdDownloadButton.replaceChildren(zoomableTelescopeTile,zoomableLabel,zoomableArrow);
    ...
    randomGalaxy.hdOverlay?.appendChild(hdArchiveButton);
    ...
    archiveFrame.addEventListener('error',()=>{
        ...
        archiveRevealTimer=0;
        setArchiveLoading(false);
        if(requested&&sourceUrl)openProviderSource(sourceUrl,'ARCHIVE FRAME FALLBACK');
    });
    ...
    function validHttpsSource(value){
        try{const url=new URL(String(value||'').trim(),location.href);return url.protocol==='https:'?url.href:''}catch(_){return ''}
    }
    function openProviderSource(value,control='PROVIDER CONTROL'){
        const sourceUrl=validHttpsSource(value);
        if(!sourceUrl){console.error(`GV-10AI ${control} INVALID HTTPS SOURCE`,{destination:currentArchiveDestination(),sourceUrl:value});return false}
        try{
            const opened=window.open(sourceUrl,'_blank','noopener,noreferrer');
            if(opened)return true;
        }catch(error){console.warn(`GV-10AI ${control} NEW-CONTEXT NAVIGATION FAILED; USING WEBVIEW FALLBACK`,error)}
        try{window.location.assign(sourceUrl);return true}catch(error){console.error(`GV-10AI ${control} NAVIGATION FAILED`,{destination:currentArchiveDestination(),sourceUrl,error});return false}
    }
    function wireUhdProviderNavigation(sourceUrl){
        const infoButton=document.querySelector('#gv-uhd-viewport-0001 .gvuhd-info');
        if(!infoButton){console.error('GV-10AI UHD PROVIDER CONTROL MISSING AFTER VIEWPORT OPEN',{destination:currentArchiveDestination(),sourceUrl});return false}
        infoButton.onclick=event=>{
            event?.preventDefault?.();
            event?.stopPropagation?.();
            openProviderSource(sourceUrl,'UHD PROVIDER CONTROL');
        };
        return true;
    }
    hdArchiveButton.addEventListener('click',event=>{
        ...
        const sourceUrl=validHttpsSource(destination?.sourceUrl);
        if(!destination||!sourceUrl){console.error('GV-10AI PROVIDER ICON ACTIVATION BLOCKED',{destination,sourceUrl:destination?.sourceUrl||''});setArchiveLoading(false);return}
        clearTimeout(providerLaunchTimer);
        setArchiveLoading(true);
        requestAnimationFrame(()=>requestAnimationFrame(()=>{
            const launched=openProviderSource(sourceUrl,'PROVIDER ICON');
            clearTimeout(providerLaunchTimer);
            providerLaunchTimer=setTimeout(()=>setArchiveLoading(false),launched?1200:0);
        }));
    },true);
    hdDownloadButton?.addEventListener('click',event=>{
        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation();
        const destination=currentArchiveDestination();
        const sourceUrl=validHttpsSource(destination?.sourceUrl);
        const viewport=window.GVUHDViewport;
        const previewImage=randomGalaxy.hdViewport?.querySelector('img');
        const previewUrl=String(previewImage?.currentSrc||previewImage?.src||destination?.hdUrl||'').trim();
        const provider=providerFor(destination);
        const providerIcon=provider==='JWST'?JWST_ICON_URL:HUBBLE_ICON_URL;
        if(!destination||!sourceUrl||!previewUrl||!providerIcon){
            console.error('GV-10AI VIEW ZOOMABLE ACTIVATION BLOCKED',{destination,sourceUrl,previewUrl,providerIcon});
            return;
        }
        if(!uhdViewportReady||!viewport||typeof viewport.open!=='function'){
            console.error('GV-10AI VIEW ZOOMABLE UHD UNAVAILABLE — CORE VIEWER REMAINS OPERATIONAL',{uhdViewportReady,uhdViewportLoadError:String(uhdViewportLoadError?.message||uhdViewportLoadError||''),uhdPromiseActive:Boolean(uhdViewportPromise)});
            return;
        }
        const archiveId=String(destination.archiveId||'').trim();
        const uhdUrl=archiveId?(provider==='JWST'?`https://cdn.esawebb.org/archives/images/publicationjpg/${archiveId}.jpg`:`https://cdn.esahubble.org/archives/images/publicationjpg/${archiveId}.jpg`):String(destination.hdUrl||previewUrl).trim();
        const payload={
            provider:provider==='JWST'?'ESA JWST':'ESA HUBBLE',
            providerIcon,
            previewUrl,uhdUrl,officialUrl:sourceUrl,
            title:String(destination.commonName||destination.designation||destination.name||'GALAXY').trim()
        };
        viewport.open(payload);
        wireUhdProviderNavigation(sourceUrl);
        const viewportRoot=document.getElementById('gv-uhd-viewport-0001');
        if(!viewportRoot?.classList.contains('open'))console.error('GV-10AI VIEWPORT OPEN CALL COMPLETED WITHOUT .open ROOT STATE',{destination,payload});
    },true);
    ...
    function positionHdInfoPanel(){
        ...
        const archiveButtonOuterSize=44,archiveInset=14;
        set(hdArchiveButton,'left',`${Math.max(0,Math.round(overlayRect.width/2+viewportWidth/2-archiveInset-archiveButtonOuterSize))}px`);
        set(hdArchiveButton,'top',`${Math.max(0,Math.round(imageTop+imageHeight-archiveInset-archiveButtonOuterSize))}px`);
        set(hdArchiveButton,'right','auto');set(hdArchiveButton,'bottom','auto');
        ...
        if(hdDownloadButton)hdDownloadButton.style.pointerEvents='auto';
    ...
    function settleHdPresentation(){
        ...
        positionHdInfoPanel();
        const image=randomGalaxy.hdImage;
        if(image instanceof HTMLImageElement&&!image.complete)image.addEventListener('load',()=>{
            if(!isHdPresentationActive())return;
            applySmartHdCrop();
            positionHdInfoPanel();
            syncHdProviderPresentation();
    ...
    window.GV10AIDiagnostics=Object.freeze({
        release:'10AI',
        get coreViewerReady(){return coreViewerReady},
        get randomGalaxyReady(){return randomGalaxyReady},
        get uhdViewportReady(){return uhdViewportReady},
        get uhdViewportLoadError(){return uhdViewportLoadError?String(uhdViewportLoadError?.message||uhdViewportLoadError):''},
        get uhdModuleVersion(){return String(window.GVUHDViewport?.version||'')},
        get providerButtonMounted(){return Boolean(document.getElementById('gv-hd-archive-button'))},
        get providerButtonParent(){const parent=document.getElementById('gv-hd-archive-button')?.parentElement;return parent?.id||parent?.className||''},
        get activeProvider(){return providerFor(currentArchiveDestination())},
        get activeSourceUrl(){return String(currentArchiveDestination()?.sourceUrl||'')},
        get zoomButtonMounted(){return Boolean(document.getElementById('gv-hd-download-button'))}
    });
    ...
    window.addEventListener('beforeunload',()=>{
        ...
        if(providerLaunchTimer)clearTimeout(providerLaunchTimer);
        if(aladinPrewarmWaitResolve){const resolve=aladinPrewarmWaitResolve;aladinPrewarmWaitResolve=null;resolve(false)}
...
})().catch(error=>{console.error('GALAXY VIEWER 10AI STARTUP FAILURE:',error);document.dispatchEvent(new CustomEvent('gv-viewer-failed',{detail:{message:String(error?.stack||error)}}));});
"""))

# GV-beta-0010AI staged