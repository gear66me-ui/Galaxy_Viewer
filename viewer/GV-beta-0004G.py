...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){
            console.error("GV-beta-0004G STARTUP FAILURE: window.A was not created");
            return;
        }
        ...
        window.A.init.then(() => {
            ...
            function syncProxyBorder(coordinateBox,proxy){const style=window.getComputedStyle(coordinateBox);const borderColor=style.borderRightColor||"rgb(236,236,236)";const borderWidth=parseFloat(style.borderRightWidth)>0?style.borderRightWidth:"1px";const borderStyle=style.borderRightStyle!=="none"?style.borderRightStyle:"solid";const radius=style.borderTopRightRadius!=="0px"?style.borderTopRightRadius:"6px";[["border-style",borderStyle],["border-width",borderWidth],["border-color",borderColor],["border-radius",radius]].forEach(([property,value])=>proxy.style.setProperty(property,value,"important"));}
            ...
            function createProxy(){let proxy=getProxy();if(proxy)return proxy;proxy=document.createElement("button");proxy.type="button";proxy.className="gv-simbad-proxy gv-command gv-copy";proxy.setAttribute("title","SIMBAD pointer");proxy.setAttribute("aria-label","SIMBAD pointer");proxy.setAttribute("aria-pressed","false");proxy.style.setProperty("--command-filter",filters.copy,"important");proxy.innerHTML=`<svg viewBox="0 0 32 32" aria-hidden="true" focusable="false"><circle cx="16" cy="16" r="8.5"></circle><circle cx="16" cy="16" r="2.2"></circle><path d="M16 2.5V8"></path><path d="M16 24V29.5"></path><path d="M2.5 16H8"></path><path d="M24 16H29.5"></path></svg>`;bindProxy(proxy);return proxy;}
        ...
        }).catch(error=>console.error("GV-beta-0004G STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0004G STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
