...
# viewer-0027
# Standalone Galaxy Viewer release copied from frozen Viewer 26.
# Only change: clear Aladin's real `toggled` target state after TAP HERE TO CLEAR.
...
A.init.then(() => {
    ...
    function restoreTargetVisual(){
        const pointer=findNativeSimbadPointer();
        if(!pointer)return;
        pointer.classList.remove("toggled");
        pointer.setAttribute("aria-pressed","false");
        pointer.blur?.();
        pointer.querySelectorAll("*").forEach(element=>{
    ...
    function scheduleTargetVisualRestore(){
        [0,40,120,300].forEach(delay=>setTimeout(restoreTargetVisual,delay));
    }
