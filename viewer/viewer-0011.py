...
# viewer-0011
# Applies only the approved finder-size corrections to viewer-0007:
# 1. Keep the finder height matched to the coordinate box.
# 2. Increase the finder width slightly.
# 3. Scale the blue finder symbol to 90%.
# 4. Preserve the live coordinate-box border hue and the approved yellow helper text.
...
/* Keep the native blue finder symbol large but inside the matched box. */
...
if old_target_css not in source:
    raise RuntimeError("viewer-0011: target CSS anchor was not found")
if old_geometry not in source:
    raise RuntimeError("viewer-0011: target geometry anchor was not found")
if old_helper_color not in source:
    raise RuntimeError("viewer-0011: helper color anchor was not found")
if old_helper_shadow not in source:
    raise RuntimeError("viewer-0011: helper shadow anchor was not found")
...
source = source.replace("# viewer-0007", "# viewer-0011", 1)

exec(compile(source, "viewer-0011.py", "exec"))