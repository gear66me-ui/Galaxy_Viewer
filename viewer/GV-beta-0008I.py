...
# GV-beta-0008I
# Derived from exact GV-beta-0008H baseline blob 778b31bd51f19101ec5b235ef5e5db3a90e4bc8b.
# Authorized 8I-2 changes: static Milky Way start, compact WE ARE HERE overlay,
# RANDOM GALAXY 0006 readable card, and first navigation only on button press.
...
#gv-we-are-here{position:absolute;left:50%;top:58%;z-index:7090;transform:translate(-50%,-50%);max-width:min(360px,82vw);padding:7px 10px;border:1px solid rgba(98,216,255,.76);border-radius:5px;background:rgba(0,5,14,.78);color:#EAF9FF;font:400 9px/1.35 "Space Age",sans-serif;letter-spacing:.8px;text-align:center;text-transform:uppercase;text-shadow:0 0 7px rgba(98,216,255,.52);box-shadow:0 0 10px rgba(98,216,255,.22);pointer-events:none;transition:opacity .2s ease}
#gv-we-are-here strong{display:block;margin-bottom:2px;color:#62D8FF;font-size:11px;font-weight:400;letter-spacing:1.1px}
...
(async()=>{
    ...
    function createBottomControls(root){
        ...
        version.setAttribute('aria-label','GALAXY VIEWER VERSION 8I');
    ...
    function createHomeOverlay(root){
        ...
        overlay.innerHTML='<strong>WE ARE HERE →</strong>CLICK RANDOM GALAXY TO NAVIGATE';
    ...
    function readCurrentRaDec(aladin,root){
        ...
        }catch(error){console.warn('GV-8I GETRADEC WARNING',error)}
        try{
        ...
        }catch(error){console.warn('GV-8I PIX2WORLD WARNING',error)}
        return null;
    }
    ...
    const hamburger=window.GalaxyViewerHamburgerMenu.init({
        ...
        onProjectionSelected(name,detail){
            ...
            }catch(error){console.error('GV-8I PROJECTION FAILURE',name,detail?.code,error)}
        }
    });
    ...
    coordinate=window.GalaxyCoordinateOverlay.mount(coordinateHost,{onFrameChange(nextFrame){
        frame=nextFrame;
        try{if(typeof aladin.setFrame==='function')aladin.setFrame(frame==='GAL'?'galactic':'ICRSd')}catch(error){console.warn('GV-8I FRAME CHANGE WARNING',error)}
        renderCoordinates();
    }});
    ...
    await loadScript(RANDOM_GALAXY_URL,'gvRandomGalaxy0006');
    if(window.GalaxyRandomGalaxy?.VERSION!=='0006')throw new Error('RANDOM GALAXY MODULE 0006 EXPORT MISSING');
    const randomGalaxy=window.GalaxyRandomGalaxy.mount(randomGalaxyHost,{aladin,randomButton:bottom.random,bindClick:true,hubbleProvider:randomHubbleProvider,currentGalaxy:HOME,catalogCount:galaxyCatalog.length});
    await randomGalaxy.ready;
    bottom.random.addEventListener('click',()=>homeOverlay.classList.add('gv-hidden'),{once:true});

    window.GV8I=Object.freeze({version:VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,reticle,versionLabel:bottom.version,homeOverlay,catalogCount:galaxyCatalog.length});
    document.dispatchEvent(new CustomEvent('gv-viewer-ready',{detail:{version:VERSION,catalogCount:galaxyCatalog.length}}));
})().catch(error=>console.error('GALAXY VIEWER 8I STARTUP FAILURE:',error));
"""))

# GV-beta-0008I staged
