...
(async()=>{
    ...
    async function loadGalaxyCatalog(catalogUrl){
    ...
    async function loadCombinedGalaxyCatalog(){
        ...
        const [hubbleData,jwstData,chandraData]=await Promise.all([
            loadGalaxyCatalog(catalogUrls.hubble),
            ...
            if(url.startsWith('https://raw.githubusercontent.com/')){
                fetch(url,{cache:'no-store'}).then(response=>{
                    ...
                    if(datasetKey==='gvRandomGalaxy0031'){
                        ...
                        if(!source.includes("host === 'chandra.harvard.edu'")||!source.includes("/(hubble|webb|chandra)/i")||!source.includes("provider: cleanText(candidate.provider"))throw new Error('RANDOM GALAXY 0031 PROVIDER COMPATIBILITY PATCH FAILED');
                    }
...
function install(){
  ...
  installHdFeedback();
  if(core.versionLabel){core.versionLabel.textContent='VERSION ACTIVE';core.versionLabel.setAttribute('aria-label','GALAXY VIEWER VERSION ACTIVE')}
