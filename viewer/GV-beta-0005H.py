...
(() => {
    ...
    async function fetchCuratedRecords(){
        ...
        return Array.isArray(payload.targets)
            ? payload.targets.filter(target=>target&&target.name).map((target,index)=>({
                ...
                ra_deg:Number(target.ra_deg),
                dec_deg:Number(target.dec_deg),
                preferred_fov_deg:Number(target.fov)||0.18,
                morphology:String(target.object_type||"Galaxy"),
                source:"CURATED",
                source_id:"CURATED:"+String(index+1).padStart(4,"0"),
                spectacular:target.spectacular===true
            }))
            : [];
    }
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){console.error("GV-beta-0005H STARTUP FAILURE: window.A was not created");return;}
        ...
        window.A.init.then(async() => {
            ...
            try{
                ...
                console.error("GV-beta-0005H CATALOG FAILURE:",error);
            }
            ...
            versionLabel.textContent="Galaxy Viewer 5H";
            const versionPalette=[
                ["#FFD166","rgba(255,209,102,.58)"],
                ["#FF7B8B","rgba(255,123,139,.58)"],
                ["#55FF88","rgba(85,255,136,.52)"],
                ["#45E7FF","rgba(69,231,255,.56)"],
                ["#C98BFF","rgba(201,139,255,.58)"]
            ];
            ...
            async function openGalaxy(record){
                ...
                try{
                    ...
                    console.error("GV-beta-0005H TARGET FAILURE:",error);
                }finally{
                    navigating=false;
        ...
        }).catch(error=>console.error("GV-beta-0005H STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0005H STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
