...
(async()=>{
    'use strict';
    const VERSION='12AR-111';
    const DISPLAY_VERSION='12AR-111';
    ...
    const AVM_LAB_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/lab/gv-avm-overlay-lab-0027.js';
...
`;
    ...
    gvAvmTrace('VIEWER_AVM0027_LOAD_START',{url:AVM_LAB_URL});
    loadScriptGithubThenLocal(AVM_LAB_URL,'gvAvmOverlayLab0027').then(()=>{
        gvAvmTrace('VIEWER_AVM0027_LOAD_OK',{version:window.GalaxyViewerAvmOverlayLab?.VERSION||''});
        const lab=window.GalaxyViewerAvmOverlayLab;
        if(!lab){gvAvmTrace('VIEWER_AVM0026_EXPORT_MISSING',{});return}
        if(lab.VERSION!=='0027')gvAvmTrace('VIEWER_AVM0026_VERSION_MISMATCH',{version:String(lab.VERSION||'')});
        try{
            gvAvmTrace('VIEWER_AVM0027_INSTALL_START',{version:lab.VERSION});
            lab.install?.({A,aladin,viewerRoot:root,randomGalaxy});
            gvAvmTrace('VIEWER_AVM0027_INSTALL_OK',{version:lab.VERSION});
        }catch(error){
