...
# GV-beta-0005V
# Standalone Galaxy Viewer release based only on final GV-beta-0005R.
# Integrates the approved active Target hierarchy directly into the complete viewer.
# Preserves the four-decimal coordinate display, 1,000-record catalog, Spectacular Mode, centered reticle, safe FOV floor, navigation, artwork, compact controls, circular comet orbit, and #B8B1F0 color.
...
#aladin-cosmic-command-test button.gv-target-proxy[aria-pressed="true"] img{
...
#aladin-cosmic-command-test button.gv-target-proxy[aria-pressed="true"] .gv-target-comet{
    animation:none!important;opacity:1!important;transform:rotate(0deg) scale(.70)!important;transform-origin:0 0!important;
}
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){console.error("GV-beta-0005V STARTUP FAILURE: window.A was not created");return;}
        window.A.init.then(async() => {
            ...
            catch(error){console.error("GV-beta-0005V CATALOG FAILURE:",error);}
            ...
            centerReticle.src=reticleUrl+"?v=5V-standalone-001";
            centerReticle.alt="";
            ...
            function getTargetStatus(){
                ...
                status.textContent="Target active · Pan locked · Tap to exit";
                return status;
            }
            function createTargetProxy(){
                ...
                proxy.innerHTML=`<img src="${targetIconUrl}?v=5V-standalone-001" alt="" aria-hidden="true" draggable="false"><span class="gv-target-comet" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i></span>`;
            ...
            function bindTargetStatus(proxy,status){
                if(status.dataset.gv5vBound==="true")return;
                status.dataset.gv5vBound="true";
            ...
            versionLabel.textContent="Galaxy Viewer 5V";
            ...
            async function openGalaxy(record){
                ...
                }catch(error){nameLabel.textContent="Could not resolve "+String(record.primary_name||"galaxy");console.error("GV-beta-0005V TARGET FAILURE:",error);}
        ...
        }).catch(error=>console.error("GV-beta-0005V STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0005V STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
