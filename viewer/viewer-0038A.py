...
/* Hard lock: neither Aladin nor the coordinate box can resize this visible target. */
...
A.init.then(() => {
    ...
    function setClearReady(){
        ...
        // Viewer-0037-Patch H: return to PAN as soon as the SIMBAD result appears.
        try{aladin.fire("default")}catch(error){
            try{if(aladin.view)aladin.view.setMode(0)}catch(innerError){
                console.warn("Galaxy Viewer could not restore Aladin PAN mode after SIMBAD lookup.",innerError);
            }
        }
        simbadModeActive=false;
        const proxy=getProxy();
        proxy?.classList.remove("gv-active");
        proxy?.setAttribute("aria-pressed","false");
        root.querySelector(".gv-simbad-helper")?.classList.remove("gv-active");
        const helperText=root.querySelector(".gv-simbad-helper-text");
        if(helperText)helperText.innerHTML="SIMBAD result shown<br>navigation restored";

        status.innerHTML="SIMBAD result displayed<br>TAP HERE TO CLEAR";
        status.classList.add("gv-visible","gv-clear-ready");
        status.setAttribute("role","button");
        status.setAttribute("aria-label","Clear SIMBAD result");
        status.tabIndex=0;
    }
    ...
    function buildTargetRow(){
        ...
        syncProxyBorder(coordinateBox,proxy);
    ...
    const popupPoll=setInterval(()=>{
        if(!document.body.contains(root)){
            clearInterval(popupPoll);observer.disconnect();return;
        }
    ...
    // Viewer-0038A: retain SIMBAD target mode through finger movement.
    // A 20-pixel tolerance records ordinary touch wobble without canceling the lookup.
    // Native SIMBAD remains armed until a result appears or TARGET is pressed again.
    const GV_SIMBAD_TOUCH_TOLERANCE=20;
    let gvSimbadPointerActive=false;
    let gvSimbadPointerId=null;
    let gvSimbadTouchStartX=0;
    let gvSimbadTouchStartY=0;
    let gvSimbadMovedBeyondTolerance=false;
    ...
    function gvSimbadPointerDown(event){
        ...
        gvSimbadTouchStartX=event.clientX;
        gvSimbadTouchStartY=event.clientY;
        gvSimbadMovedBeyondTolerance=false;
    }
    ...
    function gvSimbadPointerMove(event){
        ...
        if(!simbadModeActive||resultReady)return;
        const dx=event.clientX-gvSimbadTouchStartX;
        const dy=event.clientY-gvSimbadTouchStartY;
        if(Math.hypot(dx,dy)>=GV_SIMBAD_TOUCH_TOLERANCE){
            gvSimbadMovedBeyondTolerance=true;
            // Deliberately do not leave SIMBAD mode. Viewer 37 canceled here.
        }
    }
    ...
    function gvSimbadPointerFinish(event){
        ...
        gvSimbadTouchStartX=0;
        gvSimbadTouchStartY=0;
        gvSimbadMovedBeyondTolerance=false;
    }

    function gvBindSimbadTouchTolerance(){
        const surface=gvGetInteractionSurface();
        if(!surface||surface.dataset.gvSimbadTouchToleranceBound)return;
        surface.dataset.gvSimbadTouchToleranceBound="true";
    ...
    gvBindSimbadTouchTolerance();
    [100,300,700,1400].forEach(delay=>setTimeout(gvBindSimbadTouchTolerance,delay));
    // End Viewer-0038A touch reliability change.
});
</script>
"""))