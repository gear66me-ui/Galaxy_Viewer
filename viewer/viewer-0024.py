...
# viewer-0024
# Standalone Galaxy Viewer release copied from the approved Viewer 23 behavior.
# Changes only the post-SIMBAD visual reset so the target returns to normal size.
...
A.init.then(() => {
    ...
    window.aladin_cosmic_command_test = A.aladin("#aladin-cosmic-command-test", {
    ...
    let programmaticPointerToggle = false;
    ...
    function findNativeSimbadPointer() {
        return root.querySelector(".aladin-simbadPointerControl") || root.querySelector("[class*='simbadPointer']");
    }
    function resetSimbadInterface(message) {
    ...
    function forceTargetVisualReset() {
        const nativePointer = findNativeSimbadPointer();
        const coordinateBox = findNativeCoordinateBox();
        if (!nativePointer) return;
        const activeClasses = ["active", "selected", "on", "enabled", "pressed", "aladin-active", "aladin-selected"];
        [nativePointer, ...nativePointer.querySelectorAll("*")].forEach(element => {
            activeClasses.forEach(className => element.classList.remove(className));
            element.removeAttribute("aria-pressed");
            element.removeAttribute("data-active");
        });
        nativePointer.style.setProperty("transform", "none", "important");
        [...nativePointer.children].forEach(child => {
            child.style.setProperty("transform", "scale(0.81)", "important");
            child.style.setProperty("transform-origin", "center center", "important");
        });
        if (coordinateBox) synchronizeTargetGeometry(coordinateBox, nativePointer);
    }
    function scheduleTargetVisualReset() {
        [0, 40, 120, 300].forEach(delay => setTimeout(forceTargetVisualReset, delay));
    }
    function deactivateSimbadMode(message) {
        const nativePointer = findNativeSimbadPointer();
        if (simbadModeActive && nativePointer) {
            programmaticPointerToggle = true;
            nativePointer.click();
            programmaticPointerToggle = false;
        }
        simbadModeActive = false;
        resetSimbadInterface(message);
        scheduleTargetVisualReset();
    }
    ...
    function ensureHelper(row, nativePointer) {
        ...
        if (!stack) {
            ...
            stack.innerHTML = `<div class="gv-simbad-helper" aria-live="polite"><svg class="gv-simbad-helper-arrow" viewBox="0 0 24 14" aria-hidden="true"><path d="M22 7H6"></path><path d="M11 2L5 7L11 12"></path></svg><span class="gv-simbad-helper-text">Click target, then click<br>a galaxy or star to find info</span></div><div class="gv-simbad-live-status" aria-live="polite"></div>`;
        }
        if (stack.parentElement !== row || nativePointer.nextElementSibling !== stack) nativePointer.insertAdjacentElement("afterend", stack);
        if (!nativePointer.dataset.gvHelperBound) {
            nativePointer.dataset.gvHelperBound = "true";
            nativePointer.addEventListener("click", () => {
                if (programmaticPointerToggle) return;
                if (simbadModeActive) {
                    simbadModeActive = false;
                    resetSimbadInterface("SIMBAD cancelled — navigation restored.");
                    scheduleTargetVisualReset();
                    return;
                }
                simbadModeActive = true;
                stack.querySelector(".gv-simbad-helper")?.classList.add("gv-active");
                const text = stack.querySelector(".gv-simbad-helper-text");
                const status = stack.querySelector(".gv-simbad-live-status");
                ...
                [40, 180, 600].forEach(delay => setTimeout(mirrorNativeFeedback, delay));
            });
        }
        return stack;
    }
    ...
    function synchronizeTargetGeometry(coordinateBox, nativePointer) {
        ...
        const candidates = [coordinateBox, ...coordinateBox.querySelectorAll("*")].map(element => {
            const rect = element.getBoundingClientRect();
            const style = window.getComputedStyle(element);
            const width = parseFloat(style.borderRightWidth) || 0;
            return {rect, style, visible: style.borderRightStyle !== "none" && width > 0 && style.borderRightColor !== "transparent" && style.borderRightColor !== "rgba(0, 0, 0, 0)", rightDistance: Math.abs(rect.right - coordinateRect.right), area: rect.width * rect.height};
        }).filter(item => item.visible && item.rect.width > 0 && item.rect.height > 0).sort((a, b) => a.rightDistance - b.rightDistance || b.area - a.area);
        const borderStyle = candidates.length ? candidates[0].style : window.getComputedStyle(coordinateBox);
        const borderColor = borderStyle.borderRightColor && borderStyle.borderRightColor !== "transparent" && borderStyle.borderRightColor !== "rgba(0, 0, 0, 0)" ? borderStyle.borderRightColor : "rgb(236, 236, 236)";
        const borderWidth = parseFloat(borderStyle.borderRightWidth) > 0 ? borderStyle.borderRightWidth : "1px";
        const borderLineStyle = borderStyle.borderRightStyle !== "none" ? borderStyle.borderRightStyle : "solid";
        const radius = borderStyle.borderTopRightRadius && borderStyle.borderTopRightRadius !== "0px" ? borderStyle.borderTopRightRadius : (borderStyle.borderTopLeftRadius && borderStyle.borderTopLeftRadius !== "0px" ? borderStyle.borderTopLeftRadius : "6px");
        [["width", boxWidth + "px"], ["min-width", boxWidth + "px"], ["max-width", boxWidth + "px"], ["height", height + "px"], ["min-height", height + "px"], ["max-height", height + "px"], ["border-style", borderLineStyle], ["border-width", borderWidth], ["border-color", borderColor], ["border-radius", radius]].forEach(([property, value]) => nativePointer.style.setProperty(property, value, "important"));
    }
    ...
    function moveNativeSimbadPointer() {
        ...
        if (!row) {
            ...
            const coordinateRect = coordinateBox.getBoundingClientRect();
            if (coordinateRect.width <= 0 || coordinateRect.height <= 0 || coordinateRect.left < rootRect.left || coordinateRect.top < rootRect.top || coordinateRect.right > rootRect.right) return false;
        ...
        if (nativePointer.parentElement !== row) row.appendChild(nativePointer);
    ...
    function isNativeSimbadFeedback(element) {
        ...
        const matches = ["simbad", "pointer", "click again", "click on an object", "click an object", "select an object", "found"].some(term => text.includes(term));
        if (!matches) return false;
        const rect = element.getBoundingClientRect();
        const rootRect = root.getBoundingClientRect();
        return rect.width > 0 && rect.height > 0 && rect.top >= rootRect.top + rootRect.height * 0.55;
    }
    function isInstructionalFeedback(text) {
        const lower = text.toLowerCase();
        return ["click again", "click on an object", "click an object", "select an object", "pointer mode", "active"].some(term => lower.includes(term));
    }
    function mirrorNativeFeedback() {
        const status = root.querySelector(".gv-simbad-live-status");
        if (!status) return;
        const candidates = [...root.querySelectorAll("div, span, p")].filter(isNativeSimbadFeedback).sort((a, b) => b.getBoundingClientRect().top - a.getBoundingClientRect().top);
        ...
        if (simbadModeActive && !isInstructionalFeedback(text) && !nativeMessage.dataset.gvSimbadAutoClosed) {
            nativeMessage.dataset.gvSimbadAutoClosed = "true";
            setTimeout(() => deactivateSimbadMode("SIMBAD complete — navigation restored."), 80);
        }
    }
    ...
    function applyPalette() {
        root.querySelectorAll("*").forEach(element => {
            ...
            if (text === "ICRS" || text === "ICRSd" || /^[-+]?\d+(\.\d+)?\s+[-+]?\d+(\.\d+)?$/.test(text) || /^\d+(\.\d+)?\s*°?\s*[x×]\s*\d+(\.\d+)?\s*°?$/.test(text)) element.classList.add("gv-standard-text");
        });
        root.querySelectorAll(".aladin-layersControl").forEach(element => mark(element, "gv-layers", "layers"));
        root.querySelectorAll(".aladin-gridControl, [class*='gridControl']").forEach(element => mark(element, "gv-world", "world"));
        root.querySelectorAll(".aladin-projectionControl, [class*='projection']").forEach(element => mark(element, "gv-projection", "projection"));
        root.querySelectorAll(".aladin-fullscreen, .aladin-fullscreenControl, [class*='fullscreen']").forEach(element => mark(element, "gv-fullscreen", "fullscreen"));
        root.querySelectorAll(".aladin-location, .aladin-coordinates, .aladin-frameChoice, .aladin-fov").forEach(element => element.classList.add("gv-standard-text"));
        moveNativeSimbadPointer();
        mirrorNativeFeedback();
    }

    [250, 700, 1400, 2400].forEach(delay => setTimeout(applyPalette, delay));
