...
(async()=>{
    ...
    archiveBack.append(archiveArrow,archiveTargetTile,archiveBackLabel);
    ...
    function renderHdInfoCandidate(words,count){
        ...
        if(hdBackToSky){
            ...
            const overlapTop=Math.max(0,buttonRect.top-bodyRect.top-4);
            const overlapBottom=Math.min(bodyRect.height,buttonRect.bottom-bodyRect.top+4);
            const apertureWidth=Math.max(0,Math.ceil(bodyRect.right-buttonRect.left)+4);
            const apertureHeight=Math.max(0,Math.ceil(overlapBottom-overlapTop));
            Object.assign(aperture.style,{
                float:'right',display:'block',width:`${apertureWidth}px`,height:`${apertureHeight}px`,
                marginTop:`${overlapTop}px`,marginLeft:'10px',
