...
# GV-beta-0005R
# Standalone Galaxy Viewer release based only on GV-beta-0005Q.
...
# Replaces the square-footprint comet layer with a true circular particle orbit: one sparkling head and seven fading tail particles spanning 105 degrees, approximately 29 percent of the orbit.
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){console.error("GV-beta-0005R STARTUP FAILURE: window.A was not created");return;}
        window.A.init.then(async() => {
            ...
            catch(error){console.error("GV-beta-0005R CATALOG FAILURE:",error);}
            ...
            centerReticle.src=reticleUrl+"?v=5R-true-circular-comet-001";
            centerReticle.alt="";
            ...
            function createTargetProxy(){
                ...
                proxy.innerHTML=`<img src="${targetIconUrl}?v=5R-true-circular-comet-001" alt="" aria-hidden="true" draggable="false"><span class="gv-target-comet" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i></span>`;
            ...
            versionLabel.textContent="Galaxy Viewer 5R";
            ...
            async function openGalaxy(record){
                ...
                }catch(error){nameLabel.textContent="Could not resolve "+String(record.primary_name||"galaxy");console.error("GV-beta-0005R TARGET FAILURE:",error);}
        ...
        }).catch(error=>console.error("GV-beta-0005R STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0005R STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
