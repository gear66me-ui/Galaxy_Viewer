...
# viewer-0029
# Standalone Galaxy Viewer release copied from Viewer 28.
# Uses one canonical Aladin SIMBAD button for movement, geometry, events, and reset.
...
#aladin-cosmic-command-test .gv-native-coordinate-target-row{
    ...
    margin:0!important; padding:0!important; width:max-content!important;
...
#aladin-cosmic-command-test button.gv-native-simbad-moved{
...
#aladin-cosmic-command-test button.gv-native-simbad-moved>*{
    transform:scale(.81)!important; transform-origin:center center!important;
}
#aladin-cosmic-command-test button.gv-target-normal,
#aladin-cosmic-command-test button.gv-target-normal:hover,
#aladin-cosmic-command-test button.gv-target-normal:focus,
#aladin-cosmic-command-test button.gv-target-normal:focus-visible,
#aladin-cosmic-command-test button.gv-target-normal:active{
...
#aladin-cosmic-command-test button.gv-target-normal svg,
#aladin-cosmic-command-test button.gv-target-normal img,
#aladin-cosmic-command-test button.gv-target-normal canvas,
#aladin-cosmic-command-test button.gv-target-normal .aladin-icon,
#aladin-cosmic-command-test button.gv-target-normal [class*="icon"]{
...
#aladin-cosmic-command-test .gv-simbad-helper{
    ...
    color:#FFD166!important; font-family:"Roboto Mono","DejaVu Sans Mono",Consolas,monospace!important;
    font-size:11px!important; font-weight:700!important; line-height:1.12!important;
    letter-spacing:.15px!important; width:238px!important; max-width:238px!important;
    white-space:normal!important; text-shadow:0 0 4px rgba(255,209,102,.45)!important;
...
#aladin-cosmic-command-test .gv-simbad-live-status.gv-clear-ready{
    ...
    user-select:none!important; border-color:#7DF4FF!important;
    box-shadow:0 0 8px rgba(125,244,255,.55)!important;
}
...
A.init.then(() => {
    ...
    function synchronizeTargetGeometry(coordinateBox,button){
        ...
        [["width",boxWidth+"px"],["min-width",boxWidth+"px"],["max-width",boxWidth+"px"],["height",height+"px"],["min-height",height+"px"],["max-height",height+"px"],["border-style",borderStyle],["border-width",borderWidth],["border-color",borderColor],["border-radius",radius]].forEach(([property,value])=>{
            if(button.style.getPropertyValue(property)!==value){
                button.style.setProperty(property,value,"important");
            }
        });
    }

    function restoreTargetVisual(){
        const button=getCanonicalSimbadButton();
        const coordinateBox=findNativeCoordinateBox();
        if(!button||!coordinateBox)return false;

        button.classList.remove("toggled");
        button.classList.add("gv-target-normal");
        button.setAttribute("aria-pressed","false");
        button.blur?.();

        button.querySelectorAll("*").forEach(element=>{
        ...
        button.querySelectorAll("svg,img,canvas,.aladin-icon,[class*='icon']").forEach(element=>{
            element.style.setProperty("max-width","100%","important");
            element.style.setProperty("max-height","100%","important");
        });

        synchronizeTargetGeometry(coordinateBox,button);
        return true;
    }

    function scheduleTargetVisualRestore(){
        [0,40,120,300,700,1200].forEach(delay=>setTimeout(restoreTargetVisual,delay));
    }
    ...
    function clearSimbad(){
        ...
        try{aladin.fire("default")}catch(error){
            getCanonicalSimbadButton()?.click();
        }
        simbadModeActive=false;
        resultReady=false;
        resetHelperAndStatus();
        scheduleTargetVisualRestore();
    }
    ...
    function ensureHelper(row,button){
        ...
        if(stack.parentElement!==row||button.nextElementSibling!==stack){
        ...
        if(!button.dataset.gvHelperBound){
            button.dataset.gvHelperBound="true";
            button.addEventListener("click",()=>{
                ...
                const text=stack.querySelector(".gv-simbad-helper-text");
                if(simbadModeActive){
                    button.classList.remove("gv-target-normal");
    ...
    function moveCanonicalSimbadButton(){
        const coordinateBox=findNativeCoordinateBox();
        const button=getCanonicalSimbadButton();
        if(!coordinateBox||!button)return false;
        ...
        button.classList.add("gv-native-simbad-moved","gv-command","gv-copy");
        button.style.setProperty("--command-filter",filters.copy,"important");
        synchronizeTargetGeometry(coordinateBox,button);
        ensureHelper(row,button);
        return coordinateBox.nextElementSibling===button;
    }
    ...
    function detectSimbadPopup(){
        ...
        const isSimbad=popup.querySelector(".aladin-sp-title,.aladin-sp-content")||text.toLowerCase().includes("query in cds portal")||text.toLowerCase().includes("no match was found");
