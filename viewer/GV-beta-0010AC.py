...
(async()=>{
    ...
    const loadArchiveFrameSource=sourceUrl=>{
        const previous=archiveFrame;
        const frame=document.createElement('iframe');
        frame.id='gv-archive-frame';
        frame.title='GALAXY ARCHIVE SOURCE';
        frame.style.pointerEvents='none';
        frame.addEventListener('load',()=>{
            if(frame!==archiveFrame||archiveSourceUrl!==sourceUrl||archiveClosing)return;
            archiveLoadedUrl=sourceUrl;
            revealArchiveWhenReady();
        },{once:true});
        frame.addEventListener('error',()=>{
            if(frame!==archiveFrame||archiveSourceUrl!==sourceUrl)return;
            ...
            if(requested)window.open(sourceUrl,'_blank','noopener,noreferrer');
        },{once:true});
        archiveOverlay.replaceChild(frame,previous);
        archiveFrame=frame;
        frame.src=sourceUrl;
    };
