...
# GV-beta-0005Q
# Standalone Galaxy Viewer release based only on GV-beta-0005P.
...
# Changes only the Target animation to a centered 32x32px circular orbit with a sparkling comet head and a tapered tail spanning approximately 30% of the orbit, fully contained inside the intact Target square.
...
#aladin-cosmic-command-test button.gv-target-proxy .gv-target-comet{
    position:absolute!important;inset:0!important;width:32px!important;height:32px!important;border-radius:50%!important;
    animation:gv-target-comet-orbit 2.8s linear infinite!important;pointer-events:none!important;z-index:3!important;
}
#aladin-cosmic-command-test button.gv-target-proxy .gv-target-comet::before{
    content:""!important;position:absolute!important;inset:1px!important;border-radius:50%!important;
    background:conic-gradient(from -90deg,transparent 0 70%,rgba(98,216,255,0) 70%,rgba(98,216,255,.14) 76%,rgba(98,216,255,.32) 83%,rgba(98,216,255,.58) 91%,rgba(255,255,255,.92) 100%)!important;
    -webkit-mask:radial-gradient(circle,transparent 0 11px,#000 11.5px 14px,transparent 14.5px)!important;
    mask:radial-gradient(circle,transparent 0 11px,#000 11.5px 14px,transparent 14.5px)!important;
    filter:drop-shadow(0 0 2px rgba(98,216,255,.72))!important;
}
#aladin-cosmic-command-test button.gv-target-proxy .gv-target-comet::after{
    content:""!important;position:absolute!important;left:50%!important;top:1px!important;width:4px!important;height:4px!important;border-radius:50%!important;
    transform:translateX(-50%)!important;background:#FFFFFF!important;
    box-shadow:0 0 3px 1px #FFFFFF,0 0 7px 2px #62D8FF,1px 1px 4px rgba(255,255,255,.8)!important;
}
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){console.error("GV-beta-0005Q STARTUP FAILURE: window.A was not created");return;}
        window.A.init.then(async() => {
            ...
            catch(error){console.error("GV-beta-0005Q CATALOG FAILURE:",error);}
            ...
            centerReticle.src=reticleUrl+"?v=5Q-circular-comet-001";
            centerReticle.alt="";
            ...
            function createTargetProxy(){
                ...
                proxy.innerHTML=`<img src="${targetIconUrl}?v=5Q-circular-comet-001" alt="" aria-hidden="true" draggable="false"><span class="gv-target-comet" aria-hidden="true"></span>`;
            ...
            versionLabel.textContent="Galaxy Viewer 5Q";
            ...
            async function openGalaxy(record){
                ...
                }catch(error){nameLabel.textContent="Could not resolve "+String(record.primary_name||"galaxy");console.error("GV-beta-0005Q TARGET FAILURE:",error);}
        ...
        }).catch(error=>console.error("GV-beta-0005Q STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0005Q STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
