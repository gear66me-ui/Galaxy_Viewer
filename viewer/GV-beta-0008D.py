...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/gv-random-galaxy-0001.js?v=a12a734e51de34671e685af1fc397353c70df93d';
    ...
    const HUBBLE_DESTINATIONS=Object.freeze([
        Object.freeze({
            ...
            distance:30,
            constellation:'Virgo',
            hdUrl:'https://cdn.esahubble.org/archives/images/large/opo0328a.jpg',
        ...
        Object.freeze({
            ...
            hdUrl:'https://cdn.esahubble.org/archives/images/large/heic0506a.jpg',
        ...
        Object.freeze({
            ...
            hdUrl:'https://cdn.esahubble.org/archives/images/large/opo0501a.jpg',
        ...
        Object.freeze({
            ...
            hdUrl:'https://cdn.esahubble.org/archives/images/large/potw2235a.jpg',
            sourceUrl:'https://esahubble.org/images/potw2235a/',
            imageType:'Observation',
            category:'Galaxies',
            telescope:'Hubble Space Telescope',
    ...
    function randomHubbleProvider({excludeName}={}){
        ...
        const available=HUBBLE_DESTINATIONS.filter(destination=>destination.name.toLowerCase()!==excluded);
        const pool=available.length?available:HUBBLE_DESTINATIONS;
    ...
    function createBottomControls(root){
        ...
        version.setAttribute('aria-label','GALAXY VIEWER VERSION 8D');
    ...
    function readCurrentRaDec(aladin,root){
        ...
        }catch(error){console.warn('GV-8D GETRADEC WARNING',error)}
        try{
        ...
        }catch(error){console.warn('GV-8D PIX2WORLD WARNING',error)}
        return null;
    }
    ...
    const hamburger=window.GalaxyViewerHamburgerMenu.init({
        ...
        onProjectionSelected(name,detail){
            ...
            }catch(error){console.error('GV-8D PROJECTION FAILURE',name,detail?.code,error)}
        }
    });
    ...
    coordinate=window.GalaxyCoordinateOverlay.mount(coordinateHost,{onFrameChange(nextFrame){
        frame=nextFrame;
        try{if(typeof aladin.setFrame==='function')aladin.setFrame(frame==='GAL'?'galactic':'ICRSd')}catch(error){console.warn('GV-8D FRAME CHANGE WARNING',error)}
        renderCoordinates();
    }});
    ...
    await loadScript(RANDOM_GALAXY_URL,'gvRandomGalaxy0001');
    if(window.GalaxyRandomGalaxy?.VERSION!=='0001')throw new Error('RANDOM GALAXY MODULE 0001 EXPORT MISSING');
    const randomGalaxy=window.GalaxyRandomGalaxy.mount(randomGalaxyHost,{aladin,randomButton:bottom.random,hubbleProvider:randomHubbleProvider});
    await randomGalaxy.ready;

    window.GV8D=Object.freeze({version:VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,reticle,versionLabel:bottom.version});
    document.dispatchEvent(new CustomEvent('gv-viewer-ready',{detail:{version:VERSION}}));
})().catch(error=>console.error('GALAXY VIEWER 8D STARTUP FAILURE:',error));
"""))

# GV-beta-0008D staged
