...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0050.js';
    ...
    const moduleLoads=Promise.all([
        ...
        loadScript(RANDOM_GALAXY_URL,'gvRandomGalaxy0050'),
    ...
    function normalizeCatalogGalaxy(candidate,index,catalogKey,catalogMeta={}){
        ...
        const fov=Number.isFinite(fieldDegrees)&&fieldDegrees>0
            ? fieldDegrees/ARRIVAL_OCCUPANCY.target
    ...
    async function loadCombinedGalaxyCatalog(){
        ...
        const attach=source=>{
            loadGalaxyCatalog(source).then(data=>{
                ...
                settled++;
                maybeReleaseStartup();
                if(startupResolved&&!backgroundWorkSuspended)
                    queueMicrotask(fillPrefetchQueue);
            });
        };
    ...
    if(
        window.GalaxyRandomGalaxy?.VERSION!=='0050' ||
        typeof window.GalaxyRandomGalaxy.bootstrapHomePresentation!=='function'
    )throw new Error('RANDOM GALAXY 0050 HOME BOOTSTRAP EXPORT MISSING');
    ...
    // ==================== RANDOM NAVIGATION ====================
    if(window.GalaxyRandomGalaxy?.VERSION!=='0050')throw new Error('RANDOM GALAXY 0050 EXPORT MISSING OR VERSION MISMATCH');
    if(typeof window.GalaxyRandomNavigationWindow!=='function')throw new Error('RANDOM GALAXY 0050 NAVIGATION WINDOW EXPORT MISSING');
...
function navigateHistory(direction){
    ...
    },getBackgroundWorkSuspended:()=>backgroundWorkSuspended,suspendBackgroundWork,resumeBackgroundWork,suspendArchivePreloads,resumeArchivePreloads,getGalaxyHistory:()=>{const state=randomNavigationWindow.getState();const items=[...state.history,state.current,...[...state.forwardHistory].reverse()].filter(Boolean);return {index:state.history.length,items:items.map(item=>({name:item.name,archiveId:item.archiveId,provider:String(item?.provider||'').trim()}))}}});
