...
# GV-beta-0009E
# Derived from exact GV-beta-0009D baseline blob a28ffe2b797fbf61a9a02716d133e1b52dc18405.
# Authorized 9E changes: module 0027 wiring plus current/in-flight Hubble preload priority and duplicate-HD-request prevention.
# All unrelated 9D behavior remains frozen, including ten-second HOME travel, eighteen-second later travel, and sequential ten-image prefetch.
...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/gv-random-galaxy-0027.js?v=292fb9e8a711d7542f32ec000139f1a24bb8f7ba';
    ...
    function startPrefetch(destination,priority=false){
        ...
        const promise=prepareHdDestination(destination,controller.signal).then(item=>{
            if(key===activeTargetKey&&!activePreparedItem){
                ...
                window.__gv9eRandomGalaxy?.setPreparedHdResource?.(key,item.objectUrl,item.sourceKind,item.image);
                return;
            }
    ...
    function beginTravelHud(destination){
        ...
        const state=window.GV9E?.randomGalaxy?.getState?.()||window.__gv9eRandomGalaxy?.getState?.()||{};
    ...
    function createBottomControls(root){
        ...
        version.setAttribute('aria-label','GALAXY VIEWER VERSION 9E');
    ...
    const hamburger=window.GalaxyViewerHamburgerMenu.init({
        ...
        onProjectionSelected(name,detail){
            ...
            }catch(error){console.error('GV-9E PROJECTION FAILURE',name,detail?.code,error)}
        }
    });
    ...
    coordinate=window.GalaxyCoordinateOverlay.mount(coordinateHost,{onFrameChange(nextFrame){
        frame=nextFrame;
        try{if(typeof aladin.setFrame==='function')aladin.setFrame(frame==='GAL'?'galactic':'ICRSd')}catch(error){console.warn('GV-9E FRAME CHANGE WARNING',error)}
        renderCoordinates();
    }});
    ...
    await loadScript(RANDOM_GALAXY_URL,'gvRandomGalaxy0027');
    if(window.GalaxyRandomGalaxy?.VERSION!=='0011')throw new Error('RANDOM GALAXY 0027 PATH / 0011 VERIFIED CORE EXPORT MISSING');
    ...
    function setHistoryControls(){
        const busy=navigationPending||Boolean(window.__gv9eRandomGalaxy?.getState?.().busy);
    ...
    function navigateHistory(index){
        ...
        randomGalaxy.travelToRandom().catch(error=>{
            forcedDestination=null;pendingHistoryIndex=null;navigationPending=false;endTravelHud();setHistoryControls();console.error('GV-9E HISTORY NAVIGATION FAILURE',error);
        });
    }
    ...
    const randomGalaxy=window.GalaxyRandomGalaxy.mount(randomGalaxyHost,{
        ...
        onError(error){
            navigationPending=false;pendingHistoryIndex=null;forcedDestination=null;endTravelHud();setHistoryControls();console.error('GV-9E RANDOM GALAXY FAILURE',error);
        }
    });
    window.__gv9eRandomGalaxy=randomGalaxy;
    ...
    const deferHdUntilPrepared=async event=>{
        ...
        try{
            ...
            console.error('GV-9E HUBBLE PREPARATION WAIT FAILURE',error);
            try{randomGalaxy.showHubbleHD()}catch(fallbackError){console.error('GV-9E HUBBLE FALLBACK FAILURE',fallbackError)}
        }
    };
    ...
    setHistoryControls();
    window.GV9E=Object.freeze({version:VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,historyBackButton:bottom.back,historyForwardButton:bottom.forward,reticle,versionLabel:bottom.version,homeOverlay,catalogCount:catalogRecordCount,eligibleCatalogCount:galaxyCatalog.length,getHubblePrefetchState,startHubblePrefetch:fillPrefetchQueue,getGalaxyHistory:()=>({index:galaxyHistoryIndex,items:galaxyHistory.map(item=>({name:item.name,archiveId:item.archiveId}))})});
...
})().catch(error=>console.error('GALAXY VIEWER 9E STARTUP FAILURE:',error));
"""))

# GV-beta-0009E staged