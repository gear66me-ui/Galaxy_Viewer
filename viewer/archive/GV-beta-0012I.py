...
# Galaxy Viewer 12I repair loader.
# Frozen source baseline remains viewer/archive/GV-beta-0012F.py.
# New revisions only: compact blue diagnostics, Random readiness repair,
# subdued Earth presentation, and explicit Random preparation feedback.

display(HTML("""
<style>html,body{background:#000}#gv-12i-loader{position:fixed;inset:0;background:#000;z-index:1}</style>
<div id="gv-12i-loader" aria-hidden="true"></div>
"""))
...
(async()=>{
  ...
  const SOURCE=RAW+'viewer/archive/GV-beta-0012F.py?v=12I-base';
  ...
  const extract=source=>{
    ...
    if(html.length!==1||js.length!==1)throw new Error('12I FROZEN 12F EXTRACTION FAILED');
    return [html[0][1],js[0][1]];
  };
  const once=(text,from,to,label)=>{
    const first=text.indexOf(from);
    if(first<0)throw new Error(`12I PATCH MISSING: ${label}`);
    if(text.indexOf(from,first+from.length)>=0)throw new Error(`12I PATCH NOT UNIQUE: ${label}`);
    return text.slice(0,first)+to+text.slice(first+from.length);
  };
  try{
    const source=await read(SOURCE);
    let [html,js]=extract(source);
    html=html.replace("version.textContent='VERSION 12F'","version.textContent='VERSION 12I'");
    js=once(js,"const DISPLAY_VERSION='12F';","const DISPLAY_VERSION='12I';",'DISPLAY VERSION');
    js=once(js,
...
`    const moduleLoads=Promise.all([
        ...
        loadScript(COORDINATE_URL,'gvCoordinate0004'),
        loadScript(TARGET_URL,'gvTarget0001'),
        loadScript(RANDOM_GALAXY_URL,'gvRandomGalaxy0040')
          .then(()=>loadScript(RANDOM_PATCH_0042_URL,'gvRandomGalaxy0042'))
          .then(()=>loadScript(RANDOM_PATCH_0043_URL,'gvRandomGalaxy0043'))
          .then(()=>loadScript(RANDOM_PATCH_0044_URL,'gvRandomGalaxy0044'))
          .then(()=>loadScript(RANDOM_PATCH_0045_URL,'gvRandomGalaxy0045')),
        loadScript(DIAGNOSTICS_URL,'gvDiagnostics0002'),
...
`    const waitForAuthoritativeRandomReady=async(timeoutMs=10000)=>{
    ...
    const showRandomPreparationMessage=(text='PREPARING RANDOM GALAXY')=>{
        let note=document.getElementById('gv-random-preparing-12i');
        if(!note){note=document.createElement('div');note.id='gv-random-preparing-12i';note.style.cssText='position:fixed;left:50%;bottom:62px;z-index:2147483000;transform:translateX(-50%);padding:5px 8px;border:1px solid #7CCBFF;border-radius:5px;background:linear-gradient(145deg,rgba(8,27,58,.96),rgba(11,49,119,.94));color:#9BE5FF;font:7px monospace;letter-spacing:.35px;pointer-events:none;white-space:nowrap';document.body.appendChild(note)}
        note.textContent=text;clearTimeout(note.__timer);note.__timer=setTimeout(()=>note.remove(),2200);
    };
    const launchRandomGalaxy=async()=>{
        if(navigationPending||randomGalaxy.getState().busy)return;
        showRandomPreparationMessage();
        const nextReady=await waitForAuthoritativeRandomReady();
        if(!nextReady){
            showRandomPreparationMessage('RANDOM GALAXY PREPARATION FAILED');
...
`,
    ...
    document.getElementById('gv-12i-loader')?.remove();
    document.body.insertAdjacentHTML('beforeend',html);
    const script=document.createElement('script');script.textContent=js;document.body.appendChild(script);
  }catch(error){
    document.getElementById('gv-12i-loader')?.remove();
    const failure=document.createElement('pre');failure.style.cssText='position:fixed;inset:0;z-index:2147483647;margin:0;padding:20px;overflow:auto;background:#000;color:#9BE5FF;font:11px monospace';failure.textContent='GALAXY VIEWER 12I FAILED TO LOAD\n\n'+String(error?.stack||error);document.body.appendChild(failure);
    throw error;
  }
})();
"""))
