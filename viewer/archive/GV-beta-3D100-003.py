...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0131.js';
    ...
    const RANDOM_GALAXY_VERSION='0131';
        ...
        try{
            ...
            for(const angle of [0,90,180,270]){
                ...
                Object.assign(line.style,{
                    ...
                    height:'27px',
                    background:'#FF3B3B',
                    boxShadow:'0 0 2px rgba(255,59,59,.55)',
  ...
  return marker;
        ...
        try{
            ...
            for(const tick of northGrid.querySelectorAll('.gv-reticle-tick-major')){
                ...
                tick.style.background=COSMIC_BLUE;
                tick.style.borderRadius='0';
                tick.style.boxShadow='0 0 3px rgba(70,150,255,.78)';
  ...
  whiteSpace:'nowrap',textAlign:'center'
        ...
        // distance text while readEarthBearing() supplies the already-approved
        // direction.  The original perimeter indicator is hidden visually below.
        const earthGroup=document.createElement('div');
        earthGroup.id='gv-earth-compass-group';
        Object.assign(earthGroup.style,{position:'absolute',inset:'0',display:'none',pointerEvents:'none'});

        const earthArrow=document.createElement('div');
        earthArrow.id='gv-earth-compass-arrow';
        Object.assign(earthArrow.style,{
  position:'absolute',width:'17px',height:'12px',
  transformOrigin:'50% 50%',
  filter:'drop-shadow(0 0 3px rgba(255,216,77,.95)) drop-shadow(0 0 7px rgba(255,216,77,.48))'
        });
        const earthArrowHead=document.createElement('div');
        Object.assign(earthArrowHead.style,{
  position:'absolute',left:'1px',top:'1.5px',width:'0',height:'0',
  borderTop:'4.5px solid transparent',borderBottom:'4.5px solid transparent',
  borderLeft:'14px solid #FFD84D'
        });
        earthArrow.appendChild(earthArrowHead);

        const earthIcon=document.createElement('div');
        earthIcon.id='gv-earth-compass-icon';
        earthIcon.textContent='🌎';
        Object.assign(earthIcon.style,{
  position:'absolute',transform:'translate(-50%,-50%)',font:'17px/1 sans-serif',
  filter:'drop-shadow(0 0 3px rgba(87,255,147,.92)) drop-shadow(0 0 7px rgba(77,255,143,.38))',
  whiteSpace:'nowrap'
        });

        const earthDistance=document.createElement('div');
        earthDistance.id='gv-earth-compass-distance';
        Object.assign(earthDistance.style,{
  position:'absolute',transform:'translate(-50%,-50%)',color:COSMIC_BLUE,
  font:'600 10.5px/12px "Space Age",sans-serif',letterSpacing:'.35px',
  fontVariantNumeric:'tabular-nums',whiteSpace:'nowrap',textAlign:'center',
  textShadow:'0 0 4px rgba(223,247,255,.88),0 0 8px rgba(88,191,255,.72)'
        });
        earthGroup.append(earthDistance,earthIcon,earthArrow);
        reticle.appendChild(earthGroup);
  ...
  lastNorthBearing:null
    ...
    function readEarthBearing(aladin,root){
        try{
  if(typeof aladin.world2pix!=='function')return null;
  const center=readCurrentRaDec(aladin,root);
  if(!center)return null;
  const ra1=Number(center[0]),dec1=Number(center[1]);
  const ra2=Number(HOME.ra),dec2=Number(HOME.dec);
  if(![ra1,dec1,ra2,dec2].every(Number.isFinite))return null;

  const d=Math.PI/180;
  const dec1r=dec1*d,dec2r=dec2*d;
  let dra=(ra2-ra1)*d;
  while(dra>Math.PI)dra-=Math.PI*2;
  while(dra<-Math.PI)dra+=Math.PI*2;

  const separationCos=
      Math.sin(dec1r)*Math.sin(dec2r)+
      Math.cos(dec1r)*Math.cos(dec2r)*Math.cos(dra);
  const separation=Math.acos(Math.max(-1,Math.min(1,separationCos)));
  if(separation<1e-10)return null;

  const pa=Math.atan2(
      Math.sin(dra)*Math.cos(dec2r),
  ...
  const delta=Math.max(1e-6,Math.min(.01,fov*.08));
  const p0=aladin.world2pix(ra1,dec1,'ICRSd');
  const pN=aladin.world2pix(ra1,Math.min(89.999999,dec1+delta),'ICRSd');
  const cosDec=Math.max(.02,Math.abs(Math.cos(dec1r)));
  const pE=aladin.world2pix((ra1+delta/cosDec+360)%360,dec1,'ICRSd');
  const x0=Number(p0?.[0]),y0=Number(p0?.[1]);
  let nx=Number(pN?.[0])-x0,ny=Number(pN?.[1])-y0;
  let ex=Number(pE?.[0])-x0,ey=Number(pE?.[1])-y0;
  const nl=Math.hypot(nx,ny),el=Math.hypot(ex,ey);
  if(![nx,ny,ex,ey,nl,el].every(Number.isFinite)||nl<1e-9||el<1e-9)return null;
  nx/=nl;ny/=nl;ex/=el;ey/=el;

  const dx=Math.cos(pa)*nx+Math.sin(pa)*ex;
  const dy=Math.cos(pa)*ny+Math.sin(pa)*ey;
  if(Math.hypot(dx,dy)<1e-9)return null;
  return (Math.atan2(dx,-dy)*180/Math.PI+360)%360;
        }catch(error){
  console.warn('GALAXY VIEWER EARTH POINTER WARNING',error);
  return null;
        }
    }

    const avm3dCleanViewportStyle=document.createElement('style');
    avm3dCleanViewportStyle.id='gv-avm3d-clean-viewport';
    avm3dCleanViewportStyle.textContent=`
#gv-north-rotor,#gv-center-target,#gv-sky-physical-scale,#gv-temp-black-box-export,#gv-live-aladin-rotation{display:none !important}
...
`;
    document.head.appendChild(avm3dCleanViewportStyle);
    ...
    const avmSliderWrap=document.createElement('div');
    avmSliderWrap.id='gv-avm-slider-wrap';
    avmSliderWrap.innerHTML='<input id="gv-avm-slider" aria-label="AVM IMAGE TRANSPARENCY" type="range" min="0" max="1" step="0.01" value="1">';
    root.appendChild(avmSliderWrap);
    const avmSlider=avmSliderWrap.querySelector('#gv-avm-slider');
    avmSlider.addEventListener('input',()=>{try{aladin.getOverlayImageLayer()?.setOpacity(Number(avmSlider.value))}catch(error){console.warn('AVM OPACITY WARNING',error)}});
    let avmToken=0;
    const showAvmProviderImage=destination=>{
        const token=++avmToken;
        avmSliderWrap.style.display='none';
        avmSlider.value='1';
        const imageUrl=String(destination?.hdUrl||'').trim();
        if(!imageUrl)return;
        try{
            const image=A.image(imageUrl,{
                name:`GV AVM ${destination?.name||''}`,
                successCallback:(ra,dec,fov)=>{
                    if(token!==avmToken)return false;
                    try{
                        if(Number.isFinite(Number(ra))&&Number.isFinite(Number(dec)))aladin.gotoRaDec(Number(ra),Number(dec));
                        if(Number.isFinite(Number(fov))&&Number(fov)>0)aladin.setFov(Number(fov));
                        aladin.getOverlayImageLayer()?.setOpacity(1);
                        avmSliderWrap.style.display='block';
                    }catch(error){console.warn('AVM ARRIVAL FRAMING WARNING',error)}
                    return true;
                }
            });
            aladin.setOverlayImageLayer(image);
        }catch(error){console.warn('AVM LOAD FAILED',destination?.name,error)}
    };
    ...
    // ==================== 12AR-71 EARTH COMPASS PRESENTATION ====================
    // The Random Galaxy controller remains the authoritative source for the
    // Earth-return distance/readout lifecycle.  AR51 gives the compass its own
    // presentation elements, so no two animation loops write the same left/top.
    // Earth bearing mathematics remains readEarthBearing() above.
    function readEarthBearingFromSource(root){
        try{
            const source=document.getElementById('gv-earth-return-indicator');
            if(!source?.classList.contains('gv-visible'))return null;

            const arrow=source.querySelector('.gv-earth-return-arrow');
            if(arrow){
                ...
                if(matrix){
                    ...
                    if(parts.length>=2&&Number.isFinite(parts[0])&&Number.isFinite(parts[1])){
                        return (Math.atan2(parts[1],parts[0])*180/Math.PI+360)%360;
                    }
                }
                const inline=String(arrow.style.transform||'').match(/rotate\(([-0-9.]+)deg\)/);
                if(inline&&Number.isFinite(Number(inline[1])))return (Number(inline[1])%360+360)%360;
            }
            ...
            console.warn('GALAXY VIEWER EARTH SOURCE MIRROR WARNING',error);
            return null;
        }
    }

    function syncEarthCompassMarker(bearing){
        const state=reticle.gvDirectional;
        if(!state||!Number.isFinite(bearing))return;
        const source=document.getElementById('gv-earth-return-indicator');
        const visible=Boolean(source?.classList.contains('gv-visible'));
        state.earthGroup.style.display=visible?'block':'none';
        if(!visible)return;

        const value=source.querySelector('.gv-earth-return-value')?.textContent?.trim()||'';
        const unit=source.querySelector('.gv-earth-return-unit')?.textContent?.trim()||'';
        state.earthDistance.textContent=[value,unit].filter(Boolean).join(' ');

        // Arrow points radially OUTWARD; its TIP is locked to the
        // inner edge of the 125 px compass ring.
        const radians=bearing*Math.PI/180;
        // AR70 radial order: CENTER -> DISTANCE -> EARTH -> ARROW -> RING.
        // Arrowhead tip is 6.5 px beyond the center of its 17 px container,
        // therefore its center sits at 125 - 6.5 = 118.5 px.
        const arrowTipOffset=6.5;
        const arrowRadius=RING_RADIUS-arrowTipOffset;
        // AR71 radial order, outward -> inward:
        // RING -> OUTWARD EARTH ARROW -> EARTH ICON -> DISTANCE.
        // All presentation geometry remains inside the 125 px ring.
        const iconRadius=91;
        const distanceRadius=54;
        const xFor=r=>state.center+Math.sin(radians)*r;
        const yFor=r=>state.center-Math.cos(radians)*r;

        state.earthArrow.style.left=`${xFor(arrowRadius)}px`;
        state.earthArrow.style.top=`${yFor(arrowRadius)}px`;
        // CSS triangle is born pointing right; bearing 0 is screen-up.
        state.earthArrow.style.transform=`translate(-50%,-50%) rotate(${bearing-90}deg)`;

        state.earthIcon.style.left=`${xFor(iconRadius)}px`;
        state.earthIcon.style.top=`${yFor(iconRadius)}px`;
        state.earthDistance.style.left=`${xFor(distanceRadius)}px`;
        state.earthDistance.style.top=`${yFor(distanceRadius)}px`;
    }

    const earthCompassSourceStyle=document.createElement('style');
    earthCompassSourceStyle.id='gv-earth-compass-source-12ar52';
    earthCompassSourceStyle.textContent=`
#gv-earth-return-indicator.gv-visible{opacity:0 !important;visibility:hidden !important}
`;
    document.head.appendChild(earthCompassSourceStyle);
    ...
    const updateDirectionalReticle=()=>{
        ...
        // AR71: Earth direction comes directly from live Aladin geometry.
        // Do not mirror the hidden legacy indicator transform and do not freeze
        // during navigation: world2pix makes the marker follow live rotation.
        const earthBearing=readEarthBearing(aladin,root);
        if(Number.isFinite(earthBearing)){
            syncEarthCompassMarker(earthBearing);
        }
    };
    ...
    const randomGalaxy=window.GalaxyRandomGalaxy.mount(randomGalaxyHost,{
        ...
        onArrival(destination){
            ...
            showAvmProviderImage(destination);
        },
        onError(error){
...
`;
    ...
    (function installTemporaryBlackBoxExporter(){
        ...
        host.id='gv-temp-black-box-export';
        const rotationReadout=document.createElement('div');
        rotationReadout.id='gv-live-aladin-rotation';
        rotationReadout.dataset.gvAr63IcrsdDiagnostic='catalog-vs-live-frame';
        rotationReadout.style.whiteSpace='pre-line';
        rotationReadout.style.lineHeight='1.20';
        rotationReadout.style.minHeight='48px';
        Object.assign(rotationReadout.style,{
  color:'#58BFFF',
  font:'700 11px/1.2 monospace',
  letterSpacing:'.4px',
  textAlign:'right',
  whiteSpace:'nowrap',
  textShadow:'0 0 4px rgba(88,191,255,.72)',
  pointerEvents:'none'
        });
        const gvRotationFmt=(value)=>{
            const n=Number(value);
            return Number.isFinite(n)?n.toFixed(2):'—';
        };
        const gvRotationDelta=(from,to)=>{
        ...
        const gvRotationTarget=()=>{
            const g=window.GalaxyRandomGalaxy||{};
            const candidates=[
                g.currentDestination,
                g.currentGalaxy,
                g.activeDestination,
                g.lastDestination,
                g.prefetchRuntime?.randomGalaxy?.currentDestination,
                g.prefetchRuntime?.randomGalaxy?.currentGalaxy
            ];
            for(const c of candidates){if(c&&typeof c==='object')return c;}
            return {};
        };
        const gvRotationLabel=(target)=>{
            const designation=String(target.designation||target.catalogDesignation||target.id||target.name||target.title||'CURRENT GALAXY').replace(/\s+/g,' ').trim();
            const name=String(target.pseudo||target.commonName||target.label||'').replace(/\s+/g,' ').trim();
            if(name&&designation&&name!==designation)return `${designation} / ${name}`.slice(0,78);
            return (designation||'CURRENT GALAXY').slice(0,78);
        };
        const gvFrameLabel=()=>{
            try{if(typeof aladin?.getFrame==='function')return String(aladin.getFrame());}catch(_){}
            try{
                const text=String(document.body?.innerText||'');
                const m=text.match(/\b(ICRSd|ICRS|J2000|GAL|Galactic)\b/i);
                return m?m[1].toUpperCase():'?';
            }catch(_){return '?';}
        };
        const updateLiveAladinRotation=()=>{
            try{
                const target=gvRotationTarget();
                const catalogValue=Number(target.aladinRotation??target.rotation??target.orientation??NaN);
                const cmdValue=Number(window.GalaxyViewerNavigation?.getLastCommandedRotation?.());
                const liveValue=Number(aladin?.getRotation?.());
                const catalogText=Number.isFinite(catalogValue)?gvRotationFmt(catalogValue):'—';
                const deltaCatLive=gvRotationDelta(catalogValue,liveValue);
                const deltaCmdLive=gvRotationDelta(cmdValue,liveValue);
                rotationReadout.textContent=`${gvRotationLabel(target)}  •  CATALOG ROT ${catalogText}`;
            }catch(_){
        ...
        button.type='button';
        button.textContent='EXPORT BLACK BOX';
        ...
        status.style.opacity='.48';
        status.style.pointerEvents='none';

        const download=document.createElement('a');
        download.textContent='DOWNLOAD BLACK BOX JSON';
        download.style.display='none';
        download.style.color='#7fd6ff';
        download.style.font='700 12px monospace';
        download.style.textDecoration='underline';
        download.style.opacity='.58';
        download.style.pointerEvents='auto';

        try{
            host.dataset.gvAr69Hotdog='1';
            Object.assign(host.style,{
                position:'absolute',
                left:'50%',
                right:'auto',
                top:'max(43px,calc(env(safe-area-inset-top) + 39px))',
                bottom:'auto',
                zIndex:'7600',
                ...
                boxSizing:'border-box',
                padding:'4px 12px',
                border:'1px solid rgba(255,65,65,.72)',
                borderRadius:'999px',
                background:'rgba(28,0,0,.62)',
                pointerEvents:'none'
            });
            Object.assign(rotationReadout.style,{
                color:'#FFD6D6',
                font:'700 9px/1.2 "Roboto Mono",Consolas,monospace',
                letterSpacing:'.35px',
                textAlign:'center',
                whiteSpace:'nowrap',
                minHeight:'0',
                lineHeight:'1.2',
                textShadow:'0 0 5px rgba(255,0,0,.55)'
            });
            button.style.display='none';
            status.style.display='none';
            download.style.display='none';
        }catch(error){
            console.warn('GALAXY VIEWER HOTDOG PRESENTATION WARNING',error);
        }
        host.append(rotationReadout,button,status,download);
        ...
        button.addEventListener('click',async()=>{
            ...
            try{
                ...
                const bb=window.GalaxyBlackBox;
                if(!bb?.openDb){
                    throw new Error('GalaxyBlackBox API not loaded in this app context');
                }
                ...
                const readAll=storeName=>new Promise((resolve,reject)=>{
                    ...
                    req.onerror=()=>reject(req.error||new Error(storeName+' READ FAILED'));
                ...
                const payload={
                    recoveryVersion:'in-app-0001',
                    exportedAt:new Date().toISOString(),
                    origin:window.location.origin,
                    blackBoxVersion:bb.version||null,
                ...
                if(download.dataset.objectUrl){
                    try{URL.revokeObjectURL(download.dataset.objectUrl)}catch(_){}
                }
                ...
                window.dispatchEvent(new CustomEvent(
                    'gv-black-box-checkpoint',
                    {detail:{
                        label:'BLACK_BOX_EXPORT_PREPARED',
                        detail:{
                            runCount:runs.length,
                            sampleCount:samples.length,
                ...
                console.error('BLACK BOX EXPORT FAILURE',error);
                status.textContent=
                    'EXPORT PREP FAILED\n'+
