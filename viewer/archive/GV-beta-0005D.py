...
# GV-beta-0005D
# Standalone filtered-catalog Galaxy Viewer release based only on GV-beta-0005C.
# Starts on a random catalog galaxy, preserves Back and Next Galaxy navigation,
# and displays a permanent boxed version label in the lower-left corner.
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){console.error("GV-beta-0005D STARTUP FAILURE: window.A was not created");return;}
        ...
        window.A.init.then(async() => {
            ...
            try{
                ...
                console.error("GV-beta-0005D CATALOG FAILURE:",error);
            }

            const firstIndex=records.length?Math.floor(Math.random()*records.length):-1;
            const firstRecord=firstIndex>=0?records[firstIndex]:null;
            const firstRa=Number(firstRecord?.ra_deg);
            const firstDec=Number(firstRecord?.dec_deg);
            const firstFov=Math.min(1,Math.max(0.04,Number(firstRecord?.preferred_fov_deg)||0.10));
            const initialTarget=Number.isFinite(firstRa)&&Number.isFinite(firstDec)?firstRa+" "+firstDec:"0 0";

            const aladin=A.aladin("#aladin-cosmic-command-test",{
                target:initialTarget,survey:"P/DSS2/color",fov:firstFov,cooFrame:"ICRSd",projection:"TAN",
            ...
            versionLabel.textContent="Galaxy Viewer 5D";
            ...
            if(firstRecord){
                history=[firstIndex];historyPosition=0;
                nextButton.disabled=false;
                openGalaxy(firstRecord);
            }else{
                nameLabel.textContent="Filtered galaxy catalog unavailable";
            }
        }).catch(error=>console.error("GV-beta-0005D STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0005D STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
