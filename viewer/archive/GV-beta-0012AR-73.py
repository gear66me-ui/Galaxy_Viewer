...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0132.js';
    ...
    const RANDOM_GALAXY_VERSION='0132';
  ...
  return marker;
        ...
        try{
            ...
            for(const tick of northGrid.querySelectorAll('.gv-reticle-tick-major')){
                ...
                tick.style.background=NORTH_RED;
                tick.style.borderRadius='0';
                tick.style.boxShadow='0 0 3px rgba(255,59,59,.78),0 0 7px rgba(255,59,59,.36)';
  ...
  font:'600 10.5px/12px "Space Age",sans-serif',letterSpacing:'1.4px',
  ...
  lastNorthBearing:null
    ...
    function createBottomControls(root){
        ...
        hud.innerHTML='<div id="gv-travel-distance"><span id="gv-travel-distance-value"><span id="gv-travel-distance-integer">0</span><span id="gv-travel-distance-decimal">.</span><span id="gv-travel-distance-fraction">00</span></span><span id="gv-travel-distance-unit">MILLION LIGHT-YEARS</span></div>';
  ...
  return null;
    ...
    // ==================== 12AR-73 EARTH COMPASS PRESENTATION ====================
    ...
    function syncEarthCompassMarker(bearing){
        ...
        const source=document.getElementById('gv-earth-return-indicator');
        const sourceVisible=Boolean(source?.classList.contains('gv-visible'));

        // AR73: display is driven by valid live geometry, not by the hidden legacy source.
        // The old source can be absent/hidden while the Aladin/world2pix bearing is valid.
        state.earthGroup.style.display='block';
        state.earthGroup.dataset.earthSourceVisible=sourceVisible?'1':'0';
        ...
        const distanceText=[value,unit].filter(Boolean).join(' ');
        state.earthDistance.textContent=distanceText||'EARTH';

        // Arrow points radially OUTWARD; its TIP is locked to the inner edge of the ring.
        ...
        const earthArrowTransform=`translate(-50%,-50%) rotate(${normalizedBearing-90}deg)`;
        ...
        state.earthArrow.style.transform=earthArrowTransform;
        state.earthArrow.style.display='block';

        state.earthIcon.style.left=`${xFor(iconRadius)}px`;
        state.earthIcon.style.top=`${yFor(iconRadius)}px`;
        state.earthIcon.style.display='block';

        state.earthDistance.style.left=`${xFor(distanceRadius)}px`;
        state.earthDistance.style.top=`${yFor(distanceRadius)}px`;
        state.earthDistance.style.display='block';

        state.earthGroup.dataset.earthBearing=normalizedBearing.toFixed(3);
        state.earthGroup.dataset.earthArrowTransform=earthArrowTransform;
        state.earthArrow.dataset.earthBearing=normalizedBearing.toFixed(3);
        state.earthArrow.dataset.earthTransform=earthArrowTransform;
    }
...
`;
    ...
    const updateDirectionalReticle=()=>{
        ...
        const sourceBearing=Number.isFinite(earthBearing)?null:readEarthBearingFromSource(root);
        const finalEarthBearing=Number.isFinite(earthBearing)?earthBearing:sourceBearing;
        if(Number.isFinite(finalEarthBearing)){
            syncEarthCompassMarker(finalEarthBearing);
        }else{
            const state=reticle.gvDirectional;
            if(state?.earthGroup)state.earthGroup.style.display='none';
        }
    };

    window.GalaxyViewerEarthPointerDebug=()=>{
        const state=reticle.gvDirectional||{};
        return {
            groupDisplay:state.earthGroup?.style?.display||null,
            sourceVisible:state.earthGroup?.dataset?.earthSourceVisible||null,
            bearing:state.earthGroup?.dataset?.earthBearing||null,
            arrowLeft:state.earthArrow?.style?.left||null,
            arrowTop:state.earthArrow?.style?.top||null,
            arrowTransform:state.earthArrow?.style?.transform||null,
            datasetTransform:state.earthArrow?.dataset?.earthTransform||null,
            distanceText:state.earthDistance?.textContent||null
        };
    };
...
`;
    ...
    (function installTemporaryBlackBoxExporter(){
        ...
        rotationReadout.dataset.gvAr73RotationReadout='live-numeric-only';
        Object.assign(rotationReadout.style,{
            color:'#7CCBFF',
            font:'400 22px/1 "Space Age",sans-serif',
            letterSpacing:'1.4px',
            textAlign:'center',
            whiteSpace:'nowrap',
            minWidth:'82px',
            ...
            textShadow:'0 0 3px rgba(255,255,255,.95),0 0 8px rgba(221,248,255,.85),0 0 14px rgba(88,191,255,.72)',
            pointerEvents:'none'
        });
        const updateLiveAladinRotation=()=>{
            try{
                const liveValue=Number(aladin?.getRotation?.());
                const shown=Number.isFinite(liveValue)?liveValue:0;
                const normalized=((shown%360)+360)%360;
                const digits=String(Math.round(normalized)%360).padStart(3,'0');
                rotationReadout.innerHTML='<span style="display:inline-block;width:3ch;text-align:right;font-variant-numeric:tabular-nums">'+digits+'</span><sup style="font-size:.58em;line-height:0;position:relative;top:-.48em;margin-left:.08em;color:#DDF8FF;text-shadow:0 0 3px rgba(255,255,255,.96),0 0 8px rgba(88,191,255,.82)">°</sup>';
            }catch(_){
                rotationReadout.innerHTML='<span style="display:inline-block;width:3ch;text-align:right;font-variant-numeric:tabular-nums">000</span><sup style="font-size:.58em;line-height:0;position:relative;top:-.48em;margin-left:.08em;color:#DDF8FF;text-shadow:0 0 3px rgba(255,255,255,.96),0 0 8px rgba(88,191,255,.82)">°</sup>';
            }
        };
        updateLiveAladinRotation();
        setInterval(updateLiveAladinRotation,250);
        ...
        try{
            ...
            Object.assign(host.style,{
                ...
                boxSizing:'border-box',
                padding:'4px 12px',
                border:'1px solid rgba(124,203,255,.72)',
                borderRadius:'999px',
                background:'rgba(8,27,58,.62)',
                pointerEvents:'none'
            });
            Object.assign(rotationReadout.style,{
                color:'#7CCBFF',
