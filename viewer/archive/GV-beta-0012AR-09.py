...
(async()=>{
    ...
    const moduleLoads=Promise.all([
        ...
        loadScriptGithubThenLocal(RANDOM_GALAXY_URL,'gvRandomGalaxy0090'),
    ...
    if(
        window.GalaxyRandomGalaxy?.VERSION!=='0090' ||
        typeof window.GalaxyRandomGalaxy.bootstrapHomePresentation!=='function'
    )throw new Error('RANDOM GALAXY 0090 HOME BOOTSTRAP EXPORT MISSING');
    ...
    // Navigation 0003 is loaded globally; Random Galaxy 0090 owns the planner instance.

    // ==================== RANDOM NAVIGATION ====================
    if(window.GalaxyRandomGalaxy?.VERSION!=='0090')throw new Error('RANDOM GALAXY 0090 EXPORT MISSING OR VERSION MISMATCH');
