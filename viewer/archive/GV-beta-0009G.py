...
# GV-beta-0009G
# Derived from exact GV-beta-0009F baseline blob bf217502e99ab89a9a24e026000da0cac7b76872.
# Authorized 9G change: move only the initial observable-universe context banner and leader down so the arrow terminates at the center oval.
# All unrelated 9F behavior remains frozen, including first-trip choreography, Hubble preload priority, ten-second HOME travel, eighteen-second later travel, and sequential ten-image prefetch.
...
#gv-version-label{display:none}
#gv-universe-context{position:absolute;left:50%;top:calc(50% - 130px);z-index:7095;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;width:min(310px,76vw);pointer-events:none;transition:opacity .2s ease}
...
#gv-universe-context .gv-universe-leader{position:relative;width:1px;height:88px;background:rgba(120,255,171,.86);box-shadow:0 0 7px rgba(87,255,147,.48)}
...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/gv-random-galaxy-0028.js?v=625b8d889f67652fa19a39b8689e306ea1c1fb91';
    ...
    function startPrefetch(destination,priority=false){
        ...
        const promise=prepareHdDestination(destination,controller.signal).then(item=>{
            if(key===activeTargetKey&&!activePreparedItem){
                ...
                window.__gv9gRandomGalaxy?.setPreparedHdResource?.(key,item.objectUrl,item.sourceKind,item.image);
                return;
            }
    ...
    function fillPrefetchQueue(){
        ...
        if(candidate)startPrefetch(candidate);
    }
    ...
    function beginTravelHud(destination){
        ...
        const state=window.GV9G?.randomGalaxy?.getState?.()||window.__gv9gRandomGalaxy?.getState?.()||{};
    ...
    function randomHubbleProvider({excludeName}={}){
        ...
        if(forcedDestination){
            ...
            if(!destination){
                ...
                if(inFlight){
                    destination=setUnpreparedActive(inFlight);
                }else{
    ...
    function createBottomControls(root){
        ...
        version.setAttribute('aria-label','GALAXY VIEWER VERSION 9G');
    ...
    const hamburger=window.GalaxyViewerHamburgerMenu.init({
        ...
        onProjectionSelected(name,detail){
            ...
            }catch(error){console.error('GV-9G PROJECTION FAILURE',name,detail?.code,error)}
        }
    });
    ...
    coordinate=window.GalaxyCoordinateOverlay.mount(coordinateHost,{onFrameChange(nextFrame){
        frame=nextFrame;
        try{if(typeof aladin.setFrame==='function')aladin.setFrame(frame==='GAL'?'galactic':'ICRSd')}catch(error){console.warn('GV-9G FRAME CHANGE WARNING',error)}
        renderCoordinates();
    }});
    ...
    await loadScript(RANDOM_GALAXY_URL,'gvRandomGalaxy0028');
    if(window.GalaxyRandomGalaxy?.VERSION!=='0011')throw new Error('RANDOM GALAXY 0028 PATH / 0011 VERIFIED CORE EXPORT MISSING');
    ...
    function setHistoryControls(){
        const busy=navigationPending||Boolean(window.__gv9gRandomGalaxy?.getState?.().busy);
    ...
    function navigateHistory(index){
        ...
        randomGalaxy.travelToRandom().catch(error=>{
            forcedDestination=null;pendingHistoryIndex=null;navigationPending=false;endTravelHud();setHistoryControls();console.error('GV-9G HISTORY NAVIGATION FAILURE',error);
        });
    }
    ...
    const randomGalaxy=window.GalaxyRandomGalaxy.mount(randomGalaxyHost,{
        ...
        onError(error){
            navigationPending=false;pendingHistoryIndex=null;forcedDestination=null;endTravelHud();setHistoryControls();console.error('GV-9G RANDOM GALAXY FAILURE',error);
        }
    });
    window.__gv9gRandomGalaxy=randomGalaxy;
    ...
    const deferHdUntilPrepared=async event=>{
        ...
        try{
            ...
            console.error('GV-9G HUBBLE PREPARATION WAIT FAILURE',error);
            try{randomGalaxy.showHubbleHD()}catch(fallbackError){console.error('GV-9G HUBBLE FALLBACK FAILURE',fallbackError)}
        }
    };
    ...
    setHistoryControls();
    window.GV9G=Object.freeze({version:VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,historyBackButton:bottom.back,historyForwardButton:bottom.forward,reticle,versionLabel:bottom.version,universeContext,homeOverlay,catalogCount:catalogRecordCount,eligibleCatalogCount:galaxyCatalog.length,getHubblePrefetchState,startHubblePrefetch:fillPrefetchQueue,getGalaxyHistory:()=>({index:galaxyHistoryIndex,items:galaxyHistory.map(item=>({name:item.name,archiveId:item.archiveId}))})});
...
})().catch(error=>console.error('GALAXY VIEWER 9G STARTUP FAILURE:',error));
"""))

# GV-beta-0009G staged