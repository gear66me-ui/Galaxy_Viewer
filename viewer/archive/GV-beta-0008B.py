...
#gv-center-reticle svg{display:block;width:32px;height:32px;overflow:visible;filter:drop-shadow(0 0 2px rgba(143,234,255,.28))}
...
(async()=>{
    ...
    function createCenterReticle(root){
        ...
        reticle.innerHTML='<svg viewBox="0 0 64 64" aria-hidden="true"><ellipse cx="32" cy="32" rx="21" ry="13" fill="none" stroke="#8feaff" stroke-width="1.8"/><ellipse cx="32" cy="32" rx="13" ry="21" fill="none" stroke="#8feaff" stroke-width="1.8"/><path d="M32 7v8M32 49v8M7 32h8M49 32h8" fill="none" stroke="#4fa6ff" stroke-width="1.5" stroke-linecap="round"/><circle cx="32" cy="32" r="2.5" fill="#9d7cff"/></svg>';
    ...
    function createBottomControls(root){
        ...
        version.setAttribute('aria-label','GALAXY VIEWER VERSION 8B');
    ...
    function readCurrentRaDec(aladin,root){
        ...
        }catch(error){console.warn('GV-8B GETRADEC WARNING',error)}
        try{
        ...
        }catch(error){console.warn('GV-8B PIX2WORLD WARNING',error)}
        return null;
    }
    ...
    const hamburger=window.GalaxyViewerHamburgerMenu.init({
        ...
        onProjectionSelected(name,detail){
            ...
            }catch(error){console.error('GV-8B PROJECTION FAILURE',name,detail?.code,error)}
        }
    });
    ...
    coordinate=window.GalaxyCoordinateOverlay.mount(coordinateHost,{onFrameChange(nextFrame){
        frame=nextFrame;
        try{if(typeof aladin.setFrame==='function')aladin.setFrame(frame==='GAL'?'galactic':'ICRSd')}catch(error){console.warn('GV-8B FRAME CHANGE WARNING',error)}
        renderCoordinates();
    }});
    ...
    window.GV8B=Object.freeze({version:VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,reticle,versionLabel:bottom.version});
    document.dispatchEvent(new CustomEvent('gv-viewer-ready',{detail:{version:VERSION}}));
})().catch(error=>console.error('GALAXY VIEWER 8B STARTUP FAILURE:',error));
"""))

# GV-beta-0008B staged
