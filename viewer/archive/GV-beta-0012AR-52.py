...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0116.js';
    ...
    const RANDOM_GALAXY_VERSION='0116';
    ...
    function createCenterReticle(root){
        ...
        // AR52: the graduated compass face is one rigid rotating assembly.
        // Its rotation is driven by the SAME north bearing used by the red
        // North pointer and Space Age N, so the three can never drift apart.
  ...
  return marker;
        ...
        northPointer.style.background=NORTH_RED;
        northPointer.style.filter='drop-shadow(0 0 3px rgba(255,77,90,.92)) drop-shadow(0 0 7px rgba(255,77,90,.42))';
  ...
  transform:'translate(-50%,-50%) rotate(0deg)',color:NORTH_RED,
  font:'400 15px/1 "Space Age",sans-serif',letterSpacing:'1px',
  textShadow:'0 0 3px rgba(255,230,232,.92),0 0 7px rgba(255,77,90,.72)',
  whiteSpace:'nowrap',textAlign:'center'
        });
        reticle.appendChild(northLabel);
  ...
  transformOrigin:'50% 50%',
  filter:'drop-shadow(0 0 3px rgba(87,255,147,.90)) drop-shadow(0 0 7px rgba(77,255,143,.42))'
        });
  ...
  if(Number.isFinite(ra)&&Number.isFinite(dec)&&typeof aladin.world2pix==='function'){
      const p0=aladin.world2pix(ra,dec,'ICRSd');
      const x0=Number(p0?.[0]),y0=Number(p0?.[1]);
      if(Number.isFinite(x0)&&Number.isFinite(y0)){
          const poleRoom=Math.max(0,90-dec);
          for(const requestedDelta of [.05,.2,1]){
              const delta=Math.min(requestedDelta,poleRoom*.25);
              if(!(delta>1e-6))continue;
              const pN=aladin.world2pix(ra,dec+delta,'ICRSd');
              const xN=Number(pN?.[0]),yN=Number(pN?.[1]);
  ...
  const rotation=Number(aladin.getRotation?.());
  if(Number.isFinite(rotation))return (rotation%360+360)%360;
        }catch(error){
  console.warn('GALAXY VIEWER NORTH POINTER WARNING',error);
        }
        return null;
    }
  ...
  return null;
    ...
    function setNorthMarker(bearing){
        ...
        state.lastNorthBearing=bearing;
        // AR52 mechanical lock: grid, red pointer, and N all receive the
        // exact same north bearing on the same animation-frame update.
        state.northGrid.style.transform=`rotate(${bearing}deg)`;
        placeDirectionalElement(state.northPointer,bearing,state.northPointerRadius,0);
        const radians=bearing*Math.PI/180;
        state.northLabel.style.left=`${state.center+Math.sin(radians)*state.northLabelRadius}px`;
        state.northLabel.style.top=`${state.center-Math.cos(radians)*state.northLabelRadius}px`;
        state.northLabel.style.transform=`translate(-50%,-50%) rotate(${bearing}deg)`;
        state.northLabel.style.display='block';
    }

    // ==================== 12AR-52 EARTH COMPASS PRESENTATION ====================
    ...
    function syncEarthCompassMarker(bearing){
        ...
        const radians=bearing*Math.PI/180;
        const arrowRadius=105;
        const iconRadius=84;
        const distanceRadius=58;
...
`;
    ...
    const updateDirectionalReticle=()=>{
        ...
        // Earth bearing algorithm is unchanged; only the dedicated compass presentation moves.
        const earthBearing=readEarthBearing(aladin,root);
        if(Number.isFinite(earthBearing)){
            syncEarthCompassMarker(earthBearing);
        }
    };
    let directionalReticleFrame=0;
    const directionalReticleTick=()=>{
        updateDirectionalReticle();
        directionalReticleFrame=requestAnimationFrame(directionalReticleTick);
    };
    directionalReticleFrame=requestAnimationFrame(directionalReticleTick);
    window.addEventListener('beforeunload',()=>cancelAnimationFrame(directionalReticleFrame),{once:true});
