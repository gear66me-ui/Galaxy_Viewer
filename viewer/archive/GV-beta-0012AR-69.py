...
body.gv-hd-open #gv-center-reticle{display:none!important}
...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0129.js';
    ...
    const RANDOM_GALAXY_VERSION='0129';
        ...
        try{
            ...
            for(const angle of [0,90,180,270]){
                ...
                Object.assign(line.style,{
                    ...
                    top:`${CENTER}px`,
                    width:'5px',
                    height:`${Math.max(44,RADIUS-54)}px`,
                    borderRadius:'999px',
                    background:'rgba(255,0,0,.42)',
                    boxShadow:'0 0 5px rgba(255,0,0,.55),0 0 12px rgba(255,0,0,.25)',
  ...
  return marker;
        ...
        try{
            northGrid.dataset.gvAr69MirrorTicks='1';
            for(const tick of northGrid.querySelectorAll('.gv-reticle-tick-major')){
                tick.style.width='4px';
                tick.style.height='28px';
                tick.style.background='rgba(255,0,0,.96)';
                tick.style.borderRadius='999px';
                tick.style.boxShadow='0 0 4px rgba(255,0,0,.95),0 0 10px rgba(255,0,0,.48)';
                const match=String(tick.style.transform||'').match(/rotate\(([-0-9.]+)deg\)/);
                const angle=match?Number(match[1]):0;
                tick.style.transform=`translate(-50%,-50%) rotate(${Number.isFinite(angle)?angle:0}deg) translateY(${-RADIUS}px)`;
  ...
  transform:'translate(-50%,-50%) rotate(0deg)',color:COSMIC_BLUE,
  ...
  return null;
    ...
    // ==================== 12AR-69 EARTH COMPASS PRESENTATION ====================
    ...
    function syncEarthCompassMarker(bearing){
        ...
        // Arrow points radially OUTWARD.  Its center is 105 px from compass
        // center; with a 14 px arrowhead/body this leaves visible breathing room
        // inside the 125 px ring rather than touching the circumference.
        const radians=bearing*Math.PI/180;
        const arrowRadius=118;
        const iconRadius=94;
        const distanceRadius=66;
        ...
        state.earthArrow.style.transform=`translate(-50%,-50%) rotate(${bearing}deg)`;
...
`;
    ...
    const updateDirectionalReticle=()=>{
        ...
        if(window.GalaxyRandomGalaxy?.isNavigationPending?.())return;

        const earthBearing=readEarthBearingFromSource(root);
