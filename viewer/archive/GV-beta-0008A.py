...
(async()=>{
    ...
    function createBottomControls(root){
        ...
        version.setAttribute('aria-label','GALAXY VIEWER VERSION 8A');
        ...
        random.addEventListener('click',event=>{
            ...
            random.dispatchEvent(new CustomEvent('gv-random-galaxy-request',{bubbles:true,detail:{version:VERSION}}));
        });
    ...
    function readCurrentRaDec(aladin,root){
        ...
        }catch(error){console.warn('GV-8A GETRADEC WARNING',error)}
        try{
        ...
        }catch(error){console.warn('GV-8A PIX2WORLD WARNING',error)}
        return null;
    }
    ...
    const hamburger=window.GalaxyViewerHamburgerMenu.init({
        ...
        onProjectionSelected(name,detail){
            ...
            }catch(error){console.error('GV-8A PROJECTION FAILURE',name,detail?.code,error)}
        }
    });
    ...
    coordinate=window.GalaxyCoordinateOverlay.mount(coordinateHost,{onFrameChange(nextFrame){
        frame=nextFrame;
        try{if(typeof aladin.setFrame==='function')aladin.setFrame(frame==='GAL'?'galactic':'ICRSd')}catch(error){console.warn('GV-8A FRAME CHANGE WARNING',error)}
        renderCoordinates();
    }});
    ...
    window.GV8A=Object.freeze({version:VERSION,aladin,hamburger,coordinate,target,randomGalaxyButton:bottom.random,versionLabel:bottom.version});
    document.dispatchEvent(new CustomEvent('gv-viewer-ready',{detail:{version:VERSION}}));
})().catch(error=>console.error('GALAXY VIEWER 8A STARTUP FAILURE:',error));
"""))

# GV-beta-0008A staged
