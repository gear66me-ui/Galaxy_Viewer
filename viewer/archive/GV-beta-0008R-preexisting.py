...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/gv-random-galaxy-0015.js?v=16469b855c111c3e7e8684b02880bb5f0265407e';
    ...
    function startPrefetch(destination){
        ...
        const promise=prepareHdDestination(destination).then(item=>{
            if(key===activeTargetKey&&!activePreparedItem){
                ...
                window.__gv8rRandomGalaxy?.setPreparedHdResource?.(key,item.objectUrl,item.sourceKind,item.image);
                return;
            }
    ...
    function beginTravelHud(destination){
        ...
        const state=window.GV8R?.randomGalaxy?.getState?.()||window.__gv8rRandomGalaxy?.getState?.()||{};
    ...
    function createBottomControls(root){
        ...
        version.setAttribute('aria-label','GALAXY VIEWER VERSION 8R');
    ...
    await loadScript(RANDOM_GALAXY_URL,'gvRandomGalaxy0015');
    if(window.GalaxyRandomGalaxy?.VERSION!=='0011')throw new Error('RANDOM GALAXY 0015 PATH / 0011 VERIFIED CORE EXPORT MISSING');
    ...
    function setHistoryControls(){
        const busy=navigationPending||Boolean(window.__gv8rRandomGalaxy?.getState?.().busy);
    ...
    window.__gv8rRandomGalaxy=randomGalaxy;
    ...
    setHistoryControls();
    window.GV8R=Object.freeze({version:VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,historyBackButton:bottom.back,historyForwardButton:bottom.forward,reticle,versionLabel:bottom.version,homeOverlay,catalogCount:catalogRecordCount,eligibleCatalogCount:galaxyCatalog.length,getHubblePrefetchState,getGalaxyHistory:()=>({index:galaxyHistoryIndex,items:galaxyHistory.map(item=>({name:item.name,archiveId:item.archiveId}))})});
