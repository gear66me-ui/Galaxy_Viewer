...
# GV 12AR-26 — ICRSd restoration / rotation-frame fix.
# Baseline: GV-beta-0012AR-25.py
...
BASE_URL = 'https://raw.githubusercontent.com/gear66me-ui/Galaxy_Viewer/beta/viewer/GV-beta-0012AR-25.py'
...
def replace_once(old, new, label):
    ...
    if count != 1:
        raise RuntimeError(f'12AR-26 PATCH DRIFT [{label}] expected 1 match, found {count}')
    source = source.replace(old, new, 1)

# Roll visible/internal viewer revision only; all AR-25 functionality remains.
source = source.replace('12AR-25', '12AR-26')
...
if "cooFrame:'galactic'" in source:
    raise RuntimeError('12AR-26 ICRSD GUARD FAILED: Galactic Aladin cooFrame remains')
if "let frame='GAL'" in source:
    raise RuntimeError('12AR-26 ICRSD GUARD FAILED: Galactic coordinate default remains')

exec(compile(source, 'GV-beta-0012AR-26.py::<12AR-25-rollup>', 'exec'), globals(), globals())
