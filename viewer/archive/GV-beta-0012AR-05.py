...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0089.js';
    const NAVIGATION_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/navigation/gv-navigation-0001.js';
    ...
    const moduleLoads=Promise.all([
        ...
        loadScript(NAVIGATION_URL,'gvNavigation0001'),
        loadScript(NAVIGATION_ADMIN_URL,'gvNavigationAdmin0001'),
        loadScriptGithubThenLocal(RANDOM_GALAXY_URL,'gvRandomGalaxy0089'),
    ...
    if(
        window.GalaxyRandomGalaxy?.VERSION!=='0089' ||
        typeof window.GalaxyRandomGalaxy.bootstrapHomePresentation!=='function'
    )throw new Error('RANDOM GALAXY 0089 HOME BOOTSTRAP EXPORT MISSING');
    ...
    // ==================== NAVIGATION ENGINE ====================
    if(window.GalaxyViewerNavigation?.VERSION!=='0001')
        throw new Error('NAVIGATION MODULE 0001 EXPORT MISSING OR VERSION MISMATCH');
    ...
    const navigationPlanner=window.GalaxyViewerNavigation.create({
        catalog:galaxyCatalog
    });
    await navigationPlanner.initialize(HOME);
    window.GalaxyViewerNavigationAdmin.bind(navigationPlanner);

    // ==================== RANDOM NAVIGATION ====================
    if(window.GalaxyRandomGalaxy?.VERSION!=='0089')throw new Error('RANDOM GALAXY 0089 EXPORT MISSING OR VERSION MISMATCH');
