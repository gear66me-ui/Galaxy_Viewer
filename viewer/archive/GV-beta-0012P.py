...
(async()=>{
    'use strict';
    const VERSION='12P';
    const DISPLAY_VERSION='12P';
    const ALADIN_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/vendor/aladin/aladin-classic-0003.js?v=C55174310A587D28';
    const HAMBURGER_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/hamburger-menu/gv-hamburger-menu-0004.js?v=BDBA9D4D8F2CF099';
    const COORDINATE_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/coordinate-overlay/gv-coordinate-overlay-0004.js?v=B7DE49232D2B776C';
    const TARGET_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/target-simbad/gv-target-simbad-0002.js?v=0D6ED7C0D65AED5B';
    const SIMBAD_POINTER_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/simbad-pointer/gv-simbad-pointer-0001.js?v=6D86915BAF846F74';
    const TARGET_INFO_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/target-info/gv-target-info-0002.js?v=DD4D60F378632EFD';
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0046.js?v=E79D103014DD2109';
    ...
    const DOWNLOAD_ANALYTICS_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/download-analytics/gv-download-analytics-0004.js?v=6529192C9A1AE172';
    ...
    function startAladinPreload(){
        ...
        aladinPreloadPromise=(async()=>{
            try{
                if(!window.A?.init){
                    await loadScript(ALADIN_URL,'gvAladinLocal0003');
                }
    ...
    const moduleLoads=Promise.all([
        loadScript(HAMBURGER_URL,'gvHamburger0004'),
        loadScript(COORDINATE_URL,'gvCoordinate0004'),
        loadScript(TARGET_URL,'gvTarget0002'),
        loadScript(SIMBAD_POINTER_URL,'gvSimbadPointer0001'),
        loadScript(TARGET_INFO_URL,'gvTargetInfo0002'),
        loadScript(RANDOM_GALAXY_URL,'gvRandomGalaxy0046'),
        loadScript(DIAGNOSTICS_URL,'gvDiagnostics0004'),
        loadScript(DOWNLOAD_ANALYTICS_URL,'gvDownloadAnalytics0004')
    ]);
    ...
    const targetInfoHost=createHost(root,'gv-target-info-host');
    ...
    if(
        window.GalaxyRandomGalaxy?.VERSION!=='0046' ||
        typeof window.GalaxyRandomGalaxy.bootstrapHomePresentation!=='function'
    )throw new Error('RANDOM GALAXY 0046 HOME BOOTSTRAP EXPORT MISSING');

    window.GalaxyRandomGalaxy.bootstrapHomePresentation(root);

    // ==================== HAMBURGER MENU ====================
    if(window.GalaxyViewerHamburgerMenu?.version!=='0004')throw new Error('HAMBURGER MODULE 0004 EXPORT MISSING');
    ...
    if(window.GalaxyCoordinateOverlay?.VERSION!=='0004')throw new Error('COORDINATE MODULE 0004 EXPORT MISSING OR VERSION MISMATCH');
    ...
    // ==================== TARGET / SIMBAD ====================
    if(window.GalaxyViewerTargetSimbad?.version!=='0002')throw new Error('TARGET / SIMBAD MODULE 0002 EXPORT MISSING');
    const target=await window.GalaxyViewerTargetSimbad.init({host:targetHost,aladin});

    // ==================== SIMBAD POINTER ====================
    if(window.GalaxyViewerSimbadPointer?.VERSION!=='0001'){
        throw new Error('SIMBAD POINTER MODULE 0001 EXPORT MISSING');
    }
    const simbadPointer=await window.GalaxyViewerSimbadPointer.init({aladin});

    // ==================== TARGET INFORMATION ====================
    if(window.GalaxyViewerTargetInfo?.VERSION!=='0002'){
        throw new Error('TARGET INFO MODULE 0002 EXPORT MISSING');
    }

    const targetInfo=await window.GalaxyViewerTargetInfo.init({
        host:targetInfoHost,
        aladin,
        viewerRoot:root,
        reservedElements:[
    ...
    // ==================== RANDOM NAVIGATION ====================
    if(window.GalaxyRandomGalaxy?.VERSION!=='0046')throw new Error('RANDOM GALAXY 0046 EXPORT MISSING OR VERSION MISMATCH');
    if(typeof window.GalaxyRandomNavigationWindow!=='function')throw new Error('RANDOM GALAXY 0046 NAVIGATION WINDOW EXPORT MISSING');
...
function navigateHistory(direction){
    ...
    const launchRandomGalaxy=()=>{
        if(!window.GalaxyRandomGalaxy?.prefetchRuntime){bottom.random.disabled=true;return;}
        if(navigationPending||randomGalaxy.getState().busy)return;
