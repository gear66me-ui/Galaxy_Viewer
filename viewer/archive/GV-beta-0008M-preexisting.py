...
# GV-beta-0008M
# Derived from exact GV-beta-0008L baseline blob 635de151e7df372018e74616b5dfef9dd9642f43.
# Authorized 8M changes: retained ten-image HD runtime pool, centered compact travel/startup UI,
# native compact touch-through scientific arrival panel, visible history controls, and no post-arrival reframing.
...
.gv-galaxy-history:disabled{opacity:.55;cursor:default;box-shadow:inset 0 0 7px rgba(86,255,149,.08),0 0 5px rgba(77,255,143,.16)}
...
#gv-travel-course,#gv-travel-heading{font:400 12px/1.08 "Space Age",sans-serif;letter-spacing:.65px;color:#CFFFE0;text-align:center}
#gv-travel-heading{margin-top:1px;color:#AEEFC5}
#gv-travel-destination{margin-top:2px;font:400 14px/1.12 "Space Age",sans-serif;letter-spacing:.45px;color:#78FFAB;text-shadow:0 0 6px rgba(87,255,147,.48);text-align:center;white-space:normal;overflow-wrap:anywhere}
#gv-travel-distance{display:flex;align-items:center;justify-content:center;width:132px;height:27px;padding:0 6px;border:1px solid rgba(131,255,176,.68);border-radius:5px;background:rgba(0,12,8,.70);color:#F2FFF7;font:400 16px/27px "Space Age",sans-serif;letter-spacing:.35px;text-align:center;white-space:nowrap;font-variant-numeric:tabular-nums}
...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/gv-random-galaxy-0010.js?v=a4a9ddb3c28751dfbbf9fcdf04278c80e1020013';
    ...
    function startPrefetch(destination){
        ...
        const promise=prepareHdDestination(destination).then(item=>{
            if(key===activeTargetKey&&!activePreparedItem){
                ...
                window.__gv8mRandomGalaxy?.setPreparedHdResource?.(key,item.objectUrl,item.sourceKind);
                return;
            }
    ...
    function destinationWithPrepared(item){
        return {...item.destination,preparedHdUrl:item.objectUrl,preparedSource:item.sourceKind};
    }
    ...
    function setUnpreparedActive(destination){
        ...
        activeTargetKey=key;
        return {...destination,preparedHdUrl:'',preparedSource:''};
    }
    ...
    function beginTravelHud(destination){
        ...
        const state=window.GV8M?.randomGalaxy?.getState?.()||window.__gv8mRandomGalaxy?.getState?.()||{};
    ...
    function createBottomControls(root){
        ...
        version.setAttribute('aria-label','GALAXY VIEWER VERSION 8M');
        ...
        hud.innerHTML='<div id="gv-travel-primary"><div id="gv-travel-course">COURSE LOCKED</div><div id="gv-travel-heading">HEADING TO</div><div id="gv-travel-destination"></div></div><div id="gv-travel-distance">0.000 MLY</div>';
    ...
    const hamburger=window.GalaxyViewerHamburgerMenu.init({
        ...
        onProjectionSelected(name,detail){
            ...
            }catch(error){console.error('GV-8M PROJECTION FAILURE',name,detail?.code,error)}
        }
    });
    ...
    coordinate=window.GalaxyCoordinateOverlay.mount(coordinateHost,{onFrameChange(nextFrame){
        frame=nextFrame;
        try{if(typeof aladin.setFrame==='function')aladin.setFrame(frame==='GAL'?'galactic':'ICRSd')}catch(error){console.warn('GV-8M FRAME CHANGE WARNING',error)}
        renderCoordinates();
    }});
    ...
    await loadScript(RANDOM_GALAXY_URL,'gvRandomGalaxy0010');
    if(window.GalaxyRandomGalaxy?.VERSION!=='0010')throw new Error('RANDOM GALAXY 0010 STANDALONE CORE EXPORT MISSING');
    function historySnapshot(destination){
        const {preparedHdUrl,preparedSource,...snapshot}=destination||{};
        return Object.freeze({...snapshot});
    }
    function setHistoryControls(){
        const busy=navigationPending||Boolean(window.__gv8mRandomGalaxy?.getState?.().busy);
    ...
    function navigateHistory(index){
        ...
        randomGalaxy.travelToRandom().catch(error=>{
            forcedDestination=null;pendingHistoryIndex=null;navigationPending=false;endTravelHud();setHistoryControls();console.error('GV-8M HISTORY NAVIGATION FAILURE',error);
        });
    }
    ...
    const randomGalaxy=window.GalaxyRandomGalaxy.mount(randomGalaxyHost,{
        ...
        onError(error){
            navigationPending=false;pendingHistoryIndex=null;forcedDestination=null;endTravelHud();setHistoryControls();console.error('GV-8M RANDOM GALAXY FAILURE',error);
        }
    });
    window.__gv8mRandomGalaxy=randomGalaxy;
    ...
    setHistoryControls();
    window.GV8M=Object.freeze({version:VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,historyBackButton:bottom.back,historyForwardButton:bottom.forward,reticle,versionLabel:bottom.version,homeOverlay,catalogCount:catalogRecordCount,eligibleCatalogCount:galaxyCatalog.length,getHubblePrefetchState,getGalaxyHistory:()=>({index:galaxyHistoryIndex,items:galaxyHistory.map(item=>({name:item.name,archiveId:item.archiveId}))})});
...
})().catch(error=>console.error('GALAXY VIEWER 8M STARTUP FAILURE:',error));
"""))

# GV-beta-0008M staged
