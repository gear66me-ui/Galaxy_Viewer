...
# RANDOM GALAXY 0005: validated Hubble catalog provider, HD pinch/pan, 24.075 s travel.
...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/gv-random-galaxy-0005.js?v=c0b3cc5e2bc7bb213c0aa00a900c93488d978101';
    ...
    function validateCatalogGalaxy(candidate,index){
        ...
        return Object.freeze({...candidate,name,ra,dec,distance,constellation,fov,hdUrl:hd.href,sourceUrl:source.href,imageType:imageType||'Observation',category:category||'Galaxies',telescope:telescope||'Hubble Space Telescope'});
    }
    ...
    function createBottomControls(root){
        ...
        version.setAttribute('aria-label','GALAXY VIEWER VERSION 8H');
    ...
    function readCurrentRaDec(aladin,root){
        ...
        }catch(error){console.warn('GV-8H GETRADEC WARNING',error)}
        try{
        ...
        }catch(error){console.warn('GV-8H PIX2WORLD WARNING',error)}
        return null;
    }

    galaxyCatalog=await loadGalaxyCatalog();
    const initialGalaxy=chooseGalaxy(galaxyCatalog);
    ...
    const aladin=A.aladin('#aladin-cosmic-command-test',{
        target:`${initialGalaxy.ra} ${initialGalaxy.dec}`,
        survey:'P/DSS2/color',
        fov:1.5,
        projection:'MOL',
        showReticle:false,
    ...
    const hamburger=window.GalaxyViewerHamburgerMenu.init({
        ...
        onProjectionSelected(name,detail){
            ...
            }catch(error){console.error('GV-8H PROJECTION FAILURE',name,detail?.code,error)}
        }
    });
    ...
    coordinate=window.GalaxyCoordinateOverlay.mount(coordinateHost,{onFrameChange(nextFrame){
        frame=nextFrame;
        try{if(typeof aladin.setFrame==='function')aladin.setFrame(frame==='GAL'?'galactic':'ICRSd')}catch(error){console.warn('GV-8H FRAME CHANGE WARNING',error)}
        renderCoordinates();
    }});
    ...
    const randomGalaxy=window.GalaxyRandomGalaxy.mount(randomGalaxyHost,{aladin,randomButton:bottom.random,bindClick:true,hubbleProvider:randomHubbleProvider,currentGalaxy:initialGalaxy});
    await randomGalaxy.ready;
    randomGalaxy.travelToRandom().catch(error=>console.error('GV-8H STARTUP RANDOM GALAXY FAILURE:',error));

    window.GV8H=Object.freeze({version:VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,reticle,versionLabel:bottom.version});
    document.dispatchEvent(new CustomEvent('gv-viewer-ready',{detail:{version:VERSION}}));
})().catch(error=>console.error('GALAXY VIEWER 8H STARTUP FAILURE:',error));
"""))

# GV-beta-0008H staged
