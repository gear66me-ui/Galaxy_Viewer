...
#gv-earth-return-indicator{position:absolute;left:50%;top:50%;z-index:7350;width:118px;height:76px;transform:translate(-50%,-50%);pointer-events:none;opacity:0;visibility:hidden;transition:opacity .12s linear}
#gv-earth-return-indicator.gv-visible{opacity:1;visibility:visible}
#gv-earth-return-indicator .gv-earth-return-arrow{position:absolute;left:50%;top:50%;width:34px;height:24px;transform:translate(-50%,-50%) rotate(0deg);transform-origin:50% 50%;filter:drop-shadow(0 0 5px rgba(255,216,90,.66))}
#gv-earth-return-indicator .gv-earth-return-arrow::before{content:"";position:absolute;left:1px;top:3px;width:0;height:0;border-top:9px solid transparent;border-bottom:9px solid transparent;border-left:28px solid #FFD85A}
#gv-earth-return-indicator .gv-earth-return-readout{position:absolute;left:50%;top:50%;display:flex;flex-direction:column;align-items:center;justify-content:center;min-width:82px;text-align:center;white-space:nowrap}
#gv-earth-return-indicator .gv-earth-return-title{display:flex;align-items:center;justify-content:center;gap:4px;color:#78FFAB;font:400 9px/11px "Space Age",sans-serif;letter-spacing:.55px;text-shadow:0 0 5px rgba(87,255,147,.48)}
#gv-earth-return-indicator .gv-earth-return-icon{display:inline-block;font:17px/17px system-ui,sans-serif;filter:drop-shadow(0 0 4px rgba(87,255,147,.42))}
#gv-earth-return-indicator .gv-earth-return-distance{display:flex;align-items:baseline;justify-content:center;gap:4px;margin-top:2px}
#gv-earth-return-indicator .gv-earth-return-value{display:block;color:#FFD85A;font:400 17px/18px "Space Age",sans-serif;letter-spacing:.32px;font-variant-numeric:tabular-nums;text-shadow:0 0 4px rgba(255,242,168,.82),0 0 9px rgba(255,180,45,.34)}
#gv-earth-return-indicator .gv-earth-return-unit{display:block;color:#BFEFD0;font:400 10.5px/12px "Space Age",sans-serif;letter-spacing:.45px;text-shadow:0 0 4px rgba(87,255,147,.32)}
...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://raw.githubusercontent.com/gear66me-ui/Galaxy_Viewer/beta/viewer/modules/gv-random-galaxy-0033.js';
    ...
    async function loadCombinedGalaxyCatalog(){
        ...
        const [hubbleData,jwstData,chandraData]=await Promise.all([
            loadHubbleCatalog(catalogUrls.hubble),
            loadJwstCatalog(catalogUrls.jwst),
            loadChandraCatalog(catalogUrls.chandra)
        ]);
    ...
    function beginTravelHud(destination){
        ...
        const hudSeconds=TRAVEL_SECONDS;
    ...
    function createEarthReturnIndicator(root){
        ...
        indicator.innerHTML='<span class="gv-earth-return-arrow" aria-hidden="true"></span><span class="gv-earth-return-readout"><span class="gv-earth-return-title"><span class="gv-earth-return-icon" aria-hidden="true">🌎</span><span>EARTH</span></span><span class="gv-earth-return-distance"><span class="gv-earth-return-value">0.0</span><span class="gv-earth-return-unit">MLY</span></span></span>';
    ...
    const moduleLoads=Promise.all([
        ...
        loadScript(RANDOM_GALAXY_URL,'gvRandomGalaxy0033')
    ]);
    ...
    const earthReturnIndicator=createEarthReturnIndicator(root);
    ...
    // ==================== RANDOM NAVIGATION ====================
    if(window.GalaxyRandomGalaxy?.VERSION!=='0033')throw new Error('RANDOM GALAXY 0032 EXPORT MISSING OR VERSION MISMATCH');
    ...
    const updateEarthReturnIndicator=()=>{
        if(!earthReturnDestination||navigationPending)return;
        ...
        const bottomSafe=62;
        const sideSafe=66;
        const halfW=Math.max(20,centerX-sideSafe);
        const halfH=Math.max(20,(rootRect.height-topSafe-bottomSafe)/2);
        const apertureCenterY=topSafe+halfH;
        ...
        const y=apertureCenterY+dy*scale;
    ...
    function navigateHistory(index){
        ...
        randomGalaxy.travelToRandom().catch(error=>{
            forcedDestination=null;pendingHistoryIndex=null;navigationPending=false;hideEarthReturnIndicator();resumeBackgroundWork();resumeArchivePreloads();endTravelHud();setHistoryControls();console.error('GALAXY VIEWER HISTORY NAVIGATION FAILURE',error);
        });
    }
    ...
    const randomGalaxy=window.GalaxyRandomGalaxy.mount(randomGalaxyHost,{
        ...
        onError(error){
            navigationPending=false;pendingHistoryIndex=null;forcedDestination=null;hideEarthReturnIndicator();endTravelHud();setHistoryControls();resumeBackgroundWork();resumeArchivePreloads();console.error('GALAXY VIEWER RANDOM GALAXY FAILURE',error);
        }
    });
    ...
    const launchRandomGalaxy=()=>{
        ...
        navigationPending=true;
        hideEarthReturnIndicator();
        homeOverlay.classList.add('gv-hidden');
        universeContext.classList.add('gv-hidden');
        setHistoryControls();
        suspendBackgroundWork();
        suspendArchivePreloads();
    ...
    function restoreAfterHdClose(attempt=0){
        ...
        requestAnimationFrame(()=>forceAladinRepaint());
    }
    if(hdBackToSky)hdBackToSky.addEventListener('click',()=>{hideEarthReturnIndicator();requestAnimationFrame(()=>restoreAfterHdClose())},true);
    if(hdDownloadButton){
    ...
    function providerFor(destination){const provider=String(destination?.provider||'').trim().toUpperCase();return provider==='HUBBLE'||provider==='JWST'||provider==='CHANDRA'?provider:''}
    ...
    const handleViewerVisibility=()=>{
        ...
        requestAnimationFrame(()=>{
            reconcileViewerPresentation();
            if(viewerHiddenAt&&performance.now()-viewerHiddenAt>=5000)
                forceAladinRepaint();
            viewerHiddenAt=0;
        });
    };
    ...
    window.GalaxyViewerCore=Object.freeze({version:VERSION,displayVersion:DISPLAY_VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,historyBackButton:bottom.back,historyForwardButton:bottom.forward,reticle,versionLabel:bottom.version,universeContext,homeOverlay,catalogCount:catalogRecordCount,eligibleCatalogCount:galaxyCatalog.length,catalogDatabaseCounts,startupMetrics,getPrefetchState,getDownloadStatus,getAladinPrewarmState,fillPrefetchQueue,getGalaxyCatalog:()=>Object.freeze([...galaxyCatalog]),beginTravelHud,activateQueuedDestination:(destination,excludeName='')=>consumeReady(destination,excludeName)||setUnpreparedActive(destination),requestHdPrefetch:destination=>{if(!destination)return '';enqueuePrefetch(destination,true);fillPrefetchQueue();return destinationKey(destination)},isAladinPrepared:key=>aladinPrewarmedKeys.has(String(key||'').trim().toLowerCase()),getBackgroundWorkSuspended:()=>backgroundWorkSuspended,getChandraTestOverrideState:()=>Object.freeze({chandraTestOverrideActive,chandraTestRemaining:chandraTestQueue.length,chandraTestTotal}),getGalaxyHistory:()=>({index:galaxyHistoryIndex,items:galaxyHistory.map(item=>({name:item.name,archiveId:item.archiveId,provider:providerFor(item)}))})});
...
# Galaxy Viewer Prefetch — authoritative rolling future-ten integration layer.
display(Javascript(r"""
(()=>{
'use strict';
const VERSION='ACTIVE';
...
function install(){
  ...
  randomGalaxy.provider=async args=>{
    ...
    if(destination)core.beginTravelHud?.(destination);
  ...
  installHdFeedback();
  if(core.versionLabel){core.versionLabel.textContent='VERSION 11T';core.versionLabel.setAttribute('aria-label','GALAXY VIEWER VERSION 11T')}
