...
# GV-beta-0009F
# Derived from exact GV-beta-0009E baseline blob 8f55b7b919965e6b6b51e5bffe3f9332fe8de775.
# Authorized 9F changes: module 0028 wiring, first HOME-trip translate-then-zoom choreography, and initial observable-universe context label.
# All unrelated 9E behavior remains frozen, including Hubble preload priority, ten-second HOME travel, eighteen-second later travel, and sequential ten-image prefetch.
...
#gv-universe-context{position:absolute;left:50%;top:58px;z-index:7095;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;width:min(310px,76vw);pointer-events:none;transition:opacity .2s ease}
...
#gv-universe-context .gv-universe-leader{position:relative;width:1px;height:28px;background:rgba(120,255,171,.86);box-shadow:0 0 7px rgba(87,255,147,.48)}
...
(async()=>{
    ...
    function startPrefetch(destination,priority=false){
        ...
        const promise=prepareHdDestination(destination,controller.signal).then(item=>{
            if(key===activeTargetKey&&!activePreparedItem){
                ...
                window.__gv9fRandomGalaxy?.setPreparedHdResource?.(key,item.objectUrl,item.sourceKind,item.image);
                return;
            }
    ...
    function beginTravelHud(destination){
        ...
        const state=window.GV9F?.randomGalaxy?.getState?.()||window.__gv9fRandomGalaxy?.getState?.()||{};
    ...
    function createBottomControls(root){
        ...
        version.setAttribute('aria-label','GALAXY VIEWER VERSION 9F');
    ...
    const hamburger=window.GalaxyViewerHamburgerMenu.init({
        ...
        onProjectionSelected(name,detail){
            ...
            }catch(error){console.error('GV-9F PROJECTION FAILURE',name,detail?.code,error)}
        }
    });
    ...
    coordinate=window.GalaxyCoordinateOverlay.mount(coordinateHost,{onFrameChange(nextFrame){
        frame=nextFrame;
        try{if(typeof aladin.setFrame==='function')aladin.setFrame(frame==='GAL'?'galactic':'ICRSd')}catch(error){console.warn('GV-9F FRAME CHANGE WARNING',error)}
        renderCoordinates();
    }});
    ...
    function setHistoryControls(){
        const busy=navigationPending||Boolean(window.__gv9fRandomGalaxy?.getState?.().busy);
    ...
    function navigateHistory(index){
        ...
        randomGalaxy.travelToRandom().catch(error=>{
            forcedDestination=null;pendingHistoryIndex=null;navigationPending=false;endTravelHud();setHistoryControls();console.error('GV-9F HISTORY NAVIGATION FAILURE',error);
        });
    }
    ...
    const randomGalaxy=window.GalaxyRandomGalaxy.mount(randomGalaxyHost,{
        ...
        onError(error){
            navigationPending=false;pendingHistoryIndex=null;forcedDestination=null;endTravelHud();setHistoryControls();console.error('GV-9F RANDOM GALAXY FAILURE',error);
        }
    });
    window.__gv9fRandomGalaxy=randomGalaxy;
    ...
    const deferHdUntilPrepared=async event=>{
        ...
        try{
            ...
            console.error('GV-9F HUBBLE PREPARATION WAIT FAILURE',error);
            try{randomGalaxy.showHubbleHD()}catch(fallbackError){console.error('GV-9F HUBBLE FALLBACK FAILURE',fallbackError)}
        }
    };
    ...
    setHistoryControls();
    window.GV9F=Object.freeze({version:VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,historyBackButton:bottom.back,historyForwardButton:bottom.forward,reticle,versionLabel:bottom.version,universeContext,homeOverlay,catalogCount:catalogRecordCount,eligibleCatalogCount:galaxyCatalog.length,getHubblePrefetchState,startHubblePrefetch:fillPrefetchQueue,getGalaxyHistory:()=>({index:galaxyHistoryIndex,items:galaxyHistory.map(item=>({name:item.name,archiveId:item.archiveId}))})});
...
})().catch(error=>console.error('GALAXY VIEWER 9F STARTUP FAILURE:',error));
"""))

# GV-beta-0009F staged