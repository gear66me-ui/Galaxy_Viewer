...
# GV-beta-0008N
# Derived from exact GV-beta-0008M baseline blob e713a70d6651d85f5eeb395231da3bbd2fe1aa8f.
# Authorized 8N changes: bold CSS chevron history controls, fixed two-decimal MLY travel display,
# larger readable travel/arrival typography, five-field HD science banner, and exact retained decoded-image handoff.
...
.gv-galaxy-history{appearance:none;-webkit-appearance:none;position:relative;display:flex;align-items:center;justify-content:center;width:36px;height:36px;margin:0;padding:0;border:1px solid #B7FFD0;border-radius:6px;background:linear-gradient(145deg,rgba(8,57,38,.96),rgba(11,91,53,.94));color:transparent;box-shadow:inset 0 0 7px rgba(167,255,203,.22),0 0 8px rgba(77,255,143,.34);cursor:pointer;touch-action:manipulation;outline:none;overflow:hidden;pointer-events:auto}
.gv-galaxy-history::before,.gv-galaxy-history::after{content:"";position:absolute;left:50%;top:50%;clip-path:polygon(0 0,44% 0,100% 50%,44% 100%,0 100%,56% 50%);pointer-events:none}
.gv-galaxy-history::before{width:26px;height:28px;background:#78FFAB;transform:translate(-50%,-50%);filter:drop-shadow(0 0 4px rgba(87,255,147,.80))}
.gv-galaxy-history::after{width:20px;height:22px;background:linear-gradient(145deg,#E8FFFF,#62D8FF);transform:translate(-50%,-50%);filter:drop-shadow(0 0 3px rgba(98,216,255,.72))}
.gv-galaxy-history-back::before,.gv-galaxy-history-back::after{transform:translate(-50%,-50%) rotate(180deg)}
.gv-galaxy-history:disabled{opacity:.58;cursor:default;box-shadow:inset 0 0 6px rgba(86,255,149,.12),0 0 6px rgba(77,255,143,.20)}
...
#gv-travel-distance{display:flex;align-items:center;justify-content:center;width:132px;height:27px;padding:0 6px;border:1px solid rgba(131,255,176,.78);border-radius:5px;background:rgba(0,12,8,.76);color:#FFFFFF;font:400 18px/27px "Space Age",sans-serif;letter-spacing:.28px;text-align:center;text-shadow:0 0 4px rgba(205,255,224,.20);white-space:nowrap;font-variant-numeric:tabular-nums}
...
#gv-we-are-here .gv-home-leader{display:none}
#gv-we-are-here .gv-home-label{position:absolute;left:50%;top:66px;transform:translateX(-50%);width:min(214px,68vw);padding:5px 7px 6px;border:1px solid rgba(131,255,176,.62);border-radius:6px;background:rgba(0,14,9,.58);color:#E8FFF0;text-align:center;text-transform:uppercase;text-shadow:0 0 6px rgba(87,255,147,.42);box-shadow:0 0 8px rgba(65,255,133,.10)}
#gv-we-are-here .gv-home-origin{display:flex;align-items:center;justify-content:center;gap:5px;color:#78FFAB;font:400 13px/1.12 "Space Age",sans-serif;letter-spacing:.7px;text-align:center}
#gv-we-are-here .gv-earth-icon{display:inline-flex;align-items:center;justify-content:center;font:17px/1 system-ui,sans-serif;filter:drop-shadow(0 0 4px rgba(87,255,147,.45))}
#gv-we-are-here .gv-home-sub{margin-top:2px;color:#CFFFE0;font:400 11px/1.15 "Space Age",sans-serif;letter-spacing:.6px;text-align:center}
#gv-we-are-here .gv-home-hint{margin-top:3px;color:#A8DDB8;font:400 10px/1.15 "Space Age",sans-serif;letter-spacing:.45px;text-align:center}
...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/gv-random-galaxy-0011.js?v=a48135e520d2c1a73186def290d0ed42e8ffc9cd';
    ...
    function startPrefetch(destination){
        ...
        const promise=prepareHdDestination(destination).then(item=>{
            if(key===activeTargetKey&&!activePreparedItem){
                ...
                window.__gv8nRandomGalaxy?.setPreparedHdResource?.(key,item.objectUrl,item.sourceKind,item.image);
                return;
            }
    ...
    function beginTravelHud(destination){
        ...
        if(!hud||!destinationEl||!distanceEl)return;
        cancelAnimationFrame(travelHudFrame);
        const state=window.GV8N?.randomGalaxy?.getState?.()||window.__gv8nRandomGalaxy?.getState?.()||{};
        ...
        distanceEl.textContent='0.00 MLY';
        hud.classList.add('gv-visible');
        const started=performance.now();
        const frame=now=>{
            const t=Math.min(1,(now-started)/(TRAVEL_SECONDS*1000));
            distanceEl.textContent=(total*distanceProgress(t)).toFixed(2)+' MLY';
    ...
    function createBottomControls(root){
        ...
        version.setAttribute('aria-label','GALAXY VIEWER VERSION 8N');
        ...
        hud.innerHTML='<div id="gv-travel-primary"><div id="gv-travel-course">COURSE LOCKED</div><div id="gv-travel-heading">HEADING TO</div><div id="gv-travel-destination"></div></div><div id="gv-travel-distance">0.00 MLY</div>';
    ...
    const hamburger=window.GalaxyViewerHamburgerMenu.init({
        ...
        onProjectionSelected(name,detail){
            ...
            }catch(error){console.error('GV-8N PROJECTION FAILURE',name,detail?.code,error)}
        }
    });
    ...
    coordinate=window.GalaxyCoordinateOverlay.mount(coordinateHost,{onFrameChange(nextFrame){
        frame=nextFrame;
        try{if(typeof aladin.setFrame==='function')aladin.setFrame(frame==='GAL'?'galactic':'ICRSd')}catch(error){console.warn('GV-8N FRAME CHANGE WARNING',error)}
        renderCoordinates();
    }});
    ...
    await loadScript(RANDOM_GALAXY_URL,'gvRandomGalaxy0011');
    if(window.GalaxyRandomGalaxy?.VERSION!=='0011')throw new Error('RANDOM GALAXY 0011 STANDALONE CORE EXPORT MISSING');
    ...
    function setHistoryControls(){
        const busy=navigationPending||Boolean(window.__gv8nRandomGalaxy?.getState?.().busy);
    ...
    function navigateHistory(index){
        ...
        randomGalaxy.travelToRandom().catch(error=>{
            forcedDestination=null;pendingHistoryIndex=null;navigationPending=false;endTravelHud();setHistoryControls();console.error('GV-8N HISTORY NAVIGATION FAILURE',error);
        });
    }
    ...
    const randomGalaxy=window.GalaxyRandomGalaxy.mount(randomGalaxyHost,{
        ...
        onError(error){
            navigationPending=false;pendingHistoryIndex=null;forcedDestination=null;endTravelHud();setHistoryControls();console.error('GV-8N RANDOM GALAXY FAILURE',error);
        }
    });
    window.__gv8nRandomGalaxy=randomGalaxy;
    ...
    setHistoryControls();
    window.GV8N=Object.freeze({version:VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,historyBackButton:bottom.back,historyForwardButton:bottom.forward,reticle,versionLabel:bottom.version,homeOverlay,catalogCount:catalogRecordCount,eligibleCatalogCount:galaxyCatalog.length,getHubblePrefetchState,getGalaxyHistory:()=>({index:galaxyHistoryIndex,items:galaxyHistory.map(item=>({name:item.name,archiveId:item.archiveId}))})});
    ...
    setTimeout(fillPrefetchQueue,0);
})().catch(error=>console.error('GALAXY VIEWER 8N STARTUP FAILURE:',error));
"""))

# GV-beta-0008N staged
