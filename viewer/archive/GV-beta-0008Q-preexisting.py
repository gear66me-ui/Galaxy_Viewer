...
#gv-travel-distance{box-sizing:border-box;display:flex;flex-direction:column;align-items:center;justify-content:center;width:172px;height:27px;padding:1px 6px 0;border:1px solid rgba(131,255,176,.78);border-radius:5px;background:rgba(0,12,8,.76);color:#FFFFFF;text-align:center;text-shadow:0 0 4px rgba(205,255,224,.20);white-space:nowrap}
#gv-travel-distance-value{display:block;width:100%;height:15px;font:400 15px/15px "Space Age",sans-serif;letter-spacing:.22px;text-align:center;font-variant-numeric:tabular-nums}
#gv-travel-distance-unit{display:block;width:100%;height:8px;font:400 7px/8px "Space Age",sans-serif;letter-spacing:.45px;text-align:center;white-space:nowrap}
...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/gv-random-galaxy-0014.js?v=8fcbf8829b4623dd65d8c2f572ee1a73d8024160';
    ...
    function startPrefetch(destination){
        ...
        const promise=prepareHdDestination(destination).then(item=>{
            if(key===activeTargetKey&&!activePreparedItem){
                ...
                window.__gv8qRandomGalaxy?.setPreparedHdResource?.(key,item.objectUrl,item.sourceKind,item.image);
                return;
            }
    ...
    function formatTravelDistance(millionLy){
        ...
        if(value>=1000)return {value:(value/1000).toFixed(3),unit:'BILLION LIGHT-YEARS'};
        return {value:value.toFixed(3),unit:'MILLION LIGHT-YEARS'};
    }
    ...
    function beginTravelHud(destination){
        ...
        const state=window.GV8Q?.randomGalaxy?.getState?.()||window.__gv8qRandomGalaxy?.getState?.()||{};
    ...
    function createBottomControls(root){
        ...
        version.setAttribute('aria-label','GALAXY VIEWER VERSION 8Q');
        ...
        hud.innerHTML='<div id="gv-travel-primary"><div id="gv-travel-course">COURSE LOCKED</div><div id="gv-travel-heading">HEADING TO</div><div id="gv-travel-destination"></div></div><div id="gv-travel-distance"><span id="gv-travel-distance-value">0.000</span><span id="gv-travel-distance-unit">MILLION LIGHT-YEARS</span></div>';
    ...
    await loadScript(RANDOM_GALAXY_URL,'gvRandomGalaxy0014');
    if(window.GalaxyRandomGalaxy?.VERSION!=='0011')throw new Error('RANDOM GALAXY 0014 PATH / 0011 VERIFIED CORE EXPORT MISSING');
    ...
    function setHistoryControls(){
        const busy=navigationPending||Boolean(window.__gv8qRandomGalaxy?.getState?.().busy);
    ...
    window.__gv8qRandomGalaxy=randomGalaxy;
    ...
    setHistoryControls();
    window.GV8Q=Object.freeze({version:VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,historyBackButton:bottom.back,historyForwardButton:bottom.forward,reticle,versionLabel:bottom.version,homeOverlay,catalogCount:catalogRecordCount,eligibleCatalogCount:galaxyCatalog.length,getHubblePrefetchState,getGalaxyHistory:()=>({index:galaxyHistoryIndex,items:galaxyHistory.map(item=>({name:item.name,archiveId:item.archiveId}))})});
