...
(async()=>{
    ...
    function createCenterReticle(root){
        ...
        const EARTH_POINTER_RADIUS=RING_RADIUS-6;
        const EARTH_ICON_RADIUS=RING_RADIUS-30;
  ...
  return marker;
        ...
        const earthPointer=makeTriangle('gv-earth-pointer');

        const northLabel=document.createElement('div');
        northLabel.id='gv-north-label';
        northLabel.textContent='N';
        Object.assign(northLabel.style,{
  position:'absolute',display:'none',color:COSMIC_BLUE,
  font:'400 15px/1 "Space Age",sans-serif',letterSpacing:'1px',
  textShadow:'0 0 3px #dff7ff,0 0 6px rgba(88,191,255,.75)',
  whiteSpace:'nowrap',textAlign:'center'
        });
        reticle.appendChild(northLabel);

        const earthLeader=document.createElement('div');
        earthLeader.id='gv-earth-leader';
        Object.assign(earthLeader.style,{
  position:'absolute',display:'none',width:'0.7px',height:'7px',
  background:COSMIC_BLUE,
  boxShadow:'0 0 2px rgba(88,191,255,.45)',
  transformOrigin:'50% 0'
        });
        reticle.appendChild(earthLeader);

        const earthIcon=document.createElement('div');
        earthIcon.id='gv-earth-icon';
        earthIcon.textContent='🌎';
        Object.assign(earthIcon.style,{
  position:'absolute',display:'none',font:'17px/1 sans-serif',
  width:'26px',height:'26px',textAlign:'center',lineHeight:'24px',
  border:'0.8px solid rgba(88,191,255,.88)',borderRadius:'50%',
  boxSizing:'border-box',
  filter:'drop-shadow(0 0 3px #dff7ff) drop-shadow(0 0 6px rgba(88,191,255,.78))',
  whiteSpace:'nowrap'
        });
        reticle.appendChild(earthIcon);

        reticle.gvDirectional={
  center:CENTER,
  northPointerRadius:NORTH_POINTER_RADIUS,
  northLabelRadius:NORTH_LABEL_RADIUS,
  earthPointerRadius:EARTH_POINTER_RADIUS,
  earthIconRadius:EARTH_ICON_RADIUS,
  northPointer,
  northLabel,
  earthPointer,
  earthLeader,
  ...
  return null;
    ...
    function setNorthMarker(bearing){
        ...
        state.northLabel.style.transform='translate(-50%,-50%)';
        state.northLabel.style.display='block';
    }

    function setEarthMarker(bearing){
        const state=reticle.gvDirectional;
        if(!state||!Number.isFinite(bearing))return;
        state.lastEarthBearing=bearing;
        placeDirectionalElement(state.earthPointer,bearing,state.earthPointerRadius,180);
        placeDirectionalElement(state.earthIcon,bearing,state.earthIconRadius,0);

        const radians=bearing*Math.PI/180;
        const leaderRadius=state.earthPointerRadius-15;
        state.earthLeader.style.left=`${state.center+Math.sin(radians)*leaderRadius}px`;
        state.earthLeader.style.top=`${state.center-Math.cos(radians)*leaderRadius}px`;
        state.earthLeader.style.transform=`translate(-50%,-50%) rotate(${bearing+180}deg)`;
        state.earthLeader.style.display='block';
    }
    ...
    const updateDirectionalReticle=()=>{
        ...
        if(Number.isFinite(earthBearing))setEarthMarker(earthBearing);
        else if(Number.isFinite(state.lastEarthBearing))setEarthMarker(state.lastEarthBearing);
    };
    updateDirectionalReticle();
    const directionalReticleTimer=setInterval(updateDirectionalReticle,100);
    window.addEventListener('beforeunload',()=>clearInterval(directionalReticleTimer),{once:true});
