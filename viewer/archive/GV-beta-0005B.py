...
# GV-beta-0005B
# Standalone catalog-driven Galaxy Viewer release based only on GV-beta-0005A.
# Preserves the empty-canvas viewer and adds a minimal random-galaxy control that
# reads discovery/galaxy-catalog-beta.json. No earlier viewer is imported or executed.
...
#aladin-cosmic-command-test #gv-random-galaxy-name{
    ...
    border:1px solid rgba(98,216,255,.75)!important;
...
#aladin-cosmic-command-test button#gv-next-galaxy{
    ...
    font:700 13px/1.1 "Roboto Mono",Consolas,monospace!important;
    cursor:pointer!important;
    touch-action:manipulation!important;
    box-shadow:0 0 8px rgba(125,244,255,.35)!important;
}
#aladin-cosmic-command-test button#gv-next-galaxy:disabled{
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){
            console.error("GV-beta-0005B STARTUP FAILURE: window.A was not created");
            return;
        }
        ...
        window.A.init.then(() => {
            ...
            panel.innerHTML='<div id="gv-random-galaxy-name" role="status" aria-live="polite">Loading galaxy catalog…</div><button id="gv-next-galaxy" type="button" disabled>Next Galaxy</button>';
            ...
            function openGalaxy(record){
                const ra=Number(record?.ra_deg);
                const dec=Number(record?.dec_deg);
                const fov=Math.max(0.04,Number(record?.preferred_fov_deg)||0.25);
                if(!Number.isFinite(ra)||!Number.isFinite(dec))return;
                aladin.gotoRaDec(ra,dec);
                aladin.setFoV(fov);
                nameLabel.textContent=String(record.primary_name||"Unnamed SIMBAD object").trim()+" · FOV "+fov.toFixed(2)+"°";
            }

            function openRandomGalaxy(){
                if(!records.length)return;
                let nextIndex=Math.floor(Math.random()*records.length);
                if(records.length>1&&nextIndex===currentIndex){
                    nextIndex=(nextIndex+1)%records.length;
                }
                currentIndex=nextIndex;
                openGalaxy(records[currentIndex]);
            }

            nextButton.addEventListener("click",openRandomGalaxy);
            ...
            fetch(catalogUrl+"?v="+Date.now(),{cache:"no-store"})
                ...
                .then(payload=>{
                    records=Array.isArray(payload.records)
                        ...
                        : [];
                    if(!records.length)throw new Error("Catalog contains no SIMBAD records");
                    nextButton.disabled=false;
                    openRandomGalaxy();
                })
                .catch(error=>{
                    nameLabel.textContent="Galaxy catalog unavailable";
                    console.error("GV-beta-0005B CATALOG FAILURE:",error);
                });
        }).catch(error=>console.error("GV-beta-0005B STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0005B STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
