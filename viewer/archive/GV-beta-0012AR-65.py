...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0125.js';
    ...
    const RANDOM_GALAXY_VERSION='0125';
  ...
  return null;
    ...
    // ==================== 12AR-65 EARTH COMPASS PRESENTATION ====================
    ...
    function readEarthBearingFromSource(root){
        try{
            ...
            const dx=sx-cx;
            const dy=sy-cy;
            if(![dx,dy].every(Number.isFinite)||Math.hypot(dx,dy)<1e-6)return null;
            return (Math.atan2(dx,-dy)*180/Math.PI+360)%360;
        }catch(error){
  ...
  pointerEvents:'none'
        ...
        const updateLiveAladinRotation=()=>{
            try{
                ...
                rotationReadout.textContent=
                    `GALAXY ${gvRotationLabel(target)} / FRAME ${gvFrameLabel()} / CATALOG aladinRotation ${catalogText}\n`+
                    `LAST ROT CMD ${gvRotationFmt(cmdValue)} / ALADIN LIVE ROT ${gvRotationFmt(liveValue)} / ΔLIVE-CAT ${gvRotationFmt(deltaCatLive)} / ΔLIVE-CMD ${gvRotationFmt(deltaCmdLive)}`;
            }catch(_){
                rotationReadout.textContent='GALAXY — / FRAME ? / CATALOG aladinRotation —\nLAST ROT CMD — / ALADIN LIVE ROT — / ΔLIVE-CAT — / ΔLIVE-CMD —';
            }
        };
