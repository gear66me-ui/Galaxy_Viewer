...
# GV-beta-0005M
# Standalone Galaxy Viewer release based only on the final GV-beta-0005L.
# Preserves the 1,000-record catalog, Spectacular Mode, centered reticle, safe FOV floor, navigation, splash structure, and launcher artwork.
# Changes only the compact top control sizing, Target icon sizing, active Target status message, and approved #B8B1F0 control border/glow color.
...
#aladin-cosmic-command-test button.gv-target-proxy:active{
    ...
    flex:0 0 36px!important;margin:0 0 0 -1px!important;padding:0!important;box-sizing:border-box!important;overflow:hidden!important;
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){console.error("GV-beta-0005M STARTUP FAILURE: window.A was not created");return;}
        window.A.init.then(async() => {
            ...
            catch(error){console.error("GV-beta-0005M CATALOG FAILURE:",error);}
            ...
            centerReticle.src=reticleUrl+"?v=5M-target-status-001";
            centerReticle.alt="";
            ...
            function createTargetProxy(){
                ...
                proxy.innerHTML=`<img src="${targetIconUrl}?v=5M-target-status-001" alt="" aria-hidden="true" draggable="false">`;
            ...
            versionLabel.textContent="Galaxy Viewer 5M";
            ...
            async function openGalaxy(record){
                ...
                }catch(error){nameLabel.textContent="Could not resolve "+String(record.primary_name||"galaxy");console.error("GV-beta-0005M TARGET FAILURE:",error);}
        ...
        }).catch(error=>console.error("GV-beta-0005M STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0005M STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
