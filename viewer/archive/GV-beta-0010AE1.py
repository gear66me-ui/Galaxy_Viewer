...
(async()=>{
    ...
    async function loadCombinedGalaxyCatalog(){
        ...
        console.info('GV-10AE1 CATALOG COUNTS',catalogDatabaseCounts);
        return combined;
    }
            ...
            if(url.startsWith('https://raw.githubusercontent.com/')){
                fetch(url,{cache:'no-store'}).then(response=>{
                    ...
                    if(datasetKey==='gvRandomGalaxy0031'){
                        ...
                        if(!source.includes("host === 'chandra.harvard.edu'")||!source.includes("/(hubble|webb|chandra)/i")||!source.includes("provider: cleanText(candidate.provider"))throw new Error('RANDOM GALAXY 0031 10AE1 COMPATIBILITY PATCH FAILED');
                    }
...
})().catch(error=>{console.error('GALAXY VIEWER 10AE1 STARTUP FAILURE:',error);document.dispatchEvent(new CustomEvent('gv-viewer-failed',{detail:{message:String(error?.stack||error)}}));});
"""))

# GV-beta-0010AE1 staged