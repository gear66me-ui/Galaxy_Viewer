...
# GV-beta-0008J
# Derived from exact GV-beta-0008I baseline blob 99f10c103d865179ce498b37851025c25dc6434a.
# Authorized 8J changes: Earth-origin transparent WE ARE HERE leader to the existing reticle,
# RANDOM GALAXY 0007 interactive travel HUD, centered HD pinch, enlarged Hubble controls, and HD download action.
...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/gv-random-galaxy-0007.js?v=2a3c550a30fceec3a888026b72774899a58e2f49';
    ...
    function createBottomControls(root){
        ...
        version.setAttribute('aria-label','GALAXY VIEWER VERSION 8J');
    ...
    function readCurrentRaDec(aladin,root){
        ...
        }catch(error){console.warn('GV-8J GETRADEC WARNING',error)}
        try{
        ...
        }catch(error){console.warn('GV-8J PIX2WORLD WARNING',error)}
        return null;
    }
    ...
    const hamburger=window.GalaxyViewerHamburgerMenu.init({
        ...
        onProjectionSelected(name,detail){
            ...
            }catch(error){console.error('GV-8J PROJECTION FAILURE',name,detail?.code,error)}
        }
    });
    ...
    coordinate=window.GalaxyCoordinateOverlay.mount(coordinateHost,{onFrameChange(nextFrame){
        frame=nextFrame;
        try{if(typeof aladin.setFrame==='function')aladin.setFrame(frame==='GAL'?'galactic':'ICRSd')}catch(error){console.warn('GV-8J FRAME CHANGE WARNING',error)}
        renderCoordinates();
    }});
    ...
    await loadScript(RANDOM_GALAXY_URL,'gvRandomGalaxy0007');
    if(window.GalaxyRandomGalaxy?.VERSION!=='0007')throw new Error('RANDOM GALAXY MODULE 0007 EXPORT MISSING');
    ...
    window.GV8J=Object.freeze({version:VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,reticle,versionLabel:bottom.version,homeOverlay,catalogCount:galaxyCatalog.length});
    document.dispatchEvent(new CustomEvent('gv-viewer-ready',{detail:{version:VERSION,catalogCount:galaxyCatalog.length}}));
})().catch(error=>console.error('GALAXY VIEWER 8J STARTUP FAILURE:',error));
"""))

# GV-beta-0008J staged
