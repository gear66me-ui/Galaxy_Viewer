...
(async()=>{
    'use strict';
    const VERSION='12AR-132BR';
    const DISPLAY_VERSION='12AR-132BR';
...
`;
    ...
    gvAvmTrace('VIEWER_AVM0058_LOAD_START',{url:AVM_LAB_URL});
    loadScriptGithubThenLocal(AVM_LAB_URL,'gvAvmOverlayLab0058').then(()=>{
        gvAvmTrace('VIEWER_AVM0058_LOAD_OK',{version:window.GalaxyViewerAvmOverlayLab?.VERSION||''});
        const lab=window.GalaxyViewerAvmOverlayLab;
        if(!lab){gvAvmTrace('VIEWER_AVM0058_EXPORT_MISSING',{});return}
        if(lab.VERSION!=='0058'){
            gvAvmTrace('VIEWER_AVM0058_VERSION_MISMATCH',{version:String(lab.VERSION||'')});
            return;
        }
        try{
            gvAvmTrace('VIEWER_AVM0058_INSTALL_START',{version:lab.VERSION});
            lab.install?.({A,aladin,viewerRoot:root,randomGalaxy});
            gvAvmTrace('VIEWER_AVM0058_INSTALL_OK',{version:lab.VERSION});
        }catch(error){
            gvAvmTrace('VIEWER_AVM0058_INSTALL_FAIL',{message:String(error?.message||error||'')});
            throw error;
        }
    }).catch(error=>{gvAvmTrace('VIEWER_AVM0058_LOAD_FAIL',{message:String(error?.message||error||'')});console.error('GV AVM LAB LOAD FAILURE',error)});
