...
(async()=>{
    'use strict';
    const VERSION='12AR-132AG';
    const DISPLAY_VERSION='12AR-132AG';
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0208.js';
    ...
    const RANDOM_GALAXY_VERSION='0208';
    const AVM_LAB_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/lab/gv-avm-overlay-lab-0047.js';
...
`;
    ...
    gvAvmTrace('VIEWER_AVM0047_LOAD_START',{url:AVM_LAB_URL});
    loadScriptGithubThenLocal(AVM_LAB_URL,'gvAvmOverlayLab0047').then(()=>{
        gvAvmTrace('VIEWER_AVM0047_LOAD_OK',{version:window.GalaxyViewerAvmOverlayLab?.VERSION||''});
        const lab=window.GalaxyViewerAvmOverlayLab;
        if(!lab){gvAvmTrace('VIEWER_AVM0047_EXPORT_MISSING',{});return}
        if(lab.VERSION!=='0047')gvAvmTrace('VIEWER_AVM0047_VERSION_MISMATCH',{version:String(lab.VERSION||'')});
        try{
            gvAvmTrace('VIEWER_AVM0047_INSTALL_START',{version:lab.VERSION});
            lab.install?.({A,aladin,viewerRoot:root,randomGalaxy});
            gvAvmTrace('VIEWER_AVM0047_INSTALL_OK',{version:lab.VERSION});
        }catch(error){
            gvAvmTrace('VIEWER_AVM0047_INSTALL_FAIL',{message:String(error?.message||error||'')});
            throw error;
        }
    }).catch(error=>{gvAvmTrace('VIEWER_AVM0047_LOAD_FAIL',{message:String(error?.message||error||'')});console.error('GV AVM LAB LOAD FAILURE',error)});
