...
# GV-beta-0006A
# Standalone Galaxy Viewer release based only on final GV-beta-0005Y.
# Replaces the visible ICRSd tile with a 40 px hamburger tile, expands the coordinate tile to the available width, and enlarges the Target tile to 40 px while preserving the existing Target artwork.
# Preserves the four-decimal coordinate display, 1,000-record catalog, Spectacular Mode, centered reticle, safe FOV floor, navigation, artwork, circular comet orbit, active green field, SIMBAD behavior, and #B8B1F0 color.
...
#aladin-cosmic-command-test .gv-coordinate-target-row{
    ...
    grid-template-columns:40px minmax(0,1fr) 40px!important;column-gap:4px!important;row-gap:0!important;
...
#aladin-cosmic-command-test button.gv-menu-proxy .gv-menu-track{
    position:absolute!important;left:8px!important;width:24px!important;height:2px!important;border-radius:2px!important;
    background:#62D8FF!important;box-shadow:0 0 5px rgba(98,216,255,.9)!important;pointer-events:none!important;
}
#aladin-cosmic-command-test button.gv-menu-proxy .gv-menu-track:nth-child(1){top:11px!important}
#aladin-cosmic-command-test button.gv-menu-proxy .gv-menu-track:nth-child(2){top:19px!important}
#aladin-cosmic-command-test button.gv-menu-proxy .gv-menu-track:nth-child(3){top:27px!important}
...
#aladin-cosmic-command-test .gv-coordinate-target-row>.aladin-coordinates{
    ...
    height:40px!important;min-height:40px!important;max-height:40px!important;margin:0!important;padding:0 8px!important;
...
#aladin-cosmic-command-test button.gv-target-proxy:active{
    ...
    display:flex!important;visibility:visible!important;opacity:1!important;align-items:center!important;justify-content:center!important;
    width:40px!important;min-width:40px!important;max-width:40px!important;height:40px!important;min-height:40px!important;max-height:40px!important;
    flex:0 0 40px!important;margin:0!important;padding:0!important;box-sizing:border-box!important;overflow:hidden!important;
    background:rgba(0,0,0,.82)!important;border:1px solid #B8B1F0!important;border-radius:6px!important;
    cursor:pointer!important;touch-action:manipulation!important;outline:none!important;box-shadow:0 0 10px rgba(184,177,240,.38)!important;pointer-events:auto!important;
}
...
#aladin-cosmic-command-test button.gv-target-proxy img{
    position:relative!important;z-index:1!important;display:block!important;width:32px!important;height:32px!important;object-fit:contain!important;
...
#aladin-cosmic-command-test button.gv-target-proxy .gv-target-comet{
    position:absolute!important;left:20px!important;top:20px!important;width:0!important;height:0!important;
...
#aladin-cosmic-command-test .gv-target-status{
    ...
    width:176px!important;min-width:176px!important;max-width:176px!important;height:40px!important;min-height:40px!important;max-height:40px!important;
    margin:8px 8px 0 0!important;padding:3px 8px!important;box-sizing:border-box!important;
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){console.error("GV-beta-0006A STARTUP FAILURE: window.A was not created");return;}
        window.A.init.then(async() => {
            ...
            catch(error){console.error("GV-beta-0006A CATALOG FAILURE:",error);}
            ...
            centerReticle.src=reticleUrl+"?v=6A-responsive-topbar-001";
            centerReticle.alt="";
            ...
            function createTargetProxy(){
                ...
                proxy.innerHTML=`<img src="${targetIconUrl}?v=6A-responsive-topbar-001" alt="" aria-hidden="true" draggable="false"><span class="gv-target-comet" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i></span>`;
            ...
            function bindTargetStatus(proxy,status){
                if(status.dataset.gv6aBound==="true")return;
                status.dataset.gv6aBound="true";
            ...
            versionLabel.textContent="Galaxy Viewer 6A";
            ...
            async function openGalaxy(record){
                ...
                }catch(error){nameLabel.textContent="Could not resolve "+String(record.primary_name||"galaxy");console.error("GV-beta-0006A TARGET FAILURE:",error);}
        ...
        }).catch(error=>console.error("GV-beta-0006A STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0006A STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
