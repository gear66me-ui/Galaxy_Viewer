...
# GV-beta-0005K
# Standalone Galaxy Viewer release based only on the final GV-beta-0005J.
# Preserves the 1,000-record catalog, Spectacular Mode, centered reticle, safe FOV floor, and navigation.
# Changes only the ICRSd, coordinate, and Target control borders/glows to #A9A3D9.
...
#aladin-cosmic-command-test .gv-coordinate-target-row>.aladin-coordinates{
    ...
    box-sizing:border-box!important;overflow:hidden!important;border:1px solid #A9A3D9!important;border-radius:6px 0 0 6px!important;
...
#aladin-cosmic-command-test button.gv-target-proxy:active{
    ...
    background:rgba(0,0,0,.82)!important;border:1px solid #A9A3D9!important;border-radius:0 6px 6px 0!important;
...
(() => {
    ...
    function startGalaxyViewer(){
        if(!window.A||!window.A.init){console.error("GV-beta-0005K STARTUP FAILURE: window.A was not created");return;}
        window.A.init.then(async() => {
            ...
            catch(error){console.error("GV-beta-0005K CATALOG FAILURE:",error);}
            ...
            centerReticle.src=reticleUrl+"?v=5K-control-border-001";
            centerReticle.alt="";
            ...
            function createTargetProxy(){
                ...
                proxy.innerHTML=`<img src="${targetIconUrl}?v=5K-control-border-001" alt="" aria-hidden="true" draggable="false">`;
            ...
            versionLabel.textContent="Galaxy Viewer 5K";
            ...
            async function openGalaxy(record){
                ...
                }catch(error){nameLabel.textContent="Could not resolve "+String(record.primary_name||"galaxy");console.error("GV-beta-0005K TARGET FAILURE:",error);}
        ...
        }).catch(error=>console.error("GV-beta-0005K STARTUP FAILURE:",error));
    }
    ...
    if(loader){
        ...
        loader.addEventListener("error",()=>console.error("GV-beta-0005K STARTUP FAILURE: official Aladin 3.8.2 bundle failed to load"),{once:true});
        return;
    }
