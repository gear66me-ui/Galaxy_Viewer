...
(async()=>{
    ...
    const COORDINATE_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/coordinate-overlay/gv-coordinate-overlay-0005.js';
    const TARGET_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/target-simbad/gv-target-simbad-0004.js';
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0048.js';
    const DIAGNOSTICS_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/diagnostics/gv-diagnostics-0004.js?v=20213375C52DF374';
    ...
    const SPITZER_ICON_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/artwork/runtime/providers/spitzer/spitzer-icon.png';
    ...
    const moduleLoads=Promise.all([
        loadScript(HAMBURGER_URL,'gvHamburger0005'),
        loadScript(COORDINATE_URL,'gvCoordinate0005'),
        loadScript(TARGET_URL,'gvTarget0004'),
        loadScript(RANDOM_GALAXY_URL,'gvRandomGalaxy0048'),
        loadScript(DIAGNOSTICS_URL,'gvDiagnostics0004'),
    ...
    let catalogDatabaseCounts=Object.freeze({hubble:0,jwst:0,chandra:0,spitzer:0,total:0,eligibleHubble:0,eligibleJwst:0,eligibleChandra:0,eligibleSpitzer:0,eligibleTotal:0});
    ...
    function normalizeCatalogGalaxy(candidate,index){
        ...
        return Object.freeze({
            ...
            fov,imageFovDegrees:fieldDegrees,hdUrl:hd.href,sourceUrl:source.href,orientation,aladinRotation:Number.isFinite(archiveRotation)?archiveRotation:0,
    ...
    function normalizeJwstGalaxy(candidate,index){
        ...
        return Object.freeze({
            ...
            aladinRotation:Number.isFinite(archiveRotation)?archiveRotation:0,
    ...
    function normalizeChandraGalaxy(candidate,index){
        ...
        const archiveRotation=parseArchiveOrientation(orientation);
        return Object.freeze({
            source:'NASA CHANDRA X-RAY CENTER CATALOG FULL-0002',provider:'CHANDRA',
            archiveId:String(candidate.archiveId||'').trim(),
            name,ra,dec,distance,constellation,designation,commonName,age,
            ageYears:Number.isFinite(ageYears)&&ageYears>0?ageYears:null,
            physicalSizeLy,fov,imageFovDegrees:Number.isFinite(fieldDegrees)&&fieldDegrees>0?fieldDegrees:null,hdUrl:hd.href,sourceUrl:source.href,orientation,aladinRotation:Number.isFinite(archiveRotation)?archiveRotation:0,
    ...
    function normalizeSpitzerGalaxy(candidate,index){
        if(!candidate||typeof candidate!=='object')return null;
        const name=String(candidate.name||candidate.title||'').trim();
        const ra=Number(candidate.ra),dec=Number(candidate.dec);
        const distance=parseDistanceMly(candidate.science?.distanceMly??candidate.distance);
        ...
        let hd,source;
        try{hd=new URL(hdUrl);source=new URL(sourceUrl)}catch(_){return null}
        const approvedImageHost=host=>host==='www.spitzer.caltech.edu'||host==='spitzer.caltech.edu'||host==='astropix.org'||host.endsWith('.astropix.org');
        if(hd.protocol!=='https:'||source.protocol!=='https:'||!approvedImageHost(hd.hostname)||(source.hostname!=='www.spitzer.caltech.edu'&&source.hostname!=='spitzer.caltech.edu'))return null;
        const imageType=String(candidate.imageType||'').trim();
        if(imageType&&/\b(artwork|illustration|collage|chart|simulation|diagram|artist(?:'s)? impression)\b/i.test(imageType))return null;
        const orientation=String(candidate.orientation||'' ).trim();
        const archiveRotation=parseArchiveOrientation(orientation);
        return Object.freeze({
            source:'NASA SPITZER SPACE TELESCOPE CATALOG FULL-0009',provider:'SPITZER',
            archiveId:String(candidate.archiveId||'').trim(),
            name,ra,dec,distance,constellation,designation,commonName,age,
            ageYears:Number.isFinite(ageYears)&&ageYears>0?ageYears:null,
            physicalSizeLy,fov,imageFovDegrees:Number.isFinite(fieldDegrees)&&fieldDegrees>0?fieldDegrees:null,hdUrl:hd.href,sourceUrl:source.href,orientation,aladinRotation:Number.isFinite(archiveRotation)?archiveRotation:0,
            credit:String(candidate.credit||'NASA/JPL-Caltech').trim()||'NASA/JPL-Caltech',
            imageType:imageType||'Observation',category:'Galaxies',telescope:'Spitzer Space Telescope',
            githubImageUrl:'',sha256:String(candidate.sha256||'').trim(),catalogIndex:index
        });
    }
    ...
    async function loadMasterCatalog(){
        ...
        if(!catalogs?.hubble||!catalogs?.jwst||!catalogs?.chandra||!catalogs?.spitzer)throw new Error('MASTER CATALOG POINTERS MISSING');
        return Object.freeze({
            hubble:RAW_BETA_ROOT+catalogs.hubble,
            jwst:RAW_BETA_ROOT+catalogs.jwst,
            chandra:RAW_BETA_ROOT+catalogs.chandra,
            spitzer:RAW_BETA_ROOT+catalogs.spitzer
        });
    }
    ...
    async function loadChandraCatalog(catalogUrl){
        ...
        const raw=payload?.entries;
        const declared=Number(payload?.categoryEntryCount);
        if(payload?.version!=='full-0002')throw new Error('CHANDRA CATALOG MUST BE FULL-0002');
    ...
    async function loadSpitzerCatalog(catalogUrl){
        const response=await fetch(catalogUrl,{cache:'no-store'});
        if(!response.ok)throw new Error('FULL SPITZER CATALOG RETURNED HTTP '+response.status);
        const payload=await response.json();
        const raw=payload?.entries;
        const declared=Number(payload?.categoryEntryCount);
        if(payload?.version!=='full-0009')throw new Error('SPITZER CATALOG MUST BE FULL-0009');
        if(!Array.isArray(raw)||!raw.length||!Number.isFinite(declared)||declared!==raw.length)throw new Error('FULL SPITZER CATALOG COUNT MISMATCH');
        const eligible=raw.map(normalizeSpitzerGalaxy).filter(Boolean);
        if(!eligible.length)throw new Error('FULL SPITZER CATALOG HAS NO TARGETABLE GALAXIES');
        return Object.freeze({rawCount:raw.length,eligible:Object.freeze(eligible)});
    }
    ...
    async function loadCombinedGalaxyCatalog(){
        ...
        const state={
            ...
            spitzer:{rawCount:0,eligibleCount:0,done:false}
        };
        let settled=0;
        ...
        const publishCounts=()=>{
            ...
            catalogDatabaseCounts=Object.freeze({
                ...
                spitzer:state.spitzer.rawCount,
                total:state.hubble.rawCount+state.jwst.rawCount+state.chandra.rawCount+state.spitzer.rawCount,
                ...
                eligibleSpitzer:state.spitzer.eligibleCount,
        ...
        attach('spitzer',loadSpitzerCatalog(catalogUrls.spitzer));
    ...
    async function decodePreparedRemoteImage(url,signal=null){
        ...
        await new Promise((resolve,reject)=>{
            ...
            const onError=()=>finish(reject,new Error('HD REMOTE IMAGE PRELOAD FAILED'));
            const onAbort=()=>{
                image.src='';
                finish(reject,new DOMException('HD PRELOAD SUSPENDED','AbortError'));
            };
            image.addEventListener('load',onLoad,{once:true});
            image.addEventListener('error',onError,{once:true});
            signal?.addEventListener?.('abort',onAbort,{once:true});
            image.src=String(url||'');
            if(image.complete&&image.naturalWidth)finish(resolve,true);
        });
        if(!image.naturalWidth||!image.naturalHeight)
            throw new Error('HD REMOTE IMAGE DECODED WITHOUT IMAGE DIMENSIONS');
        return {image,objectUrl:String(url||'')};
    }
    ...
    async function prepareHdDestination(destination,signal=null){
        ...
        for(const source of sources){
            try{
                ...
                lastError=error;
                try{
                    const prepared=await decodePreparedRemoteImage(source.url,signal);
                    setHdStatus(destination,'READY',source.kind);
                    return {key:destinationKey(destination),destination,image:prepared.image,objectUrl:prepared.objectUrl,sourceUrl:source.url,sourceKind:source.kind};
                }catch(remoteError){
                    if(remoteError?.name==='AbortError'){setHdStatus(destination,'SUSPENDED',source.kind);throw remoteError}
    ...
    function ensureAladinPrewarm(){
        ...
        aladinPrewarmReady=new Promise((resolve,reject)=>{
            ...
            frame.srcdoc=`<!doctype html><html><head><link rel="stylesheet" href="https://gear66me-ui.github.io/Galaxy_Viewer/aladin-source-clone/src/css/aladin.css"><style>html,body,#gv-prewarm{margin:0;width:512px;height:512px;overflow:hidden;background:#000}</style></head><body><div id="gv-prewarm"></div><script src="${ALADIN_URL}"><\/script></body></html>`;
    ...
    function aladinReceiptMatchesDestination(receipt,destination){
        ...
        return String(receipt.key||'').trim().toLowerCase()===destinationKey(destination)&&
    ...
    async function prepareAladinDestination(destination,force=false){
        ...
        try{
            ...
            if(typeof isolated.setRotation==='function')isolated.setRotation(Number.isFinite(Number(destination.aladinRotation))?Number(destination.aladinRotation):0);
    ...
    if(
        window.GalaxyRandomGalaxy?.VERSION!=='0048' ||
        typeof window.GalaxyRandomGalaxy.bootstrapHomePresentation!=='function'
    )throw new Error('RANDOM GALAXY 0048 HOME BOOTSTRAP EXPORT MISSING');

    window.GalaxyRandomGalaxy.bootstrapHomePresentation(root);

    // ==================== HAMBURGER MENU ====================
    if(window.GalaxyViewerHamburgerMenu?.version!=='0005')throw new Error('HAMBURGER MODULE 0005 EXPORT MISSING');
    ...
    if(window.GalaxyCoordinateOverlay?.VERSION!=='0005')throw new Error('COORDINATE MODULE 0005 EXPORT MISSING OR VERSION MISMATCH');
    ...
    // ==================== RANDOM NAVIGATION ====================
    if(window.GalaxyRandomGalaxy?.VERSION!=='0048')throw new Error('RANDOM GALAXY 0048 EXPORT MISSING OR VERSION MISMATCH');
    if(typeof window.GalaxyRandomNavigationWindow!=='function')throw new Error('RANDOM GALAXY 0048 NAVIGATION WINDOW EXPORT MISSING');
...
function navigateHistory(direction){
    ...
    fillPrefetchQueue();
    randomGalaxy.ready.catch(error=>console.warn(
    ...
    window.GalaxyViewerCore=Object.freeze({version:VERSION,displayVersion:DISPLAY_VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomNavigationWindow,randomGalaxyButton:bottom.random,historyBackButton:bottom.back,historyForwardButton:bottom.forward,reticle,versionLabel:bottom.version,catalogCount:catalogRecordCount,eligibleCatalogCount:galaxyCatalog.length,catalogDatabaseCounts,startupMetrics,getPrefetchState,getRandomNavigationState,getDownloadStatus,getAladinPrewarmState,fillPrefetchQueue,getGalaxyCatalog:()=>Object.freeze([...galaxyCatalog]),activateQueuedDestination:(destination,excludeName='')=>consumeReady(destination,excludeName)||setUnpreparedActive(destination),requestHdPrefetch:destination=>{if(!destination)return '';enqueuePrefetch(destination,true);fillPrefetchQueue();return destinationKey(destination)},getHdPreparedResource:key=>{const normalized=String(key||'').trim().toLowerCase();return activePreparedItem?.key===normalized?activePreparedItem:prefetchReady.find(item=>item?.key===normalized)||historyPreparedItems.find(item=>item?.key===normalized)||null},isHdPrepared:key=>{const normalized=String(key||'').trim().toLowerCase();return Boolean(activePreparedItem?.key===normalized||prefetchReady.some(item=>item?.key===normalized)||historyPreparedItems.some(item=>item?.key===normalized))},getAladinPreparedReceipt:key=>aladinPreparedReceipts.get(String(key||'').trim().toLowerCase())||null,isAladinPrepared:key=>{
        ...
        return Boolean(normalized&&!aladinPrewarmUnavailable&&aladinPreparedReceipts.has(normalized));
        ...
        if(existing&&aladinReceiptMatchesDestination(existing,destination)&&aladinPrewarm&&aladinPrewarmHost&&aladinPrewarmLastKey===key)return existing;
        const receipt=await prepareAladinDestination(destination,true);
        return aladinReceiptMatchesDestination(receipt,destination)?receipt:null;
