...
(async()=>{
    ...
    const DISPLAY_VERSION='GV-beta-200-011';
    ...
    const NAVIGATION_RUNTIME_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/navigation-runtime/gv-navigation-runtime-0001.js';
    ...
    const moduleLoads=Promise.all([
        ...
        loadScript(NAVIGATION_RUNTIME_URL,'gvNavigationRuntime0001')
    ]);
  ...
  lastNorthBearing:null
    ...
    if(navigationState.active!==100||navigationState.reserve!==30)throw new Error('NAVIGATION RUNTIME ROUTE CONTRACT FAILED');
    const authoritativeRoute=[...navigationRuntime.active.route];
    ...
    const chooseRandom=()=>{
        if(!authoritativeRoute.length)throw new Error('NAVIGATION RUNTIME ACTIVE ROUTE EMPTY');
        const destination=authoritativeRoute[randomCursor%authoritativeRoute.length];
        randomCursor++;
    ...
    window.GalaxyViewerCore=Object.freeze({
        ...
        navigationRuntime,
        get navigationRuntimeState(){return navigationRuntime.snapshot()},
        startupMetrics,
