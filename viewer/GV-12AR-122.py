...
(async()=>{
    'use strict';
    const VERSION='12AR-122';
    const DISPLAY_VERSION='12AR-122';
    ...
    const RANDOM_GALAXY_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/random-galaxy/gv-random-galaxy-0150.js';
    ...
    const RANDOM_GALAXY_VERSION='0150';
    const AVM_LAB_URL='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/modules/lab/gv-avm-overlay-lab-0033.js';
...
`;
    ...
    )throw new Error('AR122 EARLY RANDOM PREFETCH RUNTIME INSTALL FAILED');

    // Future destinations now exist and requestHdPrefetch() has populated
    // the HD queue. This fill starts real image downloads immediately.
    ...
    gvAvmTrace('VIEWER_AVM0033_LOAD_START',{url:AVM_LAB_URL});
    loadScriptGithubThenLocal(AVM_LAB_URL,'gvAvmOverlayLab0033').then(()=>{
        gvAvmTrace('VIEWER_AVM0033_LOAD_OK',{version:window.GalaxyViewerAvmOverlayLab?.VERSION||''});
        const lab=window.GalaxyViewerAvmOverlayLab;
        if(!lab){gvAvmTrace('VIEWER_AVM0033_EXPORT_MISSING',{});return}
        if(lab.VERSION!=='0033')gvAvmTrace('VIEWER_AVM0033_VERSION_MISMATCH',{version:String(lab.VERSION||'')});
        try{
            gvAvmTrace('VIEWER_AVM0033_INSTALL_START',{version:lab.VERSION});
            lab.install?.({A,aladin,viewerRoot:root,randomGalaxy});
            gvAvmTrace('VIEWER_AVM0033_INSTALL_OK',{version:lab.VERSION});
        }catch(error){
            gvAvmTrace('VIEWER_AVM0033_INSTALL_FAIL',{message:String(error?.message||error||'')});
            throw error;
        }
    }).catch(error=>{gvAvmTrace('VIEWER_AVM0033_LOAD_FAIL',{message:String(error?.message||error||'')});console.error('GV AVM LAB LOAD FAILURE',error)});
