...
A.init.then(() => {
    ...
    if(box){
        box.innerHTML="Click Target to Find Info";
...
function setHelperActive(){
    ...
    if(box){
        box.innerHTML="✨ Click a Galaxy or Star";
...
function resetHelperAndStatus(){
    ...
    function ensureHelper(row,proxy){
        ...
        if(!stack){
            ...
            stack.innerHTML=`<div class="gv-helper-row"><div class="gv-arrow">◀</div><div class="gv-helper-box">Click Target to Find Info</div></div><div class="gv-simbad-live-status" role="status" aria-live="polite"></div>`;
        }
