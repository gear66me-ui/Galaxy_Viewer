...
#gv-apk-cover{flex-direction:column;gap:18px}#gv-apk-cover .gv-viewer-version{color:#78FFAB;background:linear-gradient(90deg,#E5FFEF 0%,#A8FFD0 30%,#78FFAB 62%,#2FCB72 100%);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;font:400 16px/1 "Space Age",sans-serif;letter-spacing:1.2px;filter:drop-shadow(0 0 5px rgba(87,255,147,.78));white-space:nowrap}
</style>
<div id="aladin-cosmic-command-test"><div id="gv-startup-wait" aria-hidden="true"></div></div>
<script>(()=>{const cover=document.getElementById('gv-apk-cover');if(!cover)return;const img=cover.querySelector('img');if(img)img.src='https://gear66me-ui.github.io/Galaxy_Viewer/viewer/artwork/runtime/navigation/galaxy-viewer-target-icon.svg';const version=document.createElement('div');version.className='gv-viewer-version';version.textContent='VERSION 12D';cover.appendChild(version)})();</script>
"""))
...
(async()=>{
    ...
    const RANDOM_GALAXY_URL='https://raw.githubusercontent.com/gear66me-ui/Galaxy_Viewer/beta/viewer/modules/gv-random-galaxy-0038.js?v=0038';
    ...
    if(
        window.GalaxyRandomGalaxy?.VERSION!=='0038' ||
        typeof window.GalaxyRandomGalaxy.bootstrapHomePresentation!=='function'
    )throw new Error('RANDOM GALAXY 0038 HOME BOOTSTRAP EXPORT MISSING');
    ...
    // ==================== RANDOM NAVIGATION ====================
    if(window.GalaxyRandomGalaxy?.VERSION!=='0038')throw new Error('RANDOM GALAXY 0038 EXPORT MISSING OR VERSION MISMATCH');
    if(typeof window.GalaxyRandomNavigationWindow!=='function')throw new Error('RANDOM GALAXY 0038 NAVIGATION WINDOW EXPORT MISSING');
