...
(async()=>{
    ...
    function createBottomControls(root){
        ...
        version.setAttribute('aria-label','GALAXY VIEWER VERSION 8C');
    ...
    function readCurrentRaDec(aladin,root){
        ...
        }catch(error){console.warn('GV-8C GETRADEC WARNING',error)}
        try{
        ...
        }catch(error){console.warn('GV-8C PIX2WORLD WARNING',error)}
        return null;
    }
    ...
    const hamburger=window.GalaxyViewerHamburgerMenu.init({
        ...
        onProjectionSelected(name,detail){
            ...
            }catch(error){console.error('GV-8C PROJECTION FAILURE',name,detail?.code,error)}
        }
    });
    ...
    coordinate=window.GalaxyCoordinateOverlay.mount(coordinateHost,{onFrameChange(nextFrame){
        frame=nextFrame;
        try{if(typeof aladin.setFrame==='function')aladin.setFrame(frame==='GAL'?'galactic':'ICRSd')}catch(error){console.warn('GV-8C FRAME CHANGE WARNING',error)}
        renderCoordinates();
    }});
    ...
    window.GV8C=Object.freeze({version:VERSION,aladin,hamburger,coordinate,target,randomGalaxy,randomGalaxyButton:bottom.random,reticle,versionLabel:bottom.version});
    document.dispatchEvent(new CustomEvent('gv-viewer-ready',{detail:{version:VERSION}}));
})().catch(error=>console.error('GALAXY VIEWER 8C STARTUP FAILURE:',error));
"""))

# GV-beta-0008C staged
