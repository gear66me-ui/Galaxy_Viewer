...
async function querySimbad(ra,dec) {
  ...
  const adql=`SELECT TOP 10 main_id,ra,dec,rvz_redshift,galdim_majaxis,galdim_minaxis,otype,sp_type,DISTANCE(POINT('ICRS',ra,dec),POINT('ICRS',${ra},${dec})) AS separation FROM basic WHERE 1=CONTAINS(POINT('ICRS',ra,dec),CIRCLE('ICRS',${ra},${dec},${radius})) ORDER BY separation ASC`;
  ...
  const rows = payload.data.map(r => {
    const row={}; payload.metadata.forEach((m,i)=>row[m.name]=r[i]); return row;
  });
  const preferred =
    rows.find(r => /^(M\s*31|MESSIER\s*31|NGC\s*224)$/i.test(String(r.main_id ?? "").trim())) ||
    rows.find(r => /ANDROMEDA/i.test(String(r.main_id ?? ""))) ||
    rows.find(r => {
      const id = String(r.main_id ?? "").toUpperCase();
      const otype = String(r.otype ?? "");
      return (
        !id.includes("[") &&
        !id.includes("LDC") &&
        !id.includes("2MASS") &&
        !id.includes("GALEX") &&
        (otype === "G" || otype === "PoG" || otype === "LSB")
      );
    }) ||
    rows[0];
  return preferred;
...
async function findGalaxy() {
  try {
    ...
    const tasks=[
      nedPromise,
      runCatalog("VizieR",()=>probeVizier(coords.ra,coords.dec)),
    ...
    const simbad=await simbadPromise;
    renderResult(simbad,coords,null);

    const ned=await nedPromise;
    if (simbad) {
      renderResult(simbad,coords,ned);
    }

    await Promise.allSettled(tasks);
    setStatus("Search complete. Galaxy table renders immediately from SIMBAD, then refines with NED/local-catalog distance fallback for nearby systems; all catalog and survey outcomes are listed below.");
