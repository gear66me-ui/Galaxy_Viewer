...
<div id="gv0011-root">
  ...
  <div class="small-note">
    This debug build waits for all requested catalog/survey work to settle before filling the main galaxy table. It also dumps every SIMBAD row returned by the cone search so we can compare the browser query against the SIMBAD website manually.
  </div>
</div>
...
async function findGalaxy() {
  try {
    ...
    setStatus('Search complete. Main table was rendered only after all catalog and survey tasks finished.');
  } catch(err){
...
def show_galaxy_viewer_gv0039():
    display(HTML(HTML_CONTENT))

show_galaxy_viewer_gv0039()
