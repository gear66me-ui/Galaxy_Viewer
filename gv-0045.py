...
<div id="gv0011-root">
  ...
  <div class="small-note">
    GV-0045 preserves the current viewer position, survey, and field of view in browser storage so switching tabs does not snap the viewer back to Andromeda. SIMBAD is queried with a 30-arcsecond cone and the first returned SIMBAD row is used as the displayed primary object.
  </div>
</div>
...
const VIEWER_STATE_KEY = "galaxy-viewer-gv0045-state";
...
async function findGalaxy() {
  try {
    ...
    saveViewerState();
    setStatus("Search complete. GV-0045 used the first SIMBAD row from the 30-arcsecond cone search and kept your current view.");
  } catch(err){
