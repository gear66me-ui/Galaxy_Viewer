...
async function querySimbad(ra,dec) {
  ...
  const adql=`SELECT TOP 5 main_id,ra,dec,rvz_redshift,galdim_majaxis,galdim_minaxis,otype,sp_type,DISTANCE(POINT('ICRS',ra,dec),POINT('ICRS',${ra},${dec})) AS separation FROM basic WHERE 1=CONTAINS(POINT('ICRS',ra,dec),CIRCLE('ICRS',${ra},${dec},${radius})) ORDER BY CASE WHEN otype = 'G' THEN 0 WHEN otype LIKE 'G%' THEN 1 ELSE 2 END, separation ASC`;
  ...
  const preferred = rows.find(r => String(r.otype ?? "").startsWith("G")) || rows[0];
  return preferred;
}
...
async function findGalaxy() {
  try {
    ...
    setStatus("Search complete. Galaxy table uses SIMBAD identity and NED distance fallback for nearby systems; all catalog and survey outcomes are listed below.");
