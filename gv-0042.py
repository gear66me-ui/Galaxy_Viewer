...
<div id="gv0011-root">
  ...
  <div class="small-note">
    Object identity and redshift are summarized from nearby catalog candidates. Whole-galaxy size prefers larger diameter measurements from returned catalog data and rejects obviously nuclear/component-sized values when possible. Catalog services may restrict browser-origin requests; each service status is reported independently without interrupting the viewer.
  </div>
</div>
...
async function findGalaxy() {
  try {
    ...
    const earlySummary=summarizeGalaxyCandidates(simbadRows,coords,null,null);
    renderResult(earlySummary,coords,null);

    const [ned,vizier,sdss,panstarrs,galex,surveyLog] = await Promise.all([
      nedPromise, vizierPromise, sdssPromise, panstarrsPromise, galexPromise, surveyPromise
    ]);
    ...
    setStatus("Search complete. GV-0042 prints raw catalog payloads in plain text so you can inspect exactly what each source returned.");
  } catch(err){
