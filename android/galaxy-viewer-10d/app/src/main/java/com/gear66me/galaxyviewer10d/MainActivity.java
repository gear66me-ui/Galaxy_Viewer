...
public final class MainActivity extends Activity {
    ...
    private void statusScreen(){panel=new LinearLayout(this);panel.setOrientation(LinearLayout.VERTICAL);panel.setGravity(Gravity.CENTER);panel.setPadding(48,48,48,48);panel.setBackgroundColor(Color.BLACK);status=new TextView(this);status.setTextColor(Color.WHITE);status.setTextSize(16);status.setGravity(Gravity.CENTER);panel.addView(status,new LinearLayout.LayoutParams(-1,-2));retry=new Button(this);retry.setText("RETRY");retry.setOnClickListener(v->resolve());LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-2,-2);p.topMargin=32;panel.addView(retry,p);show("GALAXY VIEWER 10D\n\nChecking current Viewer…",false);}
    ...
    private void open(String u){if(web!=null)web.destroy();web=new WebView(this);web.setBackgroundColor(Color.BLACK);WebSettings s=web.getSettings();s.setJavaScriptEnabled(true);s.setDomStorageEnabled(true);s.setDatabaseEnabled(true);s.setAllowFileAccess(false);s.setAllowContentAccess(false);s.setMediaPlaybackRequiresUserGesture(false);s.setCacheMode(WebSettings.LOAD_DEFAULT);s.setUserAgentString(s.getUserAgentString()+" GalaxyViewer10D/10D-generic-r3");web.setWebChromeClient(new WebChromeClient());web.setWebViewClient(new WebViewClient(){private boolean ok(Uri x){return x!=null&&"https".equalsIgnoreCase(x.getScheme())&&HOST.equalsIgnoreCase(x.getHost())&&x.getPath()!=null&&x.getPath().startsWith(PREFIX);}@Override public boolean shouldOverrideUrlLoading(WebView v,WebResourceRequest q){return !ok(q.getUrl());}@Override public boolean shouldOverrideUrlLoading(WebView v,String x){return !ok(Uri.parse(x));}@Override public void onReceivedError(WebView v,WebResourceRequest q,WebResourceError e){if(q.isForMainFrame())fail("WebView error "+e.getErrorCode()+": "+e.getDescription());}@Override public void onReceivedHttpError(WebView v,WebResourceRequest q,WebResourceResponse e){if(q.isForMainFrame()&&e.getStatusCode()>=400)fail("Viewer returned HTTP "+e.getStatusCode());}});setContentView(web);web.loadUrl(u);}
    private void fail(String d){show("GALAXY VIEWER 10D — VIEWER LOAD ERROR\n\n"+d+"\n\nResolved target:\n"+(target==null?"(none)":target)+"\n\nPointer:\n"+POINTER,true);}
