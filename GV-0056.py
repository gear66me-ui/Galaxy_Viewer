...
def _ned_query(ra, dec):
    ...
    cols = [str(i) for i in range(df.shape[1])]
    row = df.iloc[0]
...
if colab_output is not None:
    colab_output.register_callback("gv0056.ned_query", _ned_query)
...
<div class="small-note">GV-0056 is standalone. It preserves position, survey, and zoom/FOV. SIMBAD and NED use a 6-arcsecond search. Row 1 is SIMBAD row 1 and row 2 is NED row 1. The figures-of-merit table shows object name, RA / Dec, object type / size, velocity, redshift / distance, magnitude / filter, angular separation, and information.</div>
</div>
...
async function ned(ra,dec){const r=await google.colab.kernel.invokeFunction("gv0056.ned_query",[ra,dec],{});let p=r?.data?.["application/json"]??r?.data?.["text/plain"]??r;for(let i=0;i<4&&typeof p==="string";i++){try{p=JSON.parse(p)}catch(e){break}}if(p&&Array.isArray(p.rows))return p.rows;throw Error("NED callback returned no decodable rows")}
