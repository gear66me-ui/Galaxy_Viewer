...
<div id="gv0011-root">
  ...
  <div class="small-note">
    GV-0041 keeps the progress bar and viewer style from GV-0040, but now uses nearby-galaxy distance fallbacks, computes galaxy size from those fallback distances when redshift is not usable, and waits for all services to settle before rendering the final summary.
  </div>
</div>
...
const NEARBY_DISTANCE_LY = [
  {names:['M  31','M31','NGC 224','ANDROMEDA'], ly:2537000},
  {names:['M  32','M32','NGC 221'], ly:2490000},
  {names:['M 110','M110','NGC 205'], ly:2690000},
  {names:['M  33','M33','NGC 598','TRIANGULUM'], ly:2723000},
  {names:['LMC','LARGE MAGELLANIC CLOUD'], ly:163000},
  {names:['SMC','SMALL MAGELLANIC CLOUD'], ly:200000},
  {names:['NGC 6822','BARNARD'], ly:1630000}
];
...
function resolveNearbyDistanceLy(name) {
  const key = String(name || '').toUpperCase();
  for (const item of NEARBY_DISTANCE_LY) {
    for (const alias of item.names) {
      if (key.includes(alias.toUpperCase())) return item.ly;
    }
  }
  return null;
}
function resolveDistanceLy(bestRow, normalizedName) {
  const z = Number(bestRow?.rvz_redshift);
  const fromZ = distanceFromZ(z);
  if (Number.isFinite(fromZ)) return fromZ;
  return resolveNearbyDistanceLy(normalizedName || bestRow?.main_id);
}
function estimateAgeSummary(bestName) {
  const key = String(bestName || '').toUpperCase();
  if (key.includes('M  31') || key.includes('M31') || key.includes('NGC 224') || key.includes('ANDROMEDA')) {
    return '6.00 / 8.00 / 10.00';
  }
  if (key.includes('M  33') || key.includes('M33') || key.includes('NGC 598')) {
...
function pickBestSimbad(rows) {
  ...
  const ranked = rows.map(row => {
    ...
    if (/^(M\s*\d+|MESSIER\s*\d+|NGC\s*\d+|IC\s*\d+)$/i.test(id)) score += 180;
    if (otype === 'G') score += 140;
    else if (otype.startsWith('G')) score += 105;
    else if (otype === 'POG' || otype === 'LSB') score += 25;
    if (otype === 'AGN' || otype === 'SY1' || otype === 'SY2' || otype === 'X' || otype === 'XB?' || otype === 'RAD') score -= 90;
    if (Number.isFinite(maj)) score += Math.min(maj / 3, 90);
    if (Number.isFinite(sep)) score += Math.max(0, 50 - sep * 3600);
    if (id.includes('2MASS') || id.includes('GALEX') || id.includes('SDSS') || id.includes('[') || id.includes('CXO') || id.includes('XMM')) score -= 100;
    if (id === 'M  31' || id === 'M31' || id === 'NGC 224') score += 120;
    return { row, score };
...
function normalizeSimbad(rows) {
  ...
  if (!best) return null;
  const cleanedName = cleanId(best.main_id);
  const distanceLy = resolveDistanceLy(best, cleanedName);
  return {
    source: 'SIMBAD',
    name: cleanedName,
    ra: Number(best.ra),
    dec: Number(best.dec),
    age: estimateAgeSummary(cleanedName),
...
function renderResult(bestRow, snapshotRows) {
  ...
  body.innerHTML=`<tr>
    <td>${safe(primary.name || cleanId(bestRow.main_id || 'Not available'))}</td>
