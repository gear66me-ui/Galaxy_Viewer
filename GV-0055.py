...
async function findGalaxy(){try{const c=coords(document.getElementById("coordBox").value);document.getElementById("resultBody").innerHTML='<tr><td colspan="7" style="text-align:center">Waiting for final catalog aggregation…</td></tr>';status(`Searching SIMBAD, NED, VizieR, SDSS, PanSTARRS, GALEX, and configured surveys at ${c.ra.toFixed(6)} ${c.dec.toFixed(6)} …`);window.aladin.gotoRaDec(c.ra,c.dec);save({ra:c.ra,dec:c.dec});const [s,n,v,sd,p,g]=await Promise.all([run("SIMBAD",()=>simbad(c.ra,c.dec)),run("NED",()=>ned(c.ra,c.dec)),run("VizieR",()=>vizier(c.ra,c.dec)),run("SDSS",()=>sdss(c.ra,c.dec)),run("PanSTARRS",()=>panstarrs(c.ra,c.dec)),run("GALEX",()=>galex(c.ra,c.dec)),scanSurveys()]);const so=s?.[0]?{...s[0],_candidateCount:s.length}:null,no=n?.[0]?{...n[0],_candidateCount:n.length}:null;document.getElementById("resultBody").innerHTML=(resultRow(so,"SIMBAD",c)+resultRow(no,"NED",c))||'<tr><td colspan="7">No SIMBAD or NED match found.</td></tr>';debug(`DEBUG DUMP FOR ${c.ra.toFixed(6)} ${c.dec.toFixed(6)}\n######################################################\n\n`+section("SIMBAD",s)+section("NED",n)+section("VizieR",v?.data||v)+section("SDSS",sd?.data||sd)+section("PanSTARRS",p)+section("GALEX",g?.data||g));save();status("Search complete. GV-0055 used SIMBAD row 1 and NED row 1 from the 6-arcsecond search window.")}catch(e){status("Search failed: "+e.message);debug(String(e.stack||e))}}
</script>
'''
display(HTML(html))
