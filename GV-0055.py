...
def _ned_query(ra, dec):
    ...
    for i, df in enumerate(tables):
        ...
        score = (20 if "object name" in text else 0) + (10 if "ra" in text else 0) + (10 if "dec" in text else 0) + (10 if "redshift" in text else 0) + (5 if df.shape[1] >= 6 else 0)
        ranked.append((score, df.shape[0], df.shape[1], i, df, cols))
    ranked.sort(reverse=True, key=lambda x: x[:3])
    if not ranked:
        return json.dumps({"rows":[]}, ensure_ascii=False)
    _, _, _, idx, df, cols = ranked[0]
    df = df.dropna(how="all")
    if df.empty:
        return json.dumps({"rows":[]}, ensure_ascii=False)
    row = df.iloc[0]
    mapping = {cols[i].lower(): _clean(row.iloc[i]) for i in range(len(cols))}
    def pick(*keys):
    ...
    name = pick("object","name") or pick("name") or (cells[1] if len(cells)>1 else cells[0])
    ra_text = pick("ra") or pick("right","ascension") or (cells[2] if len(cells)>2 else None)
    dec_text = pick("dec") or pick("declination") or (cells[3] if len(cells)>3 else None)
    otype = pick("type") or (cells[4] if len(cells)>4 else None)
    velocity = pick("velocity") or (cells[5] if len(cells)>5 else None)
    redshift = pick("redshift") or (cells[6] if len(cells)>6 else None)
    obj = {
        "main_id": name or "Not available",
        "ra": _ra(ra_text), "dec": _dec(dec_text),
        "otype": otype, "rvz_radvel": _num(velocity), "rvz_redshift": _num(redshift),
        "sp_type": None, "galdim_majaxis": None, "galdim_minaxis": None,
        "_selectionRule": "NED row 1", "_candidateCount": int(df.shape[0]),
        "_ned_columns": cols, "_ned_raw_first_row": cells
    }
    return json.dumps({"rows":[obj], "selected_table":idx}, ensure_ascii=False, allow_nan=False)

if colab_output is not None:
    colab_output.register_callback("gv0055.ned_query", _ned_query)
...
<div class="table-wrap"><table><thead><tr><th>Object name</th><th>ICRS coordinates (RA Dec)</th><th>Z / Distance</th><th>Object size</th><th>Galaxy age</th><th>Type / Spectrum</th><th>SIMBAD information</th></tr></thead><tbody id="resultBody"><tr><td colspan="7" style="text-align:center">No search performed.</td></tr></tbody></table></div>
...
<div class="small-note">GV-0055 is standalone. It preserves position, survey, and zoom/FOV. SIMBAD and NED use a 6-arcsecond search. Row 1 is SIMBAD row 1 and row 2 is NED row 1.</div>
</div>
...
const CATALOGS=["SIMBAD","NED","VizieR","SDSS","PanSTARRS","GALEX"],KEY="galaxy-viewer-gv0055-state",LYM=3261563.777,LYP=3.261563777;
...
async function ned(ra,dec){const r=await google.colab.kernel.invokeFunction("gv0055.ned_query",[ra,dec],{});let p=r?.data?.["application/json"]??r?.data?.["text/plain"]??r;for(let i=0;i<4&&typeof p==="string";i++){try{p=JSON.parse(p)}catch(e){break}}if(p&&Array.isArray(p.rows))return p.rows;throw Error("NED callback returned no decodable rows")}
async function run(n,f){cat(n,"Searching…","warn");try{const d=await f();cat(n,d.length?"Query completed":"No match",d.length?"ok":"warn");return d}catch(e){cat(n,"Unavailable: "+e.message,"bad");return null}}
...
async function findGalaxy(){try{const c=coords(document.getElementById("coordBox").value);document.getElementById("resultBody").innerHTML='<tr><td colspan="7" style="text-align:center">Waiting for final catalog aggregation…</td></tr>';status(`Searching SIMBAD and NED at ${c.ra.toFixed(6)} ${c.dec.toFixed(6)} …`);window.aladin.gotoRaDec(c.ra,c.dec);save({ra:c.ra,dec:c.dec});const [s,n]=await Promise.all([run("SIMBAD",()=>simbad(c.ra,c.dec)),run("NED",()=>ned(c.ra,c.dec))]);const so=s?.[0]?{...s[0],_candidateCount:s.length}:null,no=n?.[0]?{...n[0],_candidateCount:n.length}:null;document.getElementById("resultBody").innerHTML=(resultRow(so,"SIMBAD",c)+resultRow(no,"NED",c))||'<tr><td colspan="7">No SIMBAD or NED match found.</td></tr>';debug(`DEBUG DUMP FOR ${c.ra.toFixed(6)} ${c.dec.toFixed(6)}\n######################################################\n\n`+section("SIMBAD",s)+section("NED",n));save();status("Search complete. GV-0055 used SIMBAD row 1 and NED row 1 from the 6-arcsecond search window.")}catch(e){status("Search failed: "+e.message);debug(String(e.stack||e))}}
</script>
'''
display(HTML(html))
